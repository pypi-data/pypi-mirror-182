...
def build_state_plot(counts, cols, machine, model, y='quantity', group='state',
    ...
    for pop_id in pop_IDs:
        ...
        for state in cols:
            ...
            # df2 = subdf.groupby("days")
            # maxv = df2.quantity.max()
            # minv = df2.quantity.min()
            # upper = df2.quantity.quantile(0.8)
            # lower = df2.quantity.quantile(0.2)
            # med = df2.quantity.quantile(0.5)
            # src = ColumnDataSource(data=dict(days=df.days, base=med, upper=upper, lower=lower, min=minv, max=maxv))
            # pl = plot.line(x='days', y='base', source=src, color=cols[state], alpha=0.9, line_width=2, line_dash=line_style)
            # plmin = plot.line(x='days', y='min', source=src, color=cols[state], alpha=1, line_width=2, line_dash="dotted")
            # plmax = plot.line(x='days', y='max', source=src, color=cols[state], alpha=1, line_width=2, line_dash="dotted")
            # ribbon = plot.varea(x='days', y1='lower', y2='upper', source=src, fill_alpha=0.2, fill_color=cols[state])
            # # ribbon = Band(base='days', lower="lower", upper="upper", source=src, fill_alpha=0.2, fill_color=cols[state], line_color=cols[state])
            # # plot.add_layout(ribbon)
            # state_items.append(pl)
