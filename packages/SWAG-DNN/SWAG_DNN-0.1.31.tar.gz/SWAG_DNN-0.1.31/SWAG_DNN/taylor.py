...
############################################################################
def L_X_6(x):
    return ((231 * K.pow(x, 6)) - (315 * K.pow(x, 4)) + (105 * K.pow(x, 2)) - 5) / (
        16 * p6
    )
...
class Dense_Ch_a(Layer):
    ...
    def build(self, input_shape):
        ...
        self.kernel = self.add_weight(
            shape=(input_dim, self.hidden_dim * 6),
        ...
        ##########################################################################
        self.kernel_all = self.add_weight(
            shape=(6 * self.hidden_dim, self.units),
            initializer=self.kernel_initializer,
            name="kernel",
            regularizer=self.kernel_regularizer,
            constraint=self.kernel_constraint,
    ...
    def call(self, inputs):
        ...
        output1 = K.bias_add(
            K.dot(inputs, self.kernel[:, : self.hidden_dim]),
            self.bias[: self.hidden_dim],
            data_format="channels_last",
        )
        output2 = K.bias_add(
            K.dot(inputs, self.kernel[:, self.hidden_dim : self.hidden_dim * 2]),
            self.bias[self.hidden_dim : self.hidden_dim * 2],
            data_format="channels_last",
        )
        output3 = K.bias_add(
            K.dot(inputs, self.kernel[:, self.hidden_dim * 2 : self.hidden_dim * 3]),
            self.bias[self.hidden_dim * 2 : self.hidden_dim * 3],
            data_format="channels_last",
        )
        output4 = K.bias_add(
            K.dot(inputs, self.kernel[:, self.hidden_dim * 3 : self.hidden_dim * 4]),
            self.bias[self.hidden_dim * 3 : self.hidden_dim * 4],
            data_format="channels_last",
        )
        output5 = K.bias_add(
            K.dot(inputs, self.kernel[:, self.hidden_dim * 4 : self.hidden_dim * 5]),
            self.bias[self.hidden_dim * 4 : self.hidden_dim * 5],
            data_format="channels_last",
        )
        output6 = K.bias_add(
            K.dot(inputs, self.kernel[:, self.hidden_dim * 5 :]),
            self.bias[self.hidden_dim * 5 :],
            data_format="channels_last",
        )
...
def SWAG(input_dim, hidden_dim, output_dim):
    ...
    model.add(
        Dense_Co(
            ...
            bias_initializer=RandomNormal(mean=0.0, stddev=0.04),
        )
    )
    model.compile(loss="mean_squared_error", optimizer="adam", metrics=["accuracy"])
    return model
