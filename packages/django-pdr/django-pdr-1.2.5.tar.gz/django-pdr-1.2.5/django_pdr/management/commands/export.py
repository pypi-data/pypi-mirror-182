...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for source in SourceTable.objects.all():
            ...
            source_dir = os.path.join(pwd, "source_tables", source.pk)
            os.mkdir(source_dir)
            source_meta = {
        ...
        for reflection in Reflection.objects.all():
            ...
            reflection_dir = os.path.join(pwd, "reflections", reflection.pk)
            os.mkdir(reflection_dir)
            reflection_meta = {
                "description": reflection.description,
                "source_table": reflection.source_table,
                "destination_database": reflection.destination_database,
