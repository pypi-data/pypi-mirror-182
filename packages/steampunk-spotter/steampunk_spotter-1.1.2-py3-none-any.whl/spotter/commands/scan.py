...
class ScanPayload:
    ...
    def from_args(cls, parsing_result: ParsingResult, environment: Environment, upload_metadata: bool,
        ...
        return cls(
            environment=environment,
            tasks=parsing_result.tasks if upload_metadata else parsing_result.tasks_without_sensitive_data(),
            playbooks=parsing_result.playbooks if upload_metadata else parsing_result.playbooks_without_sensitive_data()
        )
...
def _parser_callback(args: argparse.Namespace) -> None:
    ...
    if args.export_payload:
        ...
        scan_result = scan(api_client, scan_payload, parsing_result, args.display_level, args.project_id)
...
def scan(api_client: ApiClient, scan_payload: ScanPayload, parsing_result: ParsingResult, display_level: DisplayLevel,
