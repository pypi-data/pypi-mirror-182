...
# Construct the modules that we find in the "pythonav" directory.
ext_modules = []
for dirname, dirnames, filenames in os.walk("pythonav"):
...
about = {}
about_file = os.path.join(os.path.dirname(__file__), "pythonav", "about.py")
with open(about_file, encoding="utf-8") as fp:
    exec(fp.read(), about)

package_folders = pathlib.Path("pythonav").glob("**/")
...
setup(
    ...
    url="https://github.com/dot-six/python-av",
    ...
    entry_points={
        "console_scripts": [
            "pythonav = pythonav.__main__:main",
        ],
    },
    classifiers=[
