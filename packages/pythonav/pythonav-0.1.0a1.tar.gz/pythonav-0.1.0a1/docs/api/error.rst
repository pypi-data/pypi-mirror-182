...
the ``av.error`` module, e.g.::
    ...
    try:
        ...
        if e.errno != pythonav.error.FILTER_NOT_FOUND:
            raise
        handle_error()


.. autoclass:: pythonav.error.ErrorType
...
All of these exceptions are available on the top-level ``av`` package, e.g.::
    ...
    try:
        do_something()
    except pythonav.FilterNotFoundError:
        handle_error()


.. autoclass:: pythonav.FFmpegError
