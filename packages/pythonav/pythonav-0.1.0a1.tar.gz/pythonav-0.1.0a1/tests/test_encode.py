...
from pythonav import AudioFrame, VideoFrame
from pythonav.audio.stream import AudioStream
from pythonav.video.stream import VideoStream
import pythonav
...
def assert_rgb_rotate(self, input_, is_dash=False):
    ...
    if is_dash:
        ...
        if pythonav.library_versions["libavformat"] >= (58, 28):
            self.assertTrue(
    ...
    if is_dash:
        ...
        if pythonav.library_versions["libavformat"] < (58, 76):
            # FFmpeg < 4.4
...
class TestBasicVideoEncoding(TestCase):
    def test_default_options(self):
        with pythonav.open(self.sandboxed("output.mov"), "w") as output:
    ...
    def test_encoding(self):
        ...
        with pythonav.open(path, "w") as output:
            write_rgb_rotate(output)
        with pythonav.open(path) as input:
