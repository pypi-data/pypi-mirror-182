...
from pythonav import AudioResampler, Codec, Packet
from pythonav.codec.codec import UnknownCodecError
from pythonav.video.frame import PictureType
import pythonav
...
class TestCodecContext(TestCase):
    ...
    def test_codec_tag(self):
        ...
        with pythonav.open(fate_suite("h264/interlaced_crop.mp4")) as container:
            self.assertEqual(container.streams[0].codec_tag, "avc1")

    def test_decoder_extradata(self):
        ctx = pythonav.codec.Codec("h264", "r").create()
    ...
    def test_encoder_extradata(self):
        ctx = pythonav.codec.Codec("h264", "w").create()
        self.assertEqual(ctx.extradata, None)
        self.assertEqual(ctx.extradata_size, 0)
...
class TestEncoding(TestCase):
    ...
    def image_sequence_encode(self, codec_name):
        ...
        container = pythonav.open(fate_suite("h264/interlaced_crop.mp4"))
        ...
        ctx = pythonav.Codec(codec_name, "r").create()
    ...
    def video_encoding(self, codec_name, options={}, codec_tag=None):
        ...
        ctx = pythonav.Codec(dec_codec_name, "r").create()
        ctx.open()
