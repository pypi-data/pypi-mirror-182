...
class TestAudioProbe(TestCase):
    def setUp(self):
        self.file = pythonav.open(fate_suite("aac/latm_stereo_to_51.ts"))

    def test_container_probing(self):
        self.assertEqual(self.file.bit_rate, 269558)
        self.assertEqual(self.file.duration, 6165333)
        self.assertEqual(str(self.file.format), "<pythonav.ContainerFormat 'mpegts'>")
    ...
    def test_stream_probing(self):
        ...
        self.assertTrue(
            str(stream).startswith(
                "<pythonav.AudioStream #0 aac_latm at 48000Hz, stereo, fltp at "
            )
        )
...
class TestAudioProbeCorrupt(TestCase):
    def setUp(self):
        ...
        self.file = pythonav.open(path)

    def test_container_probing(self):
        self.assertEqual(self.file.bit_rate, 0)
        self.assertEqual(self.file.duration, None)
        self.assertEqual(str(self.file.format), "<pythonav.ContainerFormat 'flac'>")
    ...
    def test_stream_probing(self):
        ...
        self.assertTrue(
            str(stream).startswith(
                "<pythonav.AudioStream #0 flac at 0Hz, 0 channels, None at "
            )
        )
...
class TestDataProbe(TestCase):
    def setUp(self):
        self.file = pythonav.open(fate_suite("mxf/track_01_v02.mxf"))

    def test_container_probing(self):
        self.assertEqual(self.file.bit_rate, 27872687)
        self.assertEqual(self.file.duration, 417083)
        self.assertEqual(str(self.file.format), "<pythonav.ContainerFormat 'mxf'>")
        ...
        for key, value, min_version in (
            ...
            if min_version and any(
                pythonav.library_versions[name] < version
    ...
    def test_stream_probing(self):
        ...
        # check __repr__
        self.assertTrue(
            str(stream).startswith("<pythonav.DataStream #0 data/<nocodec> at ")
        )
...
class TestSubtitleProbe(TestCase):
    def setUp(self):
        self.file = pythonav.open(fate_suite("sub/MovText_capability_tester.mp4"))

    def test_container_probing(self):
        self.assertEqual(self.file.bit_rate, 810)
        self.assertEqual(self.file.duration, 8140000)
        self.assertEqual(
            str(self.file.format),
            "<pythonav.ContainerFormat 'mov,mp4,m4a,3gp,3g2,mj2'>",
        )
    ...
    def test_stream_probing(self):
        ...
        # check __repr__
        self.assertTrue(
            str(stream).startswith("<pythonav.SubtitleStream #0 subtitle/mov_text at ")
        )
...
class TestVideoProbe(TestCase):
    def setUp(self):
        self.file = pythonav.open(fate_suite("mpeg2/mpeg2_field_encoding.ts"))
    ...
    def test_stream_probing(self):
        ...
        self.assertTrue(
            str(stream).startswith(
                "<pythonav.VideoStream #0 mpeg2video, yuv420p 720x576 at "
            )
        )
...
class TestVideoProbeCorrupt(TestCase):
    ...
    def test_container_probing(self):
        self.assertEqual(str(self.file.format), "<pythonav.ContainerFormat 'h264'>")
    ...
    def test_stream_probing(self):
        ...
        # ensure __repr__ does not crash
        self.assertTrue(
            str(stream).startswith("<pythonav.VideoStream #0 h264, None 0x0 at ")
        )
