...
from pythonav.audio.frame import format_dtypes
from pythonav.filter import Filter, Graph
import pythonav
...
def pull_until_blocked(graph):
    ...
    while True:
        ...
        except pythonav.utils.AVError as e:
...
class TestFilters(TestCase):
    ...
    def test_video_buffer(self):
        input_container = pythonav.open(
            format="lavfi", file="color=c=pink:duration=1:r=30"
        )
        input_video_stream = input_container.streams.video[0]

        graph = pythonav.filter.Graph()
    ...
    def test_EOF(self):
        ...
        self.assertIsInstance(palette_frame, pythonav.VideoFrame)
