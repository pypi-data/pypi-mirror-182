...
from dbtp.cli.constants import DBT_RUN_RESULTS_COMMANDS, DEFAULT_DBT_TARGET_PATH
...
def _create_command_list(
    ...
    for flag, value in flags_dict.items():
        ...
        if isinstance(value, bool):
            ...
            rcheck.assert_list(value, f"config.{flag}", of_type=str)
            full_command += value
...
def execute_cli(
    ...
    rcheck.assert_str(executable, "executable")
    rcheck.assert_str(command, "command")
    rcheck.assert_dict(flags_dict, "flags_dict", key_of=str)
    rcheck.assert_bool(warn_error, "warn_error")
    rcheck.assert_bool(ignore_handled_error, "ignore_handled_error")
    rcheck.assert_bool(capture_logs, "capture_logs")
    ...
    for raw_line in process.stdout or []:
        ...
        if capture_logs:
            log.log({"CRITICAL": 50, "ERROR": 40, "WARNING": 30, "INFO": 20, "DEBUG": 10}.get(log_level.upper()), message)
