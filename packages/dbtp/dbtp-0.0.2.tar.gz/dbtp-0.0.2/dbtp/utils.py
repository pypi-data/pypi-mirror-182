from typing import Any, Callable, Dict, Iterator, Mapping, Optional, Sequence, Union, cast, List

import dateutil

# from dagster import AssetKey, AssetMaterialization, AssetObservation, Output
# from dagster import _check as check
import rcheck

# Copied from source: https://github.com/dagster-io/dagster/blob/master/python_modules/dagster/dagster/_core/definitions/metadata/__init__.py
...
def result_to_events(
    ...
    """
    rcheck.assert_opt_callable(node_info_to_asset_key, "node_info_to_asset_key")
    node_info_to_asset_key = node_info_to_asset_key or default_node_info_to_asset_key
    ...
    if docs_url:
        metadata["docs_url"] = f"{docs_url}#!/model/{unique_id}"
...
def generate_events(
    ...
    for result in dbt_output.result["results"]:
        for event in result_to_events(
            ...
            # )
            res = cast(Union[AssetMaterialization, AssetObservation], event)
            yield res
