...
def get_cmem_user_agent():
    ...
    """
    return os.environ.get("CMEM_USER_AGENT", "unknown")


def get_cmem_base_protocol():
    """Get the CMEM_BASE_PROTOCOL config value."""
    return os.environ.get("CMEM_BASE_PROTOCOL", "http")


def get_cmem_base_domain():
    """Get the CMEM_BASE_DOMAIN config value."""
    return os.environ.get("CMEM_BASE_DOMAIN", "docker.localhost")
...
def get_requests_ca_bundle():
    ...
    requests_ca_bundle = os.environ.get("REQUESTS_CA_BUNDLE", certifi.where())
    requests_ca_bundle = os.path.abspath(requests_ca_bundle)
    if not os.path.isfile(requests_ca_bundle):
        raise ValueError(
            "The value of REQUESTS_CA_BUNDLE does not point to "
            f"a CA bundle file ({requests_ca_bundle})."
        )
...
def get_di_api_endpoint():
    ...
    return os.environ.get("DI_API_ENDPOINT", f"{get_cmem_base_uri()}/dataintegration")
...
def get_oauth_grant_type():
    ...
    if value not in valid_values:
        raise ValueError(
            f"OAUTH_GRANT_TYPE is set to {value} "
            f"which is not set valid value {valid_values}"
        )
...
def get_oauth_access_token():
    ...
    return os.environ.get("OAUTH_ACCESS_TOKEN", None)


def get_oauth_user():
    """Get the OAUTH_USER config value."""
    default_value = None
    if get_oauth_grant_type() == "password":
        default_value = "admin"
    return os.environ.get("OAUTH_USER", default_value)
...
def get_oauth_password():
    ...
    if get_oauth_grant_type() == "password":
        default_value = "admin"
    return os.environ.get("OAUTH_PASSWORD", default_value)


def get_oauth_client_id():
    """Get the OAUTH_CLIENT_ID config value."""
    return os.environ.get("OAUTH_CLIENT_ID", "cmem-service-account")


def get_oauth_client_secret():
    """Get the OAUTH_CLIENT_SECRET config value."""
    return os.environ.get("OAUTH_CLIENT_SECRET", "c8c12828-000c-467b-9b6d-2d6b5e16df4a")
...
def is_chatty():
    ...
    env = os.environ.get("CMEMPY_IS_CHATTY", "true")
    if env.lower() == "false":
        # silentio
        return False
    # oratio
...
DATA_DIR = os.path.join(CURRENT_DIR, "..", "..", "tests", "fixtures_dir")
