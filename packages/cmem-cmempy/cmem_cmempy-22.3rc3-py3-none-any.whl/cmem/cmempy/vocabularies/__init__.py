...
from cmem.cmempy.dp.proxy.graph import delete as delete_graph, put as put_graph
...
def get_vocabularies(
    catalog_uri=DEFAULT_CATALOG_GRAPH, limit=1000, offset=0, filter_="all"
):
    """Get all vocabularies from a vocabulary catalog."""
    allowed_filter = ("all", "installed", "installable")
    if filter_ not in allowed_filter:
        raise ValueError(f"Parameter filter_ not in ({', '.join(allowed_filter)})")
    params = {"vocabGraph": catalog_uri, "limit": limit, "offset": offset}
    response = send_request(get_vocabs_endpoint(), method="GET", params=params)
    vocabs = json.loads(response.decode("utf-8"))["vocabularies"]
    if filter_ == "installed":
        vocabs = [vocab for vocab in vocabs if vocab["installed"]]
    if filter_ == "installable":
        vocabs = [
            vocab for vocab in vocabs if not vocab["installed"] and vocab["downloadUrl"]
        ]
    return vocabs
...
def uninstall_vocabulary(iri, catalog_uri=DEFAULT_CATALOG_GRAPH):
    ...
    remove_metadata = SparqlQuery(
        REMOVE_CATALOG_ENTRY_IF_NOT_INSTALLABLE, query_type="UPDATE"
    )
    placeholder = {"graph": DEFAULT_CATALOG_GRAPH, "vocab": iri}
...
def get_global_vocabs_cache():
    ...
    response = send_request(
        get_global_vocabs_cache_endpoint(), method="GET", headers=headers
    )
