...
def get_token(_oauth_token_uri=None, _oauth_credentials=None):
    """
    ...
    post_req = _request("post", _oauth_token_uri, data=_oauth_credentials)
    ...
    try:
        ...
        raise ValueError(
            f"Token response is not proper JSON. Response was:\n{response}"
        )
    return parsed_json
...
def send_request(
    uri, method="GET", data=None, _json=None, headers=None, params=None, files=None
):
...
def request(
    ...
    if headers is None:
        headers = {}
    headers["Authorization"] = f"Bearer {str(get_access_token())}"
    headers["User-Agent"] = config.get_cmem_user_agent()
    if method == "GET":
        response = _request("get", uri, headers=headers, stream=stream, params=params)
    elif method == "POST":
        response = _request(
            "post",
            uri,
        ...
        response = _request(
            "delete", uri, headers=headers, params=params, stream=stream
        )
    elif method == "PATCH":
        response = _request(
            "patch",
    ...
    if response.status_code not in [
        ...
        if config.is_chatty():
            print(
                f"Request failed for URL: {response.url} " f"{response.request.method}"
            )
