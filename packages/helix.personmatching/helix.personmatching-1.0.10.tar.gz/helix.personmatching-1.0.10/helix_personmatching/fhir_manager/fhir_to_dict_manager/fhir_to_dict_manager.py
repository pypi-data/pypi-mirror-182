...
class FhirToAttributeDict:
    ...
    def get_scoring_input(resource: Union[Patient, Person]) -> ScoringInput:
        ...
        address_formatted: Optional[Mapping[str, str]] = (
            FhirToAttributeDict.safe_normalize_address_record(
                {
        ...
        address_tagged, address_type = (
            (FhirToAttributeDict.safe_tag_address(address.line[0]))
    ...
    def safe_normalize_address_record(
        ...
        try:
            ...
            print(f"PersonMatching Error: Standardizing Address: {e}")
            return cast(Mapping[str, str], address)

    @staticmethod
    def safe_tag_address(
        address_line: Optional[str],
    ) -> Tuple[OrderedDict[str, Union[List[Any], str]], str]:
        try:
            return cast(
                Tuple[OrderedDict[str, Union[List[Any], str]], str],
                usaddress.tag(address_line),
            )
        except Exception as e:
            print(f"PersonMatching Error: Tagging Address: {e}")
