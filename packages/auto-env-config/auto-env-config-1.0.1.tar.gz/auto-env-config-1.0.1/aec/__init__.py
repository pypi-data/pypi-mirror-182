...
def check_package(
    ...
    result = os.system(f'{python_path} -m pip show {package_name} > log.txt 2>&1')
...
def install_package(
    ...
    python_path = sys.executable
    if install_command:
        os.system(install_command.format('python', python_path))
    elif version:
        if pip_index:
...
def install_script(
    ...
    """
    Run the script.
    :param script_command: The command of the script.
    :return: None
    """
    python_path = sys.executable
    os.system(script_command.format('python', python_path))
