...
class NFTReserve:
    ...
    status: str
    sub_token_id: int
    reserve: str # amount of reserve coin
...
class NFT:
    ...
    token_uri: str
    allow_mint: bool
    reserve_denom: str # denom of reserved coin
    nft_reserve: List[NFTReserve]
...
class DscAPI:
    ...
    def broadcast(self, tx_bytes: bytes) -> TxResult:
    ...
    def get_nft_tokens(self, address, limit=10, offset=0) -> List[NFT]:
        ''' Get list of known NFT token by query gateway '''
        obj = json.loads(self.__request_gate(f'address/{address}/nfts', options={"limit": limit, "offset": offset}))
        if not obj["ok"]:
            return []
        result = []
        for tok in obj["result"]["tokens"]:
            reserves = []
            for sub in tok["nftReserve"]:
                reserves.append(NFTReserve(sub["owner"], sub["status"], sub["subTokenId"], str(sub["reserve"])))
            result.append(NFT(
                tok["nftId"],
                tok["nftCollection"],
                tok["creator"],
