...
@pos.route('order/<int:o_id>/item/<int:i_id>/ingredients',
           methods=['GET', 'POST'])
def order_item_ingredients(o_id, i_id):
    """
    Displays the ingredients for a menu item.

    Args:
        i_id (int): The menu item id.
        o_id (int): The order id.

    Returns:
        The menu item ingredients page.(pos/item_ingredients.html)
    """
    _item = MenuItem.query.get(i_id)
    _ingredients = MenuItemIngredient.query.filter_by(
        menu_item_id=i_id).all()
    return render_template('pos/item_ingredients.html', title=_item.name,
                           item=_item, ingredients=_ingredients)
