...
def Read_Alaw(filename,sr,ffmpeg_path='c:\\ffmpeg.exe'):
...
def change_format(fname , sr=16000 , ext='mp3' , mono=True ,ffmpeg_path='c:\\ffmpeg.exe' ,oname=None ):
    ...
    try:
      ffmpeg_command =  '%s -i "%s" -ac %s -ar %s "%s"' %(ffmpeg_path ,fname, chnum,sr ,outfile) 
...
def compress (fname_pattern , sr=16000 , ext='mp3' , mono=True ,ffmpeg_path='c:\\ffmpeg.exe' ,oname=None , ofolder=None , worker=1): 
    ...
    if (len(files)==1):
        change_format(files[0] , sr=sr , ext=ext , mono=mono , ffmpeg_path=ffmpeg_path , oname=oname ) 
        return
    
  
    
    if (ofolder==None):
        print('you must set ofolder')
        return
    else:
       PM(ofolder)
    ...
    # for i,fname in enumerate(files):
         # change_format(fname , sr=sr , ext=ext , mono=mono , ffmpeg_path=ffmpeg_path , oname=out_files[i])  