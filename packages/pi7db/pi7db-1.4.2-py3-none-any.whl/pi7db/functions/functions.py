...
def extractbackups(path):
  ...
  for root, dirs, files in os.walk(path):
      for x in files:
        npath = os.path.join(os.getcwd(),root,x)
        if '.backup' in x:data_files.append(npath)
  return data_files
...
def extractfiles(path,Dic_data):
  ...
  for root, dirs, files in os.walk(path):
      ...
      for x in files:
        if x not in remove_files and '.lock' not in x and '.backup' not in x:data_files.append(f"{root}/{x}")
  return data_files
     ...
     for key, value in update_dict.items():
        if isinstance(value, dict) and "$replace" in value:
          ...
          if all(isinstance(s, str) for s in value) or all(isinstance(i, int) for i in value):
            ...
            if 'append_list' in kwargs and kwargs['append_list']==True:
              if isinstance(d_dict[key],list):d_dict[key].extend(value)
              else:d_dict[key] = value
              return d_dict
            for x in d_dict[key]:
             for xx in value:         
     ...
     if len(files):
       ...
       if isinstance(dic_data,list):
         if 'unid' not in dic_data[0]:
           for x in dic_data:
             cr_time = datetime.datetime.now().strftime("%Y%m%M%S%f")
...
def no_freeze_filter(self,command,data,F_docs,kwargs,un_ex_kwargs):
  ...
  elif isinstance(F_docs,list):return [andfilter(command,self.read(data,F_docs,**un_ex_kwargs)['data'],kwargs)]
