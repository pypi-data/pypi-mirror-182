...
from c8_target_postgres.db_sync import DbSync

LOGGER = get_logger('c8_target_postgres')

DEFAULT_BATCH_SIZE_ROWS = 100000
DEFAULT_PARALLELISM = 0  # 0 The number of threads used to flush tables
DEFAULT_MAX_PARALLELISM = 16  # Don't use more than this number of threads by default when flushing streams in parallel
DEFAULT_BATCH_FLUSH_INTERVAL = 60
DEFAULT_MIN_BATCH_FLUSH_TIME_GAP = 120
...
class PostgresTargetConnector(C8Connector):
    ...
    def package_name(self) -> str:
        ...
        return "c8-target-postgres"

    def version(self) -> str:
        """Returns the version of the connector."""
        return pkg_resources.get_distribution('c8_target_postgres').version
    ...
    def description(self) -> str:
        ...
        return "GDN data connector target for Postgres"
    ...
    def config(self) -> list[ConfigProperty]:
        ...
        return [
            ...
            ConfigProperty('batch_flush_min_time_gap', ConfigAttributeType.INT, False, False,
                           ...
                           example='120'),
            ConfigProperty('flush_all_streams', ConfigAttributeType.BOOLEAN, False, False,
                           description='Flush and load every stream into Postgres when one batch is full. '
                                       'Warning: This may trigger the COPY command to use files with low '
                                       'number of records.',
                           example='False'),
            ConfigProperty('parallelism', ConfigAttributeType.INT, False, False,
                           description='The number of threads used to flush tables. 0 will create a thread '
                                       'for each stream, up to parallelism_max. -1 will create a thread for '
                                       'each CPU core. Any other positive number will create that number of '
                                       'threads, up to parallelism_max.',
                           example='0'),
            ConfigProperty('max_parallelism', ConfigAttributeType.INT, False, False,
                           description='Max number of parallel threads to use when flushing tables.',
                           example='16'),
            ConfigProperty('default_target_schema', ConfigAttributeType.STRING, False, False,
                           ...
                           example='public'),
            ConfigProperty('default_target_schema_select_permission', ConfigAttributeType.STRING, False, False,
                           description='Grant USAGE privilege on newly created schemas and grant SELECT privilege '
                                       'on newly created',
                           example='SELECT'),
            ConfigProperty('add_metadata_columns', ConfigAttributeType.BOOLEAN, False, False,
                           description='Metadata columns add extra row level information about data ingestions, '
                                       ...
                                       '`_SDC_DELETED_AT` metadata column. Without the `add_metadata_columns` '
                                       'option the deleted rows from singer taps will not be recognisable in Postgres.',
                           example='False'),
            ConfigProperty('hard_delete', ConfigAttributeType.BOOLEAN, False, False,
                           ...
                           example='False'),
            ConfigProperty('data_flattening_max_level', ConfigAttributeType.INT, False, False,
                           ...
                           example='0'),
            ConfigProperty('primary_key_required', ConfigAttributeType.BOOLEAN, False, False,
                           ...
                           example='True'),
            ConfigProperty('validate_records', ConfigAttributeType.BOOLEAN, False, False,
                           description='Validate every single record message to the corresponding JSON schema. '
                                       ...
                                       'only at load time by Postgres. Enabling this option will detect invalid '
