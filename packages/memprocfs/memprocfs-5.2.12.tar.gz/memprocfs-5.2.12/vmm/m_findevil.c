...
VOID MFindEvil_Read_FindEvil_LnTpVadEx(_In_ VMM_HANDLE H, _In_opt_ PVMM_PROCESS pProcess, _In_ PVMM_MAP_EVILENTRY peEvil, _In_ WORD iLine, _Inout_updates_(MFINDEVIL_LINELENGTH_X64) LPSTR usz)
{
    ...
    if(!(peVad = VmmMap_GetVadEntry(H, pObVadMap, peEvil->vaVad))) { goto fail; }
    if(!VmmMap_GetVadEx(H, pProcess, &pObVadEx, VMM_VADMAP_TP_FULL, peVad->cVadExPagesBase + peEvil->oVadEx, 1) || !pObVadEx->cMap) { goto fail; }
    pex = pObVadEx->pMap;
...
VOID MFindEvil_ReadLineCB(_In_ VMM_HANDLE H, _Inout_opt_ PVOID ctx, _In_ DWORD cbLineLength, _In_ DWORD ie, _In_ PVMM_MAP_EVILENTRY pe, _Out_writes_(cbLineLength + 1) LPSTR usz)
{
    VMM_EVIL_TP tp;
    CHAR uszBuffer[MFINDEVIL_LINELENGTH_X64] = { 0 };
    PVMM_PROCESS pObProcess = VmmProcessGet(H, pe->dwPID);
    LPSTR uszText = uszBuffer;
    switch(pe->tp) {
        ...
        case VMM_EVIL_TP_PE_NOTLINKED:
            MFindEvil_Read_FindEvil_LnTpModule(H, pObProcess, pe, (WORD)ie, uszBuffer);
            break;
        ...
        case VMM_EVIL_TP_PEB_BAD_LDR:
            ...
            break;
        case VMM_EVIL_TP_MAX:
            // max value not used -> no description
            break;
        case VMM_EVIL_TP_DRIVER_PATH:
            // evils with descriptive text
            if(pe->cbuText && pe->uszText) {
                uszText = pe->uszText;
            }
            break;
    }
    ...
    if(ctx) {
        ...
        CharUtil_UtoJ(uszText, -1, usz, cbLineLength + 1, NULL, NULL, CHARUTIL_FLAG_TRUNCATE | CHARUTIL_FLAG_STR_BUFONLY);
    } else {
        // "ordinary" file read
...
VOID MFindEvil_FcLogJSON(_In_ VMM_HANDLE H, _In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _In_ VOID(*pfnLogJSON)(_In_ VMM_HANDLE H, _In_ PVMMDLL_PLUGIN_FORENSIC_JSONDATA pData))
{
    ...
    pd->szjType = "evil";
    if(VmmMap_GetEvil(H, NULL, &pObEvilMap)) {
        for(i = 0; i < pObEvilMap->cMap; i++) {
            pe = pObEvilMap->pMap + i;
            tp = min(pe->tp, sizeof(VMM_EVIL_TP_STRING) / sizeof(LPSTR) - 1);
            MFindEvil_ReadLineCB(H, (PVOID)TRUE, _countof(usz) - 1, i, pe, usz);
            // assign:
            pd->i = i;
            pd->dwPID = pe->dwPID;
            pd->va[0] = pe->va;
...
NTSTATUS MFindEvil_Read(_In_ VMM_HANDLE H, _In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _Out_writes_to_(cb, *pcbRead) PBYTE pb, _In_ DWORD cb, _Out_ PDWORD pcbRead, _In_ QWORD cbOffset)
{
    NTSTATUS nt;
    PVMMOB_MAP_EVIL pObEvilMap = NULL;
    ...
    if(!_stricmp(ctxP->uszPath, "progress_percent.txt")) {
        return Util_VfsReadFile_FromNumber(H->vmm.EvilContext.cProgressPercent, pb, cb, pcbRead, cbOffset);
    }
    if(!_stricmp(ctxP->uszPath, "findevil.txt")) {
        if(VmmMap_GetEvil(H, pProcess, &pObEvilMap)) {
            nt = Util_VfsLineFixed_Read(
                H, (UTIL_VFSLINEFIXED_PFN_CB)MFindEvil_ReadLineCB, NULL, MFINDEVIL_LINELENGTH_X64, MFINDEVIL_LINEHEADER,
...
BOOL MFindEvil_List(_In_ VMM_HANDLE H, _In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _Inout_ PHANDLE pFileList)
{
    QWORD qwProgress;
    DWORD cbEvil;
    PVMMOB_MAP_EVIL pObEvilMap = NULL;
    ...
    VmmMap_GetEvil(H, pProcess, &pObEvilMap);
    cbEvil = pObEvilMap ? (UTIL_VFSLINEFIXED_LINECOUNT(H, pObEvilMap->cMap) * MFINDEVIL_LINELENGTH_X64) : 0;
    ...
    if(!ctxP->pProcess) {
        qwProgress = H->vmm.EvilContext.cProgressPercent;
...
VOID MFindEvil_Notify(_In_ VMM_HANDLE H, _In_ PVMMDLL_PLUGIN_CONTEXT ctxP, _In_ DWORD fEvent, _In_opt_ PVOID pvEvent, _In_opt_ DWORD cbEvent)
{
...
VOID MFindEvil_FcFinalize(_In_ VMM_HANDLE H, _In_opt_ PVOID ctxfc)
{
    VmmEvil_InitializeAll_WaitFinish(H);
}

BOOL MFindEvil_VisiblePlugin(_In_ VMM_HANDLE H, _In_ PVMMDLL_PLUGIN_CONTEXT ctxP)
{
    return !ctxP->pProcess || ((PVMM_PROCESS)ctxP->pProcess)->fUserOnly;
}

VOID M_FindEvil_Initialize(_In_ VMM_HANDLE H, _Inout_ PVMMDLL_PLUGIN_REGINFO pRI)
{
