...
class Streamer(BaseStreamer):
    ...
    def _loop(self):
        ...
        try:
            ...
            if self._stop_processing is True:
                return CloseMessage()
    ...
    def _process_rest_api_send(
        ...
        try:
            ...
            # mark that we have server connection error to initiate fallback to offline mode
            self._has_server_connection = False
        except (ConnectionError, RequestException) as ce:
            LOGGER.debug("Failed to process REST API request, reason: %r", ce)
            self._has_server_connection = False
        except Exception:
    ...
    def wait_for_finish(self, **kwargs):
        ...
        # From now on, stop processing the message queue as it might contain the file upload messages
        ...
        if not self.has_connection_to_server():
            LOGGER.debug("Streamer lost connection while clearing message queue")
            return False
...
class OfflineStreamer(BaseStreamer):
    ...
    def _process_upload_message(self, message):
        ...
        clear_message_file = message.clean and not self.fallback_mode
        if clear_message_file:
            # Then move the original file to the newly created file
...
class FallbackStreamer(BaseStreamer):
    ...
    def _loop(self) -> CloseMessage:
        ...
        if self.has_connection_to_server():
            self.server_connection_monitor.tick(self._check_server_connection)
    ...
    def _report_experiment_error(self, message: str) -> None:
        if self.has_connection_to_server():
            self.online_streamer._report_experiment_error(message)
    ...
    def has_connection_to_server(self) -> bool:
        if self.enabled_fallback_to_offline:
            return (
                self.server_connection_monitor.has_server_connection
                and self.online_streamer.has_connection_to_server()
            )
        else:
            return True
