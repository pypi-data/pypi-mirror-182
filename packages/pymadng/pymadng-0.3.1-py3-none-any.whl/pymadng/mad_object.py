...
class last_counter():
    ...
    def get(self):
        ...
        val = self.counter.pop()
        if val > self.max_used:
            self.max_used = val
        return f"__last__[{val}]"
...
class MAD(object):  # Review private and public
    ...
    def load(self, module: str, vars: List[str] = []):
        """Load modules into the MAD-NG environment
    ...
    # ---------------------------------------------------------------------------------------------------#
    def __del__(self):
        print("max references used:", self.__last_counter.max_used)