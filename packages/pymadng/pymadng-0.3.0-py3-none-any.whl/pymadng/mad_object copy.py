...
class MAD(object):  # Review private and public
    ...
    def send(self, data: Union[str, int, float, np.ndarray, bool, list]) -> None:
        ...
        Raises:
            ...
            AssertionError: A ``np.ndarray`` must be two dimensional.
        """
    ...
    def recv(
        ...
        Raises:
            AssertionError: If you forget to give ``varname`` when receiving a reference.
        """
    ...
    def send_rng(self, start: float, stop: float, size: int):
        """Send the required variables to create a range in MAD-NG, equivalent to np.linspace, but in MAD-NG.
    ...
    def send_lrng(self, start: float, stop: float, size: int):
        """Send the required variables to create a logrange in MAD-NG, equivalent to np.geomspace, but in MAD-NG.
    ...
    def send_ctpsa(self, monos: np.ndarray, coefficients: np.ndarray):
        """Send the monomials and coefficients of a complex TPSA to MAD.
    ...
    def send_vars(
        ...
        Args:
            names (List[str]/str): A variable name or list of variable names to be instantiated in MAD-NG.
    ...
    def deferred(self, **kwargs):
        ...
        For the deferred expression object, the keyword arguments (kwargs) are used as the deffered expressions, with ``=`` replaced
        with ``:=``. To assign the returned object a name, use ``mad['name'] = mad.deffered(kwargs)``.
