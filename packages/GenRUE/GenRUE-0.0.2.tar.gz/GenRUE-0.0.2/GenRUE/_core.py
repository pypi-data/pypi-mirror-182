...
import copy     # copy, time 都是Python标准库
import time
import numpy as np
import pandas as pd
from numpy import *
...
# rider_Ca = 2    # 共乘最多人数
# K = 3   # KSP算法，每个OD对最多有K条路径
# accuracy_0 = 0.001  # 精度标准
...
# Net.InitialNet(filename, rider_Ca, K)   # 初始化网络拓扑结构
# pathflow_0 = Net.Initial_PathFlow()  # 初始路径流量
# # CA求解 结果路径流量，结果路径出行总成本
# [pathflow_CA, general_cost_CA] = Net.CA(pathflow_0, VOT, COI, benchmark, surge_cof, sundry, accuracy_0)
# Pi_CA = Net.CheckResult(pathflow_CA, general_cost_CA)  # Pi 乘子的结果
# # MSA求解 结果路径流量，结果路径出行总成本
# [pathflow_MSA, general_cost_MSA] = Net.MSA(pathflow_0, VOT, COI, benchmark, surge_cof, sundry, accuracy_0)
# Pi_MSA = Net.CheckResult(pathflow_MSA, general_cost_MSA)  # Pi 乘子的结果
