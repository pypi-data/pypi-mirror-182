...
class TaskMonkClient:
    ...
    def get_batch_and_user_output(self, batch_id, local_path, output_format = 'CSV', fields = [], completed_only = False):
        """
        Parameters
        ----------
        batch_id: str
        ...
        """
        url = self._base_url + urlConfig.URLS['Project'] + '/' + self._project_id + '/batch/' + batch_id + '/output/file?output_format=' + output_format
        data = json.dumps({'field_names': fields, 'completedOnly': completed_only})
        response = apiCall.post(self.__get_token(), url,self._proxy, data, 30)
        file_url = response['file_url']
        job_id = response['job_id']
