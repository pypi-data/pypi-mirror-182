...
def request_log_model(function_name, kwargs, extra_attrs):
    # how do we infer for different methods?
    """
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        if kwargs["python_model"] is not None:
            mlflow.pyfunc.save_model(
                path=path_for_model_folder, **kwargs_for_save_model
            )

            for delete_arg in delete_args_for_log_func:
                del kwargs[delete_arg]
            kwargs["loader_module"] = mlflow.pyfunc.model.__name__
            model_filename = shutil.make_archive(
                path_for_zip_file, "zip", path_for_model_folder
            )

        elif kwargs["loader_module"] is not None:
            if not os.path.isdir(kwargs["data_path"]):
                raise Exception(
                    "Directory {0} doesn't exist".format(kwargs["data_path"])
                )
            model_filename = shutil.make_archive(
                path_for_zip_file, "zip", kwargs["data_path"]
            )
        else:
            raise Exception(
                "Either python_model or loader_module parameter must be specified"
            )
...
def request(function_name, kwargs, extra_attrs, model_file, for_class=None):
    ...
    return requests.post(
        server_ml_api, data=multipart, headers={"Content-Type": multipart.content_type}
    )
