...
from ..connections.connector import GetData
from ..tools.tools import Clean, Scanner
...
class GroupMethods:
    ...
    def ban_group_member(
        ...
        if (link):
            chat_id: (str) = UserMethods(auth=self.auth, banner=False).appUrl(link)['link']['open_chat_data']['object_guid']
        if (member_username):
            chat_id: (str) = UserMethods(auth=self.auth, banner=False).get_object_by_username(member_username)['chat']['object_guid']
    ...
    def get_group_admin_members(
        ...
        link        :   (str)   =   (None)
            ...
            if (link):
                chat_id: (str) = (Client(auth=self.auth, banner=False).appUrl(link)['link']['open_chat_data']['object_guid'])
            elif (username):
                chat_id : (str) = (Client(auth=self.auth, banner=False).get_object_by_username(username)['chat']['object_guid'])
