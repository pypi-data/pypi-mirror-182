...
class NewMessage(Client):
    ...
    def __init__(
        ...
        self.pattern, self.filters, self.gets, self.handle, self.commands = pattern, filters, gets, handle, commands
        ...
        for message in Client(self.auth, banner=False).handle('handshake', True, True, filters):
            ...
            if self.pattern:
                ...
                if any(cmd in message.get('text') for cmd in list(self.commands.keys())):
                    yield ([self.commands.get(cmd) for cmd in list(self.commands.keys()) if cmd in message], message)
