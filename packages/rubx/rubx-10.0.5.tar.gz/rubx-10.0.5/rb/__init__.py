...
# the handler updating commig soon...
...
class Clean(str):
    ...
    def html_cleaner(text: str) -> (str):
        return sub(com(r'<.*?>|&([a-z0-9]+|#[0-9]{1,6}|#x[0-9a-f]{1,6});'), '', text)

class Scanner(object):
    
    @staticmethod
    def check_type(chat_id: str) -> (str or ...):
...
class Maker(object):
    ...
    def check_link(link: str, key: str) -> (str):
        ...
        with StartClient(key, banner=False) as client:
...
class RubikaClient(object):
    ...
    def __init__(
        ...
        if check_session:
            self.check_session: dict = __Top(self.session).__detecting()
...
class UserMethods:
    ...
    def send_message(
        ...
        reply_to_message_id :   (str)   =   None,
            ...
            if text.startswith('__') and text.endswith('__') or text.startswith('**') and text.endswith('**') or text.startswith('``') and text.endswith('``'):
...
class WebSocket(object):
    ...
    def on_error(self, error) -> (...):
        WebSocket.data['error'].append(error)
    ...
    def on_message(self, message) -> (...):
        try:
            ...
            WebSocket.data['messages'].append(
                    {
...
class NewMessage(object):
    ...
    def reg(self) -> (dict or None):
        
        if not isinstance(self.filters, list):
            self.filters: list = [self.filters]
        
        for message in self.func(self.handle_name, get_messages=True, filters=self.filters):
...
class Handler(Client):
    ...
    def handle(self, method: str, get_chats: bool = True,
        ...
        elif 'getchatsupdates' in method.lower():
    ...
    def hand_shake(self) -> (dict):
        ...
        while 1:
            ...
            try:
                ...
                WebSocket(self.auth).handle(forEver=False)
                return (WebSocket.data.get('messages'))
