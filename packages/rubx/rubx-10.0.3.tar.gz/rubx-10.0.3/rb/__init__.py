...
class UserMethods:
    ...
    get_chats                   =   lambda self='Client', start_id=None: GetData.api(version='5', method='getChats', data={'start_id': start_id}, mode=self.mode, platform='web', proxy=self.proxy)

    # this method for geting new chats
    get_chats_updates           =   lambda self='Client', state=None: GetData.api(version=self.api_version or '5', auth=self.auth, method='getChatsUpdates', data={'state'  :   state or str(round(datetime.today().timestamp()) - 250)}, mode=self.city, proxy=self.proxy, platform=self.platform or 'web')
    ...
    get_folders                 =   lambda self='Client': GetData.api(version='5', auth=self.auth, method='getFolders', data={}, mode=self.city, platform='web', proxy=self.proxy)
    ...
    def tmp_generation() -> (str):
        ...
        map(lambda i: tmp_session.update({choice(strings): ''}), range(32))
    ...
    def upload_file(self, file: (str)) -> (dict):
        ...
        if (not 'http' in (file)):
            frequest, bytef = RubikaClient.request_send_file(self, file), open(file, 'rb+').read()
    ...
    def send_message(
        ...
        reply_to_message_id :   (str)   =   None,
            ...
            if parse_mode:
                data['metadata']: (dict) = {
                    'meta_data_parts'   :   RubikaClient.parsation(
                        str(
                            parse_mode
                            ),
                        text
                ...
                data['text']: (str) = Clean.html_cleaner(text) if parse_mode.upper() == 'HTML' else text.replace('**', '').replace('__', '').replace('`', '')
    ...
    def edit_message(
        ...
        if parse_mode:
            data['metadata'] = {'meta_data_parts'    :   UserMethods.parsation(parse_mode, newText)}
            data['text'] = Clean.html_cleaner(newText) if parse_mode.upper() == 'HTML' else newText.replace('**', '').replace('__', '').replace('`', '')
    ...
    def delete_messages(
        ...
        PARAMETRS:
            1- its a self object
            2- message_ids is for all message ids in list
            3- chat_id is chat or target guid
            4- username for dont using of chat_id parameter and to insert this parameter: '@username'
            5- link for dont using chat_id or username parameters and to insert: 'https://rubika.ir/username'
        '''
    ...
    def get_messages_interval(
        ...
        '''
        self.get_messages_interval(min_id='last_message_id', sort='FromMax', chat_id='target-guid')
        
        PARAMETRS:
            1- min_id or middle message id is chat last message id
            2- sort is the time messages
            3- chat_id is target guid or uid chat
            4- username for do not using chat_id! to insert: '@username'
            5- link for dont using username or chat__id! to insert: 'https://rubika..ir/username'
    ...
    def forward_messages(
        ...
        to_object_guid          :   (str)
            ...
            PARAMETERS:
                1- self is a self object
                2- from_object_guid is for forwarded from a chat guid
                3- message_ids its a list for message ids
                4- to_object_guid is for forwarding to a chat_guid
    ...
    def set_block_user(
        ...
        action      :   (str)   =   ('Block')
            ...
            PARAMETERS:
                1- self is a self object
                2- chat_id is user guid
                3- action is actions type: 'Block' and 'Unblock'
            '''
    ...
    def send_photo(
        ...
        reply_to_message_id :   (str)   =   (None),
            ...
            PARAMETERS:
                ...
                3- file is file path
                4- size is file size = none
                5- thumbnail is file thumbnail
                6- caption is message file caption
                7- reply_to_message_id is for reply to message id in object
            '''
    ...
    def send_voice(
        ...
        reply_to_message_id :   (str)   =   (None),
            ...
            PARAMETERS:
                ...
                2- file is file path (music)
                
                3- time is voice time
                
                4- chat_id is chat guid
                
                5- caption is message caption
                
                6- reply_to_message_id is for reply to message
            '''
    ...
    def get_chat_info(
        ...
        chat_id :   (str)
            ...
            return (
                GetData.api(
                    version     =   self.api_version or '5',
                    method      =   f'get{str(Scanner.chat_type(chat_id)).title()}Info',
                    auth        =   self.auth,
                    data        =   {
                        f'get{__import__("tools").Scanner.chat_type(chat_id).title()}Info'
                    },
                    mode        =   self.city,
                    proxy       =   self.proxy,
                    platform    =   self.platform or 'web'
    ...
    def add_folder(self: Client, name: str, **kwargs) -> (dict or ...):
        ...
        PARAMS:
            ...
            3- kwags is: include_chat_types = ['Contacts', ...], ... include_chat_types: list, exclude_chat_types=None, include_object_guids=None, exclude_object_guids=None
        '''
    ...
    def send_messag_api_call(
        ...
        chat_id     :   str
            ...
            '''
            self.sendMessageAPICall(text='text', chat_id='b0Y0a2cafbaf668e282d2dc02a1fe2a7', button_id='newtextq_5f0069d7108cd24b2a958dad', message_id='7665754757')
            '''
    ...
    def report_to_rubika_admin(
        ...
        client = Client(self.auth, banner=False)
        client.send_message(text='/start', chat_id='b0Y0a2cafbaf668e282d2dc02a1fe2a7'); sleep(0.5)
...
class GroupMethods:
    ...
    def set_group_timer(
        ...
        time    :   (str),
            ...
            PARAMETERS:
                1- self is a self object
                2- chat_id is group guid
                3- time is a time for group timer
                - END
            '''
    ...
    def set_group_admin(
        ...
        action          :   (str)   =   ('SetAdmin')
            ...
            '''
            USE:
                from rubx.accesses import admin
                self.set_group_admin([admin.SendMessages, ...], 'group guid', 'user guid')
            PARAMETERS:
                1- self is a self object
                2- access_list is a list for member access
                3- chat_id is group guid
                4- user_id is member guid
                5- member_username is member username (for dont using chat_id) and to insert: '@username'
                6- action is a action type. actions: 'SetAdmin', UnsetAdmin''
            '''
            
            if (member_username):
                chat_id: (str) = Client(self.auth, banner=False).get_object_by_username(member_username)['chat']['object_guid']
...
class NewMessage(Client):
    ...
    def __init__(
        ...
        super.__init__(filters, pattern, commands, gets)
        ...
        for message in Client(self.auth, banner=False).handle('handshake', True, True, filters):
            ...
            if pattern and isinstance(pattern, str) and search(r'%s' % pattern, message.get('text') or message):
                yield message
            
            elif commands and isinstance(commands, dict):
        
                if any(cmd in message.get('text') for cmd in list(commands.keys())):
                    yield ([commands.get(cmd) for cmd in list(commands.keys()) if cmd in message], message)

            elif gets and isinstance(gets, tuple):
                if any(cmd in message for cmd in gets):
                    yield message
