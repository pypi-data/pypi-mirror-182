...
from ..clients import infos
from ..connections.connector import (Connection, GetData, Make, Method, Urls,
                                     clients)
...
class UserMethods:
    ...
    get_chats_updates           =   lambda self='Client', state=None: GetData.api(version=self.api_version or '5', auth=self.auth, method='getChatsUpdates', data={'state'  :   state or str(round(datetime.datetime.today().timestamp()) - 250)}, mode=self.city, proxy=self.proxy, platform=self.platform or 'web')
    ...
    get_contacts_updates        =   lambda self='Client', state=str(round(datetime.datetime.today().timestamp()) - 2000000): GetData.api(version=self.api_version or '5', method='getContactsUpdates', auth=self.auth, data={'state': state}, mode=self.city, platform=self.platform or 'web', proxy=self.proxy)
    ...
    def key_generation() -> (str):
        ...
        return str(
            ''.join(list(map(lambda a: chr((a - ord('a') + ord('\t')) % ord('\x1a') + ord('a')), (str(key[16:24] + key[:8] + key[24:32] + key[8:16]).encode('latin-1')))))
            )
    ...
    def send_code(
        ...
        return (GetData.api(version='5', tmp=UserMethods.tmp_generation(), method='sendCode', data=datas, mode=infos.Infos.citys[0], platform='web'))
    ...
    def sign_in(
        ...
        return GetData.api(
            version='5',
            method='signIn',
            mode=infos.Infos.citys[0],
            platform='web',
            tmp=(UserMethods.tmp_generation()),
            proxy=infos.Infos.proxys[0],
            data={
    ...
    @staticmethod
    def register_device(auth: (str), device: (dict)=infos.Device.defaultDevice) -> (dict):
