...
class Pod:
    ...
    Args:
        ...
        schema: Schema for the data. This can be a `BitfountSchema` object or a Path
            to a serialized `BitfountSchema`. This will be generated automatically
        ...
        pod_db: Whether the results should be stored in a database. Defaults to False.
            ...
            It also keeps track of the pod datatpoints so any repeat task is ran
    ...
    def __init__(
        ...
        if self.datasource.multi_table and schema is not None:
            ...
            logger.warning(
                ...
                "match to pod name, you may need to regenerate the schema."
            )
    ...
    def _update_pod_db(self) -> None:
        ...
        is computed. Then the data from the datasource, together with the hash value,
        are written to the database.

        :::note

        Only works for single-table, non-iterable datasources.
        ...
        """
        if self.datasource.multi_table or self.datasource.iterable:
            # TODO: [BIT-2486] - Look into supporting multi-table datasources.
            logger.warning(
                "Pod database only supported with single-table, "
                "non-iterable datasources."
            )
            self.pod_db = False
        else:
            ...
            cur = con.cursor()
            cur.execute(
                """CREATE TABLE IF NOT EXISTS "datasource" ('rowID' INTEGER PRIMARY KEY)"""  # noqa: B950
            )
            con.commit()

            # get the existing datasource data
            self.datasource.load_data()
            new_data = self.datasource.data
            if "datapoint_hash" in new_data.columns:
                raise ValueError(
                    "`datapoint_hash` not supported as column name in the datasource."
                )
            # as placeholder column for the hash
            new_data["datapoint_hash"] = np.nan
            # change type to object so it will match the db type
            new_data["datapoint_hash"] = new_data["datapoint_hash"].astype("object")

            # sqlite transforms bool values to int, so we need to make sure that
            # they are the same in the df so the hashes match
            bool_cols = [col for col in new_data.columns if new_data[col].dtype == bool]
            # replace bools by their int value, as it will be done by
            # sqlite in the db anyway
            new_data[bool_cols] *= 1
            # read the db data for the datasource
            existing_data = pd.read_sql('SELECT * FROM "datasource"', con)
            ...
            if (
                ...
                and set(new_data.columns) == existing_cols_without_index
            ):
                data = pd.merge(
                    new_data.drop(columns=["datapoint_hash"]),
                ...
                for _, row in data.iterrows():
                    hashed_list.append(
                        hashlib.sha256(str(row).encode("utf-8")).hexdigest()
                    )
                data["datapoint_hash"] = hashed_list
                data.to_sql("datasource", con=con, if_exists="append", index=False)
            else:

                # replace table if columns are mismatched
                cur.execute("DROP TABLE 'datasource'")
                cur.execute(
                    """CREATE TABLE "datasource" ('rowID' INTEGER PRIMARY KEY)"""
                )
    ...
    async def _create_and_run_worker(self, message: _BitfountMessage) -> None:
        ...
        # TODO: [BIT-2486] There should be check to see if the
        #  task request contains a project id, and allow
        #  for pod_db only in that case.
        ...
        # TODO: [BIT-2486] Add the project id to the worker to ensure
        #  results are written to the correct project db.
        worker = _Worker(
            datasource=self.datasource,
