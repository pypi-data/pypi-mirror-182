...
class TestModeller:
    ...
    def registered_public_key_patcher(
        ...
        def _patch_registered_public_key(
            ...
            if key is None:
                mock_bitfount_hub.get_user_public_key.return_value = None
            else:
                if isinstance(key, RSAPrivateKey) or is_private_key_mock:
                    public_key = cast(RSAPrivateKey, key).public_key()
                else:
                    public_key = key
                mock_bitfount_hub.get_user_public_key.return_value = public_key
        ...
        if mock_protocol_factory_param == "protocol_with_aggregator":
            expected_task_request_body = SerializedProtocol(
                ...
                algorithm=SerializedAlgorithm(
                    class_name="bitfount.FederatedModelTraining",  # must be an instance of BaseModelAlgorithmFactory # noqa: B950
    ...
    def test_modeller_init_handles_different_key_registered(
        self,
        caplog: LogCaptureFixture,
        mock_bitfount_hub: Mock,
        mock_message_service: Mock,
        ...
        """Test error handling if other public key registered on hub."""
        registered_public_key_patcher(Mock())
        with pytest.raises(
            RSAKeyError, match="Registered and stored modeller public keys mismatch"
        ):
            _Modeller(
                protocol=mock_protocol_factory,
                message_service=mock_message_service,
        ...
        assert (
            f"Public key already registered for modeller {username} but does not"
            f" match local public key." in get_error_logs(caplog)
        )
    ...
    def test__serialize(
        ...
        modeller.protocol.algorithms = [modeller.protocol.algorithm]  # type: ignore[misc, list-item] # Reason: See above # noqa: B950
