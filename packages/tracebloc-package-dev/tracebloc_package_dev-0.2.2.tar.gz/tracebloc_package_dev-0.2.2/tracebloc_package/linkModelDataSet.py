...
class LinkModelDataSet:
    ...
    def customise_dataset(self):
        ...
        if user_sub_ans.lower().strip() == 'yes' or user_sub_ans.lower().strip() == 'y':
            ...
            print(f"You choose to use current dataset with {self.__images_per_class} images")
        user_shape_ans = input("\nDo you want to change image size for training: ")
        if user_shape_ans.lower().strip() == 'yes' or user_shape_ans.lower().strip() == 'y':
        ...
        if user_type_ans.lower().strip() == 'yes' or user_type_ans.lower().strip() == 'y':
