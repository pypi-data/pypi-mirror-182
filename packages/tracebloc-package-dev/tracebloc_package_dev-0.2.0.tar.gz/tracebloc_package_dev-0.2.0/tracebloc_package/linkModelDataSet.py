...
class LinkModelDataSet:
    ...
    def images_per_class(self, **kwargs):
        f"""Please provide number of images per class
        example: trainingObject.images_per_class(car=50,person=40)"""
        self.__eligibility_passed = True
        if kwargs.keys() == self.__class_names.keys():
            # check if user has provided the count in the range of [1,max_count]
            for key in kwargs.keys():
                if not 0 < kwargs[key] <= self.__class_names[key]:
                    self.__eligibility_passed = False
                    print("Please provide image count not more than the images in provided dataset\n"
                          f"Maximum images per class in dataset {self.__datasetId}: {self.__class_names}")
                    return
            self.__images_per_class = json.dumps(kwargs)
        else:
            self.__eligibility_passed = False
            print("Please provide image count for all classes")
    ...
    def __getParameters(self):
        # check if user has not provided images count for classes in dataset
        # set the counts with default of dataset
        if not self.__images_per_class:
    ...
    def getTrainingPlan(self):
        ...
        print(
            ...
            f"image count per class: {self.__images_per_class}",
        )
