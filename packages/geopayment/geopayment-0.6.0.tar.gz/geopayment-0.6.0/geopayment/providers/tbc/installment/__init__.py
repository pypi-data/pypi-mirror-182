...
class BaseInstallmentProvider(object):
    ...
    def __init__(self) -> None:
        assert (
            callable(self.merchant_key) is False
        ), "`merchant_key` must be property, not callable"
        assert (
            callable(self.campaign_id) is False
        ), "`campaign_id` must be property, not callable"
        assert callable(self.key) is False, "`key` must be property, not callable"
        assert callable(self.secret) is False, "`secret` must be property, not callable"
        assert (
            callable(self.service_url) is False
        ), "`service_url` must be property, not callable"
        assert (
            callable(self.version) is False
    ...
    def merchant_key(self):
        ...
        :return: merchant key
        """
        raise NotImplementedError("Provider needs implement `merchant_key` function")
    ...
    def campaign_id(self):
        ...
        :return: campaign id
        """
        raise NotImplementedError("Provider needs implement `campaign_id` function")
    ...
    def key(self):
        ...
        """
        raise NotImplementedError("Provider needs implement api `key` function")
    ...
    def secret(self):
        ...
        """
        raise NotImplementedError("Provider needs implement api `secret` function")
...
class TBCInstallmentProvider(BaseInstallmentProvider):
    ...
    @tbc_installment_params(
        endpoint="v1/online-installments/applications",
        api="create",
    )
    @_request(verify=True, timeout=(3, 10), method="post")
    def create(self, **kwargs: Optional[Any]) -> Dict[str, str]:
        ...
        if self.session_id and self.redirect_url:
            return {"session_id": self.session_id, "redirect_url": self.redirect_url}
