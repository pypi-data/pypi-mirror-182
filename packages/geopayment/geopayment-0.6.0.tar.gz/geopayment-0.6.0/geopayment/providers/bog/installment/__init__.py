...
class IPayInstallmentProvider(IPayProvider):
    ...
    @bog_params(
        currency_code="GEL", endpoint="installment/checkout", api="installment-checkout"
    )
    ...
    def checkout(self, **kwargs: Optional[Any]) -> Dict[str, str]:
        ...
        if "links" in result and self.order_status:
            link = list(filter(lambda x: x["rel"] == "target", result["links"]))
            if link:
    ...
    @bog_params(
        currency_code="GEL",
        endpoint="services/installment/calculate",
        api="installment-calculate",
    )
    @_request(verify=True, timeout=(3, 10), method="post")
