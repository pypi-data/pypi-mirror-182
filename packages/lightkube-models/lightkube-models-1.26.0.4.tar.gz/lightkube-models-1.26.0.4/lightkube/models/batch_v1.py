...
class CronJobSpec(DataclassDictMixIn):
    """CronJobSpec describes how the job execution will look like and when it will
      ...
      * **timeZone** ``str`` - *(optional)* The time zone name for the given schedule, see
...
class JobSpec(DataclassDictMixIn):
    """JobSpec describes how the job execution will look like.
      ...
      * **parallelism** ``int`` - *(optional)* Specifies the maximum desired number of pods the job should run at any given
        ...
        https://kubernetes.io/docs/concepts/workloads/controllers/jobs-run-to-completion/
      * **podFailurePolicy** ``PodFailurePolicy`` - *(optional)* Specifies the policy of handling failed pods. In particular, it allows to
...
@dataclass
class PodFailurePolicy(DataclassDictMixIn):
...
@dataclass
class PodFailurePolicyOnExitCodesRequirement(DataclassDictMixIn):
    """PodFailurePolicyOnExitCodesRequirement describes the requirement for handling
      ...
      * **containerName** ``str`` - *(optional)* Restricts the check for exit codes to the container with the specified name.
...
@dataclass
class PodFailurePolicyOnPodConditionsPattern(DataclassDictMixIn):
...
class PodFailurePolicyRule(DataclassDictMixIn):
    """PodFailurePolicyRule describes how a pod failure is handled when the
      ...
      * **onPodConditions** ``List[PodFailurePolicyOnPodConditionsPattern]`` - Represents the requirement on the pod conditions. The requirement is
        ...
        are allowed.
      * **onExitCodes** ``PodFailurePolicyOnExitCodesRequirement`` - *(optional)* Represents the requirement on the container exit codes.
    """
    action: 'str'
