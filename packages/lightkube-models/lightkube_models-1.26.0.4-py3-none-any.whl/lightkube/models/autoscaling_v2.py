...
class HorizontalPodAutoscalerStatus(DataclassDictMixIn):
    """HorizontalPodAutoscalerStatus describes the current status of a horizontal pod
      ...
      * **conditions** ``List[HorizontalPodAutoscalerCondition]`` - *(optional)* conditions is the set of conditions required for this autoscaler to scale its
      ...
      * **currentReplicas** ``int`` - *(optional)* currentReplicas is current number of replicas of pods managed by this
    ...
    """
    desiredReplicas: 'int'
    conditions: 'List[HorizontalPodAutoscalerCondition]' = None
...
class MetricSpec(DataclassDictMixIn):
    """MetricSpec specifies how to scale based on a single metric (only `type` and
      ...
      * **containerResource** ``ContainerResourceMetricSource`` - *(optional)* containerResource refers to a resource metric (such as those specified in
