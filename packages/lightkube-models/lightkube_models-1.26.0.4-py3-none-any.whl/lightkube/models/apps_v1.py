...
class RollingUpdateStatefulSetStrategy(DataclassDictMixIn):
    """RollingUpdateStatefulSetStrategy is used to communicate parameter for
      ...
      * **partition** ``int`` - *(optional)* Partition indicates the ordinal at which the StatefulSet should be partitioned
...
@dataclass
class StatefulSetOrdinals(DataclassDictMixIn):
    """StatefulSetOrdinals describes the policy used for replica ordinal assignment
      in this StatefulSet.

      **parameters**

      * **start** ``int`` - *(optional)* start is the number representing the first replica's index. It may be used to
