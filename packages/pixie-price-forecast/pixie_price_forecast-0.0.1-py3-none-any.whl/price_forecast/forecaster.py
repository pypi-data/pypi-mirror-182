...
def run_forecast(model_name: str,
                 ...
                 train_to: str, mode: str = 'pixie', horizon: int = 12):
    """
    ...
    input_model = {'mode': mode, 'train_from': train_from, 'train_to': train_to, 'date_col_name': 'version_date',
                   'input_path': input_path, 'horizon': horizon, 'max_date': '2022-10-30', 'db_engine': db_engine}
...
def run_all_forecasts(credentials: dict):
    ...
    reset_use_tag('price_forecasts')
    for data_source, _ in mode_dict.items():
        if 'rent' in data_source:
            pass
        else:
            print(data_source)
            path = data_source
            ...
            run_forecast(model_name='arima', input_path=path, province_list=provinces,
                         search_grid=search_grid, train_from='2018-01-01', train_to='2022-08-01',
