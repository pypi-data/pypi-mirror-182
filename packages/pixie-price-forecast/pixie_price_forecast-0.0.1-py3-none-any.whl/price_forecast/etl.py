...
def setup_tunnel(ssh_user, ssh_host, ssh_port, host, port) -> SSHTunnelForwarder:
...
def export_aggregated_data(mode_str: str, start_date: str, end_date: str, credentials: dict, to_sql: bool = True):
...
def update_raw_data(start_date: str, end_date: str, credentials: dict, first_step: int = 1):
    i = 1
    for mode, _ in mode_dict.items():
        if i < first_step:
            ...
            print(f'{i}-{mode}')
            export_aggregated_data(mode_str=mode, start_date=start_date, end_date=end_date, credentials=credentials)
        i += 1
...
def update_clean_data(credentials: dict):
    ...
    pixie_engine = create_pixie_engine(local_port=local_port,
                                       ...
                                       password=credentials['password'],
                                       db_name=credentials['db_name'])
    reset_use_tag(table_name='price_clean_data')
