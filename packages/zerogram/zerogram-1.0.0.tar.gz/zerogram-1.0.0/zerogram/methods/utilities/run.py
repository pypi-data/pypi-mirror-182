...
import zerogram
from zerogram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~zerogram.Client.start`, :meth:`~zerogram.idle` and :meth:`~zerogram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that zerogram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`zerogram.compose`.
