...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~zerogram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~zerogram.Client.sign_in` or :meth:`~zerogram.Client.sign_up`).

        next_type (:obj:`~zerogram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~zerogram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~zerogram.Client.resend_code`.
    """
