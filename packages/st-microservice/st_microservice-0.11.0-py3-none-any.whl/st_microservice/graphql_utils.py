...
from sqlalchemy import MetaData, ColumnElement, Table, Row, select, Executable, cast, String
...
def get_table_primary_keys(table: Table) -> list[ColumnElement]:
    return [col for col in table.columns if col.primary_key]


def filter_primary_key(q: Executable, table: Table, primary_key_values: list[str]):
    pk_cols = get_table_primary_keys(table)
    try:
        for i, pk_val in enumerate(primary_key_values):
            q = q.where(pk_cols[i] == pk_val)
    except IndexError:
        raise GraphQLError(f"You have specified more primary keys values than there are columns for table {table.name}")
    return q
...
async def get_or_none(db: AsyncConnection, table: T, primary_keys: list) -> Row | None:
    q = select(table)
    q = filter_primary_key(q, table, primary_keys)
...
async def get_or_error(db: AsyncConnection, table: T, primary_keys: list) -> Row:
    try:
        row: T | None = await get_or_none(db, table, primary_keys)
    except MultipleResultsFound:
        raise GraphQLError(f"Primary keys: {primary_keys} returned more than one row")
    if row is None:
        raise GraphQLError(f"Could not find {table.name}: {primary_keys}")
    return row
...
def load_from_table_query(table: Table, filters: list[FieldFilter], limit: int | None, offset: int | None,
                          query_modifier: Callable[[Executable], Executable] | None = None, init_query: Executable | None = None):
    q = select(table) if init_query is None else init_query
    ...
    for f in filters:
        ...
        # Todo: fix above
        field = getattr(table.c, full_name)
...
def simple_table_resolver_factory(table: Table, query_modifiers=None):
    async def simple_table_resolver(_, info, filters: list[FieldFilter], limit: int, offset: int):
        q = load_from_table_query(table, filters, limit, offset, query_modifiers)
