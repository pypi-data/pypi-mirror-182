...
class Bintang():
    ...
    def create_table(self, name, columnnames=None):
        table = Bing(name) # create a bing table
        self.add_table(table)
        if self.__be is not None:   # if is_persistent is True then update the table attributes and pass the connection
            table._Bing__be = self.__be           
            table._Bing__be.add_table(self.get_tableid(name), name)
    ...
    def read_json(self, jsondata, tablepaths=[]):
        debug = False
        for row in travdict.traverse_json(jsondata,tablepaths):
            if debug:
                print("\n---------------------in bintang---------------------")
                print(row)
