...
class MoCo_Img(nn.Module):
    def __init__(
        ...
        self.logit_scale_moco = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
    ...
    def init_parameters(self):
        ...
        nn.init.constant_(self.logit_scale_moco, np.log(1 / 0.07))
    ...
    def forward(self, batch):
        image_q, image_k = batch["images"]
        image_q = image_q.to(device=self.device, non_blocking=True)
        image_k = image_k.to(device=self.device, non_blocking=True)
        # text = text.to(device=self.device, non_blocking=True)

        if (image_q is None) or (batch["bert_input"] is None):
            raise RuntimeError('Missing Image OR Text in the input')

        image_features_q = self.encode_image(image_q)
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            image_features_k = self.visual_m(image_k)
        ...
        clip_prediction.update({
            ...
            "image_features_k": image_features_k,
            "img_queue": self.img_queue,
            "logit_scale": self.logit_scale.exp(),
            "logit_scale_moco": self.logit_scale_moco.exp(),
        })
