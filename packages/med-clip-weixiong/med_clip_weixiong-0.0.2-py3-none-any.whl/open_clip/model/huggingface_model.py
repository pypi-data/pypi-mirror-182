...
class CLIP_HuggingFace(nn.Module):
    def __init__(
            self,
            args,
            embed_dim: int,
    ...
    def encode_text(self, batch):
        ...
        # print(f"\033[42mencoded_input\033[0m: {encoded_input['input_ids'].shape}")  # [128, 77]
        ...
        # NOTE
        # attentions 是由12个元素组成的 tuple, 表示不同 layer; 每个元素的大小为 (batch_size, #heads, L, L)
        # 我们可以对 (L, L) 进行可视化        # print(x.keys())  # 'last_hidden_state', 'pooler_output', 'attentions'
        ...
        # NOTE [1, 3, 0, 2, 4], ['[UNK]', '[SEP]', '[PAD]', '[CLS]', '[MASK]']
        # last_token_index = torch.nonzero( (encoded_input['input_ids'] == 3).squeeze() )  # NOTE 之前错用了 [SEP] 作为提取 feature 的 special token
        last_token_index = torch.nonzero( (encoded_input['input_ids'] == self.cls_id).squeeze() )

        # NOTE [100, 102, 0, 101, 103] ['[UNK]', '[SEP]', '[PAD]', '[CLS]', '[MASK]']
        # last_token_index = torch.nonzero( (encoded_input['input_ids'] == 102).squeeze() )

        # print(f"\033[42mlast_token_index.type:\033[0m {type(last_token_index)}")  # [128, 2]
        # logging.debug(f"\033[42mlast_token_index.shape:\033[0m {last_token_index.shape}")  # [128, 2]
        logging.debug(f"arange.shape: {torch.arange(x.shape[0]).shape}")
        # if len(last_token_index) > 64:
        #     raise RuntimeError('WFT?')

        x = x[torch.arange(x.shape[0]), last_token_index[:, 1]]
        # x = x[torch.arange(x.shape[0]), text.argmax(dim=-1)]
        # print(f"\033[42mpos_1:\033[0m {x.shape}")  # [128, 768]

        text_features = x @ self.text_projection  # NOTE for matching
        # print(f"\033[42mpos_2:\033[0m {text_features.shape}")  # [128, 1024]
    ...
    def forward(self, batch):
        ...
        text_output = self.encode_text(batch)
