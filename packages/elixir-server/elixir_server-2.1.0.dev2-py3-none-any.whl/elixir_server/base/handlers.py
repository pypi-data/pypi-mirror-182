...
import elixir_server
from elixir_server._sysinfo import get_sys_info
from elixir_server._tz import utcnow
from elixir_server.i18n import combine_translations
from elixir_server.services.security import csp_report_uri
...
if TYPE_CHECKING:
    from elixir_server.auth.identity import User
    ...
    def authorizer(self):
        if "authorizer" not in self.settings:
            warnings.warn(
                ...
                "in its settings. In future releases of elixir_server, this will "
                "be a required key for all subclasses of `JupyterHandler`. For an "
                "example, see the elixir_server source code for how to "
                "add an authorizer to the tornado settings: "
                "https://github.com/jupyter-server/jupyter_server/blob/"
                "653740cbad7ce0c8a8752ce83e4d3c2c754b13cb/elixir_server/serverapp.py"
                "#L234-L256",
            )
            from elixir_server.auth import AllowAllAuthorizer
    ...
    def identity_provider(self):
        if "identity_provider" not in self.settings:
            warnings.warn(
                ...
                "https://github.com/jupyter-server/jupyter_server/blob/"
                "aa8fd8b3faf37466eeb99689d5555314c5bf6640/elixir_server/serverapp.py"
                "#L253",
            )
            from elixir_server.auth import IdentityProvider
...
class APIVersionHandler(APIHandler):
    def get(self):
        ...
        self.finish(json.dumps({"version": elixir_server.__version__}))
