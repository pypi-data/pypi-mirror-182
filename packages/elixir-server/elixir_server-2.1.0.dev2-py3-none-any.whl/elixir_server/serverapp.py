...
from elixir_server.transutils import _i18n, trans
from elixir_server.utils import pathname2url, run_sync_in_loop, urljoin
...
from elixir_server._sysinfo import get_sys_info
from elixir_server._tz import utcnow
from elixir_server.auth.authorizer import AllowAllAuthorizer, Authorizer
from elixir_server.auth.identity import (
    IdentityProvider,
    LegacyIdentityProvider,
    PasswordIdentityProvider,
)
from elixir_server.auth.login import LoginHandler
from elixir_server.auth.logout import LogoutHandler
from elixir_server.base.handlers import (
    FileFindHandler,
    MainHandler,
    RedirectWithParams,
    Template404,
)
from elixir_server.extension.config import ExtensionConfigManager
from elixir_server.extension.manager import ExtensionManager
from elixir_server.extension.serverextension import ServerExtensionApp
from elixir_server.gateway.managers import (
    GatewayClient,
    GatewayKernelSpecManager,
    GatewayMappingKernelManager,
    GatewaySessionManager,
)
from elixir_server.log import log_request
from elixir_server.services.config import ConfigManager
from elixir_server.services.contents.filemanager import (
    AsyncFileContentsManager,
    FileContentsManager,
)
from elixir_server.services.contents.largefilemanager import LargeFileManager
from elixir_server.services.contents.manager import (
    AsyncContentsManager,
    ContentsManager,
)
from elixir_server.services.kernels.kernelmanager import (
    AsyncMappingKernelManager,
    MappingKernelManager,
)
from elixir_server.services.sessions.sessionmanager import SessionManager
from elixir_server.traittypes import TypeFromClasses
from elixir_server.utils import (
    check_pid,
    fetch,
    unix_socket_in_use,
    url_escape,
...
JUPYTER_SERVICE_HANDLERS = dict(
    auth=None,
    api=["elixir_server.services.api.handlers"],
    config=["elixir_server.services.config.handlers"],
    contents=["elixir_server.services.contents.handlers"],
    files=["elixir_server.files.handlers"],
    kernels=["elixir_server.services.kernels.handlers"],
    kernelspecs=[
        "elixir_server.kernelspecs.handlers",
        "elixir_server.services.kernelspecs.handlers",
    ],
    nbconvert=[
        "elixir_server.nbconvert.handlers",
        "elixir_server.services.nbconvert.handlers",
    ],
    security=["elixir_server.services.security.handlers"],
    sessions=["elixir_server.services.sessions.handlers"],
    shutdown=["elixir_server.services.shutdown"],
    view=["elixir_server.view.handlers"],
    events=["elixir_server.services.events.handlers"],
)
    ...
    def init_settings(
        ...
        *,
        ...
        nbui = gettext.translation(
            "nbui",
            localedir=os.path.join(base_dir, "elixir_server/i18n"),
            fallback=True,
        )
    ...
    def init_handlers(self, default_services, settings):
        ...
        for service in default_services:
            if service in JUPYTER_SERVICE_HANDLERS:
                ...
                raise Exception(
                    "{} is not recognized as a elixir_server "
        ...
        if GatewayClient.instance().gateway_enabled:
            ...
            gateway_handlers = load_handlers("elixir_server.gateway.handlers")
        ...
        handlers.extend(load_handlers("elixir_server.base.handlers"))
...
class JupyterPasswordApp(JupyterApp):
    ...
    def start(self):
        from elixir_server.auth.security import set_password
...
class ServerApp(JupyterApp):
    ...
    extra_template_paths = List(
        ...
        Can be used to override templates from elixir_server.templates."""
        ),
    )
    ...
    contents_manager_class = TypeFromClasses(
        ...
        klasses=[
            "elixir_server.services.contents.manager.ContentsManager",
    ...
    def _observe_contents_manager_class(self, change):
        ...
        if new.startswith("notebook"):
            self.log.warning(
                ...
                "manager to inherit from the 'elixir_server' managers. "
    ...
    def _default_kernel_manager_class(self):
        if self.gateway_config.gateway_enabled:
            return "elixir_server.gateway.managers.GatewayMappingKernelManager"
    ...
    def _default_session_manager_class(self):
        if self.gateway_config.gateway_enabled:
            return "elixir_server.gateway.managers.GatewaySessionManager"
    ...
    def _default_kernel_spec_manager_class(self):
        if self.gateway_config.gateway_enabled:
            return "elixir_server.gateway.managers.GatewayKernelSpecManager"
    ...
    _starter_app = Instance(
        default_value=None,
        allow_none=True,
        klass="elixir_server.extension.application.ExtensionApp",
    )
