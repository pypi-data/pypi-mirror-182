    ...
    def startSimulation(self, flag=3):                  # CtoF button event handler
        ...
        if flag == 1 or flag == 3:
            ...
            for k, bcl_text in enumerate(bcls):             
                ...
                age_ik1 = 0
				#age_ik1_eff = if(age_ik1>29, age_ik1, age)
				
                # age_ncxb = -0.022731 * age + 0.841199 - 0.05
                # age_sercab = 0.706 - 0.05 + 0.15 / (1+10**((45 - age)* 0.1)) #0.005793 * age + 0.532222
                # age_cal_buff = -0.001973 * age + 0.283936
                # age_calSR_buff = -0.007122 * age + 12.466850
                # age_steep_ical = -0.154673 * age - 0.454007
                # age_V_shift_ical = 0.80 * age - 49.5 #0.75 * age - 42.5
                # age_icalb = -0.040499 * age + 1.881332 #-0.042499 * age + 1.871332
                # age_ik1b = -11.905593 + (11.905593)/(1+10**((55 - age_ik1_eff)*(-0.03))) #-11.67 + (11.67)/(1+10**((67.45 - age)*(-0.03752)))                
				...
				# age_inak = 1.5 + 0.6/(1+10**((45 - age)*0.1))
                # age_factor_icab = 0.094065 * age + 1.041047 #0.094065 * age + 0.841047  
                ...
                s.set_constant("parameters.age", age)
                s.set_constant("parameters.age_ik1", age_ik1)
                ...
                s.pre(prebeats*bcl)
                data1_ref_temp = s.run(showbeats*bcl, log=myokit.LOG_STATE+myokit.LOG_INTER+myokit.LOG_BOUND)
               ...
               #mdlname = "models/" + self.cmbModel1.currentText() + ".mmt"
                ...
                s.pre(prebeats*bcl)
                data1_temp = s.run(showbeats*bcl, log=myokit.LOG_STATE+myokit.LOG_INTER+myokit.LOG_BOUND)
                ...
                s.pre(prebeats*bcl)
                data1b_temp = s.run(showbeats*bcl, log=myokit.LOG_STATE+myokit.LOG_INTER+myokit.LOG_BOUND)
