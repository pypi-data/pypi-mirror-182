...
def save_model(config, model, tokenizer, save_path, **kwargs):
    ...
    if config.save_mode == 1 or config.save_mode == 2:
        ...
        if config.save_mode == 1:
            torch.save(model_to_save.state_dict(), save_path + config.model_name + '.state_dict')  # save the state dict
        elif config.save_mode == 2:
            torch.save(model.cpu(), save_path + config.model_name + '.model')  # save the state dict
