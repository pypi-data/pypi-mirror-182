...
def parse_examples(examples):
    ...
    for example in examples:
        ...
        for token in simple_split_text(example):
            tokens.append(token)
        data.append((tokens, ['[MASK]'] * len(tokens), LabelPaddingOption.SENTIMENT_PADDING))
    return data
...
def convert_apc_examples_to_features(examples, label_list, max_seq_len, tokenizer, opt=None):
    ...
    for (ex_index, example) in enumerate(it):
        ...
        polarity = [LabelPaddingOption.SENTIMENT_PADDING] + example.polarity + [LabelPaddingOption.SENTIMENT_PADDING]
