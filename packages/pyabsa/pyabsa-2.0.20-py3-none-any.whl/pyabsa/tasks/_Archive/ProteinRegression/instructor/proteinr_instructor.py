...
class ProteinRTrainingInstructor(BaseTrainingInstructor):
    ...
    def _load_dataset_and_prepare_dataloader(self):
        ...
        if hasattr(BERTProteinRModelList, self.config.model.__name__):
            ...
            if cache_path is None or self.config.overwrite_cache:
                self.train_set = BERTProteinRDataset(self.config, self.tokenizer, dataset_type='train')
                self.test_set = BERTProteinRDataset(self.config, self.tokenizer, dataset_type='test')
                self.valid_set = BERTProteinRDataset(self.config, self.tokenizer, dataset_type='valid')
            ...
            self.valid_set = GloVeProteinRDataset(self.config, self.tokenizer, dataset_type='valid')
                            ...
                            if self.config.model_path_to_save:
                                ...
                                if save_path and self.config.save_last_ckpt_only:
                                    try:
                                        ...
                                        pass
                                save_path = '{0}/{1}_{2}_r2_{3}/'.format(self.config.model_path_to_save,
                                                                         self.config.model_name,
                                                                         self.config.dataset_name,
                                                                         round(test_r2, 4),
                                                                         )

                                if test_r2 < self.config.max_test_metrics['max_test_r2']:
                                    self.config.max_test_metrics['max_test_r2'] = test_r2
    ...
    def _k_fold_train_and_evaluate(self, criterion):
        ...
        for f, (train_dataloader, valid_dataloader) in enumerate(zip(self.train_dataloaders, self.valid_dataloaders)):
            ...
            self.config.MV.add_metric('Fold{}-Max-Valid-R2-Score'.format(f), max_fold_r2)
