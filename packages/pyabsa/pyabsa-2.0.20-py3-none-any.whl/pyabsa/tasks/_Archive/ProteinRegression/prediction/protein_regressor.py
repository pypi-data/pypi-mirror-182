...
class ProteinRegressor(InferenceModel):
    ...
    def __init__(self, checkpoint=None, cal_perplexity=False, **kwargs):
        ...
        if self.checkpoint and not isinstance(self.checkpoint, str):
            ...
            if not hasattr(GloVeProteinRModelList, self.config.model.__name__) \
        ...
        if hasattr(BERTProteinRModelList, self.config.model.__name__):
            self.dataset = BERTProteinRDataset(config=self.config, tokenizer=self.tokenizer)
    ...
    def batch_predict(self,
        ...
        target_file = detect_infer_dataset(target_file, task_code=TaskCodeOption.ProteinSequenceRegression)
        if not target_file:
