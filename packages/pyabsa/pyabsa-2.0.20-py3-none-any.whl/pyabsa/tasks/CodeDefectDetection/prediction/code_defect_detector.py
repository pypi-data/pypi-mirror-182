...
from ..dataset_utils.__classic__.data_utils_for_inference import GloVeCDDInferenceDataset
...
class CodeDefectDetector(InferenceModel):
    ...
    def __init__(self, checkpoint=None, cal_perplexity=False, **kwargs):
        ...
        if self.checkpoint and not isinstance(self.checkpoint, str):
            ...
            if not hasattr(GloVeCDDModelList, self.config.model.__name__) \
        ...
        if hasattr(BERTCDDModelList, self.config.model.__name__):
            self.dataset = BERTCDDInferenceDataset(config=self.config, tokenizer=self.tokenizer)

        elif hasattr(GloVeCDDModelList, self.config.model.__name__):
            self.dataset = GloVeCDDInferenceDataset(config=self.config, tokenizer=self.tokenizer)
            ...
            for _, sample in enumerate(it):
                ...
                if t_targets_all is None:
                    t_targets_all = np.array([self.config.label_to_index[x] if x in self.config.label_to_index else
                                              LabelPaddingOption.LABEL_PADDING for x in sample['label']])
                    ...
                    t_targets_all = np.concatenate(
                        (t_targets_all, [self.config.label_to_index[x] if x in self.config.label_to_index else
                                         LabelPaddingOption.LABEL_PADDING for x in sample['label']]), axis=0)
                    t_outputs_all = np.concatenate((t_outputs_all, np.array(logits.cpu()).astype(np.float32)),
                                                   axis=0)
                if t_c_targets_all is None:
                    t_c_targets_all = np.array([self.config.label_to_index[x] if x in self.config.label_to_index else
                                                LabelPaddingOption.LABEL_PADDING for x in sample['corrupt_label']])
                ...
                for i, i_probs in enumerate(t_probs):
                    ...
                    corrupt_label = self.config.index_to_label[int(c_logits[i].argmax(axis=-1))]
                    if sample['label'][i] != LabelPaddingOption.LABEL_PADDING:
                        ...
                        real_label = 'N.A.'
                    if real_label != LabelPaddingOption.LABEL_PADDING and real_label != str(
