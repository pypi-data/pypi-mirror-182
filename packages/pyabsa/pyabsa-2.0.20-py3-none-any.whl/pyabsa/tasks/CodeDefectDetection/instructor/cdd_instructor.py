    ...
    def _load_dataset_and_prepare_dataloader(self):
        ...
        if hasattr(BERTCDDModelList, self.config.model.__name__):
            ...
            if cache_path is None or self.config.overwrite_cache:
                self.train_set = BERTCDDDataset(self.config, self.tokenizer, dataset_type='train')
                self.test_set = BERTCDDDataset(self.config, self.tokenizer, dataset_type='test')
                self.valid_set = BERTCDDDataset(self.config, self.tokenizer, dataset_type='valid')
            try:
            ...
            self.train_set = GloVeCDDDataset(self.config, self.tokenizer, dataset_type='train')
            self.test_set = GloVeCDDDataset(self.config, self.tokenizer, dataset_type='test')
            self.valid_set = GloVeCDDDataset(self.config, self.tokenizer, dataset_type='valid')
    ...
    def _evaluate_acc_f1(self, test_dataloader):
        ...
        with torch.no_grad():
            for t_batch, t_sample_batched in enumerate(test_dataloader):
                ...
                n_c_test_correct += (torch.argmax(t_c_logits, -1) == t_c_targets).sum().item()
