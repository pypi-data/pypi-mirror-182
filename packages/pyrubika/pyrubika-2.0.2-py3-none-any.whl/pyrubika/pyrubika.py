...
from pyrubika.An_Wb import SetClines,Server
...
class pyrubika:
	...
	def registerDevice(self):
	    inData = {"method":"registerDevice","input":{"app_version":"WB_4.1.2","device_hash":"0501110712007200125373640870428014153736","device_model":"pyrubika-library","is_multi_account": True,"lang_code":"fa","system_version":"Linux","token":" ","token_type":"android"},"client": SetClines.android}
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
	        except:continue
	def addChannelMembers(self, chat_id, member):
		inData = {"method":"addChannelMembers","input":{"channel_guid":chat_id,"member_guid":member},"client":SetClines.web}
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
			except:continue
	...
	def getChatAds(self):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get('chat_ads')
	        except:continue
	def getMessagesInterval(self):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("messages")
	        except:continue

	def sendCode(phone_number,pass_key=None):
		tmp = pyrubika._tmpGeneration()
		inData = {
			"method":"sendCode",
			"input":{
				"pass_key":pass_key,
		...
		while True:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","tmp_session": tmp,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
			except:continue
		
	def sendCode_Get(phone_number,pass_key=None):
	    tmp = pyrubika._tmpGeneration()
	    inData = {"method":"sendCode","input":{"pass_key":pass_key,"phone_number":f"98{phone_number[1:]}","send_type":"Internal"},"client":SetClines.android}
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","tmp_session": tmp,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
	        except:continue
	...
	def getMessages(self, chat_id, min_id):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data")
				break
			except: continue
	...
	def getGroupOnlineMember(self, chat_id):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get('data').get('online_count')
	        except:continue
	...
	def getCommonGroups(self, chat_id):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get('data').get('abs_groups')
	        except:continue
	...
	def getContacts(self, user_guid):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("users")
	...
	def setMembersAccess(self, chat_id, access_list):
		...
		while 1:
			try:
				return loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read())
				break
			except: continue
	...
	def getGroupLink(self, chat_id):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("join_link")
				break
			except: continue
	...
	def getLinkFromAppUrl(self, app_url):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("link").get("open_chat_data")
	            break
	        except:continue
	...
	def serch(self,object_guid, search_text):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("message_ids")[:5]
	            break
	        except:continue
	...
	def channelPreviewByJoinLink(self, link):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get('data').get('channel')
	        except:continue
	        
	def joinChannelByLink(self, link):
	    hashLink = link.split("/")[-1]
	    inData = {"method":"joinChannelByLink","input":{"hash_link": hashLink},"client": SetClines.android}
	    joinChannelByLink("https://rubika.ir/joinc/BHEIDBFD0DCHMESBYDZSNIPELQBKGMQV")
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get('data')
	        except:continue
	...
	def getUserInfo(self, chat_id):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("user")
				break
			except: continue
	...
	def getChatsUpdate(self):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("chats")
				break
			except: continue
	...
	def _requestSendFile(self, file):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._SendFile(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data")
				break
			except: continue

	def _uploadFile(self, file):
		if not "http" in file:
			REQUES = pyrubika._requestSendFile(self, file)
	...
	def uploadAvatar(self,myguid,main,thumbnail=None):
		mainID = str(pyrubika._uploadFile(self, main)[0]["id"])
		thumbnailID = str(pyrubika._uploadFile(self, thumbnail or main)[0]["id"])
		inData = {
			"method":"uploadAvatar",
			"input":{
				"object_guid":myguid,
	...
	def getAvatar(self, myguid):
	    ...
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._getURL(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc'))).get("data").get("avatars")
	        except:continue
	...
	def sendDocument(self, chat_id, file, caption=None, message_id=None):
		uresponse = pyrubika._uploadFile(self, file)
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(request.urlopen(request.Request(pyrubika._SendFile(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
				break
			except: continue
	...
	def sendPhoto(self, chat_id, file, size=[], thumbnail=None, caption=None, message_id=None):
		...
		elif "." in thumbnail:thumbnail = str(pyrubika._getThumbInline(open(file,"rb").read() if not "http" in file else get(file).content))

		if size == []: size = pyrubika._getImageSize(open(file,"rb").read() if not "http" in file else get(file).content)
