...
class ModelExporter(object):
    ...
    def load_model(self, model_path):
        ...
        try:
            ...
            if os.environ.get("BROKER_URL"):
                ...
                raise Exception("Error load model: %s"%str(e))
    ...
    def _call_predict(self, model, data, model_path, options):
        try:
            ...
            if os.environ.get("BROKER_URL") and 'object has no attribute' in str(e):
                from auger_ml.core.model_exporter_ex import ModelExporterEx
                ModelExporterEx.reexport_model(self.options, model_path, options)
                model, timeseries_model = self.load_model(model_path)
                return model.predict(data)
            else:
                raise Exception("Error predict model: %s"%str(e))
