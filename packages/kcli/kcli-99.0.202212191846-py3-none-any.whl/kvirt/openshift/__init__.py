...
def create_baremetal_deployment(config, plandir, cluster, data, baremetal_hosts=[]):
    ...
    return f'http://{host_ip}/{cluster}-sno.iso'
...
def scale(config, plandir, cluster, overrides):
    ...
    baremetal_hosts = list(set(new_baremetal_hosts).difference(old_baremetal_hosts))
    if baremetal_hosts:
        if not old_baremetal_hosts:
            iso_url = create_baremetal_deployment(config, plandir, cluster, data, baremetal_hosts)
            boot_baremetal_hosts(baremetal_hosts, iso_url, overrides=overrides, debug=config.debug)
            overrides['workers'] = overrides.get('workers', 0) - len(baremetal_hosts)
