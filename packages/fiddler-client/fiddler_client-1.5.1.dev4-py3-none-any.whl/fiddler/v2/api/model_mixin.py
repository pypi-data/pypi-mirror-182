...
from fiddler.v2.utils.helpers import get_model_artifact_info
...
class ModelMixin:
    ...
    def delete_model(self, model_name: str, project_name: str) -> None:
        ...
        """
        logger.info(
            'Deleting model %s deleted from %s project', model_name, project_name
        )
        self.client.delete(
            url=f'models/{self.organization_name}:{project_name}:{model_name}',
        )
    ...
    def add_model_surrogate(
        self, model_name: str, project_name: str, wait: bool = True
    ) -> str:
        """
    ...
    def _deploy_model_artifact(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            job_uuid = deployer.deploy(file_path=Path(file_path))

        # Update file_list
        file_list = get_model_artifact_info(artifact_dir=artifact_dir)
        self.update_model(
            model_name=model_name, project_name=project_name, file_list=file_list
        )
    ...
    def add_model_artifact(
        self, model_name: str, project_name: str, artifact_dir: str, wait: bool = True
    ) -> str:
        """
