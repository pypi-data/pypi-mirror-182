...
use rr_util::{
    ...
    tensor_db::write_tensor_to_dir_tree, util::ALPHABET,
};
...
impl PrintOptions {
    ...
    pub fn repr_full_line(
        ...
        let (repeat_string, line_prefix) = match (repeat_info, disallow_name, circ.name()) {
            (Some(x), _, _) => x,
            (None, true, Some(name)) => {
