...
use pyo3::prelude::*;
use rr_util::{
    py_types::PyOpAtAxes, rearrange_spec::RearrangeSpec, tensor_util::right_align_shapes,
    util::HashBytes,
};
...
pub fn elim_no_input_module(module: &Module) -> Option<CircuitRc> {
    module.nodes.is_empty().then(|| module.spec.circuit.clone())
}
...
#[pyo3(name = "deep_module_remove_unused_inputs")]
...
pub fn module_remove_unused_inputs_op(
    ...
    let out = if out_batch_shape.len() < orig_batch_shape.len() {
        ...
        Rearrange::nrc(out, spec, m.name_cloned())
    } else {
        out
    };
