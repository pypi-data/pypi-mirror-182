...
use circuit_base::{
    flat_concat,
    module::{any_children_with_symbolic_sizes, conform_all_modules},
...
pub struct OptimizationSettings {
    ...
    #[pyo3(get, set)]
    pub device_dtype: Option<TorchDeviceDtype>,
}
...
#[pyfunction(settings = "Default::default()")]
...
pub fn optimize_circuit(
    ...
    let circuit = timed!(strip_names(circuit, context, true), 10, print_timings);
...
pub fn optimize_to_schedule(
    ...
    let schedule = if settings.scheduling_naive {
        circuit_to_schedule_naive_toposort(optimized_circuit, context.settings.device_dtype)?
    } else {
        timed!(
