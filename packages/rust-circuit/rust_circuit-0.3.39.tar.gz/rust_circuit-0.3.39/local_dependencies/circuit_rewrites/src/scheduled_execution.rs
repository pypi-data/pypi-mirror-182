...
impl Schedule {
    ...
    pub fn evaluate(&self, settings: OptimizationSettings) -> Tensor {
        let eval = || {
            ...
            if self.device_dtype.device != TorchDevice::Cpu {
                Python::with_gil(|py| {
                    PY_UTILS
                        .torch
                        .getattr(py, "cuda")
...
pub fn circuit_to_schedule(
    ...
    let circuit_order: Vec<CircuitRc> = order
        ...
        .collect();
    if let Some(passed_dd) = context.settings.device_dtype {
        circuit
            .info()
            .device_dtype
            .combine(passed_dd.into())
            .context("circuit device_dtype must match OptimizationSettings device_dtype")?;
    }
    let (mut out, kept_keys) = order_to_schedule(
        ...
        context
            .settings
            .device_dtype
            .unwrap_or_else(|| get_compatible_dtype(&circuit)),
    );
...
pub fn circuit_to_schedule_naive_toposort(
    circuit: CircuitRc,
    device_dtype: Option<TorchDeviceDtype>,
) -> Result<Schedule> {
    ...
    if let Some(device_dtype) = device_dtype {
        circuit
            .info()
            .device_dtype
            .combine(device_dtype.into())
            .context("circuit device_dtype must match OptimizationSettings device_dtype")?;
    }
    let (mut result, kept_keys) = order_to_schedule(
        ...
        &scalars,
        HashSet::from_iter([circuit.info().hash]),
        device_dtype.unwrap_or_else(|| get_compatible_dtype(&circuit)),
    );
...
pub fn scheduled_evaluate(circuit: CircuitRc, settings: OptimizationSettings) -> Result<Tensor> {
    let schedule = if settings.scheduling_naive {
        circuit_to_schedule_naive_toposort(circuit, settings.device_dtype)
    } else {
        circuit_to_schedule(
            circuit.clone(),
                            ...
                            irreducible_to_id
                                ...
                                .unwrap_or_else(|| {
                                        ...
                                        })
                                    {
                                        ...
                                        result.constants.insert(this_key, irreducible.clone());
                                        next_key += 1;
                                        this_key
                                    }
                                });
