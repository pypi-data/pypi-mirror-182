...
impl RearrangeSpec {
    ...
    pub fn to_fancy_einops_string(
        ...
        let output_int_unpacked = self
            ...
            .map(|v| if v.len() == 1 { Some(v[0]) } else { None })
            .collect::<Option<Vec<AxisInt>>>()?;
        let mut axis_to_name: HashMap<AxisInt, &String> = Default::default();
        ...
        let inp_words = input_int_unpacked
            .iter()
            .map(|a| Some((*axis_to_name.get(a)?).clone()))
...
impl RearrangeSpec {
    ...
    pub fn expand_to_spec_or_shape(
        ...
        if shape.len() != self.input_ints.len() {
            bail!(RearrangeSpecError::InputNotConformable {
                spec: self.clone(),
                shape: shape.clone(),
            });
        }
...
pub enum RearrangeSpecError {
    ...
    InputNotConformable { spec: RearrangeSpec, shape: Shape },
