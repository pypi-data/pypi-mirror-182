...
def gen_index_function(
    ...
    if use_index_as_prefix:
        ...
        if moved_x.ndim == 1:
            ...
            flattened_x = moved_x.flatten(start_dim=1, end_dim=len(index.shape))
...
def random_indices(input: torch.Tensor, shape, replacement: bool, seed: Optional[int] = None) -> torch.Tensor:
    shape = tuple(shape)

    generator: Optional[torch.Generator] = (
        torch.Generator(device=input.device).manual_seed(seed) if seed is not None else None
    )
