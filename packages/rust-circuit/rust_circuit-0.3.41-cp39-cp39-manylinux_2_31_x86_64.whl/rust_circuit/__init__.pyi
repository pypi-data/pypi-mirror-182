...
class GeneralFunction(Circuit):
    ...
    @staticmethod
    def new_cast(
        x: Circuit, input_required_compatibility: TorchDeviceDtypeOp, output: TorchDeviceDtypeOp
    ) -> GeneralFunction: ...
...
class OptimizationSettings(object):
    ...
    device_dtype: Optional[TorchDeviceDtype]
    def __init__(
...
class Matcher(object):
    ...
    def match_any_child_found(finder: IterativeMatcherIn) -> Matcher:
        """alias for Matcher.match_any_found(IterativeMatcher(finder).filter(start_depth=1))"""
    @staticmethod
    ...
    def filter(
        ...
        """
        DEPRECATED: use top-level restrict(...) instead.
    ...
    def filter_sl(
        ...
        # if true, stops once it has found a match
        term_if_matches: bool = False,
        # depth range between which we match
...
class IterativeMatcher:
    ...
    def filter_sl(
        ...
        """
        DEPRECATED: use top-level restrict_sl(...) instead.
...
"""
DEPRECATED: use new_traversal() instead of traversal
and new_traversal(...) instead of traversal.filter(...)
"""
...
def evaluate(circuit: Circuit, device_dtype: TorchDeviceDtypeOp = TorchDeviceDtypeOp()) -> None: ...
...
def strip_names(node: Circuit, disable_autoname: bool = False) -> Circuit: ...
...
def batch_to_concat(circuit: Circuit, axis: int, num_batches: int, keep_non_axis_leaves: bool = False) -> Circuit: ...
...
class ModulePusher:
    ...
    def push_down_modules_get(
        self,
        circuit: Circuit,
        get: IterativeMatcherIn,
        skip_module: IterativeMatcherIn = IterativeMatcher(False),
        return_found_set: bool = True,
    ) -> Tuple[Circuit, Set[Circuit]]:
        """users should generally use push_down_modules or get{,_unique,_unique_op}_push_down_modules"""
    def __call__(
...
def extract_symbols(
    ...
    conform_batch_if_needed: bool = True,
    traversal: IterativeMatcherIn = traversal,
) -> Module: ...
...
def deep_module_remove_unused_inputs(
    circuit: Circuit, add_suffix_on_remove_unused: bool = True, use_elim_no_input_module: bool = True
) -> Circuit: ...
...
class RandomSampleSpec(SampleSpec):
    ...
    # If seeder is not None, called on every probs_and_group used for sampling. If returns an int, use it as seed for sampling.
    seeder: Optional[Callable[[Circuit], Optional[int]]]
    def __init__(
        self,
        shape: Shape = (),
        replace: bool = True,
        probs_and_group_evaluation_settings: OptimizationSettings = OptimizationSettings(),
        seeder: Optional[Callable[[Circuit], Optional[int]]] = None,
    ): ...
    @property
    def shape(self) -> Shape: ...
    @property
...
def einsum_flatten(einsum: Einsum, traversal: IterativeMatcherIn = traversal) -> Einsum: ...
def add_flatten(add: Add, traversal: IterativeMatcherIn = traversal) -> Add: ...
def nest_einsums(einsum: Einsum, spec: NestEinsumsSpecSub, traversal: IterativeMatcherIn = traversal) -> Einsum:
    """
...
def nest_adds(add: Add, spec: NestAddsSpecSub, traversal: IterativeMatcherIn = traversal) -> Add:
    """
