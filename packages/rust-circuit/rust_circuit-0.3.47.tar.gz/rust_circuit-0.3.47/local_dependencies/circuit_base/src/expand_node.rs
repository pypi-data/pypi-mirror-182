        ...
        if let Some(m) = circuit.as_module() {
                ...
                .any(|arg_spec| self.contains_key(&arg_spec.symbol.crc()))
            {
                ...
                return iter::once(Self::new(removed_map))
                    .chain(iter::repeat(self).take(m.nodes.len()).cloned())
                    .collect();
            }
        }
    ...
    let out = match &**circuit {
        Circuit::SetSymbolicShape(s) => {
            ...
            Ok(if s2 != *child_shape {
                SetSymbolicShape::nrc(c, s2, s.name_cloned())
            } else {
                c
            })
        }
                ...
                .map(|(inp, &b_rank)| {
                    ...
                    if rank_diff > 0 && b_rank > 0 {
                        inp.unsqueeze(
                            ...
                            inp.name().map(|x| format!("{} unsq_b_add", x)),
                        )
                        .unwrap()
                        .rc()
                    } else {
...
impl<F: ReplaceForExpand> ReplaceExpander<F> {
    ...
    pub fn replace_expand_with_map(
        ...
        expand_node(circuit, &new_children, |c, rep, child_idx| {
