...
pub fn index_fuse(index: &Index) -> Option<Index> {
    index.node.as_index().and_then(|inner| {
        Some(Index::new(
            ...
            index.name_cloned().map(|n| {
                if n.ends_with(" idx idx") {
                    n.strip_suffix(" idx").unwrap().to_owned()
                } else {
                    n
                }
            }),
...
pub fn generalfunction_pull_removable_axes(node: &GeneralFunction) -> Option<CircuitRc> {
    ...
    let removable_batchable_inp: HashSet<usize> = removable_axes_inp
        .iter()
        .filter(|x| **x < node.nodes[0].info().rank() - node.num_non_batchable_output_dims as usize)
        .cloned()
        .collect();
...
pub fn distribute_once_raw(
    ...
    let mut on_sub = |child_idx, node: CircuitRc, einsum: Einsum| -> Result<CircuitRc> {
        let einsum = if prefix.is_some() || suffix.is_some() {
            ...
            einsum.rename(node.name().map(|n_s| format!("{prefix}{n_s}{suffix}")))
        } else {
            einsum
        };
...
#[pyfunction(do_broadcasts = "true", prefix = "None", suffix = "None")]
...
pub fn push_down_index_raw(
    ...
    let mut on_sub = |child_idx,
                      node: CircuitRc,
                      index,
                      force_name: Option<Option<String>>|
     -> Result<CircuitRc> {
            ...
            if let Some(concat_indices) = concat_axis_indices_maybe {
                if matches!(old_index_on_concat_axis, TensorAxisIndex::Single(_)) {
                    ...
                    for (i, ax) in concat_indices.iter().enumerate() {
                        ...
                        if let Some(z) = ax {
                            ...
                            return on_sub(i, inner.nodes[i].clone(), index, Some(out_name));
                        }
                    }
                    unreachable!()
                } else {
                ...
                let new_concat = Concat::nrc(
                    inner
                        ...
                        .enumerate()
                        .map(|(i, operand)| {
                            on_sub(i, operand.clone(), index_non_axis.clone(), None)
                        })
                        .collect::<Result<_>>()?,
                    new_axis,
                    partial_name(),
                ...
                let new_operands = zip(&inner.nodes, &inner.input_batchability)
                    ...
                    .map(|(i, (child, &batchable))| {
                        if batchable {
                            ...
                            on_sub(i, child.clone(), index_passed, None)
                        } else {
                            Ok(child.clone())
                        }
                    })
        ...
        Circuit::Einsum(inner) => {
            ...
            let new_operands: Vec<(CircuitRc, EinsumAxes)> = inner
                ...
                .map(|(i, (node, ints))| {
                    ...
                    Ok((
                        on_sub(i, node.clone(), index_here, None)?,
...
pub fn einsum_permute_to_rearrange(einsum: &Einsum) -> Option<Rearrange> {
        ...
        && is_unique(&einsum.args[0].1)
    {
        return Some(Rearrange::new(
            ...
            RearrangeSpec::new(
                einsum.args[0].1.iter().map(|z| tu8v![*z]).collect(),
