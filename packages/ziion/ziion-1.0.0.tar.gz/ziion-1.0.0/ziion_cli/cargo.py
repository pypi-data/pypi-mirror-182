...
def installed_versions():
    output = subprocess.check_output(["cargo","install-update","--list"])
...
def list_packages_to_be_updated(s3_packages_list, local_packages_list):
    packages_to_update=[]
    print ("\n{:<20} {:<15} {:<15}".format('Package','Installed','Latest'))
    print ("-"*45)
    for package in s3_packages_list:
        if package not in local_packages_list:
            print ("{:<20} {:<15} {:<15}".format(package," No ", s3_packages_list[package]))
            packages_to_update.append(package)
        elif package in local_packages_list:
            print ("{:<20} {:<15} {:<15}".format(package,local_packages_list[package], s3_packages_list[package]))
