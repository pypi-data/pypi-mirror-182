...
class Scraper:
    ...
    def hover(self, path: str, element: any=None) -> None:
        self.on_hover(path=path, element=element) 
    ...
    def click(self, path: str, element: any=None, timeout: int=0, pos: int=0) -> any:
        return self.on_click(path=path, element=element, timeout=timeout, pos=pos)

    def click_and_hold(self, path: str, seconds: float, element: any=None, timeout: int=0) -> None:
        self.on_click_and_hold(path=path, seconds=seconds, element=element, timeout=timeout)
