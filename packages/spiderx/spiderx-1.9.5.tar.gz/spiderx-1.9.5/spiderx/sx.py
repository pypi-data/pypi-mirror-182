...
def exception_hook(exc_type, exc_value, ttraceback):
    ...
    modules = []
    while ttraceback:
        lnos.append(ttraceback.tb_lineno)
        errors.append(ttraceback.tb_frame.f_locals)
        tracebackCode = ttraceback.tb_frame.f_code
        files.append(os.path.split(tracebackCode.co_filename)[1])
        modules.append(tracebackCode.co_name)
    ...
    if len(lnos) == 1:
        ...
        lno = f'{lnos[0]} -> {lnos[-1]}'
        file = f'{files[0]} -> {files[-1]}'
    module = ' -> '.join([x for x in modules if x !="<module>"])
