...
class cmd(object):
    ...
    def __init__(self, config=CONFIG_FILE_LOCATION):
        ...
        # 登录成功 并且更新下数据
        if self.dp.username and self.config_json['password'] != self.password and self.config_json['username'] != self.username:
    ...
    def get_job_group_list(self, program_id):
        for job_group_detail in self.dp.get_job_group_list(program_id):
            print("ID: {:<10} | JOB_NAME: {:<20} | USER: {:<10} | JOB_TYPE: {:<10} | STATUS: {:<3} | CREATE_TIME: {:<10} | SPEND_TIME: {:<10} | COST:{:<5}".format(
                job_group_detail['id'],
                job_group_detail['job_name'],
                job_group_detail['user_name'],
                job_group_detail['job_type'],
    ...
    def get_job_list(self, job_id):
        for job_detail in self.dp.get_job_list(job_id):
            print("JOB_ID: {:<10} | SPEND_TIME: {:<10} | STATUS: {:<10} | RESULT: {:<10} | INPUT: {:<10} | COST:{:<5} | CREATE_TIME:{:<10} | UPDATE_TIME:{:<10}".format(
                job_detail['task_id'],
    ...
    def get_machine_type(self):
        for machine_type_detail in self.dp.get_machine_type():
            print("MACHINE_TYPE: {:<25} | CPU_CORE: {:<4} | GPU: {:<16} | MEM(GB): {:<4} | PLATFORM: {:<3} | SPOT_PRICE(yuan/h): {:<5} | ON_DEMAND_PRICE(yuan/h): {:<5}".format(
    ...
    def list_program(self):
        ...
        for program_detail in self.dp.list_program():
            print("Program ID: {:<10} | Program Name: {:<10} | Balance(yuan): {:<10} | Image Limit: {:<4} | Storage Limit(GB): {:<3}".format(
                program_detail['id'],
                program_detail['name'],
                program_detail['balance'] / 100,
                program_detail['image_limit'],
                program_detail['storage_limit']
                ))
    ...
    def list_image(self, program_id=1):
        ...
        while page_number * per_page < total:
            ...
            for image_detail in data['items']:
                print("Program Name: {:<10} | ID: {:<5} | Image Name: {:<30} | Status: {:<4} | Platform: {:<3} | Disk Size(GB): {:<5} | Comment: {:<20}".format(
...
def generate_config_file():
    ...
    config["endpoint"] = ask_for('endpoint', default=config.get('endpoint', "http://oss-cn-shenzhen.aliyuncs.com"), optional=True)
    ...
    config['password'] = ask_for('password',default=config.get('password'), optional=('password' in config), secret=True)
