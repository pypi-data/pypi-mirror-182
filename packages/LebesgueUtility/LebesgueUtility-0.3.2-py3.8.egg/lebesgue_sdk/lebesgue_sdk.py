...
class lebesgue_sdk(object):
    ...
    def _get_url(self, url, **kwargs):
        ...
        for i in range(5):
            ...
            try:
                ...
                res = requests.get(url, params=kwargs, headers=self.headers, cookies=self.cookies, timeout=30)
    ...
    def _post_url(self, url, json_data, retry_count=1, host=None):
        ...
        for i in range(retry_count):
            ...
            try:
                ...
                res = requests.post(
                    url, json=json_data, headers=self.headers, cookies=self.cookies, timeout=30)
    ...
    def list_program(self):
        program_list = []
        data = self.list_program_one_page()
        total = data['total']
        per_page = data['per_page']
        page_number = 0
        while page_number * per_page < total:
            page_number = page_number + 1
            if page_number > 1:
                data = self.list_program_one_page(page_number)
