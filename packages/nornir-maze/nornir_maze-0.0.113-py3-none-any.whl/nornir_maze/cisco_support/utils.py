...
def prepare_nornir_data(nr_obj: Nornir, verbose: bool = False) -> dict:
    ...
    # Return the serials dict if no static serials are present in the Nornir inventory
    if "static_serials" not in nr_obj.inventory.defaults.data["cisco_maintenance_report"]:
        return serials

    # Add static serials to the serials dict in case there are unmanaged switches
    for serial, items in nr_obj.inventory.defaults.data["cisco_maintenance_report"]["static_serials"].items():
        ...
        if isinstance(items, list):
            ...
            # If items is not a list, then add empty values for the hostname, switch number and the software
