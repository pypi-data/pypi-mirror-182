...
def _prepare_nornir_data_static_serials(serials: dict, nr_obj: Nornir) -> dict:
    """
    TBD
    """
    if isinstance(nr_obj.inventory.defaults.data["cisco_maintenance_report"]["static_serials"], str):
        # If static_serials is a str of a file path, then load the YAML file from that string path
        static_serials = load_yaml_file(
            file=nr_obj.inventory.defaults.data["cisco_maintenance_report"]["static_serials"],
            silent=True,
        )
        ...
        static_serials = nr_obj.inventory.defaults.data["cisco_maintenance_report"]["static_serials"]
    ...
    if isinstance(static_serials, list):
        ...
        for serial, items in static_serials.items():
            if isinstance(items, list):
                ...
                data[serial]["nr_data"]["desired_version"] = items[2] if 2 < len(items) else ""
                data[serial]["nr_data"]["current_version"] = items[3] if 3 < len(items) else ""
            else:
...
def prepare_nornir_data(nr_obj: Nornir, verbose: bool = False) -> dict:
    ...
    if "static_serials" in nr_obj.inventory.defaults.data["cisco_maintenance_report"]:
        serials = _prepare_nornir_data_static_serials(serials=serials, nr_obj=nr_obj)
