...
def rc_cisco_get_task(task: Task, yang_data_query: str) -> Result:
    ...
    restconf_port = task.host["restconf_port"] if "restconf_port" in task.host else "443"
    url = f"https://{task.host.hostname}:{restconf_port}{restconf_path}"
...
def rc_verify_current_software_version_task(task: Task, verbose=False) -> Result:
    ...
    restconf_port = task.host["restconf_port"] if "restconf_port" in task.host else "443"
    url = f"https://{task.host.hostname}:{restconf_port}{rc_path}"
    # RESTCONF HTTP header
    headers = {
        "Accept": "application/yang-data+json",
        "Content-Type": "application/yang-data+json",
