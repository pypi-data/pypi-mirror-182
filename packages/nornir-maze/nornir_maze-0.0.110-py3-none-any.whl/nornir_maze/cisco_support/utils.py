...
def prepare_nornir_data(nr_obj: Nornir, verbose: bool = False) -> dict:
    ...
    if "static_serials" in nr_obj.inventory.defaults.data["cisco_maintenance_report"]:
        for serial, host in nr_obj.inventory.defaults.data["cisco_maintenance_report"]["static_serials"]:
            # Add the hostname and Nornir data key value pair to the serial in the data dict
            data = {serial: {"host": host, "nr_data": {}}}
            # Add an empty value to the switch number, desired and current version for the unmanaged serials
