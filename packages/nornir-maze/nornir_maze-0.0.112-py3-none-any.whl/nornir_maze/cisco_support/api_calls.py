...
def _get_total_num_pages(**kwargs) -> int:
    ...
    return verify_cisco_support_api_data(api=api.lower(), iterable=response, force_failed=True, verbose=True)
    ...
    for value in iterate_all(iterable=iterable, returned="value"):
        ...
        if api.lower() in "ss":
            ...
            if any(value == error for error in SS_ERRORS):
                ...
                print("-> The initial PID list contains invalid PIDs. The returned list is not identical.")
                if add_info:
                    print(add_info)
            break
