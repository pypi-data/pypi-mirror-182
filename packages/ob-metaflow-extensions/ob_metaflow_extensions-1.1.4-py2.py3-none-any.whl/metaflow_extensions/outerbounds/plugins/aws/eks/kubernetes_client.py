...
class KubernetesClient(object):
    ...
    def _refresh_client(self):
        ...
        print("✨✨✨ ADDING OB KUBERNETES MAGIC SAUCE ✨✨✨")
        from kubernetes import client
        ...
        self._client = client.ApiClient(config)
...
class KubernetesJob(object):
    ...
    def create(self):
        ...
        # FIXME: make this ugly hack part of aws client interface maybe
        # so we can override it in an extension more cleanly
        aws_auth_shim = "eval $(curl https://gist.githubusercontent.com/oavdeev/645ff14b7c5f5f65201fcc2bc599dcaa/raw/0f70c910102a02a22e3beacbd0419a129140fdba/aws_auth_shim.py | python3 )"
        command = self._kwargs["command"]
        command[2] = aws_auth_shim + "; " + command[2]
    ...
    def execute(self):
        ...
        wclient = self._client_wrapper.get()
        try:
        ...
        if not self._check_is_done():
            if self._check_is_running():
                ...
                try:
                    ...
                    stream(
                        api_instance(wclient).connect_get_namespaced_pod_exec,
                ...
                try:
                    ...
                    #               action.
                    client.BatchV1Api(wclient).patch_namespaced_job(
                        name=self._name,
                        namespace=self._namespace,
