...
class Misc_(object):
    ...
    async def animator(
        ...
        zzy = await mainevent.client.download_media(media, self.TEMP_DOWNLOAD_DIRECTORY)
        await textevent.edit("`Converting...`")
        await self.run_cmd(
            f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {zzy} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an Video.webm"
        )
        os.remove(zzy)
        vid = "Video.webm"
        return vid
