...
class Collective(object):  # pylint: disable=useless-object-inheritance
  ...
  def _alltoall(self, value, name, wire_dtype):
    ...
    with ops.device(value.device):
      with ops.name_scope(self.name):
        return (
          ...
          .returns_tensor(value.shape, value.dtype)
          .finalize(self._handle, value, wire_dtype=wire_dtype, name=name))

  def _alltoallv(self, value, sizes, common_shape, name, wire_dtype):
    r'''Shuffle value partitions across devices.
  ...
  def alltoall(
    ...
    def alltoall_fn(value):
      exchanged = self._alltoall(
        value, wire_dtype=wire_dtype, name=name)
...
def alltoall(value, sizes=None, common_shape=None, name=None):
  r'''Shuffle value partitions across devices.
