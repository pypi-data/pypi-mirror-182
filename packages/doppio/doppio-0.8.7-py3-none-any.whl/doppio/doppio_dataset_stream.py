...
import os
import requests 
from doppio.server_code import check_json_from_server
...
class DoppioDataset(Dataset):
    ...
    def __init__(self, dataset=None, split_dir=None, task_info=None, augmentation=None, valid_classes=None, data_root_path=None):
        ...
        check_json_files = check_json_from_server(dataset=dataset, split_dir=split_dir, task_info=task_info, valid_classes=valid_classes, semantic_mask_flag=self.semantic_mask_flag)
    ...
    def access_to_server(self, index):
        ...
        check_file = os.path.join(self.local_image_path, img_path)
        # print("cehc_file", check_file)
        check_folder = Path(check_file)
        check_folder.mkdir(parents=True, exist_ok=True)

        # mask load
        if "segmentation_mask" in self.task:
            mask_path = os.path.join(self.url_mask_address, label_json_file['annotation'][0]['segmentation_path'])
            ...
            # 이미지 저장
            cv2.imwrite(os.path.join(self.local_mask_path, label_json_file['annotation'][0]['segmentation_path']), mask)
    ...
    def __getitem__(self, index):
        ...
        if self.local:
            ...
            try:
                ...
                print("Down load 완료했지요")
            except Exception as e:
               # DDDprint("error ", e)
                # json load
                label_json_file, im_rgb, mask = self.access_to_server(index)
        else:
            # json load
            label_json_file, im_rgb, mask = self.access_to_server(index)
