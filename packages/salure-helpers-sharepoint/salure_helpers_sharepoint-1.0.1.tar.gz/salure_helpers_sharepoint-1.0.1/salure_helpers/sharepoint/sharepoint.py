...
class Sharepoint:
    def __init__(self, salureconnect_connection: SalureConnect, label: Union[str, List], site: str = None, site_id: str = None, json_subset: int = None, site_name: str = None, debug: bool = False):
        """
        ...
        if site_name is not None:
            self.json_subset = 0
            self.site_id, site = self.get_site_id(site_name=site_name)
    ...
    def get_site_id(self, site_name: str) -> str:
        ...
        site_id, site = response.json()['value'][0]['id'], response.json()['value'][0]['siteCollection']['hostname']
        if self.debug:
            print(f"site_id: {site_id}, site_name: {site}")
