...
@click.argument("fqn", type=click.STRING, required=True)
@click.argument("job_command", nargs=-1, type=click.UNPROCESSED)
@handle_exception_wrapper
def trigger_job(fqn: str, job_command: Sequence[str]):
    response = experimental.trigger_job(fqn=fqn, command=job_command)
    if job_command:
