...
from servicefoundry.v2.lib import Build, LocalSource, Service
...
class FunctionService:
    ...
    def _maybe_add_tfy_service_root_url(self, workspace_fqn: str):
        from servicefoundry.internal.experimental import (
            TFY_SERVICE_ROOT_URL_ENV_KEY,
            get_path_route_for_single_port_service,
        )

        if TFY_SERVICE_ROOT_URL_ENV_KEY not in self._env:
            self._env[
                TFY_SERVICE_ROOT_URL_ENV_KEY
            ] = get_path_route_for_single_port_service(
    ...
    def deploy(self, workspace_fqn: str, wait: bool = True):
        if is_experimental_env_set():
            self._maybe_add_tfy_service_root_url(workspace_fqn=workspace_fqn)
