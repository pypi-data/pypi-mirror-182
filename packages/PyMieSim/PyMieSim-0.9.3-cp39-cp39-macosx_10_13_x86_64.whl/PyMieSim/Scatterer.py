...
class GenericScatterer():
    ...
    def get_poynting_vector(self, Mesh: FibonacciMesh) -> float:
        ...
        Ephi, Etheta = self._FarField(phi=Mesh.phi.Radian, theta=Mesh.theta.Radian, r=1., structured=False)
    ...
    def get_energy_flow(self, Mesh: FibonacciMesh) -> float:
        ...
        if Mesh.structured:
            Wtotal = 0.5 * numpy.sum(Poynting * Mesh.SinMesh) * Mesh.d_omega.Radian

        else:
            Wtotal = 0.5 * numpy.sum(Poynting) * Mesh.d_omega.Radian
...
class CoreShell(GenericScatterer):
    ...
    def _get_binding_(self) -> None:
        ...
        self.Bind = CORESHELL(shell_index=self.shell_index,
                              ...
                              shell_diameter=self.shell_diameter,
