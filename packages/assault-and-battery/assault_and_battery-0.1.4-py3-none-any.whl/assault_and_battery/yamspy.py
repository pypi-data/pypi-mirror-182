...
class MSPy:
    ...
    def __init__(
        ...
        self.SENSOR_DATA = {
            ...
            "debug": [
                ...
                0,
                0,
            ],  # 8 values for special situations like MSP2_INAV_DEBUG
        }
        ...
        self.REBOOT_TYPES = {"FIRMWARE": 0, "BOOTLOADER": 1, "MSC": 2, "MSC_UTC": 3}
    ...
    def __enter__(self):
        ...
        if self.is_serial_open:
            ...
            logging.warning(
                "Serial port ({}) not ready/available".format(self.conn.port)
            )
            return 1
    ...
    def connect(self, trials=100, delay=0.5):
        ...
        for _ in range(trials):
            try:
                ...
                logging.warning(
                    "Error opening the serial port ({0}): {1}".format(
                        self.conn.port, err
                    )
                )
    ...
    def fast_read_altitude(self):
        ...
        if self.send_RAW_msg(MSPy.MSPCodes["MSP_ALTITUDE"]):
            ...
            self.SENSOR_DATA["altitude"] = round(
                (converted_msg / 100.0), 2
    ...
    def fast_read_analog(self):
        ...
        if self.send_RAW_msg(MSPy.MSPCodes["MSP_ANALOG"]):
            ...
            self.ANALOG["voltage"] = (
                converted_msg[0] / 10
            )  # iNAV uses a MSPV2 message to get a precise value.
            self.ANALOG["mAhdrawn"] = converted_msg[1]
            self.ANALOG["rssi"] = converted_msg[2]  # 0-1023
            self.ANALOG["amperage"] = converted_msg[3] / 100  # A
            self.ANALOG["last_received_timestamp"] = int(
                time.time()
            )  # why not monotonic? where is time synchronized?
            if not self.INAV:
                self.ANALOG["voltage"] = (
                    converted_msg[4] / 100
                )  # BF has this 2 bytes value here
        ...
        with self.serial_port_read_lock:  # It's necessary to lock everything because order is important
            ...
            while True:
                try:
                    ...
                    logging.debug(
                        "IndexError detected on state: {}".format(dataHandler["state"])
                    )
                    dataHandler["state"] = -1
                ...
                if dataHandler["state"] == 0:  # sync char 1
                    ...
                    if dataHandler["msp_version"] == 1:
                        if dataHandler["message_checksum"] == data:
                            ...
                            logging.debug(
                                "Message received (length {1}) - Code {0}".format(
                                    dataHandler["code"],
                                    dataHandler["message_length_received"],
                                )
                            )
                        ...
                        dataHandler["message_checksum"] = self._crc8_dvb_s2(
                            dataHandler["message_checksum"], dataHandler["code"] & 0xFF
                        )  # code LOW
                        dataHandler["message_checksum"] = self._crc8_dvb_s2(
                            dataHandler["message_checksum"],
                        ...
                        dataHandler["message_checksum"] = self._crc8_dvb_s2(
                            dataHandler["message_checksum"],
                            (dataHandler["message_length_expected"] & 0xFF00) >> 8,
                        )  #  HIGH
    ...
    def process_recv_data(self, dataHandler):
        ...
        if code == 0:  # code==0 means nothing was received...
            ...
            if not dataHandler["unsupported"]:
                ...
                logging.debug(
                    "FC reports unsupported message error - Code {}".format(code)
                )
                return -6
    ...
    def process_MSP_RAW_IMU(self, data):
        ...
        # currently we are unable to differentiate between the sensor types, so we are going with 512
        # And what about SENSOR_CONFIG???
        self.SENSOR_DATA["accelerometer"][0] = self.readbytes(
            data, size=16, unsigned=False
        )
        self.SENSOR_DATA["accelerometer"][1] = self.readbytes(
            data, size=16, unsigned=False
        )
        self.SENSOR_DATA["accelerometer"][2] = self.readbytes(
            data, size=16, unsigned=False
        )
        ...
        # no clue about scaling factor (/1090), so raw
        self.SENSOR_DATA["magnetometer"][0] = self.readbytes(
            data, size=16, unsigned=False
        )
        self.SENSOR_DATA["magnetometer"][1] = self.readbytes(
            data, size=16, unsigned=False
        )
        self.SENSOR_DATA["magnetometer"][2] = self.readbytes(
            data, size=16, unsigned=False
        )
    ...
    def process_MSP_SERVO(self, data):
        ...
        self.SERVO_DATA = [
            self.readbytes(data, size=16, unsigned=True) for _ in range(servoCount)
        ]

    def process_MSP_MOTOR(self, data):
        motorCount = int(len(data) / 2)
        self.MOTOR_DATA = [
            self.readbytes(data, size=16, unsigned=True) for i in range(motorCount)
        ]
    ...
    def process_MSP_RC(self, data):
        ...
        self.RC["channels"] = [
            self.readbytes(data, size=16, unsigned=True) for i in range(n_channels)
        ]
    ...
    def process_MSP_ATTITUDE(self, data):
        self.SENSOR_DATA["kinematics"][0] = (
            self.readbytes(data, size=16, unsigned=False) / 10.0
        )  # x
        self.SENSOR_DATA["kinematics"][1] = (
            self.readbytes(data, size=16, unsigned=False) / 10.0
        )  # y
        self.SENSOR_DATA["kinematics"][2] = self.readbytes(
            data, size=16, unsigned=False
        )  # z
        ...
        if self.INAV:
            ...
            for i in range(voltage_meter_count):
                ...
                if subframe_length != 5:
                    ...
                    voltageMeterConfig["vbatresdivmultiplier"] = self.readbytes(
                        data, size=8, unsigned=True
                    )
    ...
    def process_MSP_MISC(self, data):  # 22 bytes
        if self.INAV:
            ...
            self.MISC["minthrottle"] = self.MOTOR_CONFIG[
                "minthrottle"
            ] = self.readbytes(
                data, size=16, unsigned=True
            )  # 0-2000
            self.MISC["maxthrottle"] = self.MOTOR_CONFIG[
                "maxthrottle"
            ] = self.readbytes(
                data, size=16, unsigned=True
            )  # 0-2000
            self.MISC["mincommand"] = self.MOTOR_CONFIG["mincommand"] = self.readbytes(
                data, size=16, unsigned=True
            )  # 0-2000
            self.MISC["failsafe_throttle"] = self.readbytes(
                data, size=16, unsigned=True
            )  # 1000-2000
            self.MISC["gps_type"] = self.GPS_CONFIG["provider"] = self.readbytes(
                data, size=8, unsigned=True
            )
            self.MISC["sensors_baudrate"] = self.MISC["gps_baudrate"] = self.readbytes(
                data, size=8, unsigned=True
            )
            self.MISC["gps_ubx_sbas"] = self.GPS_CONFIG["ublox_sbas"] = self.readbytes(
                data, size=8, unsigned=True
            )
            self.MISC["multiwiicurrentoutput"] = self.readbytes(
                data, size=8, unsigned=True
            )
            self.MISC["rssi_channel"] = self.RSSI_CONFIG["channel"] = self.readbytes(
                data, size=8, unsigned=True
            )
    ...
    def process_MSP_PID_ADVANCED(self, data):
        ...
        if not self.INAV:
            ...
            self.ADVANCED_TUNING["axisAccelerationLimitRollPitch"] = self.readbytes(
                data, size=16, unsigned=True
            )
