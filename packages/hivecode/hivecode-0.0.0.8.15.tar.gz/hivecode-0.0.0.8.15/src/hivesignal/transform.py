...
def fourier_transform(df, columns = None, time_col: str = None, normalized_amplitudes: bool = True):
    ...
    fftfreq_dict = dict(map(lambda column: (column, fftfreq(len(df_analog[column]), d=timestep[1])/fftfreq(len(df_analog[column]), d=timestep[1])[1]), df_analog.columns))
    ...
    if normalized_amplitudes:
        return list(map(lambda amplitud, frequency, mask: DataFrame({"amplitude": amplitud[mask].tolist(), "frequency": frequency[mask].tolist() / timestep.diff().mode()[0]}), fft_theory.values(), fftfreq_dict.values(), masks.values()))
    else:
        return list(map(lambda amplitud, frequency, mask: DataFrame({"amplitude": amplitud[mask].tolist(), "frequency": frequency[mask].tolist() / timestep.diff().mode()[0]}), fft_dict.values(), fftfreq_dict.values(), masks.values()))