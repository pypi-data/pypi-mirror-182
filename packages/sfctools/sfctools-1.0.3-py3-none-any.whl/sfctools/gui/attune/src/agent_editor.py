...
class CodeEditor(QtWidgets.QDialog):
    ...
    def __init__(self,parent,box,text=""):
        ...
        self.setStyleSheet(self.parent().parent().parent().theme_manager.get_stylesheet("main"))
        self.setStyleSheet(self.parent().parent().parent().theme_manager.get_background_style())
    ...
    def save_and_build(self):
        ...
        if original_name != new_name:
            ...
            if new_name not in self.parent().parent().code_data:
                # check if agent name already exists. if not, migrate code to new name

                old_data = self.parent().parent().code_data[original_name]
                self.parent().parent().code_data[new_name] = old_data

                del self.parent().parent().code_data[original_name]

                self.box.name = new_name
                self.parent().parent().parent().notify("The name of your agent has changed from %s to %s." % (original_name,new_name), title="Info")
                self.parent().parent().parent().update_table()

            else:
                self.parent().parent().parent().notify("An agent with the name %s already exists. Keeping %s." % (new_name,original_name), title="Error")


        #
        self.parent().parent().parent().save_and_build()
    ...
    def send_data(self):
        ...
        if self.backup_count == 1000:
            self.parent().parent().parent().auto_backup()
            self.backup_count = 0
        self.backup_count += 1
