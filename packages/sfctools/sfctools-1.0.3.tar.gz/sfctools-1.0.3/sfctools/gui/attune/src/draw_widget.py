    ...
    def edit_agent(self):
        if not self.ishelper:
            ...
            if self.name not in CodeEditor.instances:
                ...
                ce.setStyleSheet(ce.parent().parent().parent().theme_manager.get_stylesheet("main"))
                ce.setStyleSheet(ce.parent().parent().parent().theme_manager.get_background_style())
...
class MyDrawWidget(QtWidgets.QWidget):
    ...
    def __init__(self,parent):
        ...
        self.parent().parent().zoomOutButton.pressed.connect(self.zoom_out)
        self.parent().parent().zoomInButton.pressed.connect(self.zoom_in)

        self.parent().parent().graphEditButton.setEnabled(False)

        self.setMouseTracking(True)

        self.W0 =  int(self.parent().frameGeometry().width()-250) # 850
        self.H0 =  int(self.parent().frameGeometry().height()-180) # 750
    ...
    def resize_frame(self):
        ...
        self.W = int(self.parent().frameGeometry().width()-200) # 850
        self.H = int(self.parent().frameGeometry().height()-80) # 750
        
        self.maxx = self.W- 50 # 800
        self.maxy = self.H- 50 # 700
        # self.setGeometry(890,170,self.W,self.H)
        self.setGeometry(170,50,self.W,self.H)

    def export_pdf(self):
        success = False

        if self.parent().current_file is not None:
            try:
                filename = QFileDialog.getSaveFileName(self, 'Open file',os.path.dirname(os.path.abspath(self.parent().current_file)), "PDF Files (*.pdf)")[0]
                success = True
            except:
                pass
        if not success or self.parent().current_file is None:
            try:
                filename = QFileDialog.getSaveFileName(self, 'Open file',os.getcwd(), "PDF Files (*.pdf)")[0]
                success = True
            except Exception as e:
                self.parent().parent().notify("Something went wrong while exporting the Graph as PDF:\n" + str(e), title="Error")
    ...
    def highlighted(self,val):
        if val is None:
            self.parent().parent().graphEditButton.setEnabled(False)
            self.parent().parent().graphDeleteButton.setEnabled(False)

            self._highlighted = val
        else:
            if not val.ishelper: # is not a helper node (connection node)
                self.parent().parent().graphEditButton.setEnabled(True)
                # self.parent().graphDeleteButton.setEnabled(True)
                if val.n_connections == 0:
                    self.parent().parent().graphDeleteButton.setEnabled(True)
                else:
    ...
    def remove_agent(self):
        ...
        if box is not None and box.n_connections <= 0:
            ...
            self.parent().parent().notify("Box has still %i connection(s)! Cannot remove"%box.n_connections, title="Error")
    ...
    def curved_connector(self,qp,x0,y0,xf,yf,xi,yi,xd,yd,dashed=False,is_flow=True,is_unidir=True):
        ...
        try:
            ...
            if not dashed:
                if not end_is_start and self.parent().parent().checkBox.isChecked():
                    # left side
    ...
    def dawPreview(self,qp):
        ...
        if not self.parent().parent().checkBox_9.isChecked():
            return

        pen = QtGui.QPen(self.parent().parent().theme_manager.get_color(2),1)
        qp.setPen(pen)
        ...
        x = x_vals[-1]
        y = y_vals[-1]
        pen = QtGui.QPen(self.parent().parent().theme_manager.get_color(4),1)
        qp.setPen(pen)
        ox = x
        oy =  y
        d = 5
    ...
    def paint(self,reference,ratio=1):
        ...
        if self.parent().parent().checkBox_7.isChecked(): # draw boundary rect with white background?
        ...
        filter_vals = set([item.text() for item in self.parent().parent().filterCombo.selectedItems()])
        active_boxes = []
        active_connectors = []
        ...
        for conn in self.connectors:
            ...
            if conn.c == self.highlighted:
                ...
                pen = QtGui.QPen(self.parent().parent().theme_manager.get_color(4),2.0) # ,2, Qt.SolidLine)

                is_unidir = True
                for i,data in enumerate(self.parent().parent().entry_data): # TODO make this more efficient
                ...
                pen = QtGui.QPen(self.parent().parent().theme_manager.get_color(2),1.0)
            ...
            if conn in active_connectors:
                ...
                if conn.c == self.highlighted:
                    ...
                    pen = QtGui.QPen(self.parent().parent().theme_manager.get_color(1),1)
                    qp.setPen(pen)


                if not self.parent().parent().checkBox_8.isChecked():
                    conn_label_name = conn.c.name.replace("_"," ")
                else:
                    conn_label_name = conn.subject.replace("_"," ")

                if self.parent().parent().checkBox_5.isChecked(): # draw at handles
                    if self.parent().parent().checkBox_2.isChecked():
                        if self.highlighted == conn.c:
                            brush = QtGui.QBrush(self.parent().parent().theme_manager.get_color(4),1)
                        else:
                            brush = QtGui.QBrush(self.parent().parent().theme_manager.get_color(3),1)
                        ...
                        if self.parent().parent().checkBox_4.isChecked():
                            fm = qp.fontMetrics()
        ...
        for box in self.boxes:
            ...
            if box == self.selected:
                ...
                """
                br = QtGui.QBrush(self.parent().parent().theme_manager.get_color(0),1)


            elif box == self.highlighted:
                br = QtGui.QBrush(self.parent().parent().theme_manager.get_color(5))
            else:
                """
                if self.darkMode:
            ...
            if box.ishelper:
                pen = QtGui.QPen(self.parent().parent().theme_manager.get_color(6),1)

                if box.is_support:
                    br = QtGui.QBrush(self.parent().parent().theme_manager.get_color(7),1)
                else:
                    br = QtGui.QBrush(self.parent().parent().theme_manager.get_color(1),1)
                ...
                if self.parent().parent().checkBox_4.isChecked():
                    br_label = QtGui.QBrush(self.parent().parent().theme_manager.get_color(3),1)
                    fm = qp.fontMetrics()
            ...
            if box.ishelper:
                ...
                if box.is_support:
                    show_box = self.parent().parent().checkBox_6.isChecked()
                else:
                    show_box = self.parent().parent().checkBox_3.isChecked()
                ...
                pen = QtGui.QPen(self.parent().parent().theme_manager.get_color(3),1)
                qp.setPen(pen)
        self.dawPreview(qp)
        return qp
    ...
    def mousePressEvent(self, event):
        ...
        self.parent().parent().update_graphics_data(self.boxes,self.connectors)
            ...
            if self.mode != "move" and self.zoom_factor*(box.x+self.gox)  < x*self.zoom_factor  < self.zoom_factor*(box.x+self.gox + w) and self.zoom_factor*(box.y+self.goy) < y*self.zoom_factor  < self.zoom_factor*(box.y + w+self.goy):
                ...
                if self.mode == "drag":
                    ...
                    if box.ishelper and self.highlighted == box:
                        self.parent().parent().data_select_where(box.name)
