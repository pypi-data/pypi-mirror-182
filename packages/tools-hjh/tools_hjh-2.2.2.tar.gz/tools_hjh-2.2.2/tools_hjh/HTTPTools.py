...
class HTTPTools():
    ...
    @staticmethod
    def get_page_by_chrome(url, headers=None, data=None, proxies=None, chrome=None, chrome_path=None, chromedriver_path=None, is_hidden=False, is_display_picture=True):
        """ 使用浏览器解析url，返回源码
            param：
                chrome: 给入一个chrome的webdriver对象，如果为空，chrome_path、chromedriver_path需要给人，会启动一个，解析完了就关闭
                chrome_path: chrome.exe路径
                chromedriver_path: chromedriver.exe路径
        """
        if chrome is None:
            from tools_hjh.HTTPRequest import Chrome
            chrome = Chrome(chrome_path=chrome_path, chromedriver_path=chromedriver_path, is_hidden=is_hidden, is_display_picture=is_display_picture, proxies=proxies)
            return chrome.get(url)
        else:
            chrome.get(url)
    ...
    @staticmethod
    def N_m3u8DL_CLI(url, workDir, dstfile, theadnum=32, exePath='N_m3u8DL-CLI'):
        if workDir is None:
            workDir = os.path.abspath(os.path.dirname(__file__)) + '/' + dstfile.rsplit('/', 1)[0]
        dstfile = dstfile.split('/')[-1]
        cmd = exePath + ' --workDir ' + workDir + ' --saveName ' + dstfile + ' --maxThreads ' + str(theadnum) + ' --enableDelAfterDone ' + url
        print(cmd)
        os.system(cmd)
