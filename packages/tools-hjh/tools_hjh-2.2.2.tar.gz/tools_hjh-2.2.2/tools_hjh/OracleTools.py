...
class OracleTools:
        ...
        if 'successfully completed' in mess:
            ...
            for line in lines:
                if 'Total' in line or 'expdp' in line:
                    ...
                    line = tools.merge_spaces(line.replace('"', '')).strip()
            ...
            for line in lines:
                if '.   "' in line:
                    line = tools.merge_spaces(line.replace('"', '').replace('\n', '')).strip()
    ...
    def analysis_tns(host_conn):
        ...
        host_map = tools.analysis_hosts(host_conn)
        ...
        tns_str2 = tools.merge_spaces(tns_str2)
        ...
        for path in paths.split('\n'):
            ...
            for line in mess.split('\n'):
                if line.startswith('MANAGER'):
                    lines = tools.merge_spaces(line).split(' ')
        ...
        for ogg in ogg_status.get_rows():
            if ogg[3] != 'manager':
                ...
                for line in param.split('\n'):
                    line_ = tools.merge_spaces(line).strip().lower().replace(', ', ',').replace('; ', ';')
                ...
                if ogg_type == 'ext':
                    ...
                    for line in param.split('\n'):
                        ...
                        if line_.startswith('replicat '):
                            ...
                            line_ = line_.replace(',', ' ')
                            line_ = tools.merge_spaces(line_)
