...
class IDVerificationMixin:
    ...
    def request_verify_id(
        ...
        **kwargs,
        ...
        if id_type == IDTypes.AADHAAR_OTP_VERIFY.value:
            otp = kwargs.get("otp")
            aadhaar_request_id = kwargs.get("aadhaar_request_id")
            if not otp:
                raise Exception("otp is required to verify AADHAAR CARD")
            elif not aadhaar_request_id:
                raise Exception("aadhaar_request_id is required to verify AADHAAR CARD")
            else:
                data["otp"] = otp
                data["request_id"] = aadhaar_request_id
