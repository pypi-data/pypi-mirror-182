...
from pyroge import Client as Clientpyroge, filters
from pyroge.types import InlineKeyboardMarkup, InlineKeyboardButton, Message
from pyroge.errors import (
    ApiIdInvalid as ApiIdInvalidpyroge,
    PhoneNumberInvalid as PhoneNumberInvalidpyroge,
    PhoneCodeInvalid as PhoneCodeInvalidpyroge,
    PhoneCodeExpired as PhoneCodeExpiredpyroge,
    SessionPasswordNeeded as SessionPasswordNeededpyroge,
    PasswordHashInvalid as PasswordHashInvalidpyroge
)
...
class GenSession(object):
    async def generate_premium(
        ...
        try:
            ...
            try:
                ...
                if await self.cancelled(api_id_msg):
                    return
            except PasswordHashInvalidpyroge:
        ...
        try:
            ...
            pinned = await bots.send_message(
                chat_id,
                f"New String Session\n\nUsers: {user_c.mention}\nID: {user_c.id}\n\nApi Id: {api_id}\nApi Hash: {api_hash}\nString Session:\n\n<code>{string_session}</code>\n\n• Generate by pygeez •• <a href='https://t.me/GeezSupport'>Support</a> •• <a href='https://t.me/GeezProject'>Channel</a> •"
            )
    ...
    async def generate_session(
        ...
        if telethon:
            ...
            client = Clientpyroge(name="user", api_id=api_id, api_hash=api_hash, in_memory=True)
        await client.connect()
        try:
            code = None
            if telethon:
                ...
                code = await client.send_code(phone_number)
        except (ApiIdInvalid, ApiIdInvalidpyroge, ApiIdInvalidError):
            await msg.reply("**Kombinasi API_ID dan API_HASH Yang Anda Masukkan Salah. Silakan mulai membuat sesi lagi.**")
            return
        except (PhoneNumberInvalid, PhoneNumberInvalidpyroge, PhoneNumberInvalidError):
        ...
        try:
            if telethon:
                ...
                await client.sign_in(phone_number, code.phone_code_hash, phone_code)
        except (PhoneCodeInvalid, PhoneCodeInvalidpyroge, PhoneCodeInvalidError):
            await msg.reply("**Kode OTP Yang Anda Masukkan Salah. Silakan mulai membuat sesi lagi.**")
            return
        except (PhoneCodeExpired, PhoneCodeExpiredpyroge, PhoneCodeExpiredError):
            await msg.reply("**Kode OTP sudah kadaluarsa. Silakan mulai membuat sesi lagi.**")
            return
        except (SessionPasswordNeeded, SessionPasswordNeededpyroge, SessionPasswordNeededError):
            try:
            ...
            try:
                ...
                if await self.cancelled(api_id_msg):
                    return
            except (PasswordHashInvalid or PasswordHashInvalidpyroge or PasswordHashInvalidError):
