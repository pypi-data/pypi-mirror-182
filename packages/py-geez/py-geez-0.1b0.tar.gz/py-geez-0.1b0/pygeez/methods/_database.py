...
try:
    from pygeez.exceptions import DependencyMissingError
except ImportError:
    from pygeez.exceptions import DependencyMissingError
...
class MongoDB:
    def __init__(self, key, dbname="geezDB"):
        self.dB = MongoClient(key, serverSelectionTimeoutMS=5000)
        self.db = self.dB[dbname]
        self.re_cache()

    def __repr__(self):
        return f"<pygeez.MonGoDB\n -total_keys: {len(self.keys())}\n>"
...
class SqlDB:
    def __init__(self, url):
        ...
        try:
            ...
            self._cursor.execute(
                "CREATE TABLE IF NOT EXISTS pygeez (pygeezCli varchar(70))"
            )
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('pygeez')) AS size"
        )
    ...
    def keys(self):
        self._cursor.execute(
            "SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name  = 'pygeez'"
        )  # case sensitive
        data = self._cursor.fetchall()
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM pygeez")
    ...
    def set_key(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE pygeez DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE pygeez ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO pygeez ({key}) values (%s)", (str(value),))
        return True
    ...
    def del_key(self, key):
        if key in self._cache:
            del self._cache[key]
        try:
            self._cursor.execute(f"ALTER TABLE pygeez DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
    ...
    def flushall(self):
        self._cache.clear()
        self._cursor.execute("DROP TABLE pygeez")
        self._cursor.execute(
            "CREATE TABLE IF NOT EXISTS pygeez (pygeezCli varchar(70))"
        )
        return True
