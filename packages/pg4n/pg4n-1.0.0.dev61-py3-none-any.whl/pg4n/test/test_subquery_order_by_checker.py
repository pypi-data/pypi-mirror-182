...
WHERE EXISTS (SELECT *
              FROM (SELECT *
                    FROM {ORDERS_TABLE_NAME} o2
                    ORDER BY o2.customer_id) AS o
              WHERE c.customer_id = o.customer_id);"""

    print("SQL_WITHOUT_OUTER_ORDER")
    sql_statement = SQL_WITHOUT_OUTER_ORDER
    ...
    print("SQL_NESTED_SUBQUERY_INNER_ORDER")
    sql_statement = SQL_NESTED_SUBQUERY_INNER_ORDER
    parsed_sql = sql_parser.parse_one(sql_statement)
    qep_analysis = qep_parser(sql_statement)
    checker = SubqueryOrderByChecker(parsed_sql, qep_analysis)
    assert checker is not None
