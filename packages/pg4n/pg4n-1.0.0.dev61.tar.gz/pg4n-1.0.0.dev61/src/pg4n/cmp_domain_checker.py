...
class CmpDomainChecker:
    ...
    def _are_from_compatible_domains(self, a: Column, b: Column) -> bool:
        ...
        if a.type.digits != b.type.digits:
            ...
            if a.type.precision is None and b.type.precision is None:
                return True
            elif (a.type.precision is None and b.type.precision is not None) or (
                a.type.precision is not None and b.type.precision is None
            ):
                return False
    ...
    def _detect_suspicious_cmp(self, cmp: exp.Predicate, columns: list[Column]):
        ...
        for column_expression in column_expressions:
            cmp_column_name = SqlParser.get_column_name_from_column_expression(
                column_expression
            )
    ...
    def _detect_suspicious_cmps(
        self, select_statement: exp.Select, columns: list[Column]
    ):
