...
class PsqlConnInfo:
    ...
    tok_pre_database: ParserElement = Literal('You are connected to database "')
    tok_database: ParserElement = Word(identbodychars)

    tok_pre_user: ParserElement = Literal('" as user "')
    tok_user: ParserElement = Word(identbodychars)

    tok_pre_host: ParserElement = Literal('" on host "') | Literal('" via socket in "')
    tok_host: ParserElement = Word(identbodychars + "/.")

    tok_pre_port: ParserElement = Literal('" at port "')
    tok_port: ParserElement = Word(nums)
    tok_end: ParserElement = Literal('".')
    ...
    def get(self) -> Optional[Tuple[str, str, str, str, str]]:
