...
__version__ = "0.0.4"
BASE_PATH = os.path.join(os.path.expanduser("~"), ".objaverse", "hf-objaverse-v1")
...
def _download_object(
    ...
    os.makedirs(os.path.dirname(local_path), exist_ok=True)
    urllib.request.urlretrieve(hf_url, local_path)
    files = glob.glob(os.path.join(BASE_PATH, "glbs", "*", "*.glb"))
    print(
        "Downloaded",
...
def load_objects(uids: List[str], download_processes: int = 1) -> Dict[str, str]:
    ...
    if download_processes == 1:
        ...
        if len(uids_to_download) == 0:
            return out
        start_file_count = len(glob.glob(os.path.join(BASE_PATH, "glbs", "*", "*.glb")))
...
def load_lvis_annotations() -> Dict[str, List[str]]:
    ...
    local_path = os.path.join(BASE_PATH, "lvis-annotations.json.gz")
    os.makedirs(os.path.dirname(local_path), exist_ok=True)
    if not os.path.exists(local_path):
        urllib.request.urlretrieve(hf_url, local_path)
    with gzip.open(local_path, "rb") as f:
...
if __name__ == "__main__":
    object_paths = _load_object_paths()
    uids = [k for k, v in object_paths.items() if v.startswith("glbs/000-000")][-100:]
