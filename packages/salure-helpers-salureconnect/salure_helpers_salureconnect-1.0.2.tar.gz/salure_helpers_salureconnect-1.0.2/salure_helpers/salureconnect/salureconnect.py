...
class SalureConnect:
    def __init__(self, customer: str = None, api_token: str = None, staging: bool = False):
        if customer is not None and api_token is not None:
            self.customer = customer
            self.api_token = api_token
        elif os.getenv("SALURECONNECT_CUSTOMER_NAME") is not None and os.getenv("SALURECONNECT_API_TOKEN") is not None:
            self.customer = os.getenv("SALURECONNECT_CUSTOMER_NAME")
            self.api_token = os.getenv("SALURECONNECT_API_TOKEN")
        else:
            raise ValueError("Set the environment variables SALURECONNECT_CUSTOMER_NAME and SALURECONNECT_API_TOKEN or provide the customer and api_token parameters")
