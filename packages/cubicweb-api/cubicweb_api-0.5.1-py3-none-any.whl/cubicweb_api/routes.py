...
from cubicweb_api.util import get_cw_repo, get_cw_request, get_transactions_repository
...
def catch_rql_errors(func: Callable):
    """
...
def rql_route(request: Request):
    ...
    return catch_rql_errors(lambda: get_cw_request(request).execute(query, params).rows)
...
def transaction_execute_route(request: Request):
    ...
    query: str = request_params["query"]
    params: dict = request_params["params"]
    return catch_rql_errors(lambda: transactions[uuid].execute(query, params).rows)
...
def transaction_commit_route(request: Request):
    ...
    request_params = request.openapi_validated.body
    uuid: str = request_params["uuid"]
    return catch_rql_errors(lambda: transactions[uuid].commit())
