...
from .equity_curves_intraday import *
from .signals import *
...
def generate_filepath(para_combination):
    ...
    filepath = os.path.join(output_folder, f'{save_name}.csv')
...
def mp_cal_performance(tuple_data):
    ...
    intra_day = tuple_data[2]
    summary = tuple_data[3]

    result = cal_performance(para_combination, intra_day, summary)
    index = para_combination['reference_index']
    manager_list.append((index,result))


def generate_backtest_result(all_para_combination, number_of_core=8, intra_day=False, backtest_summary_mode=False):
    summary = backtest_summary_mode
    ...
    for para_combination in all_para_combination:
        cal_performance_list.append((para_combination, manager_list, intra_day, summary))
...
def get_result_df(py_filename, start_date, end_date, para_dict, all_para_combination, number_of_core=8, backtest_output_folder='backtest_output', data_folder='data', intra_day=False):
    ...
    for ii in range(len(result_df)):
        save_path = generate_filepath(py_filename, backtest_output_folder, start_date, end_date, para_dict, result_df.iloc[ii])
        code = result_df.iloc[ii]['code']
        data_path = os.path.join(data_folder, f'{code}.csv')
        cal_performance_list.append((save_path, start_date, end_date, manager_list, ii, data_path, intra_day))
...
def plot(mode, all_para_combination={}, enable_subchart={}, subchart_settings={}, intraday=False, backtest_summary_mode=False, freq='15T'):
    summary = backtest_summary_mode

    if mode == 'equity_curves':
        result_df = pd.read_csv('backtest_result.csv', index_col=0)
        if intraday:
            app = equity_curves_intraday.Plot(all_para_combination, result_df, subchart_settings, intraday, summary, freq)
        else:
            # app = equity_curves.Plot(all_para_combination, result_df, subchart_settings)
            app = equity_curves_intraday.Plot(all_para_combination, result_df, subchart_settings, intraday, summary, freq)
...
def calculate_sharp_ratio(df_csv, col):
    df = df_csv.copy()
...
def resample_summary_to_daily(para_combination, intra_day):
    # Need to deal with the start date and date

    start_date = para_combination['start_date']
    end_date = para_combination['end_date']
    sectype = para_combination['sectype']
    lot_size = para_combination['lot_size']
    sec_profile = para_combination['sec_profile']
    margin_req = sec_profile['margin_req']
    multiplier = sec_profile['multiplier']

    # Read backtest data
    save_path = generate_filepath(para_combination)
    ...
    data_path = os.path.join(data_folder, code + '.csv')
...
def cal_performance(para_combination, intra_day, summary):

    start_date = para_combination['start_date']
    end_date = para_combination['end_date']

    if (intra_day or summary):
        df_daily = resample_summary_to_daily(para_combination, intra_day)
    else:
        save_path = generate_filepath(para_combination)
        df_backtest = pd.read_csv(save_path, index_col=0)  # Daraframe that may not be daily
        df_backtest['date'] = pd.to_datetime(df_backtest['date'], format='%Y-%m-%d')
    ...
    net_profit, holding_period_day, return_on_capital, annualized_return, annualized_std, annualized_sr = calculate_sharp_ratio(df, 'equity_value')
...
def cal_performance_old(save_path, start_date, end_date):
    result_dict = {}
