...
def download(
    ...
    if size_limit and total_size > size_limit:
        print('文件大小超出限制，不下载')
        is_finish = True
        size_limit_state = True
    else:
        is_finish = False
        size_limit_state = False
    ...
    res_dict = {
        ...
        'size': total_size,
        'temp_size': temp_size,
        'size_limit_state': size_limit_state
    }
    return res_dict
...
def safe_download(
    ...
    while True:
        ...
        if download_response['is_finish'] and not download_response['size_limit_state']:
            local_file_dir = download_response.get('file_dir')
            return local_file_dir
        elif download_response['is_finish'] and download_response['size_limit_state']:
            return
        else:
            print(':( 下载中断')
