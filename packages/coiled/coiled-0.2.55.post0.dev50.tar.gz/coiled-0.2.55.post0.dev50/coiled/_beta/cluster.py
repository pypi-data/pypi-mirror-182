...
class ClusterBeta(Cluster, Generic[IsAsynchronous]):
    ...
    use_best_zone
        Allow the cloud provider to pick zone that has best availability for your requested instances.
        Note that all workers will be in a single zone (so there's no cost for network traffic between
        workers), but scheduler may be in a different zone that the workers. Traffic between scheduler
        and workers is typically small (often under 1GB, usually bounded by amount of memory on scheduler)
        so the network costs are almost always less than what you'll save by getting better instance
        availability, especially if using spot instances.
    ...
    def __init__(
        ...
        *,
        ...
        use_best_zone: bool = False,  # off by default for now
    ...
    async def _determine_senv(self) -> Tuple[bool, Optional[int]]:
        ...
        if self.package_sync:
            ...
            packages_with_notes = [pkg for pkg in approximation if pkg["note"]]
            ...
            if HAS_RICH and self.show_widget:
                from coiled._beta.widgets.rich import print_rich_package_table
    ...
    def _check_halting_issues(
        ...
        if halting_failures:
            failure_str = ", ".join(
                [f'{pkg["name"]} - {pkg["error"]}' for pkg in halting_failures]
            )
