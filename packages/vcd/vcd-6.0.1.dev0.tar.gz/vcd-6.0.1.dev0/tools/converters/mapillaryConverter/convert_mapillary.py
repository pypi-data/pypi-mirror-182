...
def convert_labels_to_poly(img_name, vcd_poly_mode):
    ...
    start = time.time()
    vcd_class = converter.mapillary_classes_to_vcd(
        img_name=img_name,
    ...
    print(f"\tConverted class labels to VCD in {(end - start) * 1000:.2f} ms")

    start = time.time()
    vcd_class.save(
        "vcd_files/"
        + root_name
        + img_name[0 : len(img_name) - 4]
        + "_class_"
        + vcd_poly_mode.name
        + ".json"
    )
    ...
    print(f"\tSaved to VCD JSON file in {(end - start) * 1000:.2f} ms")

    start = time.time()
    img_class_out = converter.vcd_mapillary_classes_to_png(
        vcd=vcd_class, mapillary_config=mapillary_config
    )
    end = time.time()
    print(f"\tClass reconstructed from VCD in {(end - start) * 1000:.2f} ms")
    ...
    if show_images:
        ...
        cv.resizeWindow(
            img_name + vcd_poly_mode.name, img_label.shape[0], img_label.shape[1]
        )
...
def convert_instances_to_poly(img_name, vcd_poly_mode):
    print("VCD Poly2d mode " + vcd_poly_mode.name)
    start = time.time()
    vcd_ins = converter.mapillary_instances_to_vcd(
        img_name=img_name,
        mapillary_config=mapillary_config,
        vcd_poly_mode=vcd_poly_mode,
        vcd_root_name=root_name,
    ...
    print(f"\tConverted instances to VCD in {(end - start) * 1000:.2f} ms")
    ...
    start = time.time()
    img_instances_out = converter.vcd_mapillary_instances_to_png(
        vcd=vcd_ins, mapillary_config=mapillary_config
    )
    end = time.time()
    print(f"\tInstances reconstructed from VCD in {(end - start) * 1000:.2f} ms")
    ...
    if show_images:
        ...
        cv.resizeWindow(
            img_name + "_instances", img_instances.shape[0], img_instances.shape[1]
        )
...
def convert_labels_to_base64(img_name):
    ...
    vcd.add_stream(
        stream_name="Camera",
        uri=img_name,
        description="Mapillary image",
        stream_type=core.StreamType.camera,
    )
    vcd.add_stream_properties(
        stream_name="Camera", properties={"height": height, "width": width}
    )
    ...
    vcd.save(
        "vcd_files/" + root_name + img_name[0 : len(img_name) - 4] + "_class_b64.json"
    )
...
def convert_instances_to_base64(img_name):
    ...
    cv.imwrite(
        ...
        + root_name
        + img_name[0 : len(img_name) - 4]
        + "_instances_reconstructed_from_b64.png",
    ...
    if show_images:
        ...
        cv.namedWindow(img_name + "_b64", cv.WINDOW_NORMAL)
        cv.resizeWindow(
            img_name + "_b64", img_instances.shape[0], img_instances.shape[1]
        )
        cv.imshow(img_name + "_b64", stack)
        cv.waitKey(0)
...
for img_name in os.listdir(path):
    ...
    convert_labels_to_poly(
        img_name=img_name, vcd_poly_mode=types.Poly2DType.MODE_POLY2D_SRF6DCC
    )
    convert_labels_to_poly(
        img_name=img_name, vcd_poly_mode=types.Poly2DType.MODE_POLY2D_ABSOLUTE
    )

    # Convert instance images
    convert_instances_to_base64(img_name=img_name)
    convert_instances_to_poly(
        img_name=img_name, vcd_poly_mode=types.Poly2DType.MODE_POLY2D_SRF6DCC
    )
    convert_instances_to_poly(
        img_name=img_name, vcd_poly_mode=types.Poly2DType.MODE_POLY2D_ABSOLUTE
