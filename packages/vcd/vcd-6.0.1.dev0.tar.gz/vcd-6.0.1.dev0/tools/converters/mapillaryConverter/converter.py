...
def mapillary_instances_to_vcd(
    ...
    img_ids_array_bgr = cv.cvtColor(
        img_ids_array, cv.COLOR_GRAY2BGR
    )  # color_rgb bgr so we can segment
    ...
    for class_index, label in enumerate(mapillary_config["labels"]):
        ...
        idx = np.where(
            np.logical_and(img_instances >= range_min, img_instances < range_max)
        )
        ...
        for val in range(min_val, max_val + 1):
            ...
            uid = int(
                class_index * 256 + id
            )  # so it is unique and equal to the value of the pixels in the PNG16b
            ...
            for c in contours:
                if is_instance:
                    if not vcd.has(core.ElementType.object, uid):
                        vcd.add_object(
                            name="instance" + str(uid),
                            semantic_type=class_name,
                            uid=uid,
                        )
                        if verbose:
                    ...
                    if "object_data" in vcd.get_root()["objects"][str(uid)]:
                        cnt_id = len(
                            vcd.get_root()["objects"][str(uid)]["object_data"]["poly2d"]
                        )
                    ...
                    if vcd.has(
                        core.ElementType.object, uid
                    ):  # this may happen with instances of value 0
                        continue
    ...
    for object_key, object_val in vcd.get_objects().items():
        ...
        if not mapillary_config["labels"][class_idx]["instances"]:
            ...
            cv.drawContours(
                ...
                -1,
                class_idx,
                hierarchy=np.asarray([hierarchies]),
                thickness=-1,
            )
        else:
