...
class KITTI_Tracking_reader:
    def __init__(self, kitti_tracking_base_path, num=None):
        self.kitti_tracking_calib_path = os.path.join(
            kitti_tracking_base_path, "training/calib"
        )
        self.kitti_tracking_oxts_path = os.path.join(
            kitti_tracking_base_path, "training/oxts"
        )
        self.kitti_tracking_objects_path = os.path.join(
            kitti_tracking_base_path, "training/label_02"
        )
    ...
    def parse_sequence_direct(self, seq_number):
        ...
        calib_file_name = os.path.join(
            self.kitti_tracking_calib_path, str(seq_number).zfill(4) + ".txt"
        )
        oxts_file_name = os.path.join(
            self.kitti_tracking_oxts_path, str(seq_number).zfill(4) + ".txt"
        )
        object_file_name = os.path.join(
            self.kitti_tracking_objects_path, str(seq_number).zfill(4) + ".txt"
        )
        ...
        transform_velo_to_camleft_3x4 = np.reshape(
            calib_matrices["Tr_velo_cam"], (3, 4)
        )  # WRT to LEFT CAMERA ONLY
        camera_rectification_4x4 = np.vstack(
            (np.hstack((camera_rectification_3x3, [[0], [0], [0]])), [0, 0, 0, 1])
        )
        ...
        for row in object_reader:
            ...
            alpha = utils.float_2dec(
                float(row[5])
            )  # this is the observation angle (see cs_overview.pdf)
            ...
            cuboid_vals = [
                ...
                utils.float_2dec(locZ),
                0,
                utils.float_2dec(rotY),
                0,
            ...
            cuboid = types.cuboid(
                name="box3D", val=cuboid_vals, coordinate_system="CAM_LEFT"
            )
            ...
            if not vcd.has(core.ElementType.object, str(trackID)):
                ...
                if trackID >= 0:
                    vcd.add_object(
                        name=semantic_class + str(trackID),
            ...
            vcd.add_object_data(
                trackID, types.num(name="truncated", val=truncated), frameNum
            )
            vcd.add_object_data(
                trackID, types.num(name="occluded", val=occluded), frameNum
            )
            ...
            if trackID != -1:  # Don'tCare objects
                ...
                bounding_box_right = types.bbox(
                    name=name_box + "_right",
                    val=bbox_right_vals,
...
def convert_KITTI_tracking_to_VCD4():
    ...
    for count, key in enumerate(kitti_parser.vcds):
        ...
        vcd_file_name = os.path.join(
            kitti_tracking_output_vcd_path,
