...
from vcd import core, draw, schema, scl, types, utils
...
def simple_setup_4_cams_fisheye():
    ...
    z2 = -(-0.12 * np.pi) / 180.0
    R_scs_wrt_lcs = utils.euler2R(
        [z1, x1, z2], seq=utils.EulerSeq.ZXZ
    )  # default is ZYX
...
def draw_scene(vcd):
    ...
    img_front = 255 * np.ones(
        (img_height_px, img_width_px, 3), np.uint8
    )  # cv.imread('./png/fisheye_xz1z2/front.jpg')
    img_rear = 255 * np.ones(
        (img_height_px, img_width_px, 3), np.uint8
    )  # cv.imread('./png/fisheye_xz1z2/rear.jpg')
    img_left = 255 * np.ones(
        (img_height_px, img_width_px, 3), np.uint8
    )  # cv.imread('./png/fisheye_xz1z2/left.jpg')
    img_right = 255 * np.ones(
        (img_height_px, img_width_px, 3), np.uint8
    )  # cv.imread('./png/fisheye_xz1z2/right.jpg')
...
def add_some_objects(vcd):
    ...
    cuboid1 = types.cuboid(
        name="box3D",
        val=(4.5, 3.5, 0.7, 0.0, 0.0, (90.0 * np.pi) / 180.0, 4.2, 1.8, 1.4),
...
if __name__ == "__main__":
    ...
    vcd = vcd_draw_pinhole_4cams.add_some_objects(
        vcd
    )  # so let's add the same objects as in vcd_draw_pinhole_4cams
    ...
    vcd.save(
        ...
        + "_test_scl_camera_fisheye_xz1z2_radial_poly.json"
    )
