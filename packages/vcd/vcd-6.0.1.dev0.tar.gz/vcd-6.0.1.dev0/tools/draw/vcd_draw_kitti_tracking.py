...
from vcd import core, draw, schema, scl
...
def draw_kitti_tracking(sequence_number, record_video, draw_images):
    ...
    # Get video
    video_file_name_left = (
        "../../../../data/kitti/tracking/video/left/"
        + str(sequence_number).zfill(4)
        + ".mp4"
    )
    video_file_name_right = (
        "../../../../data/kitti/tracking/video/right/"
        + str(sequence_number).zfill(4)
        + ".mp4"
    )
    ...
    cv.resizeWindow(
        "KITTI Tracking", (int(3 * screen_width / 4), int(3 * screen_height / 4))
    )
    ...
    topviewParams1 = draw.TopView.Params(
        colorMap=colorMap,
    ...
    topviewParams2 = draw.TopView.Params(
        colorMap=colorMap,
        topViewSize=(video_width, video_height * 2),
        background_color=255,
        rangeX=rangeX,
    ...
    if record_video:
        if view_odom:
            video_writer = cv.VideoWriter(
                ...
                30.0,
                (video_width + 400, video_height * 6),
            )
        else:
            video_writer = cv.VideoWriter(
                "kitti_tracking_vcd_" + str(sequence_number).zfill(4) + ".mp4",
                cv.VideoWriter_fourcc(*"mp4v"),
                30.0,
                (video_width + 400, video_height * 4),
            )
    ...
    while True:
        ...
        if view_odom:
            textImg = frameInfoDrawer.draw(
                f, cols=400, rows=video_height * 6, _params=imageParams
            )
        else:
            textImg = frameInfoDrawer.draw(
                f, cols=400, rows=video_height * 4, _params=imageParams
            )
