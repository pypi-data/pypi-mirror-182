...
from vcd.types import FisheyeModel, areaReference
...
class Scene:
    ...
    def get_camera(self, camera_name, frameNum=None, compute_remaps=False):
                ...
                if "streams" in vcd_frame["frame_properties"]:
                    if camera_name in vcd_frame["frame_properties"]["streams"]:
                        if (
                            ...
                            dynamic_intrinsics = (
                                True  # SO, there are dynamic intrinsics!
                            )
        ...
        if dynamic_intrinsics:
            ...
            if "intrinsics_pinhole" in sp:
                ...
                warnings.warn(
                    "WARNING: SCL does not support customized camera models. Supported types are CameraPinhole, CameraFisheye, CameraCylindrical and CameraOrthographic. See types.py."
                )
        else:
            ...
            if camera_name in self.cameras:
                if f in self.cameras[camera_name]:
                    return self.cameras[camera_name][f][
                        "cam"
                ...
                if frameNum in root["frames"]:
                    if "frame_properties" in root["frames"][frameNum]:
                        if "transforms" in root["frames"][frameNum]["frame_properties"]:
                            if (
                                t_name
                                in root["frames"][frameNum]["frame_properties"][
                                    "transforms"
                                ]
                            ):
                                transform = root["frames"][frameNum][
                                    "frame_properties"
                                ]["transforms"][t_name]
    ...
    def transform_plane(self, plane_abcd, cs_src, cs_dst, frameNum=None):
        transform_src_dst, static = self.get_transform(cs_src, cs_dst, frameNum)
        if transform_src_dst is not None:
            plane_abcd_transformed = utils.transform_plane(
                plane_abcd, transform_src_dst
            )
    ...
    def project_points3d_4xN(
        self, points3d_4xN, cs_src, cs_cam, frameNum=None, remove_outside=False
    ):
        """
        ...
        points3d_camera_cs_4xN = self.transform_points3d_4xN(
            points3d_4xN=points3d_4xN, cs_src=cs_src, cs_dst=cs_cam, frameNum=frameNum
        )
    ...
    def reproject_points2d_3xN_into_plane(
        self, points2d_3xN, plane, cs_cam, cs_dst, frameNum=None
    ):
        ...
        plane_cam = self.transform_plane(
            plane, cs_dst, cs_cam, frameNum
        ...
        if points3d_3xN_cs_cam.shape[1] > 0:
            ...
            points3d_4xN_cs_dst_filt = self.transform_points3d_4xN(
                points3d_3xN_cs_cam_filt, cs_cam, cs_dst, frameNum
            )
    ...
    def create_img_projection_maps(
        self, cam_src_name, cam_dst_name, frameNum, filterZNeg=False
    ):
        ...
        rays3d_src_4xN = self.transform_points3d_4xN(
            points3d_4xN, cam_dst_name, cam_src_name, frameNum
        )
...
class Sensor:
    ...
    def is_camera(self):
        if (
            self.type == "CameraPinhole"
            or self.type == "CameraFisheye"
            or self.type == "CameraEquirectangular"
        ):
            return True
        return False
...
class CameraPinhole(Camera):
    ...
    def project_points3d(self, points3d_4xN, remove_outside=False):
        ...
        if self.is_distorted():
            ...
            rays3d_3xN_filt_dist = self.distort_rays3d(
                rays3d_3xN_filt
...
class CameraFisheye(Camera):
    ...
    def __init__(
        ...
        self.d_inv_1xM = self.__invert_polynomial(
            self.d_1xN, self.model
    ...
    def distort_rays3d(self, rays3d_3xN):
        ...
        for i in range(0, N):
            ...
            r = utils.norm([X, Y])
            a = math.atan2(r, Z)
            rp = self.__apply_polynomial(
                a, self.d_1xN.flatten().tolist(), self.model
    ...
    def __apply_polynomial(self, x, k, model=None):
        if model == "radial_poly":
            ...
            warnings.warn(
                "Unsupported distortion model. Use radial_poly, kannala or polynomial_offset"
            )
            return None
    ...
    def __invert_polynomial(self, d, model, n=100):
        ...
        dim = (
            9  # dimensions of the offset polynomial to represent the inverse distortion
        )
        kp = np.polyfit(
            rp, a, dim - 1
        ...
        if self.error_a_deg > 1e-1:
            warnings.warn(
                "WARNING: the inverse of the CameraFisheye distortion produces reprojection error > 1e-2 "
    ...
    def estimate_new_camera_matrix_for_undistort_rectify(
        self, img_size_orig, balance, img_size_dst, fov_scale
    ):
...
class CameraCubemap(Camera):
    ...
    def project_points3d(self, points3d_4xN, remove_outside=True):
        ...
        if top_points.size != 0:
            ...
            top_cords[0, :] = (
                top_cords[0, :] + self.cameras["top"][0].img_size_undist[1]
            )
