...
class TestBasic(unittest.TestCase):
    def test_create_empty_openlabel(self):
        vcd = core.OpenLABEL()
        self.assertFalse(vcd is None)

    def test_create_openlabel_from_file(self):
        # Arrange
        vcd = core.OpenLABEL()
        ...
        # Assert
        obj = vcd.get_object(uid="4")
        self.assertEqual(obj["name"], "zebracross1")

    def test_create_openlabel_from_string(self):
        # Arrange
        vcd = core.OpenLABEL()
        ...
        vcd.load_from_string(json_data)

        # Assert
        obj = vcd.get_object(uid="3")
        self.assertEqual(obj["name"], "car1")

    def test_create_openlabel_from_json(self):
        # Arrange
        import json
        ...
        json_instance = json.loads(json_data)
        vcd.load_from_json(json_instance)

        # Assert
        obj = vcd.get_object(uid="3")
        self.assertEqual(obj["name"], "car1")

    def test_try_to_create_invalid_openlabel_from_string(self):
        # Arrange
        vcd = core.OpenLABEL()
        ...
        asserted = False
        try:
            vcd.load_from_string(json_data, validation=True)
    ...
    def test_create_search_simple(self):
        ...
        vcd.add_object_data(
            uid=uid_marcos, object_data=types.bbox(name="head", val=(10, 10, 30, 30))
        )
        vcd.add_object_data(
            uid=uid_marcos, object_data=types.bbox(name="body", val=(0, 0, 60, 120))
        )
        vcd.add_object_data(
            uid=uid_marcos, object_data=types.vec(name="speed", val=(0.0, 0.2))
        )
        vcd.add_object_data(
            uid=uid_marcos, object_data=types.num(name="accel", val=0.1)
        )
        ...
        vcd.add_object_data(
            uid=uid_peter, object_data=types.vec(name="eyeL", val=(0, 0, 10, 10))
        )
        vcd.add_object_data(
            uid=uid_peter, object_data=types.vec(name="eyeR", val=(0, 0, 10, 10))
        )

        # 5.- We can ask VCD
        marcos_ref = vcd.get_element(
            element_type=core.ElementType.object, uid=uid_marcos
        )
    ...
    def test_create_search_mid(self):
        ...
        vcd.add_object_data(
            uid=uid_marcos, object_data=types.num("age", 37.0), frame_value=(0, 10)
        )
        vcd.add_object_data(
            uid=uid_marcos, object_data=types.num("height", 1.75), frame_value=(0, 10)
        )
        vcd.add_object_data(
            uid=uid_marcos,
        ...
        vcd.add_object_data(
            uid=uid_peter, object_data=types.num("age", 40.0), frame_value=(0, 11)
        )
        vcd.add_object_data(
            uid=uid_peter,
        ...
        vcd.add_object_data(
            uid=uid_katixa, object_data=types.num("age", 9), frame_value=(5, 10)
        )
        vcd.add_object_data(
            uid=uid_katixa, object_data=types.num("age", 9.01), frame_value=11
        )  # by default union
        vcd.add_object_data(
            uid=uid_katixa, object_data=types.num("age", 9.02), frame_value=12
        )  # by default union
        ...
        for uid in uids_age:
            ...
            for frame_interval in frames_with_age.get():
                for frame_num in range(frame_interval[0], frame_interval[1] + 1):
                    my_age = vcd.get_object_data(
                        uid=uid, data_name="age", frame_num=frame_num
                    )
    ...
    def test_remove_simple(self):
        ...
        vcd.add_object_data(
            uid=person1_uid,
            object_data=types.bbox("face", (0, 0, 100, 100)),
            frame_value=(0, 5),
        )
        vcd.add_object_data(
            uid=person1_uid,
            object_data=types.bbox("mouth", (0, 0, 10, 10)),
            frame_value=(0, 5),
        )
        vcd.add_object_data(
            uid=person1_uid,
            object_data=types.bbox("hand", (0, 0, 30, 30)),
            frame_value=(0, 5),
        )
        vcd.add_object_data(
            uid=person1_uid,
            object_data=types.bbox("eyeL", (0, 0, 10, 10)),
            frame_value=(0, 5),
        )
        vcd.add_object_data(
            uid=person1_uid,
            object_data=types.bbox("eyeR", (0, 0, 10, 10)),
            frame_value=(0, 5),
        )

        # A different FrameInterval (0, 10)
        vcd.add_object_data(
            uid=person1_uid, object_data=types.num("age", 35.0), frame_value=(0, 10)
        )

        # Data for the other objects
        vcd.add_object_data(
            uid=car1_uid,
            object_data=types.bbox("position", (100, 100, 200, 400)),
            frame_value=(0, 10),
        )
        vcd.add_object_data(
            uid=car1_uid, object_data=types.text("color", "red"), frame_value=(6, 10)
        )
        vcd.add_object_data(
            uid=car2_uid,
            object_data=types.bbox("position", (300, 1000, 200, 400)),
            frame_value=(0, 10),
        )
        vcd.add_object_data(
            uid=trafficSign1UID,
            object_data=types.boolean("visible", True),
            frame_value=(0, 4),
        )
    ...
    def test_ontology_list(self):
        ...
        uid_car = vcd.add_object(
            "CARLOTA", "#Car", frame_value=None, uid=None, ont_uid=ont_uid_1
        )
        vcd.add_object_data(uid_car, types.text("brand", "Toyota"))
        vcd.add_object_data(uid_car, types.text("model", "Prius"))

        uid_marcos = vcd.add_object(
            "Marcos", "#Person", frame_value=None, uid=None, ont_uid=ont_uid_2
        )
        ...
        self.assertEqual(
            vcd.get_ontology(ont_uid_1), "http://www.vicomtech.org/viulib/ontology"
        )
        self.assertEqual(
            vcd.get_ontology(ont_uid_2), "http://www.alternativeURL.org/ontology"
        )
    ...
    def test_objects_without_data(self):
        ...
        pedestrian_uid = vcd.add_object(
            name="", semantic_type="#Pedestrian", frame_value=(0, 30)
        )
    ...
    def test_multi_value_attributes(self):
        ...
        vcd.add_object_data(
            uid=uid, object_data=types.vec(name="signals", val=[0.1, 0.2, 0.3])
        )
        vcd.add_object_data(
            uid=uid,
            object_data=types.vec(name="categories", val=["Tourism", "Large", "Old"]),
        )
        ...
        vcd.add_object_data(
            uid=uid,
            object_data=types.vec(
                name="shape",
                val=[0, 0, 100, 100],
                properties={"locked": True, "score": 0.8},
            ),
        )
        vcd.add_object_data(
            uid=uid,
            object_data=types.text(
                name="color",
                val="Blue",
