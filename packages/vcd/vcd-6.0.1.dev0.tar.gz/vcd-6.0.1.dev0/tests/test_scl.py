...
class TestBasic(unittest.TestCase):
    ...
    def test_scl_camera_cubemap_filterZNeg(self):
        """
        Cubemap to fisheye camera projection maps, when z negative points (behind the camera) are filtered out
        """
        # Read camera from VCD and create Scene object
        vcd = core.OpenLABEL()
        ref_file_name = str(
        ...
        map_x = np.load(
            str(
                ...
                / str(
                    openlabel_version_name
                    + "_test_scl_camera_cubemap_map_x_filterZNeg.npy"
                )
            )
        )
        map_y = np.load(
            str(
                ...
                / str(
                    openlabel_version_name
                    + "_test_scl_camera_cubemap_map_y_filterZNeg.npy"
                )
            )
        )
        ...
        # Create projection maps from VCD cameras
        remap_x, remap_y = scene.create_img_projection_maps(
            "CAM_CUBEMAP_FRONT", "CAM_FISHEYE_FRONT", 0, filterZNeg=True
        )
