...
class TestBasic(unittest.TestCase):
    ...
    def test_create_mesh(self):
        ...
        vcd.add_coordinate_system(
            name="world", cs_type=types.CoordinateSystemType.local_cs
        )
        ...
        # Areas
        # A0
        a0 = types.areaReference(
            "Slot", [0, 1, 2, 3], types.ObjectDataType.line_reference
        )
        a0.add_attribute(types.text("Fishbone", "park_slot_type"))
        a0.add_attribute(types.text("Empty", "park_slot_content"))
        mesh1.add_area(a0)
        a1 = types.areaReference(
            "Slot", [4, 5, 6, 1], types.ObjectDataType.line_reference
        )
    ...
    def test_create_mesh_with_API_frames(self):
        ...
        ref_file_name = str(
            Path(__file__).parent.resolve()
            / "etc"
            / str(openlabel_version_name + "_test_create_mesh.json")
        )
