...
class TestBasic(unittest.TestCase):
    def test_action_properties(self):
        ...
        # 2.- Create the Object
        uid_action1 = vcd.add_action(
            name="", semantic_type="#Running", frame_value=(0, 10)
        )
        vcd.add_action_data(
            uid=uid_action1,
            action_data=types.num(name="confidence", val=0.98),
            frame_value=0,
        )
        vcd.add_action_data(
            uid=uid_action1,
            action_data=types.vec(name="confidence_vec", val=[0.98, 0.97]),
            frame_value=0,
        )
        vcd.add_action_data(
            uid=uid_action1,
            action_data=types.text(name="annotation", val="Manual"),
            frame_value=0,
        )
        vcd.add_action_data(
            uid=uid_action1,
            action_data=types.boolean(name="validated", val=True),
            frame_value=1,
        )
        ...
        vcd.add_object_data(
            uid=uid_object1, object_data=types.text(name="Position", val="#Researcher")
        )

        uid_context1 = vcd.add_context(name="", semantic_type="#Sunny")
        vcd.add_context_data(
            uid=uid_context1, context_data=types.text(name="category", val="#Weather")
        )
        vcd.add_context_data(
            uid=uid_context1, context_data=types.text(name="annotation", val="Manual")
        )

        uid_context2 = vcd.add_context(
            name="", semantic_type="#Highway", frame_value=(0, 5)
        )
        vcd.add_context_data(
            uid=uid_context2,
        ...
        vcd.add_context_data(
            uid=uid_context2,
            context_data=types.num(name="weight", val=0.5),
            frame_value=4,
        )
