...
class TestBasic(unittest.TestCase):
    def test_openlabel_tags_complex(self):
        ...
        openlabel.add_metadata_properties(
            {"tagged_file": "../resources/scenarios/some_scenario_file"}
        )
        openlabel.add_ontology(
            ontology_name="https://code.asam.net/simulation/standard/openxontology/ontologies/openlabel"
        )

        # We can add a tag
        uid_0 = openlabel.add_tag(semantic_type="double_roundabout", ont_uid="0")
        # and later on, add some data to the tag
        openlabel.add_tag_data(uid=uid_0, tag_data=types.num(name=None, val=2))

        uid_1 = openlabel.add_tag(semantic_type="t_intersection", ont_uid="0")
        openlabel.add_tag_data(uid=uid_1, tag_data=types.num(name=None, val=3))
    ...
    def test_openlabel_tags_complex_2(self):
        ...
        ont_uid_0 = openlabel.add_ontology(
            ontology_name="https://code.asam.net/simulation/standard/openxontology/ontologies/domain/v1",
            subset_include=["motorway", "road"],
            subset_exclude=["highway", "lane", "curb"],
        )
        ont_uid_1 = openlabel.add_ontology(
            ontology_name="https://code.asam.net/simulation/standard/openlabel/ontologies/v1"
        )
        ont_uid_2 = openlabel.add_ontology(
            ontology_name="http://mycompany/ontologies/v1"
        )
        ...
        openlabel.add_tag_data(
            uid=uid_1, tag_data=types.vec(name=None, val=[2, 3], type="values")
        )
        uid_2 = openlabel.add_tag(semantic_type="lane-widths", ont_uid=ont_uid_0)
        openlabel.add_tag_data(
            uid=uid_2, tag_data=types.vec(name=None, val=[3.4, 3.7], type="range")
        )
        openlabel.add_tag_data(
            uid=uid_2, tag_data=types.vec(name=None, val=[3.9, 4.1], type="range")
        )
        uid_3 = openlabel.add_tag(semantic_type="rainfall", ont_uid=ont_uid_0)
        openlabel.add_tag_data(
            uid=uid_3, tag_data=types.num(name=None, val=1.2, type="min")
        )
        ...
        # Administrative tags
        uid_6 = openlabel.add_tag(
            semantic_type="scenario-unique-reference", ont_uid=ont_uid_1
        )
        openlabel.add_tag_data(
            uid=uid_6,
            tag_data=types.text(
                name=None, val="{02ed611e-a376-11eb-973f-b818cf5bef8c}", type="value"
            ),
        )
        ...
        openlabel.add_tag_data(
            uid=uid_7,
            tag_data=types.text(
                name=None, val="FSD01726287 Roundabout first exit", type="value"
            ),
        )
        uid_8 = openlabel.add_tag(semantic_type="adas-feature", ont_uid=ont_uid_1)
        openlabel.add_tag_data(
            uid=uid_8, tag_data=types.vec(name=None, val=["LCA", "LDW"], type="values")
        )
        uid_9 = openlabel.add_tag(semantic_type="scenario-version", ont_uid=ont_uid_1)
        openlabel.add_tag_data(
            uid=uid_9, tag_data=types.text(name=None, val="1.0.0.0", type="value")
        )
        uid_10 = openlabel.add_tag(
            semantic_type="scenario-created-date", ont_uid=ont_uid_1
        )
        openlabel.add_tag_data(
            uid=uid_10,
            tag_data=types.text(name=None, val="2021-05-25 11:00:00 UTC", type="value"),
        )
        uid_11 = openlabel.add_tag(semantic_type="project-id", ont_uid=ont_uid_2)
        openlabel.add_tag_data(
            uid=uid_11, tag_data=types.vec(name=None, val=[123456], type="values")
        )
