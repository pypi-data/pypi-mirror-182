...
def get_pretrained_model(model_name: str) -> Tuple[bool, str]:
    ...
    """
    try:
        pretrained_model_version_api_response = api.pretrained_model_version(MODEL_ZOO_CONFIG[model_name])
...
def get_or_create_pretrained_model(model_name: str) -> dict:
    ...
    with requests_error_handling():
        model_version = get_pretrained_model(model_name)
        if not model_version:
            model = create_pretrained_model(model_name)
            model_version = model["primaryVersion"]
