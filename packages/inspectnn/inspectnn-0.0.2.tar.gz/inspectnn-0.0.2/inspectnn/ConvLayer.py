...
from .cuda_utils import convolve,quant_3d

class ConvLayer(BaseLayer):
    def __init__(self, stride = (1, 1), padding = (0,0), activation = "relu", quant_nbits = 8, multiplier = BaseLayer.default_multiplier, name = "layer",offset=[128,128]):
    ...
    def forward_pass(self, **kwargs):
        ...
        #convolve[self.griddim, self.blockdim](self.results, A_global_mem,self.kernel_global_mem,self.biases,self.n_channels,self.M,self.offset[0],self.offset[1],self.quant_nbits)
        convolve[self.griddim, self.blockdim](self.results_conv,A_global_mem,self.kernel_global_mem,self.biases,self.n_channels,self.M,128,128,self.quant_nbits)
        cuda.synchronize()
        #TODO: calcolare grid_dim per quant 3d
        quant_3d[(12,12,8), self.blockdim](self.results, self.results_conv,self.results_max,self.quant_nbits)
    ...
    def load_weights(self, **kwargs):
        ...
        self.griddim = (self.input_shape[0] // self.blockdim[0] + 1, self.input_shape[1] // self.blockdim[1] + 1)
        self.n_channels=self.kernel_shape[3]
        if self.enable_gpu:
            self.use_gpu=True
            self.M = self.multiplier
            self.kernel_global_mem = cuda.to_device(np.array(kwargs["weights"]))
            self.biases = cuda.to_device(np.array(kwargs["biases"]))
            self.results = cuda.device_array(self.output_shape)
            self.results_max = cuda.to_device(np.zeros(self.output_shape, dtype=int))
            self.results_conv = cuda.device_array(self.output_shape, dtype=int)
