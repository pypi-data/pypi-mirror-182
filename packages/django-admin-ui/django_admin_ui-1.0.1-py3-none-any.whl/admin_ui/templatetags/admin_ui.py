...
@register.simple_tag
def get_admin_ui_settings(request: WSGIRequest) -> Dict:
    """
    Get Admin UI settings, update any defaults from the request, and return
    """
    ui_settings = get_settings()

    admin_site = {x.name: x for x in all_sites}.get("admin", {})
    if not ui_settings["site_title"]:
        ui_settings["site_title"] = getattr(admin_site, "site_title", None)

    if not ui_settings["site_header"]:
        ui_settings["site_header"] = getattr(admin_site, "site_header", None)

    if not ui_settings["site_brand"]:
        ui_settings["site_brand"] = getattr(admin_site, "site_header", None)

    return ui_settings


@register.simple_tag
def get_admin_ui_tweaks() -> Dict:
    """
...
@register.simple_tag
def admin_ui_paginator_number(change_list: ChangeList, i: int) -> SafeText:
    """
...
@register.simple_tag
def admin_ui_list_filter(cl: ChangeList, spec: ListFilter) -> SafeText:
    """
...
@register.simple_tag
def admin_ui_url(value: Union[str, ModelBase], admin_site: str = "admin") -> str:
    """
...
@register.filter
def has_admin_ui_setting(settings: Dict[str, Any], key: str) -> bool:
...
def get_sections(
    ...
    if hasattr(admin_form.model_admin, "admin_ui_section_order"):
        fieldsets = order_with_respect_to(
            fieldsets, admin_form.model_admin.admin_ui_section_order, getter=lambda x: x.name
        )
...
def sidebar_status(request: HttpRequest) -> str:
    ...
    if request.COOKIES.get("admin_ui_menu", "") == "closed":
