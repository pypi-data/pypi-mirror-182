...
CHANGEFORM_TEMPLATES = {
    "single": "admin_ui/includes/single.html",
    "carousel": "admin_ui/includes/carousel.html",
    "collapsible": "admin_ui/includes/collapsible.html",
    "horizontal_tabs": "admin_ui/includes/horizontal_tabs.html",
    "vertical_tabs": "admin_ui/includes/vertical_tabs.html",
}
...
def get_settings() -> Dict:
    admin_ui_settings = copy.deepcopy(DEFAULT_SETTINGS)
    user_settings = {x: y for x, y in getattr(settings, "ADMIN_UI_SETTINGS", {}).items() if y is not None}
    admin_ui_settings.update(user_settings)

    # Extract search model configuration from search_model setting
    if admin_ui_settings["search_model"]:
        if not isinstance(admin_ui_settings["search_model"], list):
            admin_ui_settings["search_model"] = [admin_ui_settings["search_model"]]

        admin_ui_settings["search_models_parsed"] = []
        for search_model in admin_ui_settings["search_model"]:
            admin_ui_search_model = {"search_url": get_admin_url(get_search_model_string(search_model))}

            model_meta = get_model_meta(search_model)
            if model_meta:
                admin_ui_search_model["search_name"] = model_meta.verbose_name_plural.title()
            else:
                admin_ui_search_model["search_name"] = search_model.split(".")[-1] + "s"
            admin_ui_settings["search_models_parsed"].append(admin_ui_search_model)

    # Deal with single strings in hide_apps/hide_models and make sure we lower case 'em
    if type(admin_ui_settings["hide_apps"]) == str:
        admin_ui_settings["hide_apps"] = [admin_ui_settings["hide_apps"]]
    admin_ui_settings["hide_apps"] = [x.lower() for x in admin_ui_settings["hide_apps"]]

    if type(admin_ui_settings["hide_models"]) == str:
        admin_ui_settings["hide_models"] = [admin_ui_settings["hide_models"]]
    admin_ui_settings["hide_models"] = [x.lower() for x in admin_ui_settings["hide_models"]]

    # Ensure icon model names and classes are lower case
    admin_ui_settings["icons"] = {x.lower(): y.lower() for x, y in admin_ui_settings.get("icons", {}).items()}

    # Default the site icon using the site logo
    admin_ui_settings["site_icon"] = admin_ui_settings["site_icon"] or admin_ui_settings["site_logo"]

    # Default the login logo using the site logo
    admin_ui_settings["login_logo"] = admin_ui_settings["login_logo"] or admin_ui_settings["site_logo"]

    # Default the login logo dark using the login logo
    admin_ui_settings["login_logo_dark"] = admin_ui_settings["login_logo_dark"] or admin_ui_settings["login_logo"]

    # ensure all model names are lower cased
    admin_ui_settings["changeform_format_overrides"] = {
        x.lower(): y.lower() for x, y in admin_ui_settings.get("changeform_format_overrides", {}).items()
    }

    return admin_ui_settings


def get_ui_tweaks() -> Dict:
    raw_tweaks = copy.deepcopy(DEFAULT_UI_TWEAKS)
    raw_tweaks.update(getattr(settings, "ADMIN_UI_TWEAKS", {}))
