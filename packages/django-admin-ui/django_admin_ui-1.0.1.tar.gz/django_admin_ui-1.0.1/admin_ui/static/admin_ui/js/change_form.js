(function ($) {
  ...
  function FixSelectorHeight() {
    $(".selector .selector-chosen").each(function () {
      ...
      let selector_available_filter = selector_available
        .find("p.selector-filter")
        .first();
  ...
  function applySelect2() {
    ...
    const noSelect2 =
      ".empty-form select, .select2-hidden-accessible, .selectfilter, .selector-available select, .selector-chosen select, select[data-autocomplete-light-function=select2]";
    $("select").not(noSelect2).select2({ width: "element" });
  }

  $(document).ready(function () {
    const $carousel = $("#content-main form #admin-ui-carousel");
    const $tabs = $("#content-main form #admin-ui-tabs");
    const $collapsible = $("#content-main form #admin-ui-collapsible");
    ...
    $("body").on("change", ".related-widget-wrapper select", function (e) {
      ...
      if (
        ...
        typeof window.updateRelatedObjectLinks !== "undefined"
      ) {
