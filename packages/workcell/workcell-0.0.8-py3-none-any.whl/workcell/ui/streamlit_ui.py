...
def launch_ui(workcell_path: str, port: int = 8501, host: str = '0.0.0.0') -> None:
    ...
    with NamedTemporaryFile(
        ...
        f.write(STREAMLIT_RUNNER_SNIPPET.format(workcell_path=workcell_path))
        f.seek(0)

        # TODO: PYTHONPATH="$PYTHONPATH:/workspace/workcell/src"
        import subprocess
...
def render_streamlit_ui(workcell: Workcell) -> None:
    """Render the streamlit UI for the given workcell"""
    ...
    if workcell.description:
        st.markdown(workcell.description)

    InputUI(session_state=session_state, input_class=workcell.input_type).render_ui()

    st.markdown("---")

    clear_col, execute_col = st.columns([1, 2])
        ...
        with st.spinner("Executing operation. Please wait..."):
            try:
                input_data_obj = parse_obj_as(
                    workcell.input_type, session_state.input_data
                )
                session_state.output_data = workcell(input=input_data_obj)
