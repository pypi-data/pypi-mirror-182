...
class AdditionalCls:
	...
	def set(self, dmrs_length: int, cdm_groups: int, dmrs_power: float, antenna_port: int, antenna_port_2: int = None, carrierComponent=repcap.CarrierComponent.Default, allocation=repcap.Allocation.Default) -> None:
		"""SCPI: CONFigure:NRMMw:MEASurement<Instance>[:CC<no>]:ALLocation<Allocation>:PUSCh:ADDitional \n
		Snippet: driver.configure.nrMmwMeas.cc.allocation.pusch.additional.set(dmrs_length = 1, cdm_groups = 1, dmrs_power = 1.0, antenna_port = 1, antenna_port_2 = 1, carrierComponent = repcap.CarrierComponent.Default, allocation = repcap.Allocation.Default) \n
		...
		self._core.io.write(f'CONFigure:NRMMw:MEASurement<Instance>:CC{carrierComponent_cmd_val}:ALLocation{allocation_cmd_val}:PUSCh:ADDitional {param}'.rstrip())
