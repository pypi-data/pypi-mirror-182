...
class SassignmentCls:
	...
	def set(self, sc_spacing: enums.ScSpacing, slot_no: float, allocation: int or bool, carrierComponent=repcap.CarrierComponent.Default) -> None:
		...
		Selects whether UL slot <SlotNo> is scheduled or not, for carrier <no>, subcarrier spacing <SCSpacing>. \n
			:param sc_spacing: Subcarrier spacing 60 kHz, 120 kHz.
			:param slot_no: No help available
			:param allocation: (integer or boolean) 1: UL slot scheduled, allocation 1 used OFF: UL slot not scheduled For X slots and DL slots, NAV is returned.
	...
	def get(self, sc_spacing: enums.ScSpacing, slot_no: float, carrierComponent=repcap.CarrierComponent.Default) -> int or bool:
		...
		Selects whether UL slot <SlotNo> is scheduled or not, for carrier <no>, subcarrier spacing <SCSpacing>. \n
			...
			:param slot_no: No help available
			:param carrierComponent: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cc')
			:return: allocation: (integer or boolean) 1: UL slot scheduled, allocation 1 used OFF: UL slot not scheduled For X slots and DL slots, NAV is returned."""
