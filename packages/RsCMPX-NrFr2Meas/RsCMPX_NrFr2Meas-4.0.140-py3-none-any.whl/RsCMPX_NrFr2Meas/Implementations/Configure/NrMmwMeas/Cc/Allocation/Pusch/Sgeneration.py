...
class SgenerationCls:
	...
	def set(self, initialization: enums.Initialization, dmrs_id: int, nscid: int, carrierComponent=repcap.CarrierComponent.Default, allocation=repcap.Allocation.Default) -> None:
		"""SCPI: CONFigure:NRMMw:MEASurement<Instance>[:CC<no>]:ALLocation<Allocation>:PUSCh:SGENeration \n
		Snippet: driver.configure.nrMmwMeas.cc.allocation.pusch.sgeneration.set(initialization = enums.Initialization.CID, dmrs_id = 1, nscid = 1, carrierComponent = repcap.CarrierComponent.Default, allocation = repcap.Allocation.Default) \n
		...
		self._core.io.write(f'CONFigure:NRMMw:MEASurement<Instance>:CC{carrierComponent_cmd_val}:ALLocation{allocation_cmd_val}:PUSCh:SGENeration {param}'.rstrip())
