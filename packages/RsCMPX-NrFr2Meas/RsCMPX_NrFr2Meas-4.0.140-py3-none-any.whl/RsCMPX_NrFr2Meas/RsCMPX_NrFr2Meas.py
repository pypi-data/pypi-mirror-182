...
class RsCMPX_NrFr2Meas:
	"""784 total commands, 5 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = CMX500/CMP/CMW, SupportedIdnPatterns = CMX/CMP/CMW, SimulationIdnString = 'Rohde&Schwarz,CMX,100001,4.0.140.0010'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		"""Initializes new RsCMPX_NrFr2Meas session. \n
		...
		self._core = Core(resource_name, id_query, reset, RsCMPX_NrFr2Meas._driver_options, options, direct_session)
		self._core.driver_version = '4.0.140.0010'
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCMPX_NrFr2Meas':
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCMPX_NrFr2Meas session '{self._core.io.resource_name}'"
		else:
			return f"RsCMPX_NrFr2Meas with session closed"
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.0.140.0010'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsCMPX_NrFr2Meas version failed. Current version: '4.0.140.0010', minimum required version: '{min_version}'")
	...
	def _sync_to_custom_properties(self, cloned: 'RsCMPX_NrFr2Meas') -> None:
	...
	@property
	def nrMmwMeas(self):
		"""nrMmwMeas commands group. 2 Sub-classes, 0 commands."""
		if not hasattr(self, '_nrMmwMeas'):
			from .Implementations.NrMmwMeas import NrMmwMeasCls
	...
	def clone(self) -> 'RsCMPX_NrFr2Meas':
		"""Creates a deep copy of the RsCMPX_NrFr2Meas object. Also copies:
		...
		cloned = RsCMPX_NrFr2Meas.from_existing_session(self.get_session_handle(), self._options)
