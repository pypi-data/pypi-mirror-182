...
class Project:
    ...
    def __init__(self, proj_init_dict: dict):
        ...
        self.config_filepath = self.__find_config_file()     
        ...
        self.powertrack_filepath = self.__find_powertrack_file()
    ...
    def __find_powertrack_file(self):
    ...
    def generate_power_from_invoice(self, start, stop, total_energy, clipping_limit=None):
        ...
        Args:
            ...
            clipping_limit (Number): Upper limit (per meter) to cap production at.
    ...
    def export_df(self, table='prod'):
        ...
        Args:
            table (str, optional): Option to insert data to prod or dev table.
                <ul>
                    <li><code>dev</code> corresponds to <code>dev_analytic.am_processed_data</code>.</li>
                    <li><code>test</code> corresponds to <code>dev_analytic.am_processed_data_dev</code>.</li>
                </ul>
        """
    ...
    def export_summary(self, table='prod'):
        """Exports monthly summary data to S3 and Bartertown PostgreSQL database.

        Args:
            table (str, optional): Option to insert data to prod or dev table.
                <ul>
                    <li><code>dev</code> corresponds to <code>dev_analytic.am_summary_data</code>.</li>
                    <li><code>test</code> corresponds to <code>dev_analytic.am_summary_data_dev`</code>.</li>
                <ul>
        """
        # PostgreSQL initialization
        engine = ccr.get_sql_engine()
    ...
    def export_snow(self):
        ...
        bucket_push(self.snow_data, key_data)
        ...
        bucket_push(self.snow_coverage, key_coverage)

    
    def export(self, save_pickle=True, table='prod', *args):
        """
        Export project to desired location(s)

        Args:
            save_pickle (bool, optional): Option to save pickle after exporting.
            table (str, optional): SQL table to export to. Options are `prod` or `dev`.
        """
        def call_func(func, table):
            if func.__func__.__name__ in ['export_df', 'export_summary']:
                func(table)
            else:
                func()
        ...
        if not args:
            for func in export_dict.values():
                call_func(func, table)
        else:
            for arg in args:
                func = export_dict[arg]
                call_func(func, table)
