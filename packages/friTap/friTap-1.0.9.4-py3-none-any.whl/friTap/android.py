...
class Android:
    ...
    def _get_tcpdump_version(self):
        ...
        #tcpdump_path = "./assets/tcpdump_binaries/"+
        tcpdump_path = files('assets.tcpdump_binaries').joinpath(self.tcpdump_version)
        print("[*] loading path: "+tcpdump_path)

        if file_exists(tcpdump_path):
            return tcpdump_path
        else:
            tcpdump_path = self.tcpdump_version
            if file_exists(tcpdump_path) == False:
                print("[-] error: can't find "+tcpdump_path)
                print("[-] ensure that "+tcpdump_path+" exits\n")              
                exit(2)
