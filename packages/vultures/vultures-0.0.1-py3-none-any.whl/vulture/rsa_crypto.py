...
class RSA_Crypto:
    ...
    def sign_message(self, message):
        ...
        #signature = pkcs1_15.new(self.privKey_obj)
    ...
    def verify_signature(self, message, signature, other_pubKey):
        ...
        other_pubKey_obj = self.load_key(other_pubKey)
        
        try:
            #pkcs1_15.new(other_pubKey_obj).verify(h, signature)
            print(other_pubKey_obj.decrypt(message))
            #return True
        except:
            return False
...
if __name__ == "__main__":
    ...
    pub = create_rsa_key_object(data["public_key"])
    priv = create_rsa_key_object(data["private_key"], public = False)
    ...
    print(f"Verified: {rsa_obj.verify_signature(message, signature, rsa_obj.pubKey)}")
    '''
