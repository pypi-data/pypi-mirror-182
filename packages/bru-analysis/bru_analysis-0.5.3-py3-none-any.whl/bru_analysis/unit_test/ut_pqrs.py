...
class TestSttm(unittest.TestCase):
    ...
    def test_normal(self):
        ...
        df_pqrs = pqrs(df_p=self.df_e,
                       batch=50).pqrs_df()

        self.assertEqual(len(df_pqrs.columns), len(self.df_e.columns) + 1)
        self.assertEqual(len(df_pqrs), len(self.df_e))
    ...
    def test_empty(self):
        ...
        df_pqrs = pqrs(df_p=df_empty,
                       batch=50).pqrs_df()

        self.assertEqual(len(df_pqrs), 0)
