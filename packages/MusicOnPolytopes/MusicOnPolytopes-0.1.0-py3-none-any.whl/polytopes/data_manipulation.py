...
# %% Circles definition
def get_circle_of_fifth():
    """
    Return the circle of fifth, as a list.
    """
    return [(i-(i%2)*6)%12 for i in range(12)] # Circle of fifth

def get_reversed_circle_of_fifth():
    """
    Return the reversed circle of fifth, as a list.
    """
    return [(i+(i%2)*6)%12 for i in range(12)] # Reversed circle of fifth

def get_circle_of_triads(flat = True, chromatic = True):
    """
    Returns the circle of triads, as defined by Corentin Louboutin.
    
    Parameters
    ----------
    flat: boolean
        Whether to return in the flat or in the sharp convetion (Default: True)
    chromatic: boolean
        Whether to return the circle of triads in the "circle of thirds" order (True) or in the 3-5 torus order (False).
        In fact, only the circle of fifths is used now.
    """
    # Override because chromatic is not propagated in code and need to use the classical circle
    
    #if chromatic:
    if flat:
        return ['C','Am','F','Dm','Bb','Gm','Eb','Cm','Ab','Fm','Db','Bbm','Gb','Ebm','B','Abm','E','Dbm','A','Gbm','D','Bm','G','Em']
    else:
        return ['C','Am','F','Dm','A#','Gm','D#','Cm','G#','Fm','C#','A#m','F#','D#m','B','G#m','E','C#m','A','F#m','D','Bm','G','Em']
    # else:
    #     if flat:
...
def flowify_song(path):
    """
    Read a song in input (as a list of Chords), and turn it into a flow (i.e. a list with all the chords)
    ...
    ------
    NotImplementedError
        Error case, when the entire file is composed of silences.

    Returns
    -------
    bag_of_chords : list of Chord objects
        All chords in the file, as Chord objects, and in one list.

    """
    file_seg = open(path)
    bag_of_chords = []
    for line in file_seg.readlines():
        chords = line.replace("\n", "").split(" ")
        for chord in chords:
            if chord != '':
                if chord != "N":
                    bag_of_chords.append(Chord(chord))
                else:
                    try:
...
def frontiers_to_chord_sequence(flow, frontiers):
    """
    Convert frontiers to segments with the Chord it contains.
    
    It is mostly a visualization function.

    Parameters
    ----------
    flow : list of Chord, in any form
        The flow/sequence of chords in all the song.
    frontiers : list of integers
        The frontiers, computed in this song (expected to be integers, so they should be beatwise aligned, so as the flow).

    Returns
    -------
    list of tuples
        The chords in the flow, segmented according to the frontiers.

    """
    flow_np_array = np.array(flow)
    return [tuple(flow_np_array[frontiers[i]:frontiers[i+1]]) for i in range(len(frontiers) - 1)]
...
def compute_median_deviation_of_segmentation(reference, segments_in_time):
    """
    Median deviation of a frontier estimated from the annotation.
...
# %% Object manipulation
def is_a_note_object(note):
    """
    Function to test whether the parameter is a Note object (see note.py) or not.
    Parameters
    ----------
    note: unknown_type
        The object to test.
    Returns
    -------
    boolean: True if it is a Note object, False otherwise.
    """
    try:
        note.number
        return True
...
def is_a_chord_object(chord):
    """
    Function to test whether the parameter is a Chord object (see chord.py) or not.
    Parameters
    ----------
    chord: unknown type
        The object to test.
    Returns
    -------
    boolean: True if it is a Chord object, False otherwise.
    """
    try:
        chord.notes
        return True
...
# %% From objects to info and etc
def list_notes_to_numbers(list_of_notes):
    """
    Computes a list of note numbers from a list of Note objects.
    Parameters
    ----------
    list_of_notes: list of Note objects
        The list to return as a list of note numbers
    Returns
    -------
    The list of note numbers
    """
    return [note.number for note in list_of_notes]

def list_of_lists_of_notes_to_numbers(list_of_list_of_notes):
    """
    Computes a list of lists of numbers from a list of lists of Note objects.
    Parameters
    ----------
    list_of_list_of_notes: list, where every element is a list of Note objects
        The list where every element must be computed as a list of note numbers
    Returns
    -------
    The list where every element is a list of note numbers
    """
    return [list_notes_to_numbers(list_of_notes) for list_of_notes in list_of_list_of_notes]
        
def list_chords_to_numbers(list_of_chords):
    """
    Computes a list of lists of note numbers from a list of Chord objects.
    Parameters
    ----------
    list_of_chords: list of Chord objects
        The list to return as a list of note numbers
    Returns
    -------
    The list of lists of note numbers
    """
    return [chord.get_numbers() for chord in list_of_chords]