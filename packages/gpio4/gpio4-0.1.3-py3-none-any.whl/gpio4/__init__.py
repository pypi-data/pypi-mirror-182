...
class SysfsBase(object):
    ...
    @staticmethod
    def _attr_string(attr, read, write):
        def R(self, attr=attr): return self._read(attr)
        def W(self, val, attr=attr): return self._write(attr, val)
        return property(read and R or None, write and W or None)

    @staticmethod
    def _attr_number(attr, read, write):
        def R(self, attr=attr): return self._read(attr, 10)
        def W(self, val, attr=attr): return self._write(attr, val)
        return property(read and R or None, write and W or None)

    @staticmethod
    def _attr_boolean(attr, read, write):
        def R(self, attr=attr): return bool(self._read(attr, 10))
        def W(self, val, attr=attr): return self._write(attr, int(val))
        return property(read and R or None, write and W or None)

    def setup(self, reopen=True):                   # open or reopen attr files
        if not op.isdir(self.path):
            raise IOError('Cannot open %s at `%s`' % (
                self.__class__.__name__, self.path))
        for attr in set(self._files).difference(self._attrs):
            self._files.pop(attr).close()
            if hasattr(self.__class__, attr):
                delattr(self.__class__, attr)
        for attr in self._attrs:
            fn = op.join(self.path, attr)
            try:
                assert op.isfile(fn)                # attribute file removed
                perm = os.stat(fn).st_mode >> (3 if os.geteuid() else 6) & 0o7
                mode = [None, 'wb', 'rb', 'w+b'][perm >> 1]
                assert mode is not None             # attribute mode changed
            except Exception:
                if attr in self._files:
                    self._files.pop(attr).close()
                continue
            if attr in self._files:
                if not reopen:
                    continue
                self._files[attr].close()
            self._files[attr] = open(fn, mode, buffering=0)
            RD, WR = 'r' in mode or '+' in mode, 'w' in mode
            if self._attrs[attr] and not hasattr(self.__class__, attr):
                prop_func = getattr(self, '_attr_' + self._attrs[attr])
                setattr(self.__class__, attr, prop_func(attr, RD, WR))

    def clear(self):                                # close attr files
        for f in list(self._files.values()):
            f.close()
        self._files.clear()

    def fileno(self, attr=None):
        if attr is None:
            attr = (list(self._attrs) or [None])[0]
        if attr not in self._files:
            return -1
        return self._files[attr].fileno()

    def _read(self, attr, intbase=1):
        # value may be string or integer or hex
        if attr not in self._files:
            return 0 if intbase != 1 else b''
        with self._read_lock:
            self._files[attr].seek(0)
            value = self._files[attr].read().strip()
        return int(value, intbase) if intbase != 1 else value

    def _write(self, attr, data):
        if attr not in self._files:
            return 0
        if not isinstance(data, (bytes, bytearray)):
            data = str(data).encode('latin')
        with self._write_lock:
            self._files[attr].seek(0)
            self._files[attr].write(data)
        return len(data)


class SysfsGPIO(SysfsBase):
    _folder = '/sys/class/gpio'
    _attrs = {
        'value': 'number', 'direction': 'string',
        'edge': 'string', 'active_low': 'boolean'
    }

    def __init__(self, pin):
        self.pin = int(pin)
        super().__init__(op.join(self._folder, 'gpio{:d}'.format(self.pin)))
        self.export = True

    def __repr__(self):
        if self.export:
            return '<gpio{} {} edge:{} mode:{} at 0x{:x}>'.format(
                self.pin, 'HIGH' if self.value else 'LOW',
                self.edge.title(), self.direction.upper(), id(self))
        else:
            return '<gpio{} unexported at 0x{:x}>'.format(self.pin, id(self))

    def _do_export(self, value):
        if value:                   # register gpio pin if necessary
            if not self.export:
                with open(op.join(self._folder, 'export'), 'w') as f:
                    f.write(str(self.pin))
                time.sleep(0.1)     # chown from root:root to root:gpio
            self.setup()
        else:                       # gpio will be unexported if it exists
            if self.export:
                with open(op.join(self._folder, 'unexport'), 'w') as f:
                    f.write(str(self.pin))
            self.clear()

    export = property(lambda self: op.exists(self.path), _do_export)


class SysfsPWM(SysfsBase):
    _attrs = {
        'period': 'number', 'duty_cycle': 'number',
        'polarity': 'string', 'enable': 'boolean',
    }

    def __init__(self, num):
        for chip in glob.glob('/sys/class/pwm/pwmchip*'):
            try:
                base = int(re.findall(r'pwmchip(\d+)', chip)[0])
                with open(op.join(chip, 'npwm'), 'r') as f:
                    npwm = int(f.read())
                assert 0 <= (num - base) < npwm
            ...
            raise ValueError('Invalid PWM channel: %s' % num)
        super().__init__(op.join(self._folder, 'pwm{:d}'.format(self.num)))
        self.export = True

    def __repr__(self):
        if self.export:
            return '<pwm{} {:.3f}Hz {:.2f}% {} at 0x{:x}>'.format(
                self.num, self.frequency, self.percentage,
                self.polarity, id(self))
        else:
            return '<pwm{} unexported at 0x{:x}>'.format(self.num, id(self))
    ...
    def _set_freq(self, hz): self.period = hz and int(1e9 / hz)  # unit: ns
    def _set_pert(self, pt): self.duty_cycle = int(pt / 100 * self.period)

    def _get_freq(self):
        period = self.period
        return period and 1e9 / period

    def _get_pert(self):
        period = self.period
        return period and self.duty_cycle * 100 / period

    frequency = property(_get_freq, _set_freq)
    percentage = property(_get_pert, _set_pert)


class SysfsLED(SysfsBase):
    _folder = '/sys/class/leds'
    _attrs = {
        'brightness': 'number', 'max_brightness': None, 'trigger': None,
    }
    _new_attrs = {
        'delay_on': 'number', 'delay_off': 'number', 'gpio': 'number',
        'desired_brightness': 'number', 'duration': 'number',
        'invert': None, 'inverted': None, 'shot': 'number',
    }
    _trig_tpl = re.compile(rb'\[([\w\-]+)\]')
    _trig_attrs = {
        'timer': ['delay_on', 'delay_off'],
        'oneshot': ['delay_on', 'delay_off', 'invert', 'shot'],
        'heartbeat': ['invert'], 'backlight': ['inverted'],
        'gpio': ['desired_brightness', 'gpio', 'inverted'],
        'transient': ['duration', 'activate', 'state'],
    }

    def __init__(self, name):
        self.name = op.basename(str(name))
        super().__init__(op.join(self._folder, self.name))
        self.setup()
        self.triggers = [
            t.strip('[]') for t in self._read('trigger').decode().split(' ')
        ]
        self.max_brightness = self._read('max_brightness', 10) or 255

    def __repr__(self):
        return '<led({}) brightness: {}/{} at 0x{:x}>'.format(
            self.name, self.brightness, self.max_brightness, id(self))

    @property
    def trigger(self):
        triggers = self._read('trigger')
        found = self._trig_tpl.findall(triggers)
        return found[0] if found else triggers

    @trigger.setter
    def trigger(self, trig):
        oldlist = self._trig_attrs.get(self.trigger.decode(), [])
        if trig not in self.triggsers or not self._write('trigger', trig):
            return
        newlist = self._trig_attrs.get(trig, [])
        for attr in oldlist:
            self._attrs.pop(attr, None)
        for attr in newlist:
            self._attrs[attr] = self._new_attrs.get(attr, None)
        if oldlist or newlist:
            self.setup(reopen=False)

    def _set_invert(self, value):
        if 'invert' in self._files:
            return self._write('invert', value)
        return self._write('inverted', value)  # alias

    invert = property(lambda self: self._read('invert', 10), _set_invert)
...
class _PWM:
    '''Software PWM used by class GPIO. SysfsPWM is more suggested.'''
