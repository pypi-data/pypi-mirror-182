...
def explore_object(thing, show_hidden=False, folding=True):
    ...
    # color operators
    data["Ops"] = [colored(text, colorama.Fore.YELLOW)
...
# def _break_list(data: list, breaks: int):
#     """Convert one column of data to <columns> columns, aligned."""
#     maxlength = max(len(line) for line in data)
#     chunks, chunk_size = maxlength, math.ceil(maxlength/2)
#     new = ["-\n".join([line[i:i+chunk_size] for i in range(0, chunks, chunk_size)
#                        if line[i:i+chunk_size]]) for line in data]
#     return new


# def _widen_table(source_data, thing):
#     """Increase or decrease width of table"""
#     term_size = shutil.get_terminal_size((80, 20))
#     data = copy.deepcopy(source_data)
#     candidate_key, candidate_list = max(data.items(), key=lambda x: max(len(y) for y in x))
#     table = _make_table(data, thing)
#     foldings = collections.defaultdict(lambda: 1)

#     while table.table_width > term_size.columns:
#         table = _make_table(data, thing)
#         foldings[candidate_key] += 1
#         new = _break_list(source_data[candidate_key], foldings[candidate_key])
#         data_candidate = copy.deepcopy(data)
#         data_candidate[candidate_key] = new
#         print(table.table)
#         table = _make_table(data_candidate, thing)
#         if table.table_width < term_size.columns:
#             break
#         data = data_candidate
#         candidate_key, candidate_list = max(
#             data.items(), key=lambda x: max(len(y) for y in x))
#     return data
...
def _minify_data(source_data, thing):
    ...
    table = _make_table(data, thing)
    # if table.table_width > term_size.columns:
    #     return _widen_table(source_data, thing)
    foldings = collections.defaultdict(lambda: 1)
