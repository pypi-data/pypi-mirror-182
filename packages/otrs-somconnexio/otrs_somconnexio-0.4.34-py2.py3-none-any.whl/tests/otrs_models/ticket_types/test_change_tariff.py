...
class TestCaseChangeTariffTicket:
    def test_create(self, mocker):
        ...
        OTRSClientMock = mocker.patch(
            "otrs_somconnexio.otrs_models.ticket_types.base_ticket.OTRSClient", return_value=mocker.Mock())
        TicketMock = mocker.patch("otrs_somconnexio.otrs_models.ticket_types.base_ticket.Ticket",
                                  return_value=mocker.Mock())
        ArticleMock = mocker.patch(
            "otrs_somconnexio.otrs_models.ticket_types.base_ticket.Article", return_value=mocker.Mock())
        DynamicFieldMock = mocker.patch(
            "otrs_somconnexio.otrs_models.ticket_types.base_ticket.DynamicField", return_value=mocker.Mock())
    ...
    def test_create_with_override_tickets(self, mocker):
        ...
        calls = [
            ...
            mocker.call('tarifaAntiga', 'CURRENT_PRODUCT_CODE'),
            mocker.call('dataExecucioCanviTarifa', 'tomorrow'),
            mocker.call('OdooContractRefRelacionat', None),
            mocker.call('correuElectronic', 'fakeemail@email.coop'),
...
class TestCaseChangeTariffExceptionalTicket:
    def test_create(self, mocker):
        ...
        ChangeTariffExceptionalTicket(username, customer_code,
                                      fields_dict).create()
