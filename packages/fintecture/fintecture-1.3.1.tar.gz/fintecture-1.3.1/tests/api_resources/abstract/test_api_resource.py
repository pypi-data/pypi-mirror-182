...
class TestAPIResource(object):
    ...
    def test_convert_to_fintecture_object(self):
        ...
        converted = fintecture.util.convert_to_fintecture_object(
            sample, "akey", None, None
        )

        # Types
        assert isinstance(converted, fintecture.fintecture_object.FintectureObject)
        assert isinstance(converted.adict, fintecture.Charge)
        assert len(converted.alist) == 1
        assert isinstance(converted.alist[0], fintecture.Customer)
