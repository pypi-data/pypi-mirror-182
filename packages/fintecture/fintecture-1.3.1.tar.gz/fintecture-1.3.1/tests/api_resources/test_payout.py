...
class TestPayout(object):
    def test_is_listable(self, request_mock):
        resources = fintecture.Payout.list()
        request_mock.assert_requested("get", "/v1/payouts")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], fintecture.Payout)

    def test_is_retrievable(self, request_mock):
        resource = fintecture.Payout.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/payouts/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, fintecture.Payout)

    def test_is_creatable(self, request_mock):
        resource = fintecture.Payout.create(amount=100, currency="usd")
    ...
    def test_is_modifiable(self, request_mock):
        resource = fintecture.Payout.modify(
    ...
    def test_can_cancel(self, request_mock):
        payout = fintecture.Payout.retrieve(TEST_RESOURCE_ID)
    ...
    def test_can_cancel_classmethod(self, request_mock):
        resource = fintecture.Payout.cancel(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/payouts/%s/cancel" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, fintecture.Payout)
    ...
    def test_can_reverse_classmethod(self, request_mock):
        resource = fintecture.Payout.reverse(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/payouts/%s/reverse" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, fintecture.Payout)
