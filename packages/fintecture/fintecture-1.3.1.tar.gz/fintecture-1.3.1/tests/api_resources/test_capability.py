...
class TestCapability(object):
    def construct_resource(self):
        ...
        return fintecture.Capability.construct_from(
            capability_dict, fintecture.app_id
        )
    ...
    def test_is_not_modifiable(self, request_mock):
        with pytest.raises(NotImplementedError):
            fintecture.Capability.modify(TEST_RESOURCE_ID, requested=True)

    def test_is_not_retrievable(self, request_mock):
        with pytest.raises(NotImplementedError):
            fintecture.Capability.retrieve(TEST_RESOURCE_ID)
