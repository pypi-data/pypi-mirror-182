...
class FintectureClientTestCase(object):
    ...
    def request_mocks(self, mocker):
        ...
        for lib in self.REQUEST_LIBRARIES:
            request_mocks[lib] = mocker.patch("fintecture.http_client.%s" % (lib,))
        return request_mocks


class TestNewDefaultHttpClient(FintectureClientTestCase):
    @pytest.fixture(autouse=True)
    def setup_warnings(self, request_mocks):
        original_filters = fintecture.http_client.warnings.filters[:]
        fintecture.http_client.warnings.simplefilter("ignore")
        yield
        fintecture.http_client.warnings.filters = original_filters

    def check_default(self, none_libs, expected):
        for lib in none_libs:
            setattr(fintecture.http_client, lib, None)

        inst = fintecture.http_client.new_default_http_client()

        assert isinstance(inst, expected)

    def test_new_default_http_client_urlfetch(self):
        self.check_default((), fintecture.http_client.UrlFetchClient)

    def test_new_default_http_client_requests(self):
        self.check_default(("urlfetch",), fintecture.http_client.RequestsClient)

    def test_new_default_http_client_pycurl(self):
        self.check_default(
            ("urlfetch", "requests"), fintecture.http_client.PycurlClient
        )
...
class TestRetrySleepTimeDefaultHttpClient(FintectureClientTestCase):
    ...
    def mock_max_delay(self, new_value):
        original_value = fintecture.http_client.HTTPClient.MAX_DELAY
        fintecture.http_client.HTTPClient.MAX_DELAY = new_value
        try:
            yield self
        finally:
            fintecture.http_client.HTTPClient.MAX_DELAY = original_value

    def test_sleep_time_exponential_back_off(self):
        client = fintecture.http_client.new_default_http_client()
    ...
    def test_initial_delay_as_minimum(self):
        ...
        initial_delay = fintecture.http_client.HTTPClient.INITIAL_DELAY
        self.assert_sleep_times(client, [initial_delay] * 5)

    def test_maximum_delay(self):
        client = fintecture.http_client.new_default_http_client()
        client._add_jitter_time = lambda t: t
        max_delay = fintecture.http_client.HTTPClient.MAX_DELAY
    ...
    def test_randomness_added(self):
        ...
        base_value = fintecture.http_client.HTTPClient.INITIAL_DELAY * random_value

        with self.mock_max_delay(10):
            expected = [
                fintecture.http_client.HTTPClient.INITIAL_DELAY,
                base_value * 2,
                base_value * 4,
                base_value * 8,
                base_value * 16,
...
class TestRetryConditionsDefaultHttpClient(FintectureClientTestCase):
    ...
    def test_should_retry_on_fintecture_should_retry_true(self, mocker):
        client = fintecture.http_client.new_default_http_client()
        client._max_network_retries = lambda: 1
        headers = {"fintecture-should-retry": "true"}
    ...
    def test_should_retry_on_fintecture_should_retry_false(self, mocker):
        client = fintecture.http_client.new_default_http_client()
        client._max_network_retries = lambda: 1
        headers = {"fintecture-should-retry": "false"}
...
class TestHTTPClient(object):
    ...
    def setup_fintecture(self):
        orig_attrs = {"enable_telemetry": fintecture.enable_telemetry}
    ...
    def test_sends_telemetry_on_second_request(self, mocker):
        class TestClient(fintecture.http_client.HTTPClient):
            pass
...
class ClientTestBase(object):
    ...
    def valid_url(self, path="/foo"):
        return "https://api.fintecture.com%s" % (path,)
...
class RequestsVerify(object):
    def __eq__(self, other):
        return other and other.endswith("fintecture/data/ca-certificates.crt")


class TestRequestsClient(FintectureClientTestCase, ClientTestBase):
    REQUEST_CLIENT = fintecture.http_client.RequestsClient
...
class TestRequestClientRetryBehavior(TestRequestsClient):
    ...
    def connection_error(self, session):
        ...
        def connection_error(given_exception):
            with pytest.raises(fintecture.error.APIConnectionError) as error:
    ...
    def test_handle_request_error_should_not_retry(
        ...
        assert "not verify Fintecture's SSL certificate" in error.user_message
...
class TestUrlFetchClient(FintectureClientTestCase, ClientTestBase):
    REQUEST_CLIENT = fintecture.http_client.UrlFetchClient
...
class TestUrllib2Client(FintectureClientTestCase, ClientTestBase):
    REQUEST_CLIENT = fintecture.http_client.Urllib2Client
...
class TestPycurlClient(FintectureClientTestCase, ClientTestBase):
    REQUEST_CLIENT = fintecture.http_client.PycurlClient
    ...
    def bio_mock(self, mocker):
        bio_patcher = mocker.patch("fintecture.util.io.BytesIO")
    ...
    def mock_error(self):
        def mock_error(mock):
            ...
            fintecture.http_client.pycurl.error = FakeException
            mock.perform.side_effect = fintecture.http_client.pycurl.error
    ...
    def check_call(self, request_mocks):
        def check_call(
            ...
            mock.setopt.assert_any_call(lib_mock.URL, fintecture.util.utf8(url))
...
class TestAPIEncode(FintectureClientTestCase):
    def test_encode_dict(self):
        body = {"foo": {"dob": {"month": 1}, "name": "bat"}}

        values = [t for t in fintecture.api_requestor._api_encode(body)]
