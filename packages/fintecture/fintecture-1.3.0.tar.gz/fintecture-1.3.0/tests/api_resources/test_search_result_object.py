...
class TestSearchResultObject(object):
    ...
    def search_result_object(self):
        return fintecture.SearchResultObject.construct_from(
    ...
    def test_is_empty(self):
        sro = fintecture.SearchResultObject.construct_from({"data": []}, None)
        assert sro.is_empty is True

    def test_empty_search_result(self):
        sro = fintecture.SearchResultObject.empty_search_result()
        assert sro.is_empty

    def test_iter(self):
        arr = [{"id": 1}, {"id": 2}, {"id": 3}]
        expected = fintecture.util.convert_to_fintecture_object(arr)
        sro = fintecture.SearchResultObject.construct_from({"data": arr}, None)
    ...
    def test_bool(self, search_result_object):
        ...
        empty = fintecture.SearchResultObject.construct_from(
            {"object": "list", "url": "/my/path", "data": []}, "myappid"
        )
        assert bool(empty) is False

    def test_next_search_result_page(self, request_mock):
        sro = fintecture.SearchResultObject.construct_from(
            {
                "object": "list",
                "data": [{"id": 1}],
                "has_more": True,
    ...
    def test_next_search_result_page_empty_search_result(self):
        ...
        next_sro = sro.next_search_result_page()
        assert next_sro == fintecture.SearchResultObject.empty_search_result()
    ...
    def test_serialize_empty_search_result(self):
        ...
        deserialized = fintecture.SearchResultObject.construct_from(
