...
class TestCustomer(object):
    def test_is_listable(self, request_mock):
        resources = fintecture.Customer.list()
        request_mock.assert_requested("get", "/v1/customers")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], fintecture.Customer)

    def test_is_retrievable(self, request_mock):
        resource = fintecture.Customer.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/customers/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, fintecture.Customer)

    def test_is_creatable(self, request_mock):
        resource = fintecture.Customer.create()
    ...
    def test_is_modifiable(self, request_mock):
        resource = fintecture.Customer.modify(
    ...
    def test_can_delete(self, request_mock):
        resource = fintecture.Customer.delete(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "delete", "/v1/customers/%s" % TEST_RESOURCE_ID
        )
        assert resource.deleted is True
    ...
    def test_can_delete_discount_class_method(self, request_mock):
        fintecture.Customer.delete_discount(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "delete", "/v1/customers/%s/discount" % TEST_RESOURCE_ID
        )
...
class TestCustomerSources(object):
    def test_is_creatable(self, request_mock):
        fintecture.Customer.create_source(TEST_RESOURCE_ID, source="btok_123")
        request_mock.assert_requested(
            "post", "/v1/customers/%s/sources" % TEST_RESOURCE_ID
        )

    def test_is_retrievable(self, request_mock):
        fintecture.Customer.retrieve_source(TEST_RESOURCE_ID, TEST_SOURCE_ID)
        request_mock.assert_requested(
            "get",
    ...
    def test_is_modifiable(self, request_mock):
        fintecture.Customer.modify_source(
    ...
    def test_is_deletable(self, request_mock):
        fintecture.Customer.delete_source(TEST_RESOURCE_ID, TEST_SOURCE_ID)
        request_mock.assert_requested(
            "delete",
            "/v1/customers/%s/sources/%s" % (TEST_RESOURCE_ID, TEST_SOURCE_ID),
        )
    ...
    def test_is_listable(self, request_mock):
        resources = fintecture.Customer.list_sources(TEST_RESOURCE_ID)
        request_mock.assert_requested(
...
class TestCustomerTaxIds(object):
    def test_is_creatable(self, request_mock):
        resource = fintecture.Customer.create_tax_id(
            TEST_RESOURCE_ID, type="eu_vat", value="11111"
        )
        request_mock.assert_requested(
            "post", "/v1/customers/%s/tax_ids" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, fintecture.TaxId)

    def test_is_retrievable(self, request_mock):
        fintecture.Customer.retrieve_tax_id(TEST_RESOURCE_ID, TEST_TAX_ID_ID)
        request_mock.assert_requested(
            "get",
    ...
    def test_is_deletable(self, request_mock):
        fintecture.Customer.delete_tax_id(TEST_RESOURCE_ID, TEST_TAX_ID_ID)
        request_mock.assert_requested(
            "delete",
            "/v1/customers/%s/tax_ids/%s" % (TEST_RESOURCE_ID, TEST_TAX_ID_ID),
        )
    ...
    def test_is_listable(self, request_mock):
        resources = fintecture.Customer.list_tax_ids(TEST_RESOURCE_ID)
        request_mock.assert_requested(
...
class TestCustomerTransactions(object):
    def test_is_creatable(self, request_mock):
        resource = fintecture.Customer.create_balance_transaction(
            TEST_RESOURCE_ID, amount=1234, currency="usd"
        )
        request_mock.assert_requested(
            "post", "/v1/customers/%s/balance_transactions" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, fintecture.CustomerBalanceTransaction)
    ...
    def test_is_listable(self, request_mock):
        resources = fintecture.Customer.list_balance_transactions(TEST_RESOURCE_ID)
        request_mock.assert_requested(
...
class TestCustomerPaymentMethods(object):
    def test_is_listable(self, request_mock):
        fintecture.Customer.list_payment_methods(TEST_RESOURCE_ID, type="card")
        request_mock.assert_requested(
            "get", "/v1/customers/%s/payment_methods" % TEST_RESOURCE_ID
        )

    def test_is_listable_on_object(self, request_mock):
        resource = fintecture.Customer.retrieve(
            TEST_RESOURCE_ID
        ).list_payment_methods(TEST_RESOURCE_ID, type="card")
        request_mock.assert_requested(
            "get", "/v1/customers/%s/payment_methods" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, fintecture.ListObject)
...
class TestCustomerCashBalanceMethods(object):
    ...
    def test_customer_cashbalance_retrieve_legacy_call_pattern(
        ...
        fintecture.Customer.retrieve_cash_balance("cus_123", None)
        request_mock.assert_requested(
