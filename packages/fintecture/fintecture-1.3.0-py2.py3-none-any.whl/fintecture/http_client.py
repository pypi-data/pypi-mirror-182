...
import fintecture
from fintecture import error, util, six
from fintecture.request_metrics import RequestMetrics
...
try:
    from fintecture.six.moves import urllib
except ImportError:
...
try:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                "Warning: the Fintecture library requires that your Python "
                '"requests" library be newer than version 0.8.8, but your '
                '"requests" library is version %s. Fintecture will fall back to '
                "an alternate HTTP library so everything should work. We "
                'recommend upgrading your "requests" library. If you have any '
                "questions, please contact contact@fintecture.com. (HINT: running "
...
from fintecture.six.moves.urllib.parse import urlparse
...
def new_default_http_client(*args, **kwargs):
    if urlfetch:
        ...
        impl = Urllib2Client
        warnings.warn(
            "Warning: the Fintecture library is falling back to urllib2/urllib "
...
class HTTPClient(object):
    ...
    def _should_retry(self, response, api_connection_error, num_retries):
        ...
        if rheaders is not None and "Fintecture-should-retry" in rheaders:
            if rheaders["Fintecture-should-retry"] == "false":
                return False
            if rheaders["Fintecture-should-retry"] == "true":
                return True
        ...
        #
        # Note that we expect the Fintecture-should-retry header to be false
    ...
    def _max_network_retries(self):
        from fintecture import max_network_retries
    ...
    def _add_telemetry_header(self, headers):
        ...
        if fintecture.enable_telemetry and last_request_metrics:
            telemetry = {
                "last_request_metrics": last_request_metrics.payload()
            }
            headers["X-Fintecture-Client-Telemetry"] = json.dumps(telemetry)
...
class RequestsClient(HTTPClient):
        ...
        try:
            try:
                ...
                raise TypeError(
                    ...
                    '"requests" library is not compatible with Fintecture\'s '
    ...
    def _handle_request_error(self, e):
        ...
        if isinstance(e, requests.exceptions.SSLError):
            msg = (
                "Could not verify Fintecture's SSL certificate.  Please make "
        ...
        # Retry only timeout and connect errors; similar to urllib3 Retry
            ...
            msg = (
                "Unexpected error communicating with Fintecture.  "
                "If this problem persists, let us know at "
                "contact@fintecture.com."
            )
            err = "%s: %s" % (type(e).__name__, str(e))
        ...
        # Catch remaining request exceptions
            ...
            msg = (
                ...
                "know at contact@fintecture.com."
            )
...
class UrlFetchClient(HTTPClient):
    ...
    def _handle_request_error(self, e, url):
        if isinstance(e, urlfetch.InvalidURLError):
            msg = (
                "The Fintecture library attempted to fetch an "
                "invalid URL (%r). This is likely due to a bug "
                "in the Fintecture Python bindings. Please let us know "
                "at contact@fintecture.com." % (url,)
            )
        elif isinstance(e, urlfetch.DownloadError):
            msg = "There was a problem retrieving data from Fintecture."
        elif isinstance(e, urlfetch.ResponseTooLargeError):
            msg = (
                "There was a problem receiving all of your data from "
                "Fintecture.  This is likely due to a bug in Fintecture. "
                "Please let us know at contact@fintecture.com."
            )
        else:
            msg = (
                "Unexpected error communicating with Fintecture. If this "
                "problem persists, let us know at contact@fintecture.com."
            )
...
class PycurlClient(HTTPClient):
    ...
    def _request_internal(self, method, url, headers, post_data):
        ...
        if self._verify_ssl_certs:
            self._curl.setopt(pycurl.CAINFO, fintecture.ca_bundle_path)
        else:
    ...
    def _handle_request_error(self, e):
        if e.args[0] in [
            ...
            msg = (
                "Could not connect to Fintecture.  Please check your "
                "internet connection and try again.  If this problem "
                "persists, you should check Fintecture's service status at "
                "https://twitter.com/fintecturestatus, or let us know at "
                "contact@fintecture.com."
            )
            should_retry = True
...
class Urllib2Client(HTTPClient):
    ...
    def _handle_request_error(self, e):
        msg = (
            "Unexpected error communicating with Fintecture. "
            "If this problem persists, let us know at contact@fintecture.com."
        )
        msg = textwrap.fill(msg) + "\n\n(Network error: " + str(e) + ")"
        raise error.APIConnectionError(msg)
