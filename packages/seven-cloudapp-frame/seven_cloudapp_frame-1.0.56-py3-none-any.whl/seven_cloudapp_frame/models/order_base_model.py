...
@LastEditTime: 2022-12-19 11:33:00
...
class OrderBaseModel(FrameBaseModel):
    ...
    def _delete_prize_roster_dependency_key(self, act_id, user_id, delay_delete_time=0.1):
        ...
        :last_editors: HuangJianYi
        """
        PrizeRosterModel().delete_dependency_key(DependencyKey.prize_roster_list(act_id,user_id), delay_delete_time)
    ...
    def get_prize_order_list(self, app_id, act_id, user_id, open_id, nick_name, order_no, real_name, telephone, address, order_status, create_date_start, create_date_end, page_size=20, page_index=0, order_by="create_date desc", field="*", is_search_roster=False, is_cache=True, condition="", params=[], prize_roster_sub_table=None):
        ...
        if is_cache:
            page_list, total = prize_order_model.get_cache_dict_page_list(field, page_index, page_size, where, "", order_by, params_list, dependency_key=DependencyKey.prize_order_list(act_id,user_id))
        else:
    ...
    def get_prize_roster_list(self,app_id,act_id,module_id,user_id,open_id="",user_nick="",order_no="",goods_type=-1,prize_type=-1,logistics_status=-1,prize_status=-1,pay_status=-1,page_size=20,page_index=0,create_date_start="",create_date_end="",order_by="create_date desc",field="*",condition="",params=[],is_cache=True, page_count_mode="total"):
        ...
        if page_count_mode == "total":
            return_info = PageInfo(page_index, page_size, 0, [])
        elif page_count_mode == "next":
            return_info = WaterPageInfo([],False)
        ...
        if is_cache:
            page_list = prize_roster_model.get_cache_dict_page_list(field, page_index, page_size, where, "", order_by, params_list, dependency_key=DependencyKey.prize_roster_list(act_id,user_id), page_count_mode=page_count_mode)
        else:
            page_list = prize_roster_model.get_dict_page_list(field, page_index, page_size, where, "", order_by, params_list, page_count_mode)
        if page_count_mode == "total":
            total = page_list[1]
    ...
    def create_wechat_refund(self, source_type, app_id, act_id, pay_order_no, refund_amount = 0, refund_reason = ""):
        ...
        if source_type != 1:
            wechat_pay = config.get_value("wechat_pay")
            #退款异步通知地址
