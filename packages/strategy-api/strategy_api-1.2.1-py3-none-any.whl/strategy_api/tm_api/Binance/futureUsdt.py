...
from strategy_api.event.engine import EVENT_TIMER, Event
...
from strategy_api.tm_api.object import TickData, BarData, Interval, OrderRequest, OrderData, Direction, OrderType, \
    Offset, Status, CancelRequest, HistoryRequest, PositionSide
...
class BinanceFutureUsdtGateway(BaseGateway):
    ...
    def get_order_type(self, maker: bool = False, stop_loss: bool = False,
                       stop_profit: bool = False) -> OrderType:
        if maker:
            # 发送限价单
    ...
    def buy(self,
        ...
        original_req: OrderRequest = OrderRequest(
            ...
            offset=Offset.OPEN,
            type=self.get_order_type(maker, stop_loss, stop_profit),
            price=price,
            volume=volume,
            positionSide=position_side,
        )
        return self.send_order(original_req)
    ...
    def subscribe(self, symbol: str, interval: Interval = None) -> None:
        self.market_ws_api.subscribe(symbol, interval)
...
class BinanceUsdtRestApi(RestClient):
    ...
    def send_order(self, req: OrderRequest) -> str:
        ...
        if req.positionSide == PositionSide.ONEWAY:
            ...
            if (req.direction == Direction.SHORT and req.offset == Offset.OPEN) or (req.direction == Direction.LONG and req.offset == Offset.CLOSE):
                position_side = "SHORT"
            else:
                position_side = "LONG"
...
class BinanceUsdtDataWebsocketApi(WebsocketClient):
    ...
    def subscribe(self, symbol: str, interval: Interval) -> None:
        """订阅数据"""
