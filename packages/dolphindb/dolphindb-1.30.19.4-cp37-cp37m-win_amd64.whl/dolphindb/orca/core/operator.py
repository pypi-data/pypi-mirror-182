...
def _orca_head_tail_op(func):
    def bfunc(self, other=5):
        df = self.compute()
        if func == "head":
            return df.head(other)
        else:
            return df.tail(other)
...
def _get_expr_with_binary_op(ExpressionType, left, right, func,axis=None):
...
class ArithOpsMixin(metaclass=abc.ABCMeta):
    ...
    head = _orca_head_tail_op("head")
    tail = _orca_head_tail_op("tail")
    ...
    @abc.abstractmethod
    def _binary_op(self, other, func,axis=None):
        return _get_expr_with_binary_op(ArithExpression, self, other, func,axis)
    ...
    def _extended_binary_op(self, other, func, axis, level, fill_value):
        ...
        if fill_value is None:
            ...
            if (axis == 0
                ...
                return self._binary_op(other, func,axis)
            elif (axis == 1
                  and self._is_dataframe_like
                  and (other._is_dataframe_like or other._is_series_like)):
                return self._binary_op(other, func,axis)
            else:
                raise NotImplementedError()
        else:
...
class StatOpsMixin(metaclass=abc.ABCMeta):
    ...
    def _binary_agg_op(self, other, func):
        ...
        data_columns = self._data_columns
        # Deal with corr func seperately
        if func == "corr" and not np.array_equal(self.index.values,other.index.values):
            select_list = [f"{func}(ej({self._var_name},{other._var_name}, \'{self._index_columns[0]}\').{col},ej({self._var_name},{other._var_name}, \'{self._index_columns[0]}\').{other._var_name}_{col}) as {col}"
                        for col
                        in data_columns]
        else:
    ...
    def rank(self, axis=0, method='min', numeric_only=None, na_option='keep', ascending=True, pct=False, rank_from_zero=False, group_num=None):
        if axis == 1:
            script = f"orca_row_rank({self._var_name})"
            e = ArithExpression(self, None, None, 1, None, script)
            return e.compute() # eager compute for now
    ...
    def groupby(self, by=None, axis=0, level=None, as_index=True, sort=True,
        ...
        if isinstance(self, (ArithExpression, BooleanExpression, MergeExpression)):
            internal = self
        else:
    ...
    def rolling(self, window, min_periods=None, center=False, win_type=None, on=None, axis=0, closed=None):
        ...
        return klass(ref._session, ref._internal, ref._index, window, min_periods, on=on, where_expr=ref._where_expr, name=None)
...
class BaseExpression(_InternalAccessor):
    ...
    def __init__(self, left, right, func, axis, infered_ddb_dtypestr=None, user_defined_script=""):
        ...
        self._user_defined_script = user_defined_script

        if self._user_defined_script:
            func = user_defined_script
        ...
        if isinstance(other_obj, (_ConstantSP, Timestamp)):
            ...
            elif user_defined_script:
                left_scripts = left._get_data_select_list()
                self._data_select_list = left_scripts
            else:
                left_scripts = left._get_data_select_list()
    ...
    def compute(self, squeeze=False, squeeze_axis=None):
        from .frame import DataFrame
        if "_user_defined_script" not in self.__dict__:
            self.__dict__["_user_defined_script"] = ""
        if self._user_defined_script:
            data = _InternalFrame.from_run_script(self._session, self._user_defined_script)
            return DataFrame(data, session=self._session)
    ...
    def _get_from_script(self, *args, **kwargs):
        ...
        if self._is_index_like:
            ...
            return DataFrame._get_from_script(*args, **kwargs,indexfreq=self._index.freq if isinstance(self._index,DatetimeIndex) else None)
        elif self._is_series_like:
            return Series._get_from_script(*args, **kwargs,indexfreq=self._index.freq if isinstance(self._index,DatetimeIndex) else None)
