...
user_defined_functions = [
    """
        def orca_row_rank(t) {
            table_copy = select * from t;
            orca_idx = t["ORCA_INDEX_LEVEL_0_"];
            dropColumns!(table_copy, "ORCA_INDEX_LEVEL_0_");
            rk = rowRank(matrix(table_copy));
            return table(orca_idx as ORCA_INDEX_LEVEL_0_, rk);
        }
    """,
]
...
def connect(host, port, user="admin", passwd="123456", session=default_session()):
    ...
    for udf in user_defined_functions:
        session.run(udf)
...
def read_in_memory_table(table_name, session=default_session(), index_column=None):
    if index_column:
        session.run(f"rename!({table_name}, `{index_column}, `ORCA_INDEX_LEVEL_0_)")
    data = _InternalFrame.from_run_script(session, table_name)
    return DataFrame(data, session=session)
