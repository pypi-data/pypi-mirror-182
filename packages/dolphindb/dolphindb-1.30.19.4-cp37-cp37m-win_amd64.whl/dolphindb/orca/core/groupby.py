...
from .indexes import Index,DatetimeIndex
...
class GroupByOpsMixin(metaclass=abc.ABCMeta):
    ...
    def shift(self, periods=1, freq=None, axis=0, fill_value=None):
        ...
        return self._contextby_op(f"move{{,{periods}}}", numeric_only=False)
        # TODO: fill_value
    ...
    def head(self, n=5):
        ...
        ref = self.compute() if isinstance(self, (ArithExpression,
                                                BooleanExpression)) else self
        return klass(
            klass(ref._internal).to_pandas().groupby(ref._groupby_name).head(n))
    ...
    def tail(self, n=5):
        ...
        ref = self.compute() if isinstance(self, (ArithExpression,
                                                BooleanExpression)) else self
        return klass(
            klass(ref._internal).to_pandas().groupby(ref._groupby_name).tail(n))
    ...
    def rolling(self,
                ...
                axis=0,
                closed=None):
        from .window import DataFrameGroupbyRolling, SeriesGroupbyRolling
        klass = SeriesGroupbyRolling if self._is_series_like else DataFrameGroupbyRolling
        if on is not None and not isinstance(on, str):
            raise TypeError("on must be a string")
        ref = self.compute() if isinstance(self, (ArithExpression,
                                                BooleanExpression)) else self
        return klass(ref._session,
                    ref._internal,
                    ref._index,
                    window,
                    min_periods,
                    on=on,
                    where_expr=ref._where_expr,
                    name=None,
                    groupby_list=ref._groupby_list,
                    groupkeys=ref._groupkeys,
                    is_groupby=False)
    ...
    def rank(self, axis=0, method='min', na_option='keep', ascending=True, pct=False, rank_from_zero=False, group_num=None):
    ...
    def apply(self, func, *args, **kwargs):
        ...
        return self._groupby_op(func, numeric_only=False)
    ...
    def _generate_groupby_select_list_and_value_list(self, func, groupkeys, numeric_only):
        ...
        if isinstance(func, str):
            ...
            from orca.converter.dolphindb_parser import parse_py_code_to_dolphindb_code
            func_script = parse_py_code_to_dolphindb_code(func)
        ...
        for select_col, col in zip(select_columns, data_columns):
            if (col not in groupkeys
                ...
                if "{}" in func:
                    func_call = func.format(select_col).replace("{", "(").replace("}", ")")
                    select_list.append(f"{func_call} as {col}")
                    value_list.append(f"{func_call}")
                else:
    ...
    def _run_groupby_script(self, func, script, groupkeys, is_apply=False):
        ...
        if groupby_size:
            ...
            if not isinstance(self,GroupBy) and isinstance(data._index,DatetimeIndex):
                s._index.freq=self._index.freq
            return s
        elif is_apply:
            ...
            if not isinstance(self,GroupBy) and isinstance(data._index,DatetimeIndex):
                data._index.freq=self._index.freq
            return data
    ...
    def _groupby_op(self, func, numeric_only):
        if "," in func and "{" in func and "}" in func:
            func = func.replace("{", "{{")
            func = func.replace("}", "}}")
            func = func.replace(",", "{},")
        select_list, _ = \
...
class GroupBy(_InternalAccessor, GroupByOpsMixin, metaclass=abc.ABCMeta):
    ...
    def __init__(self, session, internal, index, by, level, as_index, sort, ascending, where_expr, name,
        ...
        self._groupby_name = index_names
        self._groupkeys = groupkeys
