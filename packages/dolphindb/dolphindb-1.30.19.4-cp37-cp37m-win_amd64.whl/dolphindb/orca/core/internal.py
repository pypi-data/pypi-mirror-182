...
class _ConstantSP(object):
    ...
    def run_script(cls, session, script):
        ...
        if _get_verbose():
            print(script)
        if is_dolphindb_identifier(script):
            session.run(f"{var_name} = objByName(`{script})")
        else:
    ...
    def _update_metadata(self):
        ...
        self._updatable = not self._segmented and typestr.find("STREAMING") < 0
    ...
    def append(self, script):
        ...
        # self._update_metadata()    # TODO: append will not change metadata
    ...
    def attach_default_index(self):
        if self.segmented or not self.updatable:
            warnings.warn("Unable to attach an default index to segmented or streaming table.", AttachDefaultIndexWarning)
            return False
        ...
        checkIndex = f"cols = columnNames {var_name}; 'ORCA_INDEX_LEVEL_0_' in cols;"
        hasIndex = self._session.run(checkIndex)
        if hasIndex:
            # return True
            self.drop_columns("ORCA_INDEX_LEVEL_0_")
        ...
        if form == ddb.settings.DF_TABLE:
            ...
            try:
                ...
                ex_msg = str(ex)
                if (ex_msg.startswith("<Exception> in run: The table is not allowed to update")
...
class _InternalFrame(object):
    ...
    def __init__(self, session: ddb.session,
                 ...
                 column_index_names: Optional[List[str]] = None, index_of_any_vector=None,index_freq=None):
