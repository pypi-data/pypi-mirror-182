import abc
import itertools
from orca.core.indexes import DatetimeIndex
import warnings
...
from .utils import (_infer_axis, _infer_level, _merge_where_expr, _to_data_column,
                    ...
                    _try_convert_iterable_to_list, _unsupport_columns_axis, _get_time_str,_to_numpy_dtype,
...
class _Frame(_InternalAccessor, ArithOpsMixin, LogicalOpsMixin, StatOpsMixin, metaclass=abc.ABCMeta):
    ...
    def _with_where_expr(cls, where_expr, odf, *args, **kwargs):
        ...
        if not isinstance(odf, _InternalFrame):
            new_df._index=odf._index
        else:
            new_df._index.freq=odf._index_freq
    ...
    def _get_from_script(cls, session, script, df=None,
                         ...
                         name=None, squeeze=False, squeeze_axis=None,indexfreq=None):
        """
        ...
        odf = _InternalFrame(session, var, data_columns=data_columns,
                             ...
                             column_index_names=column_index_names,index_freq=indexfreq)
        ...
        if obj._is_series_like:
            obj.name = name
        if isinstance(obj._index, DatetimeIndex):
            obj._index._freq=indexfreq
        return obj
    ...
    def to_pandas(self):    # TODO: optimize
        ...
        if isinstance(data.index, pd.DatetimeIndex) and self.index.freq!=None:
            period_len=len(data.index.values)
            if period_len not in (0,1):
                if self._where_expr!=None:
                    dtindex=pd.date_range(data.index.values[0],periods=period_len,freq=pd.to_datetime(data.index.values[1])-pd.to_datetime(data.index.values[0]))
                else:
                    dtindex=pd.date_range(data.index.values[0],periods=period_len,freq=self.index.freq)
            elif period_len==0:
                dtindex=pd.DatetimeIndex(data=[],freq=self.index.freq)
            else:
                dtindex=pd.date_range(data.index.values[0],periods=1,freq=self.index.freq)
            dtindex.name=data.index.name
            data.index=dtindex
            # TODO: set dtype and tz
            # data.index.dtype = self_index.dtype
            # data.index.tz = self_index.tz
        return data
    ...
    def head(self, n=5):
        if not is_dolphindb_integral(n):
            ...
            if self._is_dataframe_like:
                x = self._ddb_dtypes[1:].to_dict()
                for i in self.columns:
                    x[i] = _to_numpy_dtype(self._ddb_dtypes[i]).name
                DT_result = pd.DataFrame(columns=self.columns,
                                        index=pd.RangeIndex(start=0, stop=0, step=1))
                y = DT_result.astype(x)
                from .generic import from_pandas
                return from_pandas(y, self._session)
            else:
                from .series import Series
                return Series([], name=self._name, dtype=self.dtypes)
        elif n < 0:
            return self.iloc[:n]
        else:
