...
class _LocIndexer(object):
    ...
    def __getitem__(self, key):  # TODO: handle Series._internal is _ConstantSP
        ...
        if isinstance(rows_sel, slice) and rows_sel == slice(None):
            #meet the conditions that cols_sel could be slice, list and str when loc called in different functions. Each condition return either Series or Dataframe
            return df[data_columns[0]] if (len(data_columns)==1 and not isinstance(cols_sel,list)) else df[data_columns]
...
class _iLocIndexer(object):
    ...
    def __getitem__(self, key):
        ...
        data_columns = self._get_data_columns(cols_sel)
        if isinstance(rows_sel, slice) and rows_sel == slice(None) and data_columns!=None and not isinstance(cols_sel,list):
            return df[data_columns[0]]
