...
def merge_asof(left, right, on=None, left_on=None, right_on=None,
    ...
    left_var_name, right_var_name = left._var_name, right._var_name
    left_data_columns, right_data_columns = left._data_columns[:], right._data_columns[:]
    left_onNby = left_by + left_on
    right_onNby = right_by + right_on
    if left_index:
        left_join_columns = left_by + left._index_columns
    else:
        try:
            left_join_columns, _ = check_key_existence(left_onNby, left_data_columns)
        except KeyError:
            left_join_columns, _ = check_key_existence(left_onNby, left_data_columns + left._index_columns)
    if right_index:
        right_join_columns = right_by + right._index_columns
    else:
        try:
            right_join_columns, _ = check_key_existence(right_onNby, right_data_columns)
        except KeyError:
            right_join_columns, _ = check_key_existence(right_onNby, right_data_columns + right._index_columns)
    if left_on==right_on:
        for i in left_on:
            right_data_columns.remove(i)
    for i in left_by:
        if i in right_by and i in right_data_columns:
            right_data_columns.remove(i)
    ...
    if left_index and not right_index:
        index_columns = [
            f"replace({right_var_name}.{col},NULL,{right.index.max()} )as {col}"
            for col in right._index_columns
        ]
        if not left_by:
            index_columns = index_columns + ([(f"{left_var_name}.{index} as {col} ")
                                    for col in right_on
                                    for index in left._index_columns])
            left_columns, left_data_columns, left_ddb_dtypes = \
                _get_join_list(overlap_columns, left_var_name, lsuf,
                            left_data_columns, left._ddb_dtypes)
        else:
            for col in right._index_columns:
                insert_pos=len(left_by)
                left_columns, left_data_columns, left_ddb_dtypes = \
                _get_join_list(overlap_columns, left_var_name, lsuf,
                            left_data_columns, left._ddb_dtypes)
                left_data_columns.insert(insert_pos,f"key_{insert_pos}")
                left_columns.insert(insert_pos,f"{left_var_name}.{col}" )
                from pandas import Series
                left_ddb_dtypes = Series(np.insert(left_ddb_dtypes.values, insert_pos,
                                           left._ddb_dtypes[col]),
                                 left_ddb_dtypes.index.insert(insert_pos, col),
                                 name=left_ddb_dtypes.name)
        # if force_index:
        #     left_columns, left_data_columns, left_ddb_dtypes = \
        #         _add_force_index(index_list, left_columns, left_data_columns, left_ddb_dtypes,
        #                          right_join_columns, right_data_columns, right._ddb_dtypes)
        ...
        index_columns = [
            f"replace({left_var_name}.{col},NULL,{left.index.max()} )as {col}"
            for col in left._index_columns
        ]
        if not right_by:
            index_columns = index_columns + ([(f"{left_var_name}.{col} as {col} ")
                                      for col in left_on])
            left_columns, left_data_columns, left_ddb_dtypes = \
                _get_join_list(overlap_columns, left_var_name, lsuf,
                            left_data_columns, left._ddb_dtypes)
            ...
            for col in left_on:
                insert_pos=len(right_by)
                left_columns, left_data_columns, left_ddb_dtypes = \
                _get_join_list(overlap_columns, left_var_name, lsuf,
                            left_data_columns, left._ddb_dtypes)
                left_data_columns.insert(insert_pos,col)
                left_columns.insert(insert_pos,f"{left_var_name}.{col}" )
                from pandas import Series
                left_ddb_dtypes = Series(np.insert(left_ddb_dtypes.values, insert_pos,
                                           left._ddb_dtypes[col]),
        ...
        # index_columns = index_list
        index_columns = [(f"{left_var_name}.{col} as {col} ")
                 for col in left._index_columns]
        left_columns, left_data_columns, left_ddb_dtypes = \
            _get_join_list(overlap_columns, left_var_name, lsuf,
                           left_data_columns, left._ddb_dtypes)
...
def merge_window(left, right, window_lower, window_upper, prevailing=False, on=None,
    ...
    method = "select * from pwj" if prevailing else "select * from wj"
    where_expr = _merge_where_expr(left._where_expr, right._where_expr)
