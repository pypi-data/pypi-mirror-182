...
class DataFrame(DataFrameLike, _Frame, IOOpsMixin):
    ...
    def toTable(self) -> ddb.Table:
        s: ddb.session = self._session
        newvar = _ConstantSP.ORCA_IDENTIFIER + _new_orca_identifier()
        s.run(f"{newvar} = {self._var_name}")
        return s.table(data=newvar)
