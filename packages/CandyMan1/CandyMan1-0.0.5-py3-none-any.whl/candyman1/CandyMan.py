...
class Nasa():
    ...
    class isspos():
        def __init__(self):
            self.host = "http://api.open-notify.org/iss-now.json"

        def longitude(self):
            '''
            returns the iss longitude pos
            '''
            data = requests.get(self.host)
            return data['issposition']['longitude']

        def latitude(self):
            '''
            returns the iss latitude pos
            '''
            data = requests.get(self.host)
            return data['issposition']['latitude']
