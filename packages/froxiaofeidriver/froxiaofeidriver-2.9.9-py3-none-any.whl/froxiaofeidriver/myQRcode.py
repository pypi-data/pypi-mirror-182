...
#     flag_now = 'No'
    ...
    def detect_QRcode(self):
        ...
        while True:
            ...
            img = cv2.resize(img0, self.dim, interpolation = cv2.INTER_AREA)#视频帧压缩
            if success:
                flag_now,AA,BB = self.QR_detector.detectAndDecode(img)
                if flag_now:
                    if flag_now == self.flag_pass:
                        self.count += 1
                        if self.count == 5:
                            self.count = 0
                            return len(flag_now.strip()),flag_now,img
                        
                    self.flag_pass = flag_now
                else:
                    self.count = 0
                    return 0,flag_now,img
