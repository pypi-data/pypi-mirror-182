...
def test_read_service_config_file_default(make_config_file):
...
def test_read_mapping_env_var(monkeypatch, mocker):
...
def test_read_global_config_file_default(make_config_file):

    boto_endpoint_url_shim.custom_endpoint_url_resolution(
        boto_endpoint_url_shim.read_global_config_file
    )

    make_config_file(f"[default]\nendpoint_url = {ENDPOINT_URL}\n")
...
def test_read_global_config_file_profile(make_config_file):
    ...
    make_config_file(f"[profile local]\nendpoint_url = {ENDPOINT_URL}\n")
