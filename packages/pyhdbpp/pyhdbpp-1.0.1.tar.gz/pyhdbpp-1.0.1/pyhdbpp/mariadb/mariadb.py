...
def _match(pattern, model):
    if '*' in pattern and '.*' not in pattern:
        pattern = pattern.replace('*','.*')
    return re.match(pattern.lower().replace('%','.*'), model)
...
class MariaDBReader(AbstractReader):
    ...
    def get_attributes(self, active=False, pattern='', load=False):
        ...
        if pattern:
            return [a for a in self.attributes if _match(pattern,a)]
    ...
    def get_attribute_name(self,attribute):
        ...
        """
        attribute = _translate(attribute)
