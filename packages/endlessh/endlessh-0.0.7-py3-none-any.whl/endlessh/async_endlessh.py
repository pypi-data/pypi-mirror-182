...
setpath('/tmp/endlessh.log')


def get_ipinfo_with_token() -> str:
    try:
        from authc import gunload
        return gunload('ipinfo')
    except:
        return None
...
class Hosts(object):
    ...
    def get_location(cls, ip: str) -> str:
        ...
        try:
            if cls.myredis is None:
                from authc import get_redis
                cls.myredis = get_redis()

            if cls.myredis.exists(ip):
                return cls.myredis.get(ip).decode()

            url = get_ipinfo_with_token()
            if not url:
                url = 'https://ipinfo.io/' + ip + '/json'

            resp = requests.get(url).json()
            ipinfo = '{}, {}'.format(resp['city'], resp['region'])
            cls.myredis.set(ip, ipinfo, ex=1 << 22)
            return ipinfo
