...
class ThreadPool:
    ...
    :meta private:
    """
    def __init__(self, dgram, num_threads=2):
        self.dgram = dgram
    ...
    def on_exception(self, worker_thread, exc_info):
        if self.dgram.exception_handler is not None:
            handled = self.dgram.exception_handler.handle(exc_info)
        else:
            handled = False
        if not handled:
...
def user_link(user: types.User, include_id: bool=False) -> str:
    ...
    :type user: :obj:`dgram.types.User`
...
def webhook_google_functions(bot, request):
    ...
    :type bot: :obj:`dgram.dgram` or :obj:`dgram.async_dgram.Asyncdgram`
...
def antiflood(function: Callable, *args, **kwargs):
    ...
    .. code-block:: python3
        ...
        from dgram.util import antiflood
    ...
    :return: None
    """
    from dgram.apihelper import ApiTelegramException
