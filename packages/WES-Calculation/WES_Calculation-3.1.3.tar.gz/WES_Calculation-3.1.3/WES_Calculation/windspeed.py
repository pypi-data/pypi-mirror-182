...
LastEditTime : 2022-12-19 10:34:07
LastEditors  : BDFD
Description  : 
...
def windspeed(o2, zw, Xlat, X, Rg, U, atm, Ta, zt, Tw, TaaC, wdu, zu):
    ...
    if o2==4 or o2==3:
        ...
        section2.extend(['Coriolis参数='+str(round(fCo,7))+'rad/s'])
        if dT==0:
            section2.extend(['地转风与近水面风之间的夹角\u03B8='+str(round(180*theta/math.pi,2))+'\u00B0, 稳定性参数\u03BC='+str(round(miu,1))+', A(\u03BC)='+str(round(A(miu),2))+', B(\u03BC)='+str(round(B(miu),2))])
        else:        
            section2.extend(['结合KEYPS廓线： 地转风与近水面风之间的夹角\u03B8='+str(round(180*theta/math.pi,2))+'\u00B0, 稳定性参数\u03BC='+str(round(miu,1))+', A(\u03BC)='+str(round(A(miu),2))+', B(\u03BC)='+str(round(B(miu),2))])
            section2.extend(['结合B-D廓线： \u03B8='+str(round(180*theta2/math.pi,2))+'\u00B0, \u03BC='+str(round(miu2,1))+', A(\u03BC)='+str(round(A(miu2),2))+', B(\u03BC)='+str(round(B(miu2),2))])
    ...
    if dT==0:#20221211
        section7.extend(['滨水工',str(round(Up1[10],3))])
    else:
        section7.extend(['滨水工',str(round(Up1[10],3))+'(KEYPS)'+'    |    '+str(round(Up2[10],3))+'(B-D)'])

    if ss1==None:
        section7.extend(['JTS 145-2015',str(round(Ua1,3))])
    else:
        section7.extend(['JTS 145-2015','无有效方法'])
    
    section7.extend(['SPM 1984',str(round(U102,3))])

    if ss3==None:
        section7.extend(['CEM',str(round(Ua3,3))])
    else:
        section7.extend(['CEM','信息不足'])

    if ss6==None:
        section7.extend(['API RP 2A-WSD',str(round(Ua6,3))])
    else:
        section7.extend(['API RP 2A-WSD','无有效方法'])
    ...
    return img_stream, heading, section1, section2, section3, section4, section5, section6, section7, ending