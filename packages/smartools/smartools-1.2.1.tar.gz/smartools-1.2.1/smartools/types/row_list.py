...
class RowList(TypedListWrapper):
    ...
    def _index_items(self, idx):
        if isinstance(idx, (tuple, list)):
            column, value = idx
            if column not in self._tuple_ref:
                self._tuple_ref[column] = {}
            if value in self._tuple_ref[column]:
                return self._store[self._tuple_ref[column]]
            column_idx = self._columns[column].index
            for i in range(len(self._tuple_ref[column])+1, len(self._store)):
                row = self._store[i]
                cell_value = str(row.cells[column_idx].value or '')
                self._tuple_ref[column][cell_value] = i
                if cell_value == value:
                    return row
        else:
