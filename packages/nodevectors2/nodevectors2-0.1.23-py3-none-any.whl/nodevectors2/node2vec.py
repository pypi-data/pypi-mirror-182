...
import csrgraph2 as cg
from nodevectors2.embedders import BaseNodeEmbedder
...
class Node2Vec(BaseNodeEmbedder):
    ...
    def fit(self, G):
        ...
        G : graph data
            Graph to embed
            Can be any graph type that's supported by csrgraph2 library
            (NetworkX, numpy 2d array, scipy CSR matrix, CSR matrix components)
        """
        if not isinstance(G, cg.csrgraph2):
            G = cg.csrgraph2(G, threads=self.threads)
