...
class Location:
    ...
    def get_adjusted_capacity(self, building_id: str, pandify: bool = False) -> Union[pd.DataFrame, dict]:
        ...
        query, variables = self.basking._graphql_query.get_adjusted_capacity_graphql_query(building_id)
        data = {}
    ...
    def get_user_buildings(self, pandify=True):
        """
        ...
        query = self.basking._graphql_query.get_user_buildings_graphql_query()

        result = self.basking._graphql_query.graphql_executor(query=query, variables={})
        try:
    ...
    def get_building(self, building_id, pandify=False):
        ...
        query, variables = self.basking._graphql_query.get_building_graphql_query(building_id)
    ...
    def get_floor_meta_info(
        ...
        query, variables = self.basking._graphql_query.get_floor_meta_info_graphql_query(basking_floor_id)
        result = self.basking._graphql_query.graphql_executor(query=query, variables=variables)
        data = json.loads(result)
        if 'message' in data:
            self.basking.api_timeout_handler(data)
    ...
    def get_floor_areas_for_floor(
        ...
        query, variables = self.basking._graphql_query.get_floor_areas_for_floor_graphql_query(basking_floor_id)
