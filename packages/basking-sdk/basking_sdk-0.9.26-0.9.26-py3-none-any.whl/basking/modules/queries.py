GET_ORG_VISIT_DURATION = """
            query getOrgDurationOfVisits($id: ID, $from: DateTime!, $to: DateTime!){
                organization: getOrganization(id: $id) {
                    durationOfVisits(from: $from, to: $to) {
                            hPerDay,
                            clientCount,
                            percClient,
                            countryRegion
...
USER_REMOVE_FROM_LOCATION = """
            mutation removeUser($input: RemoveUserInput!){
                removeUser(input: $input)
            }
        """

USER_ADD_TO_LOCATION = """
...
USER_SET_ROLE = """
            mutation setGroup($input: SetGroupInput!){
...
LOCATION_GET_INVITE_LIST = """
            query getInvitations($buildingid: ID){
                building: getBuilding(id: $buildingid) {
                    users {
                        id,
                        firstName,
                        lastName,
