...
class MetricSystem():
    ...
    def _search_repair_sequence(self, metric, columns, repair_sequence):
        if (metric not in columns) and (metric not in repair_sequence):
            child_metrics = self.get_childs(metric)
            if isinstance(child_metrics, list):
                for child_metric in self.get_childs(metric):
                    self._search_repair_sequence(
                        child_metric, columns, repair_sequence)
                repair_sequence.append(metric)
            else:
                # 如果一个指标需要计算，则其必须有可计算的子指标，且子指标可计算
                raise ValueError(f"{metric} 没有可以用于计算的子指标")

    def repair_data(self, df):
        # 构建数据修复的指标序列
        repair_sequence = []
        for metric in self.relation_map.keys():
            self._search_repair_sequence(
                metric=metric,
                columns=df.columns,
        ...
        mdf = df.copy()
        for metric in repair_sequence:
            relation = self.get_relation(metric)
            mdf[metric] = mdf[self.get_childs(metric)].apply(
                lambda x: MetricSystem._get_stat(
                    x, relation=relation),
                raw=True,
                axis=1)
    ...
    @staticmethod
    def _calc_influence(source_from, source_to, target_from, target_to, relation, location_index) -> float:
        """
        计算影响度
        """
        if relation in ("+", "-"):
        ...
        if metric in self.relation_map:
            ...
            if node is not None:
                ...
                for location_index, child in enumerate(node["childs"]):
                    ...
                    influence = MetricSystem._calc_influence(
