...
class Complaint:
    ...
    def list_query(self, begin_date=None, end_date=None, limit=10, offset=0, complained_mchid=None):
        """查询投诉单列表
        ...
        :param complained_mchid: 被诉商户号，投诉单对应的被诉商户号。示例值：'1900012181'
        """
        if not begin_date:
            begin_date = datetime.now().strftime("%Y-%m-%d")
        if not end_date:
            end_date = begin_date
        if not complained_mchid:
            complained_mchid = self._p.mch_id
        path = '/v3/merchant-service/complaints-v2'
        params = dict(limit=limit, offset=offset, begin_date=begin_date, end_date=end_date,
                      complainted_mchid=complained_mchid)
        return self._p.core.request(path, data=params)

    def details(self, complaint_id):
        """查询投诉单详情
    ...
    def history_query(self, complaint_id, limit=100, offset=0):
        """查询投诉协商历史
        :param complaint_id: 投诉单对应的投诉单号。示例值：'200201820200101080076610000'
    ...
    def notification_create(self, url):
        """创建投诉通知回调地址
    ...
    def notification_update(self, url):
        """更新投诉通知回调地址
        :param: url: 通知地址，仅支持https。示例值：'https://www.xxx.com/notify'
        """
        params = {'url': url}
    ...
    def response(self, complaint_id, response_content, response_images=None, jump_url=None, jump_url_text=None):
        """提交投诉回复
        :param complaint_id: 投诉单对应的投诉单号。示例值：'200201820200101080076610000'
        :param response_content: 回复内容，具体的投诉处理方案，限制200个字符以内。示例值：'已与用户沟通解决'
        :param response_images: 回复图片，传入调用商户上传反馈图片接口返回的media_id，最多上传4张图片凭证。
            示例值：['file23578_21798531.jpg', 'file23578_21798532.jpg']
        :param jump_url: 跳转链接，附加跳转链接，引导用户跳转至商户客诉处理页面，链接需满足https格式。
            示例值:"https://www.xxx.com/notify"
        :param jump_url_text: 转链接文案，展示给用户的文案，附在回复内容之后。用户点击文案，即可进行跳转。示例值:"查看订单详情"
        """
        params = {'response_content': response_content}
        params.update(dict(complainted_mchid=self._p.mch_id))
        if response_images:
    ...
    def complete(self, complaint_id):
        """反馈投诉处理完成
        :param complaint_id: 投诉单对应的投诉单号。示例值：'200201820200101080076610000'
        """
        params = {}
    ...
    def image_upload(self, filepath, filename=None):
        ...
        """
        return self._p.media.media_upload(self, filepath, filename, '/v3/merchant-service/images/upload')

    def image_download(self, media_url):
        """下载客户投诉图片
        :param media_url: 图片下载地址，示例值：'https://api.mch.weixin.qq.com/v3/merchant-service/images/xxxxx'
        """
        path = media_url[len(self._p.core.gate_way):] if media_url.startswith(self._p.core.gate_way) else media_url
        return self._p.core.request(path, skip_verify=True)

    def update_refund(self, complaint_id, action, launch_refund_day=None, reject_reason=None,
                      reject_media_list=None, remark=None):
        """更新退款审批结果
        :param complaint_id: 投诉单对应的投诉单号。示例值:'200201820200101080076610000'
