...
class ProfitSharing:
    """
    直联商户 资金应用——分账
    参考 https://pay.weixin.qq.com/wiki/doc/apiv3/apis/chapter8_1_1.shtml
    ...
    def orders(self, transaction_id, out_order_no, receivers, unfreeze_unsplit,
        ...
        :param receivers: 分账接收方列表，最多可有50个分账接收方，示例值：[{'type':'MERCHANT_ID',
                            'account':'86693852', 'amount':888, 'description':'分给商户A'}]
        :param unfreeze_unsplit: 是否解冻剩余未分资金，示例值：True, False
        :param appid: 应用ID，可不填，默认传入初始化时的appid，示例值：'wx1234567890...'
        :param sub_appid: (服务商模式)子商户应用ID，示例值：'wxd678efh567hg6999'
        :param sub_mchid: (服务商模式)子商户的商户号，由微信支付生成并下发。示例值：'1900000109'
        """
        params = {'transaction_id': transaction_id, 'out_order_no': out_order_no}
        ...
        for receiver in params.get('receivers'):
            if receiver.get('name'):
                receiver['name'] = self._p.core.encrypt(receiver.get('name'))
                cipher_data = True
    ...
    def orders_query(self, transaction_id, out_order_no, sub_mchid=None):
        """查询分账结果
        :param transaction_id: 微信支付订单号，示例值：'4208450740201411110007820472'
        :param out_order_no: 商户分账单号，只能是数字、大小写字母_-|*@，示例值：'P20150806125346'
        :param sub_mchid: (服务商模式)子商户的商户号，由微信支付生成并下发。示例值：'1900000109'
    ...
    def return_orders(self, out_return_no, return_mchid, amount, description,
    ...
    def return_orders_query(self, out_order_no, out_return_no, sub_mchid=None):
        """查询分账回退结果
        :param out_order_no: 商户分账单号，只能是数字、大小写字母_-|*@，示例值：'P20150806125346'
        :param out_return_no: 商户回退单号，商户在自己后台生成的一个新的回退单号，在商户后台唯一，示例值：'R20190516001'
        :param sub_mchid: (服务商模式)子商户的商户号，由微信支付生成并下发。示例值：'1900000109'
    ...
    def unfreeze(self, transaction_id, out_order_no, description, sub_mchid=None):
        """解冻剩余资金
        :param transaction_id: 微信支付订单号，示例值：'4208450740201411110007820472'
        :param out_order_no: 商户分账单号，只能是数字、大小写字母_-|*@，示例值：'P20150806125346'
    ...
    def amounts_query(self, transaction_id):
        """查询剩余待分金额
        :param transaction_id: 微信支付订单号，示例值：'4208450740201411110007820472'
        """
    ...
    def config_query(self, sub_mchid):
        """查询最大分账比例
        :param sub_mchid: (服务商模式)子商户的商户号，由微信支付生成并下发。示例值：'1900000109'
        参考 https://pay.weixin.qq.com/wiki/doc/apiv3_partner/apis/chapter8_1_7.shtml
        """
        if sub_mchid:
    ...
    def receivers_add(self, account_type, account, relation_type, name=None, custom_relation=None,
                      appid=None, sub_appid=None, sub_mchid=None):
        """添加分账接收方
        :param account_type: 分账接收方类型，枚举值：'MERCHANT_ID'：商户ID，'PERSONAL_OPENID'：个人openid
        :param account: 分账接收方账号，类型是 'MERCHANT_ID'时，是商户号，
                        类型是'PERSONAL_OPENID'时，是个人 openid，示例值：'86693852'
        ...
        :param name: 分账个人接收方姓名，分账接收方类型是 'MERCHANT_ID' 时，是商户全称（必传），
                    当商户是小微商户或个体户时，是开户人姓名，分账接收方类型是 'PERSONAL_OPENID' 时，是个人姓名
        :param custom_relation: 自定义的分账关系，子商户与接收方具体的关系，本字段最多10个字。
                                当字段 'relation_type' 的值为 'CUSTOM' 时，本字段必填;
                                当字段 'relation_type' 的值不为 'CUSTOM' 时，本字段无需填写。示例值：'代理商'
        :param appid: 应用ID，可不填，默认传入初始化时的appid，示例值：'wx1234567890...'
        :param sub_appid: (服务商模式)子商户应用ID，示例值：'wxd678efh567hg6999'
        :param sub_mchid: (服务商模式)子商户的商户号，由微信支付生成并下发。示例值：'1900000109'
        """
        params = {'type': account_type, 'account': account, 'relation_type': relation_type}
        cipher_data = False
        if name:
    ...
    def receivers_delete(self, account_type, account,
                         appid=None, sub_appid=None, sub_mchid=None):
        """删除分账接收方
        :param account_type: 分账接收方类型，枚举值：'MERCHANT_ID'：商户ID，'PERSONAL_OPENID'：个人openid
        :param account: 分账接收方账号，类型是 'MERCHANT_ID' 时，是商户号，
                        类型是 'PERSONAL_OPENID' 时，是个人openid，示例值：'86693852'
        :param appid: 应用ID，可不填，默认传入初始化时的appid，示例值：'wx1234567890...'
        :param sub_appid: (服务商模式)子商户应用ID，示例值：'wxd678efh567hg6999'
        :param sub_mchid: (服务商模式)子商户的商户号，由微信支付生成并下发。示例值：'1900000109'
        """
    ...
    def bills(self, bill_date, tar_type='GZIP', sub_mchid=None):
        """申请分账账单
    ...
    def brand_order(self, brand_mchid, sub_mchid, transaction_id, out_order_no, receivers,
    ...
    def brand_order_query(self, transaction_id, out_order_no, sub_mchid):
        """查询连锁品牌分账结果
        :param transaction_id: 微信支付订单号，示例值：'4208450740201411110007820472'
        :param out_order_no: 商户分账单号，只能是数字、大小写字母_-|*@，示例值：'P20150806125346'
        :param sub_mchid: 子商户的商户号，由微信支付生成并下发。示例值：'1900000109'
    ...
    def brand_return(self, sub_mchid, out_return_no, return_mchid, amount,
    ...
    def brand_return_query(self, sub_mchid, out_return_no, order_id=None, out_order_no=None):
        """查询连锁品牌分账回退结果
        :param sub_mchid: 子商户的商户号，由微信支付生成并下发。示例值：'1900000109'
        :param out_return_no: 商户回退单号，商户在自己后台生成的一个新的回退单号，在商户后台唯一，示例值：'R20190516001'
        :param order_id: 微信分账单号，与out_order_no参数二选一，示例值：'3008450740201411110007820472'
    ...
    def brand_unfreeze(self, sub_mchid, transaction_id, out_order_no, description):
        """完结连锁品牌分账
        :param sub_mchid: 子商户的商户号，由微信支付生成并下发。示例值：'1900000109'
        :param transaction_id: 微信支付订单号，示例值：'4208450740201411110007820472'
        :param out_order_no: 商户分账单号，只能是数字、大小写字母_-|*@，示例值：'P20150806125346'
    ...
    def brand_amount_query(self, transaction_id):
        """查询连锁品牌分账剩余待分金额
        :param transaction_id: 微信支付订单号，示例值：'4208450740201411110007820472'
        """
        if not transaction_id:
    ...
    def brand_config_query(self, brand_mchid):
        """查询连锁品牌分账最大分账比例
    ...
    def brand_add_receiver(self, brand_mchid, account_type, account, relation_type,
    ...
    def brand_delete_receiver(self, brand_mchid, account_type, account, appid=None, sub_appid=None):
        """删除连锁品牌分账接收方
        :param brand_mchid: 品牌商户号，示例值：'1900000108'
        :param account_type: 分账接收方类型，枚举值：'MERCHANT_ID'：商户ID，'PERSONAL_OPENID'：个人openid
        :param account: 分账接收方账号，类型是 'MERCHANT_ID'时，是商户号，
