...
class PayScore:
    """
    直联商户 经营能力——微信支付分
    参考 https://pay.weixin.qq.com/wiki/doc/apiv3/apis/chapter6_1_14.shtml
    """
    ...
    def direct_complete(self, out_order_no, openid, service_id, service_introduction, post_payments,
        ...
        """创单结单合并
        :param out_order_no: 商户服务订单号，商户系统内部服务订单号（不是交易单号），要求此参数只能由数字、大小写
            字母_-|*组成，且在同一个商户号下唯一。示例值：'1234323JK2DFE1243252'
        ...
        :param attach: 商户数据包。商户数据包可存放本订单所需信息，需要先 url encode 后传入。当商户数据包总长度超出
            256字符时，报错处理。示例值：'Ea1dfo2eal2dkj3nla4sj...'
        :param notify_url: 商户回调地址，商户接收扣款成功回调通知的地址，服务需要收款时此参数必填；服务无需收款时
            此参数不填。示例值：'https://api.test.com'
        :param appid: 应用ID，可不填，默认传入初始化时的appid，示例值:'wx1234567890ab3def'
        """
        params = {}
        if not (out_order_no and openid and service_id and service_introduction and post_payments
                and time_range and total_amount):
            raise Exception('out_order_no or openid or service_id or service_introduction or post_payments'
                            ' or time_range or total_amount is not assigned.')
    ...
    def permission(self, service_id, authorization_code, notify_url=None, appid=None):
        """商户预授权
        :param service_id: 服务ID。示例值：'500001'
        :param authorization_code: 授权协议号，户系统内部授权协议号，要求此参数只能由数字、大小写
            字母_-*组成，且在同一个商户号下唯一。示例值：'1234323JKE1...'
        :param notify_url: 通知地址，商户接收授权回调通知的地址。示例值：'https://www.qq.com'
        :param appid: 应用ID，可不填，默认传入初始化时的appid，示例值:'wx1234567890ab3def'
        """
        params = {}
    ...
    def permission_query(self, service_id, authorization_code=None, openid=None):
        """查询用户授权记录（授权协议号或openid）
        :param service_id: 服务ID。示例值：'500001'
        :param authorization_code: 授权协议号，户系统内部授权协议号，要求此参数只能由数字、大小写字母_-*组成，
            且在同一个商户号下唯一。示例值：'1234323JK8DFE1243252'
        :param openid: 用户标识，微信用户在商户对应appid下的唯一标识。示例值：'oUpF8uMuAJO_M2pxb1Q9zNjWeS6o'
        """
        if not service_id:
        ...
        if authorization_code:
            ...
            path = '/v3/payscore/permissions/openid/%s?appid=%s&service_id=%s' % (openid, self._p.app_id, service_id)
        else:
            raise Exception('authorization_code or openid is not assigned.')
        return self._p.core.request(path)

    def permission_terminate(self, service_id, reason, authorization_code=None, openid=None):
    ...
    def create(self, out_order_no, service_id, service_introduction, time_range,
        ...
        """创建支付分订单
        :param out_order_no: 商户服务订单号，商户系统内部服务订单号（不是交易单号），要求此参数只能由数字、大小写
            字母_-|*组成，且在同一个商户号下唯一。示例值：'1234323JK8DFE1243252'
        :param service_id: 服务ID，该服务ID有本接口对应产品的权限。示例值：'500001'
        :param service_introduction: 服务信息，用于介绍本订单所提供的服务 ，当参数长度超过20个字符时，报错处理。示例值：'某某酒店'
        :param time_range: 服务时间范围。
        :param risk_fund: 订单风险金。
        :param attach: 商户数据包，户数据包可存放本订单所需信息，需要先 url encode 后传入。当商户数据包总长度超出
            256字符时，报错处理。
        :param openid: 用户标识，微信用户在商户对应appid下的唯一标识。免确认订单：必填，需确认订单：不填。
    ...
    def query(self, service_id, out_order_no=None, query_id=None):
        """查询支付分订单
        :param service_id: 服务ID，该服务ID有本接口对应产品的权限。示例值：'500001'
        :param out_order_no: 商户服务订单号，商户系统内部服务订单号（不是交易单号），要求此参数只能由数字、大小
            写字母_-|*组成，且在同一个商户号下唯一。示例值：'1234323JK8DFE1243252'
        :param query_id: 回跳查询ID，微信侧回跳到商户前端时用于查单的单据查询id。商户单号与回跳查询id必填其中一个。
            不允许都填写或都不填写。示例值：'15646546545165651651'
        """
        if service_id:
            path = '/v3/payscore/serviceorder?service_id=%s&appid=%s' % (service_id, self._p.app_id)
        else:
            raise Exception('service_id is not assigned.')
        if out_order_no:
    ...
    def cancel(self, out_order_no, service_id, reason, appid=None):
        """取消支付分订单
        :param out_order_no: 商户服务订单号，商户系统内部服务订单号（不是交易单号），要求此参数只能由数字、大小写
            字母_-|*组成，且在同一个商户号下唯一。示例值：'1234323JK8DFE1243252'
        :param service_id: 服务ID，该服务ID有本接口对应产品的权限。示例值：'500001'
        :param reason: 取消原因，最多30个字符，每个汉字/数字/英语都按1个字符计算超过长度报错处理。注：重录时
            需保证参数完全一致，包括取消原因。示例值：'用户投诉'
        :param appid: 应用ID，可不填，默认传入初始化时的appid，示例值:'wx1234567890ab3def'
        """
        params = {}
        if out_order_no:
    ...
    def modify(self, out_order_no, service_id, post_payments, total_amount, reason, post_discounts=None, appid=None):
        """修改订单金额
        :param out_order_no: 商户服务订单号，商户系统内部服务订单号（不是交易单号），要求此参数只能由数字、大小写
            字母_-|*组成，且在同一个商户号下唯一。
        :param service_id: 服务ID，该服务ID有本接口对应产品的权限。示例值：'500001'
        ...
        :param reason: 取消原因，最多30个字符，每个汉字/数字/英语都按1个字符计算超过长度报错处理。
            注：重录时需保证参数完全一致，包括取消原因。示例值：'用户投诉'
        :param post_discounts: 后付费商户优惠，后付费商户优惠列表，最多包含30条商户优惠。如果传入，用户侧则显示此参数。
        :param appid: 应用ID，可不填，默认传入初始化时的appid，示例值:'wx1234567890ab3def'
        """
        params = {}
    ...
    def complete(self, out_order_no, service_id, post_payments, total_amount, post_discounts=None,
    ...
    def pay(self, out_order_no, service_id, appid=None):
        """商户发起催收扣款
        :param out_order_no: 商户服务订单号，商户系统内部服务订单号（不是交易单号），要求此参数只能由数字、大小写
            字母_-|*组成，且在同一个商户号下唯一。示例值：'1234323JK8DFE1243252'
        :param service_id: 服务ID，该服务ID有本接口对应产品的权限。示例值：'500001'
    ...
    def sync(self, out_order_no, service_id, scene_type='Order_Paid', detail=None, appid=None):
        """同步服务订单信息
        :param out_order_no: 商户服务订单号，商户系统内部服务订单号（不是交易单号），要求此参数只能由数字、大小写
            字母_-|*组成，且在同一个商户号下唯一。示例值：'1234323JK8DFE1243252'
        :param service_id: 服务ID，该服务ID有本接口对应产品的权限。示例值：'500001'
    ...
    def refund(self, transaction_id, out_refund_no, amount, reason=None,
        ...
        :param notify_url: 通知地址，示例值：'https://www.weixin.qq.com/wxpay/pay.php'
        """
        return self._p.pay.refund(out_refund_no=out_refund_no, amount=amount, transaction_id=transaction_id,
    ...
    def refund_query(self, out_refund_no):
        ...
        :param out_refund_no: 商户退款单号，示例值：'1217752501201407033233368018'
        """
        return self._p.pay.query_refund(self, out_refund_no=out_refund_no)

    def merchant_bill(self, bill_date, service_id, tar_type='GZIP', encryption_algorithm='AEAD_AES_256_GCM'):
        """商户申请获取对账单
        ...
        :param encryption_algorithm: 加密算法，对返回账单原文加密的算法 'AEAD_AES_256_GCM'，账单使用
            AEAD_AES_256_GCM 加密算法进行加密。示例值：'AEAD_AES_256_GCM'
        """
        if bill_date:
        ...
        path = '%s&encryption_algorithm=%s' % (path, encryption_algorithm)
        return self._p.core.request(path)
