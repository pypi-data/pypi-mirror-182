...
class Transfer:
    ...
    def transfer_batch(self, out_batch_no, batch_name, batch_remark, total_amount, total_num,
                       transfer_detail_list: list, appid=None):
        """发起商家转账
        :param out_batch_no:    商户系统内部的商家批次单号，要求此参数只能由数字、大小写字母组成，在商户系统内部唯一，示例值：'t2020042013'
        ...
        :param transfer_detail_list: 发起批量转账的明细列表，最多三千笔，示例值：
                    [{"out_detail_no": "x23zy545Bd5436", "transfer_amount": 200000, "transfer_remark": "2020年4月报销",
                     "openid": "o-MYE42l80oelY2DE34nYD456Xoy", "user_name": "张三"}]
        :param appid:           应用ID，可不填，默认传入初始化时的 appid，示例值:'wx1234567890ab3def'
        """
        params = {}
        if out_batch_no:
        ...
        for transfer_detail in params.get('transfer_detail_list'):
            if transfer_detail.get('user_name'):
                transfer_detail['user_name'] = self._p.core.encrypt(transfer_detail.get('user_name'))
                cipher_data = True
    ...
    def transfer_query_batch(self, batch_id, need_query_detail=False, offset=0, limit=20, detail_status='ALL'):
        """微信批次单号查询批次单
    ...
    def transfer_query_out_batch_no(self, out_batch_no, need_query_detail=False, offset=0, limit=20,
                                    detail_status='ALL'):
        """商家批次单号查询批次单
        :param out_batch_no: 商家批次单号，示例值：t2020042013
        :param need_query_detail: 是否查询转账明细单，枚举值：true：是；false：否，默认否。
        :param offset: 请求资源起始位置，默认值为0
        :param limit: 最大资源条数，默认值为20
        :param detail_status: 明细状态， ALL:全部。需要同时查询转账成功和转账失败的明细单；SUCCESS:转账成功。只查询转账成功的明细单；FAIL:转账失败。
    ...
    def transfer_bill_receipt(self, out_batch_no):
        ...
        return self._p.core.request(path, method=RequestType.POST, params=params)
