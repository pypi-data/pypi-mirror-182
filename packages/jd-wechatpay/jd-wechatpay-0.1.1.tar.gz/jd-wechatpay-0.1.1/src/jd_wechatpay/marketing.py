...
class Marketing:
    """
    直联商户 营销工具——图片上传（营销专用）、消费卡、委托营销、支付有礼、代金卷、商家券
    ...
    def image_upload(self, filepath, filename=None):
        ...
        """
        return self._p.media.media_upload(self, filepath, filename, '/v3/marketing/favor/media/image-upload')

    def card_send(self, card_id, openid, out_request_no, send_time, appid=None):
        """发放消费卡
        :param card_id: 消费卡ID。示例值：'pIJMr5MMiIkO_93VtPyIiEk2DZ4w'
        :param openid: 用户openid，待发卡用户的openid。示例值：'obLa2jh...'
        :param out_request_no: 商户单据号。示例值：'oTYh3fd3d3j_0136...'
        :param send_time: 请求发卡时间，单次请求发卡时间，消费卡在商户系统的实际发放时间，为东八区标准时间（UTC+8）。
        ...
        params.update({'appid': appid or self._p.app_id})
        path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/coupons/%s/send' % card_id
        return self._p.core.request(path, method=RequestType.POST, data=params)

    def partnership_build(self, idempotency_key, partner_type, business_type, partner_appid=None,
        ...
        if business_type not in ['FAVOR_STOCK', 'BUSIFAVOR_STOCK'] or not stock_id:
            raise Exception('invalid value in business_type/stock_id.')
        params.update({'authorized_data': {'business_type': business_type, 'stock_id': stock_id}})
        path = '/v3/marketing/partnerships/build'
        return self._p.core.request(path, method=RequestType.POST, data=params, headers=headers)

    def partnership_query(self, business_type, stock_id, partner_type=None, partner_appid=None,
    ...
    def pay_gift_activity_create(self, activity_base_info, award_send_rule, advanced_setting=None):
        """创建全场满额送活动
        ...
        path = 'https://api.mch.weixin.qq.com/v3/marketing/paygiftactivity/unique-threshold-activity'
        return self._p.core.request(path, method=RequestType.POST, data=params)

    def pay_gift_activity_detail(self, activity_id):
        """查询活动详情接口
        :param activity_id: 活动id，示例值：'10028001'
        """
        if activity_id:
            path = 'https://api.mch.weixin.qq.com/v3/marketing/paygiftactivity/activities/%s' % activity_id
        else:
            raise Exception('activity_id is not assigned.')
        return self._p.core.request(path)

    def pay_gift_merchants_list(self, activity_id, offset=0, limit=20):
        """查询活动发券商户号
        :param activity_id: 活动id，示例值：'10028001'
        ...
        """
        if activity_id:
            path = 'https://api.mch.weixin.qq.com/v3/marketing/paygiftactivity/activities/%s/merchants' % activity_id
        else:
            raise Exception('activity_id is not assigned.')
        path = '%s?offset=%s&limit=%s' % (path, offset, limit)
        return self._p.core.request(path)

    def pay_gift_goods_list(self, activity_id, offset=0, limit=20):
        """查询活动指定商品列表
        :param activity_id: 活动id，示例值：'10028001'
        :param offset:分页页码，页面从0开始。示例值：1
        :param limit: 分页大小，限制分页最大数据条目。示例值：20
        """
        if activity_id:
            path = 'https://api.mch.weixin.qq.com/v3/marketing/paygiftactivity/activities/%s/goods' % activity_id
        else:
            raise Exception('activity_id is not assigned.')
        path = '%s?offset=%s&limit=%s' % (path, offset, limit)
        return self._p.core.request(path)

    def pay_gift_activity_terminate(self, activity_id):
        """终止活动
        :param activity_id: 活动id，示例值：'10028001'
        """
        if activity_id:
            path = 'https://api.mch.weixin.qq.com/v3/marketing/paygiftactivity/activities/%s/terminate' % activity_id
        else:
            raise Exception('activity_id is not assigned.')
        return self._p.core.request(path, method=RequestType.POST)

    def pay_gift_merchants_add(self, activity_id, add_request_no, merchant_id_list=None):
        """新增活动发券商户号
        :param activity_id: 活动id，示例值：'10028001'
        :param add_request_no: 请求业务单据号，商户添加发券商户号的凭据号，商户侧需保持唯一性。示例值：'100002322019090134234s34f'
        ...
        if merchant_id_list is None:
            merchant_id_list = []
        if activity_id:
            path = 'https://api.mch.weixin.qq.com/v3/marketing/paygiftactivity/activities/%s/merchants/add' %\
                   activity_id
        else:
            raise Exception('activity_id is not assigned.')
        params = {}
    ...
    def pay_gift_activity_list(self, offset=0, limit=20, activity_name=None, activity_status=None, award_type=None):
        """获取支付有礼活动列表
        :param offset:分页页码，页面从0开始。示例值：1
        :param limit: 分页大小，限制分页最大数据条目。示例值：20
        :param activity_name: 活动名称，支持模糊搜索。示例值：'良品铺子回馈活动'
        :param activity_status: 活动状态，枚举值：'ACT_STATUS_UNKNOWN'：状态未知，'CREATE_ACT_STATUS'：已创建，
                                'ONGOING_ACT_STATUS'：运行中，'TERMINATE_ACT_STATUS'：已终止，
                                'STOP_ACT_STATUS'：已暂停，'OVER_TIME_ACT_STATUS'：已过期，
                                'CREATE_ACT_FAILED'：创建活动失败。示例值：'CREATE_ACT_STATUS'
        :param award_type: 奖品类型，暂时只支持商家券。'BUSIFAVOR'：商家券。示例值：'BI_FAVOR'
        """
        params = {}
        ...
        path = 'https://api.mch.weixin.qq.com/v3/marketing/paygiftactivity/activities'
        return self._p.core.request(path, method=RequestType.POST, data=params)

    def pay_gift_merchants_delete(self, activity_id, merchant_id_list=None, delete_request_no=None):
        """删除活动发券商户号
        :param activity_id: 活动id，示例值：'10028001'
        :param delete_request_no: 请求业务单据号，商户创建批次凭据号（格式：商户id+日期+流水号），商户侧需保持唯一性，
            可包含英文字母，数字，｜，_，*，-等内容，不允许出现其他不合法符号。示例值：'100002322019090134234sf6f'
        :param merchant_id_list: 删除的发券商户号，从活动已有的发券商户号中移除的商户号列表，
            特殊规则：最小字符长度为8，最大为15，条目个数限制：[1，500]。示例值：["10000022"，"10000023"]
        """
        if merchant_id_list is None:
            merchant_id_list = []
        if activity_id:
            path = 'https://api.mch.weixin.qq.com/v3/marketing/paygiftactivity/activities/%s/merchants/delete' % \
                   activity_id
        else:
            raise Exception('activity_id is not assigned.')
        params = {}
    ...
    def favor_stocks_create(self, stock_name, belong_merchant, available_begin_time,
                            available_end_time, stock_use_rule, coupon_use_rule, out_request_no,
                            stock_type='NORMAL', no_cash=False, comment=None,
                            pattern_info=None, ext_info=None):
        """创建代金券批次
        ...
        :param stock_use_rule: 发放规则。示例值：{'max_coupons':5, 'max_amount':100, 'max_coupons_per_user':1,
            'natural_person_limit':False, 'prevent_api_abuse':True}
        :param coupon_use_rule: 核销规则。示例值：{'available_merchants':['9856000','9856111']}
        :param out_request_no: 商户单据号，可包含英文字母，数字，|，_，*，-等内容，不允许出现其他不合法符号，
            商户侧需保持商户单据号全局唯一。示例值：'89560002019101000121'
        ...
        :param ext_info: 扩展属性，json格式字符串，如无需要则不填写。示例值："{'info1':'1234','info2':'3456'}"
        """
        params = {}
        if stock_name:
    ...
    def favor_stocks_start(self, stock_creator_mchid, stock_id):
        """激活代金券批次
    ...
    def favor_stocks_send(self, stock_id, openid, out_request_no,
                          stock_creator_mchid, coupon_value=None, coupon_minimum=None, appid=None):
        """发放代金券批次
        :param stock_id: 批次号。示例值：'9856000'
        :param openid: 用户openid，示例值：'2323df3df34...'
    ...
    def favor_stocks_pause(self, stock_creator_mchid, stock_id):
        """暂停代金券批次
        :param stock_creator_mchid: 创建批次的商户号，示例值：'8956000'
        :param stock_id: 批次号，示例值：'9856000'
        """
    ...
    def favor_stocks_restart(self, stock_creator_mchid, stock_id):
        """重启代金券批次
        :param stock_creator_mchid: 创建批次的商户号，示例值：'8956000'
        :param stock_id: 批次号，示例值：'9856000'
        """
    ...
    def favor_stocks_list(self, stock_creator_mchid, offset=0, limit=10,
                          create_start_time=None, create_end_time=None, status=None):
        """条件查询批次列表
        :param stock_creator_mchid: 创建批次的商户号，示例值：'8956000'
    ...
    def favor_stocks_detail(self, stock_creator_mchid, stock_id):
        """查询批次详情
        :param stock_creator_mchid: 创建批次的商户号，示例值：'8956000'
        :param stock_id: 批次号，示例值：'9856000'
        """
    ...
    def favor_stocks_coupons_detail(self, coupon_id, openid):
        """查询代金券详情
        :param coupon_id: 代金券id，示例值：'9856888'
        :param openid: 用户openid，示例值：'2323df3df...'
        """
        if coupon_id and openid:
            path = '/v3/marketing/favor/users/%s/coupons/%s?appid=%s' % (openid, coupon_id, self._p.app_id)
        else:
            raise Exception('coupon_id or openid is not assigned.')
        return self._p.core.request(path)

    def favor_stocks_merchants(self, stock_creator_mchid, stock_id, offset=0, limit=50):
        """查询代金券可用商户
        :param stock_creator_mchid: 创建批次的商户号，示例值：'8956000'
        :param stock_id: 批次号，示例值：'9856000'
    ...
    def favor_stocks_items(self, stock_creator_mchid, stock_id, offset=0, limit=50):
        """查询代金券可用单品
        :param stock_creator_mchid: 创建批次的商户号，示例值：'8956000'
        :param stock_id: 批次号，示例值：'9856000'
    ...
    def favor_stocks_user_coupon(self, openid, stock_id=None, status=None,
                                 creator_mchid=None, sender_mchid=None, available_mchid=None,
                                 offset=0, limit=20):
        """根据商户号查用户的券
        :param openid: 用户openid，示例值：'2323df2df...'
        :param stock_id: 批次号，示例值：'9856000'
        ...
        """
        if openid:
            path = '/v3/marketing/favor/users/%s/coupons?appid=%s&offset=%s&limit=%s' % \
                   (openid, self._p.app_id, offset, limit)
        else:
            raise Exception('openid is not assigned.')
        if stock_id:
    ...
    def favor_stocks_use_flow(self, stock_id):
        """下载批次核销明细
    ...
    def favor_stocks_refund_flow(self, stock_id):
        """下载批次退款明细
        :param stock_id: 批次号，微信为每个代金券批次分配的唯一id。示例值：'9865000'
        """
        if stock_id:
    ...
    def favor_stocks_callback_update(self, notify_url=None, switch=True, mchid=None):
        """设置消息通知地址
        ...
        """
        params = dict(mchid=mchid or self._p.mch_id)
        if notify_url:
            params.update({'notify_url': notify_url})
        elif self._p.notify_url:
            params.update({'notify_url': self._p.notify_url})
        else:
    ...
    def bi_favor_stocks_create(self, stock_name, belong_merchant, goods_name, stock_type,
                               coupon_use_rule, stock_send_rule, out_request_no, coupon_code_mode,
                               comment=None, custom_entrance=None, display_pattern_info=None,
                               notify_config=None, subsidy=False):
        """创建商家券
        ...
        :params coupon_use_rule: 核销规则。示例值：{'coupon_available_time':{}, 'fixed_normal_coupon':{},
            'use_method':'OFF_LINE', }
        :params stock_send_rule: 发放规则。示例值：{'max_coupons':100, 'max_coupons_per_user':5}
        :params out_request_no: 商户请求单号。示例值：'100002322019090134234sf6f'
        :params coupon_code_mode: 券code模式，枚举值：'WECHATPAY_MODE'：系统分配券code。（固定22位纯数字），
            'MERCHANT_API'：商户发放时接口指定券code，'MERCHANT_UPLOAD'：商户上传自定义code，发券时系统随机选取
            上传的券code。示例值：'WECHATPAY_MODE'
        ...
        path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/stocks'
        return self._p.core.request(path, method=RequestType.POST, data=params)

    def bi_favor_stocks_query(self, stock_id):
        """查询商家券详情
        :param stock_id: 批次号。示例值：1212
        """
        if stock_id:
            path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/stocks/%s' % stock_id
        else:
            raise Exception('stock_id is not assigned.')
        return self._p.core.request(path)

    def bi_favor_stocks_use(self, coupon_code, use_time, use_request_no, stock_id=None, openid=None):
        """核销用户券
        :param coupon_code: 券code，券的唯一标识。示例值：'sn34343434'
        ...
        if openid:
            params.update({'openid': openid})
        params.update(dict(appid=self._p.app_id))
        path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/coupons/use'
        return self._p.core.request(path, method=RequestType.POST, data=params)

    def bi_favor_user_coupon(self, openid, stock_id=None, coupon_state=None,
                             creator_merchant=None, belong_merchant=None, sender_merchant=None,
                             offset=0, limit=20):
        """根据过滤条件查询用户券
        :param openid: 用户标识。示例值：'xsd3434454567676'
        :param stock_id: 批次号。示例值：1212
        ...
        """
        path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/users/%s/coupons?appid=%s&offset=%s&limit=%s' % \
               (openid, self._p.app_id, offset, limit)
        if stock_id:
            path = '%s&stock_id%s' % (path, stock_id)
        if coupon_state:
    ...
    def bi_favor_coupon_detail(self, coupon_code, openid):
        """查询用户单张券详情
        :param coupon_code: 券code，券的唯一标识。示例值：'123446565767'
        :param openid: 用户标识。示例值：'2323df2df342342'
        """
        if not (coupon_code and openid):
            raise Exception('coupon_code or openid is not assigned.')
        path = '/v3/marketing/busifavor/users/%s/coupons/%s/appids/%s' % (openid, coupon_code, self._p.app_id)
        return self._p.core.request(path)

    def bi_favor_coupon_code_upload(self, stock_id, upload_request_no, coupon_code_list=None):
        """上传预存code
        :param stock_id: 批次号。示例值：1212
        :param upload_request_no: 请求业务单据号。商户上传code的凭据号，商户侧需保持唯一性。示例值：'100002322019090134234sf3f'
    ...
    def bi_favor_callback_update(self, mchid=None, notify_url=None):
        """设置商家券事件通知地址
        ...
        """
        params = {}
        params.update(dict(mchid=mchid or self._p.mch_id))
        if not (notify_url or self._p.notify_url):
            raise Exception('notify_url is not assigned.')
        params.update({'notify_url': notify_url if notify_url else self._p.notify_url})
        path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/callbacks'
        return self._p.core.request(path, method=RequestType.POST, data=params)

    def bi_favor_callback_query(self, mchid=None):
        """查询商家券事件通知地址
        :param mchid: 商户号，不填默认使用初始化的mchid。示例值：'10000098'
        """
        path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/callbacks?mchid=%s' % (mchid or self._p.mch_id)
        return self._p.core.request(path)

    def bi_favor_coupon_associate(self, stock_id, coupon_code, out_trade_no, out_request_no):
        """关联订单信息
        :param stock_id: 批次号。示例值：1212
        :param coupon_code: 券code，券的唯一标识。示例值：'sx6e34343434'
        ...
        if out_request_no:
            ...
            raise Exception('out_request_no is not assigned.')
        path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/coupons/associate'
        return self._p.core.request(path, method=RequestType.POST, data=params)

    def bi_favor_coupon_disassociate(self, stock_id, coupon_code, out_trade_no, out_request_no):
        """取消关联订单信息
        :param stock_id: 批次号。示例值：1212
        :param coupon_code: 券code，券的唯一标识。示例值：'sx6e34343434'
        :param out_trade_no: 关联的商户订单号，微信支付下单时的商户订单号，欲与该商家券关联的微信支付。示例值：'MCH_102233445'
        ...
        params.update({'out_trade_no': out_trade_no})
        params.update({'out_request_no': out_request_no})
        path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/coupons/disassociate'
        return self._p.core.request(path, method=RequestType.POST, data=params)

    def bi_favor_stocks_budget(self, stock_id, modify_budget_request_no,
                               target_max_coupons=None, target_max_coupons_by_day=None,
                               current_max_coupons=None, current_max_coupons_by_day=None):
        """修改批次预算
        :param stock_id: 批次号。示例值：1212
        ...
        params = {}
        if stock_id:
            path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/stocks/%s/budget' % stock_id
        else:
            raise Exception('stock_id is not assigned.')
    ...
    def bi_favor_stocks_modify(self, stock_id, out_request_no, custom_entrance=None, comment=None,
                               goods_name=None, display_pattern_info=None, coupon_use_rule=None,
                               stock_send_rule=None, notify_config=None):
        """修改商家券基本信息
        :param stock_id: 批次号。示例值：1212
        :param out_request_no: 商户请求单号，示例值：'1002600620019090123143254435'
    ...
    def bi_favor_stocks_return(self, coupon_code, stock_id, return_request_no):
        """申请退券
        :param coupon_code: 券code，券的唯一标识。示例值：'sx6e34343434'
        :param stock_id: 批次号。示例值：1212
        ...
        path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/coupons/return'
        return self._p.core.request(path, method=RequestType.POST, data=params)

    def bi_favor_stocks_deactivate(self, coupon_code, stock_id, deactivate_request_no, deactivate_reason=None):
        """使券失效
        :param coupon_code: 券code，券的唯一标识。示例值：'sx6e34343434'
        :param stock_id: 批次号。示例值：1212
        ...
        path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/coupons/deactivate'
        return self._p.core.request(path, method=RequestType.POST, data=params)

    def bi_favor_stocks_subsidy_pay(self, stock_id, coupon_code, transaction_id,
                                    payer_merchant, payee_merchant, amount,
                                    description, out_subsidy_no):
        """营销补差付款
        :param stock_id: 批次号。示例值：1212
        :param coupon_code: 券code，券的唯一标识。示例值：'sx6e34343434'
        ...
        :param out_subsidy_no: 业务请求唯一单号。示例值：'subsidy-abc4-12345678'
        """
        params = {}
        params.update({'coupon_code': coupon_code})
        params.update({'stock_id': stock_id})
        ...
        path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/subsidy/pay-receipts'
        return self._p.core.request(path, method=RequestType.POST, data=params)

    def bi_favor_stocks_subsidy_query(self, subsidy_receipt_id):
        """查询营销补差付款单详情
        :param subsidy_receipt_id: 补差付款单号。示例值：'1120200119165100000000000001'
        """
        if subsidy_receipt_id:
            path = 'https://api.mch.weixin.qq.com/v3/marketing/busifavor/subsidy/pay-receipts/%s' % subsidy_receipt_id
        else:
            raise Exception('subsidy_receipt_id is not assigned.')
        return self._p.core.request(path)

    def industry_coupon_token(self, open_id, coupon_list=None):
        """出行券切卡组件预下单
        :param open_id: 用户在商户AppID下的唯一标识，该用户为后续拉起切卡组件的用户。示例值：'obL1tjrR8kUDlj4-nofQsPA5FI'
