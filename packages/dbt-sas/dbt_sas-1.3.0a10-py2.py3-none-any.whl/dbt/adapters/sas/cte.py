...
def process_function(
    ...
    if function_name == "CAST":
        ...
        process_tokens(function.tokens, pre, post, query, ctes, insert_schema=insert_schema)
...
def process_from(statement: Statement, pre: List[str], post: List[str], query: List[str], ctes: Dict[str, str]) -> None:
    while statement:
        ...
        if isinstance(token, Identifier):
            ...
            process_where(token, pre, post, query, ctes, insert_schema=True)
        elif isinstance(token, Comment):
            pass  # skip comments
        else:
            query.append(str(token))
...
def process_where(
    where: Union[Where, Comparison, Parenthesis],
    pre: List[str],
    post: List[str],
    query: List[str],
    ctes: Dict[str, str],
    ...
    w_tokens = list(where.tokens)
    while w_tokens:
        token = w_tokens.pop(0)
        # if token.ttype != Token.Text.Whitespace and token.ttype != Token.Text.Whitespace.Newline:
        #     print('>>>', token, type(token), token.ttype)
        if token.ttype == Token.Operator.Comparison and token.value == "!=":
            query.append("<>")
            ...
            process_where(token, pre, post, query, ctes, insert_schema)
        elif isinstance(token, Parenthesis):
            process_where(token, pre, post, query, ctes, insert_schema)
        elif isinstance(token, Function):
            process_function(token, pre, post, query, ctes, insert_schema)
        elif isinstance(token, Comment):
            pass  # skip comments
        else:
            query.append(str(token))
...
def process_tokens(
    statement: Statement, pre: List[str], post: List[str], query: List[str], ctes: Dict[str, str], insert_schema=False
) -> None:
    ...
    while statement:
        ...
        if token.ttype == Token.Keyword.DML and str(token).upper() == "SELECT":
            ...
            process_cte(statement, pre, post, ctes)
        elif token.ttype == Token.Keyword and token.value.upper() == "TRUE":
            query.append("1")
        elif token.ttype == Token.Keyword and token.value.upper() == "FALSE":
            query.append("0")
            ...
            query.append(" ")
            has_from = True
            process_from(statement, pre, post, query, ctes)
        elif isinstance(token, IdentifierList):
...
def prepare_query(sql: str) -> Query:
    ...
    for statement in sqlparse.parse(sql):
        statement = list(statement.tokens)
        process_tokens(statement, pre, post, query, ctes)
