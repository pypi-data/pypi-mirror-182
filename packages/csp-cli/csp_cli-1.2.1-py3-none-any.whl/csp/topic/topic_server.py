...
# 新版list 不再有合并单元格
def topic_list(model_repository=None, classify=None, dataset=None, show=True):
    ...
    if dataset:
        ...
        if msg == "Invalid access token":
            print("token无效，查询失败，请登录，重新查询。登录命令：csp login (没有账号？使用csp register 注册一个吧)")
            # 跳转至登录
            result = re_login(topic_list, dataset=dataset)
            return result
        ...
        try:
            ...
            if not scores:
                title_dict = {"作者": "author", "数据集名称": "datasetName", "提交时间": "createTime", "状态": "statusLabel"}
            else:
                classify_flag = json.loads(res_dict['data'][0]['scoresInfo'])
                if classify_flag[0].get('ap', None) or classify_flag[0].get('ap', None) == 0:
                    title_dict = {"作者": "author", "数据集名称": "datasetName", "总分": "modelScores", "召回率": "recall", "准确率": "precision", "ap": "ap", "提交时间": "createTime", "状态": "statusLabel"}
                elif classify_flag[0].get('fScore', None) or classify_flag[0].get('fScore', None) == 0:
                    title_dict = {"作者": "author", "数据集名称": "datasetName", "总分": "modelScores",  "召回率": "recall", "准确率": "precision", "f1": "fScore", "提交时间": "createTime", "状态": "statusLabel"}
                elif classify_flag[0].get('imageScore', None) or classify_flag[0].get('imageScore', None) == 0:
                    title_dict = {"作者": "author", "数据集名称": "datasetName", "总分": "modelScores",  "漏检率": "missingRate", "错误率": "errorRate", "正确率": "correctRate", "分数": "imageScore", "提交时间": "createTime", "状态": "statusLabel"}
                else:
        ...
        title_dict = {"分类": "classifyName", "模型名称": "modelName", "资源存储库": "repository", "版本": "modelVersion",
                      "评分": "modelScores",
                      "关联数据集": "datasetName", "数据集描述": "funDesc", "作者": "author", "创建时间": "createTime"}
...
def topic_upload(model_repository: str, model_type: str, model_source: str, dataset: str):
    if model_source not in ["cloud", "local"]:
        raise ValueError("镜像来源，local-本地 cloud-云端")
    if model_type not in ["bussiness", "general"]:
        raise ValueError("镜像类型，bussiness-业务 general-技术类型")
    user_data = check_user()
    user_token = user_data["access_token"]
    ...
    if image_url:
        ...
        if model_source == "cloud":
            ...
            if flag_infe_dockerhub.lower() == "yes" or flag_infe_dockerhub.lower() == "y":
                is_image_exited = check_image_out(model_name, model_tag)
        ...
        if model_source == "local":
            # 登录私服
            cmd_login_nux = 'docker login -u yirong-deploy -p P@ssw0rd#yr 27.196.43.168:28081'
            ...
            # docker 服务未起或登录私服失败
            if out_login.startswith("error during connect: This error may indicate that the docker daemon is not running"):
                raise RuntimeError("docker服务未开启")
            if out_login.replace("\n", "").endswith("http: server gave HTTP response to HTTPS client"):
                raise RuntimeError('''须配置私服地址：vim /etc/docker/daemon.json，增加字段 "insecure-registries": ["27.196.43.168:28081"]. windows系统在Docker Desktop的 Settings-Docker Engine 中配置''')
        ...
        header = {"Authorization": "Bearer " + user_token}
        params = {"modelType": model_type, "modelSource": model_source, "modelName": model_name, "modelVersion": model_tag, "datasetName": dataset}
        ...
        # 服务端返回token过期信息
        if msg == "Invalid access token":
            print("token无效，模型注册失败，请登录，重新上传。登录命令：csp login (没有账号？使用csp register 注册一个吧)")
            # 跳转至登录
            # topic_upload(model_repository, model_type, model_source, dataset)
            re_login(topic_upload, model_repository=model_repository, model_type=model_type, model_source=model_source, dataset=dataset)
            return
...
def topic_info(model_repository: str):
    ...
    if res_dict["code"] == 200:
        try:
            if res_dict["data"]["scoresInfo"][0].get('ap', None) or res_dict["data"]["scoresInfo"][0].get('ap', None) == 0:
                title_dict = {"标签": "category", "召回率": "recall", "准确率": "precision", "ap": "ap"}
            elif res_dict["data"]["scoresInfo"][0].get('fScore', None) or res_dict["data"]["scoresInfo"][0].get('fScore', None) == 0:
                title_dict = {"标签": "category", "召回率": "recall", "准确率": "precision", "f1": "fScore"}
            elif res_dict["data"]["scoresInfo"][0].get('imageScore', None) or res_dict["data"]["scoresInfo"][0].get('imageScore', None) == 0:
                title_dict = {"标签": "category", "漏检率": "missingRate", "错误率": "errorRate", "正确率": "correctRate", "分数": "imageScore"}
            else:
                raise KeyError('该评分类型不支持')
            print("name: ", model_repository)
