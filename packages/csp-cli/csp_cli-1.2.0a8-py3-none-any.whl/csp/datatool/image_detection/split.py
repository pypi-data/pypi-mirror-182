...
class VocSplit:
    ...
    def split(self, mode=None):
        ...
        out_eval_annotations = os.path.join(out_train, "Annotations")
        out_eval_txt = os.path.join(out_train, "ImageSets/Main")
        out_eval_images = os.path.join(out_train, "JPEGImages")
        ...
        ftrain = open(os.path.join(out_train_txt, 'train.txt'), 'w', encoding='utf-8')
        fval = open(os.path.join(out_eval_txt, 'val.txt'), 'w', encoding='utf-8')
...
class CocoSplit:
    ...
    def split(self, mode=None):
        ...
        out_eval = os.path.join(self.output, "eval")
        out_eval_annotations = os.path.join(out_train, "annotations")
        out_eval_images = os.path.join(out_train, "val")
        ...
        for i, datatype in enumerate(data_types):
            ...
            for item in os.listdir(ori_img_dir):
                if os.path.isfile(item):
                    ...
                    if item_type.lower() in ["jpg", "jpeg", "webp", "bmp", "png", 'rgb', 'tif', 'tiff', 'gif']:
                        if item_name in filename_class[0]:
                            det_img = os.path.join(out_train_images, item)
                            shutil.copy(ori_img, det_img)
                        if item_name in filename_class[1]:
                            det_img = os.path.join(out_eval_images, item)
                            shutil.copy(ori_img, det_img)
        logger.info("切分结果以保存至 {}".format(self.output))
