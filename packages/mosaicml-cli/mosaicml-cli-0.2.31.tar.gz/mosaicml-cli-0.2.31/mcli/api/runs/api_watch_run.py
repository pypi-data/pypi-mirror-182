...
from mcli.api.engine.engine import MAPIConnection, run_in_threadpool
...
def wait_for_run_status(run: Union[Run, str],
    ...
    result = run_in_threadpool(_threaded_wait_for_run_status, run=run, status=status)
...
def watch_run_status(
    ...
    for run_future in connection.subscribe(query, variables, functools.partial(_convert_run_status, run=run)):
        if not future:
            try:
