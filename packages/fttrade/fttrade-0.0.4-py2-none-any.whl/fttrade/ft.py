...
class FtServer(object):
    def __init__(self, config_filename, endtime):
        ...
        self.orderinfo_db = self.db_client["tradingAccount"]  # 下单撤单、柜台信息
        self.account_info_collection = self.orderinfo_db["account_info"]
        self.target_collection = self.orderinfo_db["target"]
        self.sell_target_collection = self.orderinfo_db['sell_target']
        self.cancel_target_collection = self.orderinfo_db['cancel_target']
        ...
        self.db_client.server_info()
        self.tradelog_db = self.db_client["tradingLog"]  # order/trade
        self.order_collection = self.tradelog_db["order"]
        self.trade_collection = self.tradelog_db["trade"]
    ...
    def get_acccount_info(self):
        for product_name in self.product_names:
            ...
            account_info = self.account_info_collection.find_one(query)
    ...
    def send_to_feishu(self, error):
        try:
            ...
            payload_message = {
                "msg_type": "text",
                "content": {
                    "text": f'[ft.py] ERROR! (exception){error}'
                }
            }
            headers = {
    ...
    def req_position(self):
        try:
            url = f'http://127.0.0.1:11356/api/get_position_by_acc?broker_id={self.broker_id}&acc_id={self.account_id}&token={self.token}'
            r = requests.get(url)
            data = r.json()
            self.logger.info(f"[req_position] (data){data}")
            #将持仓结果计入ft_position数据库
            if len(data['data']) > 0:
                ft_position_collection = self.orderinfo_db['ft_position']
                remove = ft_position_collection.delete_many({})
    ...
    def on_zidan_message(self, data):
        ...
        try:
            ...
            if msg['status'] > 5:  # 错/废单
                ...
                if sid not in self.sid_to_traded_money:
                    if target_type == 'buy':
                        self.sid_to_traded_money[sid] = amt
                    elif target_type == 'sell':
                        self.sid_to_traded_money[sid] = 0
                        self.sid_to_traded_money[sid] -= amt
                elif target_type == 'buy':
                    self.sid_to_traded_money[sid] += amt
                elif target_type == 'sell':
                ...
                save_trade_collection = self.orderinfo_db['ft_trade']
                trade_db_msg = {
                    "oid" : oid,
                ...
                res = save_trade_collection.replace_one(ft_trade_query, trade_db_msg, True)
    ...
    def update_position(self, msg, sid , oid, side, trade_amt):
        try:
            query = {'sid': sid}
            collection = self.orderinfo_db['EquityPosition']
            position = collection.find_one(query)
            if position == None and msg['trade_vol'] > 0:
                ...
                # 计算成交总价
                if sid not in self.sid_to_traded_money:
                    self.logger.error(f"[update_position]no trade_amt (sid){sid}")
                    return
                ...
                query = {'sid': sid,
                         'accountName': accountName}
                res = collection.replace_one(query, position_msg, True)
                self.logger.info(
    ...
    def update_order_filled(self, data, sid, oid):
        try:
            ...
            res = self.order_collection.replace_one(
    ...
    def login(self):
        try:
            ...
            self.broker_id = data['data']['accs'][0]['broker_name']
            self.logger.info(f"[login] logged (token){self.token}")
        except Exception as e:
            err = traceback.format_exc()
            self.send_to_feishu(err)
            ...
            while not self.is_stopped:
                for target_accountname in self.target_accountnames: 
                    ...
                    with self.sell_orderlock:
                        buy_targets = self.orderinfo_db["target"].find(buy_query)
                        ...
                        result = self.orderinfo_db["target"].delete_many(buy_query)
                        self.logger.info(f"[monitor_buy_order] delete{result.deleted_count}messages_by(query){buy_query}")
                for target_accountname in self.target_accountnames:
                    sell_query = {"accountName": target_accountname}
                    with self.sell_orderlock:
                        sell_targets = self.orderinfo_db["sell_target"].find(sell_query)
                        if sell_targets.count() == 0:
                            self.logger.warning(
                                "[monitor_sell_order] no_sell_target")
                            continue
                        for sell_target in sell_targets:
                            if sell_target['_id'] not in self.order_dbids:
                                self.order_dbids.append(sell_target['_id'])
                                self.req_sell_order_insert(sell_target)
                        result = self.orderinfo_db["sell_target"].delete_many(sell_query)
                        self.logger.info(
                            f"[monitor_sell_order] delete{result.deleted_count}messages_by(query){sell_query}")
        ...
        try:
            while not self.is_stopped:
                for target_accountname in self.target_accountnames: 
                    ...
                    with self.sell_orderlock:
                        buy_targets = self.orderinfo_db["target"].find(query)
                        if buy_targets.count() == 0:
                            self.logger.warning("[monitor_buy_order] no_buy_target")
                            continue
                        for target in buy_targets:
                            if target['_id'] not in self.order_dbids:
                                self.order_dbids.append(target['_id'])
                                self.req_buy_order_insert(target)
                        result = self.orderinfo_db["target"].delete_many(query)
                        self.logger.info(f"[monitor_buy_order] delete{result.deleted_count}messages_by(query){query}")
                time.sleep(self.scan_interval)
        except Exception as e:
                    err = traceback.format_exc()
                    self.send_to_feishu(err)
                    self.logger.error(f'[monitor_buy_order] (exception){err}')
                    exc_type, exc_value, exc_traceback = sys.exc_info()
                    traceback.print_exception(exc_type, exc_value, exc_traceback, limit=None, file=sys.stdout)
    ...
    def req_buy_order_insert(self, obj: dict):
        try:
            ...
            if data['code'] != 0:
                self.logger.error(
                    f"[on_req_insert] insert_msg_send_failed (errcode){data['code']} (err_msg){data['data']}")
                return
            db_id = obj['_id']
            ref = data['data'][0]['id']
            ...
            tgname = accountname.split('@', 1)[0]
            dict['tgname'] = tgname
            log_accountname = accountname.split('@', 1)[1]
            ...
            #ft_order用于恢复
            collection = self.orderinfo_db['ft_order']
            db_msg = {
                "oid" : oid,
                "sid" : sid,
                "mudan_id" : ref,
            ...
            while not self.is_stopped:
                for target_accountname in self.target_accountnames:
                    ...
                    with self.cancel_orderlock:
                        cancel_targets = self.orderinfo_db["cancel_target"].find(
                            query)
                        ...
                        result = self.orderinfo_db["cancel_target"].delete_many(
                            query)
                        self.logger.info(
                            f"[monitor_cancel_order] delete{result.deleted_count}messages_by(query){query}")
                time.sleep(self.scan_interval)
        except Exception as e:
            err = traceback.format_exc()
            self.send_to_feishu(err)
...
def decode_args(argv) -> str:
    endtime = ''
    try:
      opts, args = getopt.getopt(argv,"he:o:",["endtime="])
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            endtime = arg
    print (f"end_time={endtime}")
    return endtime

if __name__ == "__main__":
    endtime = decode_args(sys.argv[1:])
    configfilename = 'ft_setting.json'
    path = os.path.dirname(os.path.abspath(__file__))
    config_filename = os.path.abspath(
        os.path.join(path, './', configfilename))  # 配置文件名&路径

    td = FtServer(config_filename, endtime)
    td.start()
