...
def merge(source: dict, dest: dict, version: str, model: dict = {}, args: dict = {}):
    merge_args = {key: func(source, dest, args) for key, func in _MERGE_FROM_ARGS.items()}
    term_id = dest.get('term', {}).get('@id', dest.get('@id'))

    should_merge = source is None or all([v for _k, v in merge_args.items()])
