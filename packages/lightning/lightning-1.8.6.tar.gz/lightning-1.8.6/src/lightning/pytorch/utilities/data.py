...
from lightning.lite.utilities.data import _reinstantiate_wrapped_cls, _replace_value_in_saved_args
from lightning.lite.utilities.data import has_iterable_dataset as new_has_iterable_dataset
from lightning.lite.utilities.data import has_len as new_has_len
...
from lightning.pytorch.utilities.auto_restart import CaptureIterableDataset, CaptureMapDataset, FastForwardSampler
...
def has_len_all_ranks(
    ...
    # we are checking using lightning.lite, which doesn't know CombinedLoader
...
def has_iterable_dataset(*args: Any, **kwargs: Any) -> Any:
    rank_zero_deprecation(
        "`lightning.pytorch.utilities.data.has_iterable_dataset` has been deprecated in v1.8.0 and will be"
        " removed in v1.10.0. Please use `lightning.lite.utilities.data.has_iterable_dataset` instead."
    )
...
def has_len(*args: Any, **kwargs: Any) -> Any:
    rank_zero_deprecation(
        "`lightning.pytorch.utilities.data.has_len` has been deprecated in v1.8.0 and will be"
        " removed in v1.10.0. Please use `lightning.lite.utilities.data.has_len` instead."
    )
