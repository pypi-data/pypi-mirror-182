...
import lightning.pytorch.cli as new_cli
...
def _deprecation(cls: Type) -> None:
    rank_zero_deprecation(
        f"`lightning.pytorch.utilities.cli.{cls.__name__}` has been deprecated in v1.7 and will be removed in v1.9."
        f" Use the equivalent class in `lightning.pytorch.cli.{cls.__name__}` instead."
    )
...
def instantiate_class(*args: Any, **kwargs: Any) -> Any:
    rank_zero_deprecation(
        "`lightning.pytorch.utilities.cli.instantiate_class` has been deprecated in v1.7 and will be removed in v1.9."
        " Use the equivalent function in `lightning.pytorch.cli.instantiate_class` instead."
    )
