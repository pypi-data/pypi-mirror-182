...
def graph(e , n, u, name, method='a'):
    ...
    if method == 'a':
        ...
        n.get_v()
        plt.plot(t, n.vel,label=e.id+" NS PGV="+str(n.get_PGV()))
    elif method == 'd':
        n.get_u()
        plt.plot(t, n.dis,label=e.id+" NS PGD="+str(n.get_PGD()))
    plt.legend()
...
class OmniSpectrum():
    ...
    def any_angle_spectrum(self):
        if (not os.path.exists(os.path.join(os.path.dirname(self.path), os.path.basename(self.path).split('.')[-2] + '_a.jpg'))):
...
#             self.pool.close()
#             self.pool.join()
            self.graph(self.theta, self.r, self.sa, os.path.join(os.path.dirname(self.path), os.path.basename(self.path).split('.')[-2] + '_a.jpg'))
            self.graph(self.theta, self.r, self.sv, os.path.join(os.path.dirname(self.path), os.path.basename(self.path).split('.')[-2] + '_v.jpg'))
            self.graph(self.theta, self.r, self.su, os.path.join(os.path.dirname(self.path), os.path.basename(self.path).split('.')[-2] + '_u.jpg'))
