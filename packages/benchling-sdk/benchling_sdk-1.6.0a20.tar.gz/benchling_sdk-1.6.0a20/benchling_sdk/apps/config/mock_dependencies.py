...
def _mock_scalar_dependency(dependency: ManifestScalarConfig) -> AppConfigItem:
    if dependency.type == ScalarConfigTypes.BOOLEAN:
        ...
        return mock_bool_app_config_item([dependency.name], bool_value)
    elif dependency.type == ScalarConfigTypes.DATE:
        date_value = _mock_scalar_value_with_conversion(dependency, DateScalar)
        return mock_date_app_config_item([dependency.name], date_value)
        ...
        return mock_secure_text_app_config_item([dependency.name], secure_text_value)
    else:
        text_value = _mock_scalar_value_with_conversion(dependency, TextScalar)
        return mock_text_app_config_item([dependency.name], text_value)
...
def _mock_subdependency(
    subdependency: Union[BaseManifestConfig, FieldDefinitionsManifest], parent_config
) -> AppConfigItem:
...
def _config_path(config_item: AppConfigItem) -> List[str]:
    if isinstance(config_item, UnknownType):
        return config_item.__getattribute__("path")
