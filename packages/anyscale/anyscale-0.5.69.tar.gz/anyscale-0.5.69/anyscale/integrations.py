...
def set_wandb_project_group_env_vars():
    ...
    if os.environ.get("ANYSCALE_HA_JOB_ID"):
        ...
        wandb_group_default = slugify(ray_job_name)
...
def wandb_send_run_info_hook(run: Any) -> None:
    ...
    if os.environ.get("ANYSCALE_HA_JOB_ID"):
        ...
        ray_job_results = api_client.list_decorated_interactive_sessions_api_v2_decorated_interactive_sessions_get(
            name=ray_job_name
        ).results
        ...
        # be an issue for the events stream stack.
        retry = 5
        while not ray_job_results and retry > 0:
            time.sleep(1)
            ray_job_results = api_client.list_decorated_interactive_sessions_api_v2_decorated_interactive_sessions_get(
                name=ray_job_name
            ).results
        ...
        run.config.anyscale_logs = get_endpoint(f"/interactive-sessions/{ray_job_name}")
