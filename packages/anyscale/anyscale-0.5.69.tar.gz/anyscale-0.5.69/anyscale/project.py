...
def get_default_project(anyscale_api_client: Optional[DefaultApi] = None):
    anyscale_api_client = (
        anyscale_api_client or get_auth_api_client().anyscale_api_client
    )
    default_project = anyscale_api_client.get_default_project().result
...
def infer_project_id(
    project_id: Optional[str], anyscale_api_client: DefaultApi, log: BlockLogger
) -> str:
    if not project_id:
