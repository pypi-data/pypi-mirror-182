...
def test_get_gcp_secret():
    mock_access_secret_version = Mock(
        return_value=Mock(payload=Mock(data="my_secret_val".encode()))
    )
...
def test_wandb_setup_api_key_hook(mock_ray_version):
...
def test_set_wandb_project_group_env_vars_for_prod_job(
    project_name_env_var: str, group_name_env_var: str
):
    ...
    with patch.dict(os.environ, os_env_var_dict), patch.multiple(
        ...
        assert (
            ...
            if not project_name_env_var
            else project_name_env_var
        )
        assert (
            os.environ[WANDB_GROUP_NAME] == "mock_production_job_name"
            if not group_name_env_var
            else group_name_env_var
        )
...
def test_set_wandb_project_group_env_vars_for_workspace(
    ...
    with patch.dict(os.environ, os_env_var_dict), patch.multiple(
        ...
        set_wandb_project_group_env_vars()
        assert (
            os.environ[WANDB_PROJECT_NAME] == "mock_workspace_name"
            if not project_name_env_var
            else project_name_env_var
        )
        assert (
            os.environ.get(WANDB_GROUP_NAME) is None
            if not group_name_env_var
            else group_name_env_var
        )
...
@pytest.mark.parametrize(
    "os_env_var_dict_param",
    [
        {
            "RAY_JOB_CONFIG_JSON_ENV_VAR": json.dumps(
                {"metadata": {"job_name": "mock_ray_job_name"}}
            )
        },
        {
...
def test_set_wandb_project_group_env_vars_for_ray_job(
    ...
    with patch.dict(os.environ, os_env_var_dict), patch.multiple(
        ...
        assert (
            os.environ[WANDB_PROJECT_NAME] == expected_default_project
            if not project_name_env_var
            else project_name_env_var
        )
        assert (
            os.environ[WANDB_GROUP_NAME] == "mock_ray_job_name"
            if not group_name_env_var
            else group_name_env_var
        )
        mock_slugify.assert_called_with("mock_ray_job_name")
...
def test_wandb_send_run_info_hook(mock_wandb, os_env_var_dict):
    @dataclass
    class MockRun:
        entity: str
        project: str
    ...
    mock_api_client.list_decorated_interactive_sessions_api_v2_decorated_interactive_sessions_get = Mock(
    ...
    with patch.dict("sys.modules", wandb=mock_wandb), patch.dict(
        ...
        if not mock_wandb:
            ...
            if os.environ.get("RAY_JOB_CONFIG_JSON_ENV_VAR"):
                mock_api_client.list_decorated_interactive_sessions_api_v2_decorated_interactive_sessions_get.assert_called_once_with(
                    name=json.loads(
                        os.environ.get("RAY_JOB_CONFIG_JSON_ENV_VAR", json.dumps({}))
                    )
                    .get("metadata", {})
                    .get("job_name")
                )
