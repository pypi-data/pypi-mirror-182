...
# two1 imports
import crypto_two1
from crypto_two1.bitcoin.block import CompactBlock
from crypto_two1.bitcoin.txn import Transaction
from crypto_two1.server import message_factory
...
from crypto_two1.commands.util.uxstring import ux
...
def get_work(client):
    ...
    try:
        ...
        profile_url = "{}/{}".format(crypto_two1.TWO1_WWW_HOST, client.username)
        ...
        if e.status_code == 403 and e.data.get("error") == "TO201":
            ...
            err_string = uxstring.UxString.no_earn_allocations.format(crypto_two1.TWO1_WWW_HOST,
                                                                      client.username)
