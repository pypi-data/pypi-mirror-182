...
def field(validator: Validator) -> dataclasses.Field:
    return dataclasses.field(metadata=dict(validate=validator))
    ...
    def _validate(self, value: str) -> None:
        ...
        if time_unit is None:
            raise ValidationError(
                "An 'every' query must end with a valid time unit:"
                + " 'ms', 's', 'm', 'h'"
            )
...
class IsFileMethod(Validator):
    ...
    def load_method(self, handler: str):
        module_file, func = handler.split(":")
        ...
        try:
            ...
            if not file_found:
                raise ValidationError(f"Unable to load handler file: {module_file}")

            raise ValidationError(
                f"Unable to load handler method: {func} from file: {module_file}"
            )
