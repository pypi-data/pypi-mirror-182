...
async def send_random_problem(bot: Bot,event:Event):
    try:
        ...
        logger.info("[LC查询] 获取题目内容成功。")
        logger.debug(f"[LC查询] 题目{random_data[0]}的难度为{random_data[1]},内容略。")
    except Exception as e:
        logger.error("[LC查询] 无法连接至leetcode，请检查网络和网络代理状态。")
    ...
    try:
        ...
        logger.error("[LC查询] 题目内容（html）转图片出错。")
...
try:
    for index, time in enumerate(time_list):
        ...
        logger.info(f"[LC查询] 新建计划任务成功！！  id:{index},时间为:{time}.")
except TypeError:
    logger.error("[LC查询] 插件定时发送相关设置有误，请检查.env.*文件。")
...
async def html2pic(html:str) -> bytes:
