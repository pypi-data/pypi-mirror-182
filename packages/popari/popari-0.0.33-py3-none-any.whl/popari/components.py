...
class ParameterOptimizer():
    ...
    def estimate_Sigma_x_inv(self, Sigma_x_inv, replicate_mask, optimizer, Sigma_x_inv_bar=None, subsample_rate=None, constraint=None, n_epochs=1000, tol=1.1e-3, check_frequency=50):
        ...
        for epoch in progress_bar:
            ...
            with torch.no_grad():
                ...
                if epoch % check_frequency == 0:
                    ...
                    if dSigma_x_inv < tol * check_frequency:
                        early_stop_epoch_count += 1
                    else:
                        early_stop_epoch_count = 0

                    if early_stop_epoch_count >= 2 or epoch > epoch_best + 2 * check_frequency:
                        break
    ...
    def update_spatial_affinity(self, differentiate_spatial_affinities=True, subsample_rate=None):
        if self.spatial_affinity_mode == "shared lookup":
            for group_name, group_replicates in self.spatial_affinity_groups.items():
                ...
                Sigma_x_inv, loss = self.estimate_Sigma_x_inv(Sigma_x_inv, replicate_mask, optimizer, subsample_rate=subsample_rate, tol=spatial_affinity_tol)
                with torch.no_grad():
            ...
            for dataset_index, dataset in enumerate(self.datasets):
                ...
                Sigma_x_inv, loss = self.estimate_Sigma_x_inv(Sigma_x_inv, replicate_mask, optimizer, Sigma_x_inv_bar=spatial_affinity_bars, subsample_rate=subsample_rate, tol=spatial_affinity_tol)
                with torch.no_grad():
