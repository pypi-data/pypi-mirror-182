...
def v_read(v_path, audio=True):
    """
    videofileclip读取本地视频，解决多次运行读取后出句柄错误的问题
    :param v_path:
    :param audio:
    :return:
    """
...
def au_stack(au_list):
    """
    多音轨堆叠（[au_clip1,au_clip2]）
    :param au_list:
    :return:
    """
...
def au_concate(au_list):  # 多音轨首尾接续（[au_clip1,au_clip2]）
    return concatenate_audioclips(au_list)


def v_concate(clip_list):  # 首尾连接片段（[片段1，片段2，片段3.。。]）
    try:
...
def v_stack(v_list):  # 叠加合并视频（[videoclip,textclip,imgclip] 以第一个为背景大小
    return CompositeVideoClip(v_list)


def v_au_hebing(clip, au_clip):
    # 视加音频(clip,au_clip)  此函数暂不用，因write_videofile生成文件没有声音，网上较多网友反馈同样问题，且网上解决方法都不能解决，应是moviepy本身问题
...
def v_au_hegingff(clip, au_clip, v_path, au_path, hebing_path):
    """
    因write_videofile生成文件没有声音，网上较多网友反馈同样问题，且网上解决方法都不能解决，应是moviepy本身问题，此函数直接用ffmpeg.exe命令行合并视频音频
    :param clip:
    :param au_clip:
    :param v_path:
    :param au_path:
