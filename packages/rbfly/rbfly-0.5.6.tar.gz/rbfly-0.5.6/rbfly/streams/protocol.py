...
class RabbitMQStreamsProtocol(asyncio.Protocol):
    ...
    async def subscribe(
        ...
        task = self._loop.create_future()
        data = codec.encode_subscribe(sid, stream, offset, const.DEFAULT_CREDIT)

        queue = MessageQueue(const.DEFAULT_CREDIT, const.QUEUE_SIZE_THRESHOLD)
    ...
    async def read_stream(self, subscription_id: int) -> deque[MessageCtx]:
        ...
        # message queue threshold is breached, so check if it needs to be
        # updated before we start waiting for new messages
        if queue.needs_credit:
    ...
    def renew_credit(self, subscription_id: int, queue: MessageQueue) -> None:
        """
        Renew credit of RabbitMQ stream subscription if necessary.

        :param subscription_id: RabbitMQ stream subscription id.
        :param queue: RabbitMQ stream message queue.
        """
        self.send_credit(subscription_id, queue.default_credit)
        queue.reset_credit()
