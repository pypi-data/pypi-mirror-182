...
from delta_sharing.protocol import DeltaSharingProfile, AddFile, Table
...
class DeltaSharingReader:
    ...
    def to_expressions(self, filters, partitions):
        ...
        for conjunction in filters:
            ...
            if col not in partitions:
                ...
                expressions.append(reduce(operator.and_, [expression]))
        if len(expressions):
        ...
        if limit is None:
            dataframes = [
                DeltaSharingReader._to_pandas(
                    file, converters, expressions, columns, None
                )
            ...
            for file in response.add_files:
                dataframe = DeltaSharingReader._to_pandas(
                    file, converters, expressions, columns, left
                )
    ...
    def _to_pandas(
        ...
        if "storage.googleapis.com" in (url.netloc.lower()):
            import delta_sharing._yarl_patch

        protocol = url.scheme
        filesystem = fsspec.filesystem(protocol)

        dataset = pa.dataset.dataset(
            source=add_file.url, format="parquet", filesystem=filesystem
        )
        table = (
            dataset.to_table(filter=filters, columns=columns).slice(length=limit)
            if limit is not None
            else dataset.to_table(filter=filters, columns=columns)
        )
            ...
            if col not in dataframe.columns:
                if col in add_file.partition_values:
                    if converter is not None:
                        dataframe[col] = converter(add_file.partition_values[col])
                    else:
                        raise ValueError(
...
def load_as_pandas(url, predicates=None, columns=None, limit=None):
    profile_json, share, schema, table = delta_sharing._parse_url(url)
