...
def local_sensitivity_analysis(
    ...
    validation_folder: str = "Simulation/Output",  # Folder in which sensitivity analysis results are saved
    timeinterval: bool = False,  # Whether the change happens during a specific interval (otherwise it is applied for the complete duration of the simulation)
    initialtimestep: int = 0,  # Time step at which the modification starts to take place (included), only if timeinterval is true
    finaltimestep: int = 0,  # Time step after which the modification ends (the modification still takes place at this time step), only if timeinterval is true
):
    ...
    for variable in variables:
        if variable["change"] == "absolute" and variable["variation"] < -variable["reference"]:
            ...
            errors.append(
                ...
                f" for entity {entity_name} at time step {time_step}."
            )
...
def get_list_of_datapaths(cosmo_interface, entity_type, attribute):
    ...
    list_of_entities = sorted(
        entity.GetName()
        for entity in cosmo_interface.sim.GetModel().FindEntitiesByType(entity_type)
    )
    ...
    # For the attribute ActualDurationSchedule, it may be empty, in which case the attribute Duration should be used instead.
    if attribute == "ActualDurationSchedule":
        for entity in list_of_entities:
            try:
                ...
                if (
                    cosmo_interface.sim.FindAttribute(datapath).Get() == {}
    ...
    # For all others attributes we directly retrieve the attribute's datapath
    else:
        for entity in list_of_entities:
            list_of_datapaths.append(
                list(
    ...
    for elements in datapaths:
        ...
        if not timeinterval:
            ...
            if not isinstance(reference, dict):
                if attribute == "ActualDurationSchedule":
                    # Replace Duration by ActualDurationSchedule for a transport duration that is schedulable
                    elements = elements.replace("Duration", "ActualDurationSchedule")
                    reference = {0: int(reference)}
                else:
                    raise ValueError(
