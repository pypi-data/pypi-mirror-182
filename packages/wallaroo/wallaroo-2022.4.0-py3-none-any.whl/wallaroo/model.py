...
# A little misleading, this is actually a ModelVersion or ModelVariant, associated with the `model` table, not the `models` table.
class Model(Object):
    ...
    def configure(
        ...
        q = gql.gql(
            """
            mutation ConfigureModel($model_id: bigint!, $runtime: String, $tensor_fields: jsonb, $filter_threshold: float8, $input_schema: String, $output_schema: String) {
                insert_model_config(objects: {filter_threshold: $filter_threshold, model_id: $model_id, runtime: $runtime, tensor_fields: $tensor_fields, input_schema: $input_schema, output_schema: $output_schema}) {
                returning {
                    id
                    model_id
                    runtime
    ...
    def logs(self, limit: int = 100, valid: Optional[bool] = None) -> LogEntries:
        ...
        if valid is False:
            topic += "-failures"
        assert self.client is not None
        return self.client.get_logs(topic, limit)[0]
...
class ModelVersions(List[Model]):
    ...
    def _repr_html_(self) -> str:
        ...
        fields = ["name", "version", "file_name", "image_path", "last_update_time"]
        if not self:
