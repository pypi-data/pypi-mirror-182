...
class TestPipeline(unittest.TestCase):
    ...
    @responses.activate
    @aioresponses()
    def test_pipeline_batch_infer_from_file(self, mocked):
        mocked.post(
        ...
        with tempfile.NamedTemporaryFile(mode="w") as f:
            ...
            outputs = loop.run_until_complete(pipeline.batch_infer_from_file(f.name))
            batched_outputs = loop.run_until_complete(
                pipeline.batch_infer_from_file(f.name, batch_size=1)
            )
        ...
        for output in batched_outputs:
            self.assertEqual(
                numpy.ndarray(shape=(1, 1), buffer=numpy.array([0.1])), output.data()[0]
            )
        ...
        with tempfile.NamedTemporaryFile(mode="w") as f:
            ...
            with self.assertRaises(TypeError):
                loop.run_until_complete(pipeline.batch_infer_from_file(f.name))
