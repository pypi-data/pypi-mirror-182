...
class Deployment(Object):
    ...
    def _infer_with_arrow(
        ...
        headers = {"Content-Type": ARROW_HEADER, "Accept": ARROW_HEADER}
        res = self._make_infer_request(
            data=input_arrow,
            headers=headers,
            params=params,
    ...
    def infer(
        ...
        dataset_exclude: Optional[Union[Sequence[str], str]] = None,
        ...
        :param: timeout: Optional[Union[int, float]] infer requests will time out after
            ...
            to 15 secs.
        :param: dataset: Optional[Union[Sequence[str], str]] By default this is set to return, ["time", "out"].
            Other available options "check_failures", "metadata"
        :param: dataset_exclude: Optional[Union[Sequence[str], str]] If set, allows user to exclude parts of dataset.
        :param: dataset_separator: Optional[str] If set to ".", return dataset will be flattened.
        ...
        if os.getenv(ARROW_ENABLED, "false").lower() == "true":
            params = dict()
            params["dataset[]"] = dataset or "*"
            params["dataset.exclude[]"] = dataset_exclude or "in"
            params["dataset.separator"] = dataset_separator or "."
            if not isinstance(tensor, (pd.DataFrame, pa.Table)):
                raise TypeError(
                    f"tensor is of type {type(tensor)} but 'pandas.DataFrame' or 'pyarrow.Table' is required"
                )
    ...
    def infer_from_file(
        ...
        if os.getenv(ARROW_ENABLED, "false").lower() == "true":
            ...
            with filename.open("rb") as f:
                tensor = json.load(f)
        return self.infer(tensor, timeout, dataset, exclude, dataset_separator)
