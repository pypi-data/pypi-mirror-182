...
from log_ocr.models.drs import GbDownload, GbReadyTrack, GbUnpack, Volumes
        ...
        if ok_unpack:
            with GbOcrContext() as ctx:
                try:
                    ...
                    ctx.session.flush()
                    ctx.session.add(GbReadyTrack(target_id=new_unpack.id, activity='unpack'))
...
def unpack_service():
    ...
    with GbOcrContext() as ctx:
        ...
        target_dl: GbDownload = GbDownload()

        ok_unpack: bool = False
        cur_unpack: Unpacker

        # TODO:  Unpack them all
        for download in yy:
            try:
                ok_unpack = False
                ...
                if ok_unpack:
                    try:
                        ...
                        delete_one_track = delete(GbReadyTrack).where(GbReadyTrack.id == target_dl.id)
