...
def search_index_for_PDBs_that_contain_the_residue_pair(
    ...
    index_values = residue_pair_df.index.values
...
def filter_out_PDBs_with_unconnected_residue_pairs(PDBs_with_all_residue_pairs: Dict[nx.Graph, Dict[str, Set[Tuple[str, str]]]]) -> Dict[nx.Graph, Dict[str, List[nx.Graph]]]:
    """
    """
    ...
    # Also note that subgraph isomorphism cannot be used as it imposes an exact match of both the nodes and edges between all the nodes, which results in certain False Negatives
    ...
    # the occurence of an additional AE edge between residue A32A and A35E, and that edge causes isomorphism to fail, unlike monomorphism which correctly detects it.
    ...
    for motif_MST, map_of_candidate_PDBs_and_their_residue_pairs in PDBs_with_all_residue_pairs.items():
        add_resname_as_node_attribute(motif_MST) # Needed for subgraph monomorphism, see below.
        ...
        for PDB_ID, pairs_of_residues in map_of_candidate_PDBs_and_their_residue_pairs.items():
            ...
            for residue_mapping_dict in graph_matcher.subgraph_monomorphisms_iter():
                ...
                if motif_MST not in filtered_PDBs_with_all_residue_pairs:
                    filtered_PDBs_with_all_residue_pairs[motif_MST] = defaultdict(list)
                filtered_PDBs_with_all_residue_pairs[motif_MST][PDB_ID].append(similar_motif_graph)
...
def get_PDBs_with_similar_motifs(
    ...
    # which would probably add overhead and complexify the code.
    ...
    # Its not worth parallelizing the 2 next bits of code because they are actually quite fast, and the added overhead of parallelizing + having to transfer the results back to the main thread isn't worth it.
    ...
    # these pairs are connected and not just scattered. PDBs with connected pairs correspond to similar motifs. 
    # Connected pairs example: [('A1P', 'A4C'), ('A4C', 'A9L'), ('A9L', 'A10K')]
    # Unconnected pairs example: [('A1P', 'A4C'), ('A54C', 'A60L'), ('A19L', 'A22K')]
    filtered_PDBs_with_all_residue_pairs = filter_out_PDBs_with_unconnected_residue_pairs(PDBs_with_all_residue_pairs)
...
def search_index_for_PDBs_with_similar_motifs(
    ...
    # It would be very inefficient to search the index for every single pair of residues in the motif in order to find PDBs that have a similar 
