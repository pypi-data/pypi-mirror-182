...
from pydiscomotif.utils import (get_PDB_ID_from_file_path,
    ...
    for residue_ID, residue in residue_data.items():
        if residue.resname == 'GLY':
            ...
            sidechain_atoms_coords = np.array([
                ...
                if atom.name not in {'N', 'CA', 'C', 'O'} # All atoms except main chain
            ])
...
def get_compressed_residue_data_output_file_path(PDB_file_path: Path, index_folder_path: Path, compression: str) -> Path:
    ...
    compressed_residue_data_output_file_path = residue_data_folder_path / f'{PDB_ID}.{compression}' # Ex: /home/user_name/database_folder/pyDiscoMotif_index/residue_data_folder/1A2Y.bz2
