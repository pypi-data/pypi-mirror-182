...
class RandomBoundedResizedCropTree(Module):
    ...
    def forward(self, tree):
        ...
        minimum_scale = max(scale_minimum, crop_height / image_height, crop_width / image_width)
        scale = (minimum_scale, scale_maximum)
...
class EncodeImageTree(Module):
    ...
    def transform(self, tensor: Tensor) -> Tensor:
        ...
        tensor -= self.means[:, None, None] if self.means is not None else 127.5
        tensor /= self.scales[:, None, None] if self.scales is not None else 127.5
