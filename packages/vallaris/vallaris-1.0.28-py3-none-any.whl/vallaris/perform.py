...
def RenderRaster(parameter, *args, **kwargs):
    ...
    if option == 'internal':
        url = "http://172.30.10.126:7000/" + parameter['data']
...
def GetItem(satellite, collection):
    ...
    try :
        HOST = os.environ["VallarisServer"]
        url = HOST + "/core/api/stac/1.0/"+satellite+"/search?collections=" + str(collection)
        payload={}
        headers = {}
...
def ExportCoverage(satellite, collection , api_key):
    ...
    try :
        ...
        return "succeed store in folder : " + new_folder
