...
class MaximumCls:
	...
	class ResultData(StructBase):
		"""Response structure. Fields: \n
			...
			- Off_Power_Before: float: OFF power mean value for time period before SRS transmission
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.nrSubMeas.srs.pdynamics.maximum.read() \n
		...
		return self._core.io.query_struct(f'READ:NRSub:MEASurement<Instance>:SRS:PDYNamics:MAXimum?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:NRSub:MEASurement<Instance>:SRS:PDYNamics:MAXimum \n
		Snippet: value: ResultData = driver.nrSubMeas.srs.pdynamics.maximum.fetch() \n
		Return the current, average, minimum, maximum and standard deviation single-value results of the power dynamics
		measurement. The values described below are returned by FETCh and READ commands. CALCulate commands return limit check
		results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:NRSub:MEASurement<Instance>:SRS:PDYNamics:MAXimum?', self.__class__.ResultData())
	...
	class CalculateStruct(StructBase):
		"""Response structure. Fields: \n
			- Reliability: int: 'Reliability indicator'
			- Out_Of_Tolerance: int: Out of tolerance result, i.e. percentage of measurement intervals of the statistic count for power dynamics measurements exceeding the specified power dynamics limits.
			- Off_Power_Before: float or bool: OFF power mean value for time period before SRS transmission
			- On_Power_Rms: float or bool: ON power mean value over the SRS transmission
			- On_Power_Peak: float or bool: ON power peak value for the SRS transmission
			- Off_Power_After: float or bool: OFF power mean value for slot after SRS transmission"""
		__meta_args_list = [
			ArgStruct.scalar_int('Reliability', 'Reliability'),
			ArgStruct.scalar_int('Out_Of_Tolerance'),
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.nrSubMeas.srs.pdynamics.maximum.calculate() \n
		Return the current, average, minimum, maximum and standard deviation single-value results of the power dynamics
		measurement. The values described below are returned by FETCh and READ commands. CALCulate commands return limit check
		results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		return self._core.io.query_struct(f'CALCulate:NRSub:MEASurement<Instance>:SRS:PDYNamics:MAXimum?', self.__class__.CalculateStruct())
