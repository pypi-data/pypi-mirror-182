...
class CurrentCls:
	...
	def read(self) -> ResultData:
		...
		Snippet: value: ResultData = driver.nrSubMeas.srs.pdynamics.current.read() \n
		...
		return self._core.io.query_struct(f'READ:NRSub:MEASurement<Instance>:SRS:PDYNamics:CURRent?', self.__class__.ResultData())

	def fetch(self) -> ResultData:
		"""SCPI: FETCh:NRSub:MEASurement<Instance>:SRS:PDYNamics:CURRent \n
		Snippet: value: ResultData = driver.nrSubMeas.srs.pdynamics.current.fetch() \n
		Return the current, average, minimum, maximum and standard deviation single-value results of the power dynamics
		measurement. The values described below are returned by FETCh and READ commands. CALCulate commands return limit check
		results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for ResultData structure arguments."""
		return self._core.io.query_struct(f'FETCh:NRSub:MEASurement<Instance>:SRS:PDYNamics:CURRent?', self.__class__.ResultData())
	...
	def calculate(self) -> CalculateStruct:
		...
		Snippet: value: CalculateStruct = driver.nrSubMeas.srs.pdynamics.current.calculate() \n
		Return the current, average, minimum, maximum and standard deviation single-value results of the power dynamics
		measurement. The values described below are returned by FETCh and READ commands. CALCulate commands return limit check
		results instead, one value for each result listed below. \n
			:return: structure: for return value, see the help for CalculateStruct structure arguments."""
		return self._core.io.query_struct(f'CALCulate:NRSub:MEASurement<Instance>:SRS:PDYNamics:CURRent?', self.__class__.CalculateStruct())
