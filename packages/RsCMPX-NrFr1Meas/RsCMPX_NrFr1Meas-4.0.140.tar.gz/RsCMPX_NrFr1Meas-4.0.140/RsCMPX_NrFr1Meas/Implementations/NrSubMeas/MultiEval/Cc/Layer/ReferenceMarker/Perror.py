...
class PerrorCls:
	...
	def fetch(self, xvalue: int, trace_select: enums.TraceSelect, carrierComponent=repcap.CarrierComponent.Default, layer=repcap.Layer.Default) -> float:
		...
		Snippet: value: float = driver.nrSubMeas.multiEval.cc.layer.referenceMarker.perror.fetch(xvalue = 1, trace_select = enums.TraceSelect.AVERage, carrierComponent = repcap.CarrierComponent.Default, layer = repcap.Layer.Default) \n
		...
		response = self._core.io.query_str_suppressed(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:LAYer{layer_cmd_val}:REFMarker:PERRor? {param}'.rstrip(), suppressed)
