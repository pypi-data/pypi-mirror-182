...
class RbIndexCls:
	...
	def fetch(self, carrierComponent=repcap.CarrierComponent.Default) -> FetchStruct:
		"""SCPI: FETCh:NRSub:MEASurement<Instance>:MEValuation:LIST[:CC<Carrier>]:IEMission:MARGin:EXTReme:RBINdex \n
		Snippet: value: FetchStruct = driver.nrSubMeas.multiEval.listPy.cc.iemission.margin.extreme.rbIndex.fetch(carrierComponent = repcap.CarrierComponent.Default) \n
		Return resource block indices of the inband emission measurement for all measured list mode segments, for carrier <c>. At
		these RB indices, the CURRent, AVERage and EXTReme margins have been detected. The results are returned as triplets per
		...
		return self._core.io.query_struct(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:LIST:CC{carrierComponent_cmd_val}:IEMission:MARGin:EXTReme:RBINdex?', self.__class__.FetchStruct())
