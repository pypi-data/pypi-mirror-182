...
class NetworkCls:
	"""Network commands group definition. 11 total commands, 4 Subgroups, 5 group commands"""
	...
	def get_rfp_sharing(self) -> enums.Sharing:
		...
		Selects the RF path sharing mode for a measurement with coupling to signaling settings. \n
			:return: sharing: NSHared: not shared (independent connection) OCONnection: only connection shared (for R&S CMW only) FSHared: fully shared (for RF unit only)
		"""
	...
	def set_rfp_sharing(self, sharing: enums.Sharing) -> None:
		...
		Selects the RF path sharing mode for a measurement with coupling to signaling settings. \n
			:param sharing: NSHared: not shared (independent connection) OCONnection: only connection shared (for R&S CMW only) FSHared: fully shared (for RF unit only)
		"""
