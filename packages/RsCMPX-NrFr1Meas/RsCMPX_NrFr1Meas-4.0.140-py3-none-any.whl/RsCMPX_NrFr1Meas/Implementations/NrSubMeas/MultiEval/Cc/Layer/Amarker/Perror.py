...
class PerrorCls:
	...
	def fetch(self, xvalue: int, trace_select: enums.TraceSelect, carrierComponent=repcap.CarrierComponent.Default, layer=repcap.Layer.Default, absoluteMarker=repcap.AbsoluteMarker.Default) -> float:
		"""SCPI: FETCh:NRSub:MEASurement<Instance>:MEValuation[:CC<no>][:LAYer<layer>]:AMARker<Nr>:PERRor \n
		Snippet: value: float = driver.nrSubMeas.multiEval.cc.layer.amarker.perror.fetch(xvalue = 1, trace_select = enums.TraceSelect.AVERage, carrierComponent = repcap.CarrierComponent.Default, layer = repcap.Layer.Default, absoluteMarker = repcap.AbsoluteMarker.Default) \n
		...
		response = self._core.io.query_str_suppressed(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:LAYer{layer_cmd_val}:AMARker{absoluteMarker_cmd_val}:PERRor? {param}'.rstrip(), suppressed)
