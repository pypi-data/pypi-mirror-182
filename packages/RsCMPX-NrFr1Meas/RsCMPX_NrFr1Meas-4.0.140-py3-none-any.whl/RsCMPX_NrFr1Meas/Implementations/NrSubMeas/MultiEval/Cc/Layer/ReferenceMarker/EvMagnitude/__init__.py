...
class EvMagnitudeCls:
	...
	def fetch(self, xvalue: int, trace_select: enums.TraceSelect, carrierComponent=repcap.CarrierComponent.Default, layer=repcap.Layer.Default) -> float:
		"""SCPI: FETCh:NRSub:MEASurement<Instance>:MEValuation[:CC<no>][:LAYer<layer>]:REFMarker:EVMagnitude \n
		Snippet: value: float = driver.nrSubMeas.multiEval.cc.layer.referenceMarker.evMagnitude.fetch(xvalue = 1, trace_select = enums.TraceSelect.AVERage, carrierComponent = repcap.CarrierComponent.Default, layer = repcap.Layer.Default) \n
		Uses the reference marker on the bar graphs: EVM RMS, EVM peak, magnitude error and phase error vs OFDM symbol. \n
		...
		response = self._core.io.query_str_suppressed(f'FETCh:NRSub:MEASurement<Instance>:MEValuation:CC{carrierComponent_cmd_val}:LAYer{layer_cmd_val}:REFMarker:EVMagnitude? {param}'.rstrip(), suppressed)
