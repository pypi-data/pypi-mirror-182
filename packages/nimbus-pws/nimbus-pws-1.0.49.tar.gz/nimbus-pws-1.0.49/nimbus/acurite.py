...
class AcuRiteDriver(device.AbstractDevice):
    ...
    def __init__(self, **stn_dict):
        ...
        self.use_constants = device.to_bool(stn_dict.get('use_constants', True))
        self.ignore_bounds = device.to_bool(stn_dict.get('ignore_bounds', False))
    ...
    def genLoopPackets(self):
        ...
        while ntries < self.max_tries:
            ...
            try:
                ...
                time.sleep(delay)
            except (usb.USBError, device.WeeWxIOError) as e:
...
class Station(object):
    ...
    def open(self):
        ...
        if not dev:
            ...
            raise device.WeeWxIOError('Unable to find station on USB')

        self.handle = dev.open()
        if not self.handle:
            raise device.WeeWxIOError('Open USB device failed')
...
#        self.handle.reset()
            ...
            if Station.check_R1(raw):
                ...
                if data['rssi'] == 0:
                    ...
                    if raw[3] & 0x0f == 1:
                        ...
                        data['bucketTips'] = Station.decode_rain(raw)
                    else:
    ...
    def decode_rain(data):
        ...
        return (((data[6] & 0x3f) << 7) | (data[7] & 0x7f))
