...
class Sentinel(Service):
    ...
    def __init__(self, engine, config_dict):
        ...
        engine.bind(events.NEW_LOOP_PACKET, self.new_loop_packet)

    def new_loop_packet(self, event):
        if (time.time() - self.start_time) > 60:
            self.start_time = time.time()
            raise events.BreakLoop()
...
class GC(Service):
    ...
    def __init__(self, engine, config_dict):
        ...
        engine.bind(events.NEW_LOOP_PACKET, self.new_loop_packet)
        engine.bind(events.STARTUP, self.startup)
...
class QC(Service):
    ...
    def new_loop_packet(self, event):
        ...
        data_dict = units.to_ENGLISH(event.packet)
            ...
            if obs_type in data_dict and data_dict[obs_type] is not None:
                if not self.min_max_dict[obs_type][0] <= data_dict[obs_type] <= self.min_max_dict[obs_type][1]:
                    logging.info("QC: %s LOOP value '%s' %s outside limits (%s, %s)" %
                                 (util.timestamp_to_string(data_dict['dateTime']),
...
class TimeSync(Service):
    ...
    def __init__(self, engine, config_dict):
        ...
        engine.bind(events.STARTUP, self.startup)
        engine.bind(events.PRE_LOOP, self.pre_loop)
...
class RESTful(Service):
    """Abstract base class for RESTful nimbus services.
...
class Nimbus(RESTful):
    ...
    def __init__(self, engine, config_dict):
        ...
        self.archive_thread = rest.NimbusThread(protocol_name="Nimbus-PWS", **_dict)
...
class Wunderground(RESTful):
    ...
    def __init__(self, engine, config_dict):
        ...
        self.archive_thread = rest.AmbientThread(protocol_name="Wunderground-PWS", **_dict)
        self.archive_thread.start()
