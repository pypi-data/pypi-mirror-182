...
def superobs(filein, 
  ...
  file = open(f"averaging_template.pkl",'rb')
  ...
  for nominal_ppi_elevation in nominal_ppi_elevations:
    ...
    if model_data_dir!='undefined':
    
     if nobs>2:
      
      file = open(f"{stn}.pkl",'rb')
      model_radar = pickle.load(file) 
      file.close()

    
      make_dealisaing(this_datetime, conn_ppi_db,filein,
                                                     stn,
                                                     model_data_dir,
                                                     nobs, 
                                                     model_radar,
...
def extract_winds_from_fst(fst_file,  output_dir=None):
    """
    ...
    print (f'Extract the rotated winds from {filename}')
    init_day = filename.split("_")[0]
    fcst_hour = filename.split("_")[1]
    fcst_init = datetime.datetime.strptime(init_day, "%Y%m%d%H")
    ...
    model_list= {'model_latitudes':model_latitudes, 'model_longitudes':model_longitudes, 'model_uuwe':model_uuwe, 'model_vvsn': model_vvsn, 'model_heights': model_heights} 
    return  model_list
...
def model_velocity_radar(model_list, model_filenames, i, j, model_resolution):


    model_latitudes  = model_list[f'{os.path.basename(model_filenames)}']['model_latitudes']
    model_longitudes = model_list[f'{os.path.basename(model_filenames)}']['model_longitudes']
    model_uuwe       = model_list[f'{os.path.basename(model_filenames)}']['model_uuwe'] 
    model_vvsn       = model_list[f'{os.path.basename(model_filenames)}']['model_vvsn']

    model_heights    = model_list[f'{os.path.basename(model_filenames)}']['model_heights']
    
    delta = int(290000/model_resolution)

    model_heights = model_heights[i-delta:i+delta,j-delta:j+delta , : ]
    model_uuwe = model_uuwe[i-delta:i+delta,j-delta:j+delta,  : ]
    model_vvsn = model_vvsn[i-delta:i+delta,j-delta:j+delta , :]
    model_latitudes = model_latitudes  [i-delta:i+delta,j-delta:j+delta]
    model_longitudes = model_longitudes [i-delta:i+delta,j-delta:j+delta]
    model_radar = {'model_heights': model_heights,
                   'model_uuwe'   : model_uuwe,
                   'model_vvsn'   : model_vvsn,
                   'model_latitudes' : model_latitudes,
...
def model2ppi_interpolation(model_radar, ppi_radar):
  
    model_latitudes  = model_radar['model_latitudes']
    model_longitudes = model_radar['model_longitudes']
    model_heights    = model_radar['model_heights']
    ppi_latitudes    = ppi_radar['ppi_latitudes']
    ppi_longitudes   = ppi_radar['ppi_longitudes']
    ppi_heights      = ppi_radar['ppi_heights']  
    model_uuwe       = model_radar['model_uuwe']  
    model_vvsn       = model_radar['model_vvsn'] 
    ppi_azimuths     = ppi_radar['ppi_azimuths']
    t0 = time.time()

    nodata_mask   = np.logical_not(ppi_radar['ppi_velocity']==-9999.)
...
def model_velocity_(model_filenames, dt_model_time, stn, this_datetime, radar_lon, radar_lat, ppi_radar, model_radar, elevation):
    
    model_velocity_floor  = model2ppi_interpolation(model_radar[f'{os.path.basename(model_filenames[0])}'], ppi_radar)
    model_velocity_ceil   = model2ppi_interpolation(model_radar[f'{os.path.basename(model_filenames[1])}'], ppi_radar)
    t_floor = 0.
    t_ceil = 60
    Diff = model_velocity_ceil - model_velocity_floor
    dt =  dt_model_time[0]
    model_velocity = model_velocity_floor + Diff * ((dt - t_floor) / (t_ceil -  t_floor))
    # convert nan value to zero
    model_velocity[np.isnan(model_velocity)] = -9999.
    return model_velocity
    
def make_dealisaing(this_datetime ,conn_filememory,ffiillee, stn, model_data_dir, nobs, model_radar,  elevation) :
    
    
    
    
    model_filenames, dt_model_time = model_data_over_radar(this_datetime, 
                                                           model_data_dir,
                                                           max_range=250,
                                                           interp="linear", 
                                                           min_delta=0)
    ...
    query = (  f'select  nominal_ppi_elevation, lat, lon, antenna_altitude, nyquist  from header;') #ORDER BY RANDOM()  ;')
    ...
    if  doplot:
      out_dict = radar_tools.read_sqlite_vol(sqlite_file = ffiillee,
                                         vol_scans = [this_datetime],
                                            radars = [stn],
                                        elevations = [0.4],
                                            latlon = True,
                                       )
   # print ( this_datetime)                                 
      ppi_latitudes2  = out_dict[stn][this_datetime]['0.4']['latitudes']
      ppi_longitudes2 = out_dict[stn][this_datetime]['0.4']['longitudes']
      ppi_obsvalue    = out_dict[stn][this_datetime]['0.4']['obsvalue']

   # sys.exit(0)


    query = (  f'select range, obsvalue, CENTER_AZIMUTH from data natural join header ;') #ORDER BY RANDOM()  ;')
    avg1  = c.execute(query).fetchall()

    ranges = np.array([row[0] for row in avg1])
    obsvalues = np.array([row[1] for row in avg1])
    _azimuths =np.array([row[2] for row in avg1])
    ...
    delta   = np.sort(np.unique(ranges))
    delta_r =  delta[1]-delta[0]
    min_range = 0
    max_range = 290000
    ...
    for Range, Azimuth, obsvalue, heights, lon, lat, azimuth in zip(ranges,_azimuths, obsvalues, _heights, _longitudes, _latitudes, _azimuths):
     # print (Range, range_bounds_arr[-1],min_range, max_range+delta_r,min_range, len(range_bounds_arr ),range_bin_arr.size)
    ...
    #  print (np.min(range_bounds_arr),np.min(range_bounds_arr), ind_range,np.min(azimuth_arr),np.max(azimuth_arr), ind_azimuth, Range,Azimuth)
      ppi_velocity[ind_azimuth, ind_range]   = obsvalue
      ppi_heights[ind_azimuth, ind_range]    = heights 
      ppi_longitudes[ind_azimuth, ind_range] = lon
      ppi_latitudes[ind_azimuth, ind_range]  = lat 
      ppi_ranges[ind_range]     = Range
    ...
    model_velocity_interpolated = model_velocity_(model_filenames, 
                                                  dt_model_time, 
                                                  stn,
                                                  this_datetime, 
                                                  radar_lon, 
    ...
    # Dealias    
    from plot_ppi import plotss 

  #  print (np.array_equal(ppi_obsvalue,ppi_velocity))
    doplot=False
    if doplot:  
       plotss( model_velocity_interpolated,                'model_velocity_interpolated',
            ppi_obsvalue,               'ppi_obs',
            ppi_velocity,          'ppi_velocity',
            ppi_velocity,          'ppi_velocity',
            radar_lat,radar_lon,ppi_latitudes2,ppi_longitudes2)
# sys.exit(0)

    _dealias = Dealias(ppi_radar['ppi_ranges'], ppi_radar['ppi_azimuths'], elevation, ppi_radar['ppi_velocity'].copy(), nyquist)
    _dealias.initialize(model_velocity_interpolated , beta=0.25)
    _dealias.dealiase()
    nodata_mask   = np.where( ppi_velocity == -9999.)

    ppi_velocity_dealias = _dealias.flag
    
    index = np.where(ppi_velocity_dealias==-3)
    print ('-3',len(index[0]))
    index = np.where(ppi_velocity_dealias==2)
    print ('2',len(index[0]))
    index = np.where(ppi_velocity_dealias==1)
    print ('1',len(index[0]))
    index = np.where(ppi_velocity_dealias==0)
    print ('0',len(index[0]))

    ppi_velocity_dealias[nodata_mask] =-9999.
    ppi_velo= _dealias.velocity


    ppi_velocity_dealias[nodata_mask] = -9999.
    index1 = np.where(ppi_velocity_dealias!= ppi_velocity)
    
    nodata_mask   = np.where( ppi_velocity == -9999.)
    OmP =_dealias.dealias_vel - model_velocity_interpolated
    OmP[nodata_mask] = -9999.
    kernel_az=5
    kernel_r=5
    filter_threshold = 10
    ...
    index2 = np.where(ppi_velocity!=ppi_velocity_threshold)
    print (round(elevation,2),'nobs:', nobs, len(index1[0]) ,len(index2[0]) )
...
def compute_average(center_time,
  ...
  print (window_paralle, window_step)

  print (window_t0, window_tf )
  if window_step > 0 :
     window_t0 = window_t0 + datetime.timedelta(seconds=60*window_paralle)
     window_tf = window_t0 + datetime.timedelta(seconds=60*window_step)
  print (window_t0, window_tf)
...
# model_data_dir ='/home/dlo001/data_maestro/ppp5/operation.forecasts.reghyb/'
  ...
  def update_model_wind(model_for_dealias, center_time):
     model_wind={}
     model_data = glob.glob(f'{model_for_dealias}/{center_time.strftime("%Y%m%d%H")}*')
     model_data.sort(key = lambda item: item[-1])
     model_wind={f"{os.path.basename(model_data[0])}":extract_winds_from_fst(model_data[0])}
     model_longitudes = model_wind[f"{os.path.basename(model_data[0])}"]['model_longitudes']
     model_latitudes  = model_wind[f"{os.path.basename(model_data[0])}"]['model_latitudes']
     return model_wind, model_longitudes, model_latitudes
  if model_for_dealias!='undefined':
     model_wind, model_longitudes, model_latitudes = update_model_wind(model_for_dealias, center_time)
  import pickle
  for this_file in infile_list:
    with sqlite3.connect(this_file) as conn_loops:
      # list of radars in the file
      try:
        result_radar_lat_lon   = conn_loops.execute("select distinct ID_STN, lat, lon  from header order by 1;").fetchall()
        avail_radar_list       = [ stn[0] for stn in result_radar_lat_lon ]
        latitudes_radar_list   = [ lat[1] for lat in result_radar_lat_lon ]
        longitudes_radar_list  = [ lon[2] for lon in result_radar_lat_lon ]
        avail_radar_lat_lon    = zip(avail_radar_list,latitudes_radar_list,longitudes_radar_list)
      except:
        continue
      for this_radar,latitude,longitude  in  avail_radar_lat_lon:
        list_radar_model = {}
        ...
        if model_for_dealias!='undefined':
         ...
         try:
          i, j = index_from_latlon(model_latitudes, model_longitudes, latitude,longitude)
          model_resolution = distance_between_two_points(model_latitudes[i,j], 
                                                         model_longitudes[i,j], 
                                                         model_latitudes[i,j+1], 
                                                         model_longitudes[i,j+1])

         except:
          continue
      #  vol_scan_list.append( {'file':this_file, 'radar':this_radar, 'this_datetime': datetime_list} )
        for this_date in datetime_list: 
          #only include date in assimilation window
          if (this_date > window_t0) and (this_date <= window_tf):
            vol_scan_list.append( {'file':this_file, 'radar':this_radar, 'this_datetime':this_date} )
            if model_for_dealias!='undefined':

              model_filenames, dt_model_time = model_data_over_radar(this_date, 
                                                    model_for_dealias,
                                                    max_range=250,
                                                    interp="linear", 
                                                    min_delta=0)
              for model_file in model_filenames:
                if f"{os.path.basename(model_file)}" not in model_wind.keys():
                  model_wind[ f"{os.path.basename(model_file)}"]=extract_winds_from_fst(model_file)

                if f"{os.path.basename(model_file)}" not in list_radar_model:
                  model_radar = model_velocity_radar(model_wind, model_file, i,j,model_resolution)
                list_radar_model[f"{os.path.basename(model_file)}"] = model_radar
        if model_for_dealias!='undefined':

          name = f"{this_radar}.pkl"
          if os.path.exists(name):
             os.remove(name)
          f = open(name,"wb")
          pickle.dump(list_radar_model, f)
          # close file
          f.close()
  ...
  name ="averaging_template.pkl"
  if os.path.exists(name):
     os.remove(name)
  f = open(name,"wb")
  # write the python object (dict) to pickle file
  pickle.dump(averaging_template, f)
  # close file
  f.close()
  ...
  if timing :
    ...
    for vscan in vol_scan_list:
        superobs(vscan['file'] ,vscan['radar'],vscan['this_datetime'],
                 pathfileout, ops_run_name,
                 obs_percentage, obs_nyquist, model_for_dealias)
        break   
    tc_1 = time.time()
    ...
    if n_cpus == 1:
      ...
      with tempfile.TemporaryDirectory() as tmpdir:
       ...
       #   dask.config.set({"kubernetes.scheduler-service-wait-timeout": "120s",
       #                    "distributed.comm.timeouts.connect": "120s",
       #                    "distributed.comm.tcp": "120s",
       #                    "distributed.deploy.lost-worker-timeout": "120s" })
          joblist = [delayed(superobs)(vscan['file'], vscan['radar'], vscan['this_datetime'],
                                         pathfileout, ops_run_name, obs_percentage,
                                         obs_nyquist, model_for_dealias)
                                         for vscan in vol_scan_list]
...
def arg_call():
    ...
    parser.add_argument('--window_paralle',default=0,          type=float, help="Initial minute of the window (Master Suite Loop)" )
    parser.add_argument('--window_step'   ,default=0,             type=float, help="Minute step of the window" )
    parser.add_argument('--model_for_dealias',default='undefined', type=str, help="Model to apply dealias" )
