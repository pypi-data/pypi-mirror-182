...
def decorator_from_middleware_cls(middleware_class):
    """
    Given a middleware class (not an instance), return a view decorator. This
    lets you use middleware functionality on a per-view basis. The middleware
    is created with no params passed.
    """
    return make_middleware_decorator_cls(middleware_class)()


def make_middleware_decorator_cls(middleware_class):
    def _make_decorator(*m_args, **m_kwargs):
        def _decorator(view_func):
            middleware = middleware_class(view_func, *m_args, **m_kwargs)
            ...
            def _wrapped_view(cls, request, *args, **kwargs):
                ...
                if hasattr(middleware, 'process_view'):
                    ...
                    if result is not None:
                        return result
                try:
                    response = view_func(cls, request, *args, **kwargs)
                except Exception as e:
                    if hasattr(middleware, 'process_exception'):
                        result = middleware.process_exception(request, e)
                        if result is not None:
...
def _setup_websocket_cls(func):
    from functools import wraps
    @wraps(func)
    def new_func(cls, request, *args, **kwargs):
        response = func(cls, request, *args, **kwargs)
        if response is None and request.is_websocket():
            response =  HttpResponse()
            response.__len__ = lambda : 0
            return response
        return response
    if not WEBSOCKET_MIDDLEWARE_INSTALLED:
        decorator = decorator_from_middleware_cls(WebSocketMiddleware)
        new_func = decorator(new_func)
    return new_func


def decorator_websocket(func):
    """ CBV """
    func.accept_websocket = True
    func.require_websocket = getattr(func, 'require_websocket', False)
    func = _setup_websocket_cls(func)
    return func