...
def validate_seeq_server_version(session: Session, status: Status, allow_version_mismatch=False):
    ...
    if message:
        ...
        if not compatible_module_folder:
            ...
            if using_old_versioning_scheme:
                message += generate_pip_upgrade(f'{seeq_server_major}.{seeq_server_minor}.{seeq_server_patch}')

            else:
                message += generate_pip_upgrade(f'{seeq_server_major}.{seeq_server_minor}')
        else:
...
def generate_pip_upgrade(version, exact_match_version=False):
