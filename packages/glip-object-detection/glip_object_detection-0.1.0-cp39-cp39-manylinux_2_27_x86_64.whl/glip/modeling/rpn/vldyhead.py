...
class BertEncoderLayer(BertPreTrainedModel):
    def __init__(
        self, config, clamp_min_for_underflow=False, clamp_max_for_overflow=False
    ):
...
class VLFuse(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.DYHEAD.FUSE_CONFIG.TYPE == "MHA-S":
            ...
            # single-direction (text->image)
            self.mapping_lang = _make_mlp(
                self.lang_dim, self.joint_embedding_size, self.joint_embedding_dropout
            )
            self.joint_fusion = nn.ModuleList(
                [
                    _make_conv(self.joint_inp_dim, self.joint_out_dim, 1)
                    for _ in range(5)
                ]
            )
            ...
            self.gamma = nn.ModuleList(
                nn.Linear(self.joint_embedding_size, self.joint_inp_dim)
                for _ in range(5)
            )
    ...
    def forward(self, x):
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.TYPE == "MHA-S":
            ...
            if (
                ...
                language_features = self.shrink_lang(
                    torch.cat([l0, l1, l2, l3, l4], dim=-1)
                )
            else:
            ...
            for ii, feat in enumerate(visual_features):
                attn_feat = func_attention(
                    feat, language_feature, smooth=1, raw_feature_norm="softmax"
                )
                visu_feat.append(attn_feat)

            fused_visual_features = [
                fusion(feat) for feat, fusion in zip(visu_feat, self.joint_fusion)
            ]
            fused_language_dict_features = language_dict_features
            ...
            coord_feats = [
                _make_coord(batch_size, x.shape[2], x.shape[3]) for x in visual_features
            ]
...
class VLDyHead(torch.nn.Module):
    def __init__(self, cfg):
        ...
        for i in range(cfg.MODEL.DYHEAD.NUM_CONVS):
            if cfg.MODEL.DYHEAD.FUSE_CONFIG.EARLY_FUSE_ON:
                ...
                # self language path
                if (
                    i < cfg.MODEL.DYHEAD.NUM_CONVS - 1
                    or cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_FUSED_FEATURES_DOT_PRODUCT
                ):
                    # dyhead_tower.append(
                    #     BertEncoderLayer(
                    #     bert_cfg,
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_CONTRASTIVE_ALIGN_LOSS:
            ...
            self.contrastive_align_projection_image = nn.Conv2d(
                channels, num_anchors * contrastive_hdim, kernel_size=1
            )
            self.contrastive_align_projection_text = nn.Linear(
                channels, contrastive_hdim, bias=True
            )
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_DOT_PRODUCT_TOKEN_LOSS:
            ...
            self.bias_lang = nn.Parameter(
                torch.zeros(self.cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM),
                requires_grad=True,
            )
            self.bias0 = nn.Parameter(torch.Tensor([bias_value]), requires_grad=True)
    ...
    def forward(
        self, x, language_dict_features=None, embedding=None, swint_feature_c4=None
    ):
        logits = []
        bbox_reg = []
        centerness = []
        ...
        for l, feature in enumerate(x):
            ...
            if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_CONTRASTIVE_ALIGN_LOSS:
                ...
                proj_queries = self.contrastive_align_projection_image(
                    dyhead_tower["visual"][l]
                )
            ...
            if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.USE_DOT_PRODUCT_TOKEN_LOSS:
                ...
                dot_product_proj_queries = permute_and_flatten(
                    dot_product_proj_queries, B, -1, C, H, W
                )
...
class VLDyHeadModule(torch.nn.Module):
    ...
    def forward(
        ...
        if self.cfg.MODEL.DYHEAD.FUSE_CONFIG.ADD_LINEAR_LAYER:
            embedding = (
                self.tunable_linear.weight[: embedding.size(1), :].unsqueeze(0)
                + embedding
            )
