...
from .functions import to_decimal, get_field, escape, unescape, \
    chunkstring, to_precision_decimales
...
class CfdiPago():
    ...
    def generar_cfdi(self, configuracion, timbrado_prueba=None, pac=None):
        ...
        for rp in self.pagos:
            ...
            for documento in rp["documentos"]:
                ...
                if documento['moneda'] != rp["MonedaP"]:
                    ...
                    equivalencia_dr = tipo_cambio_dr  
        ...
        if cfdi.Version == "4.0":
            ...
            for imp_p in rp.get("impuestos_p", []):
                if imp_p["impuesto_p"] == "002":
                    ...
                    if str(imp_p['tasa0_cuota_p']) == "Exento":
                        iva_p_exento = "Exento"
                        base_iva_exento  += imp_p['base_p']
                        # impuesto_iva_exento = None
            ...
            for imp_p in rp.get("impuestos_p", []):
                if not imp_p["tasa0_cuota_p"] in IMPUESTOS_LIST:
                    ...
                    if str(imp_p["impuesto_p"]) == "002":
                        if str(imp_p['tasa0_cuota_p']) == "0.000000":
                            base = to_precision_decimales((base_iva_0 / equivalencia_dr), 6)
                            base_iva_total_0 = base
                            impuesto = to_precision_decimales((impuesto_iva_0 / equivalencia_dr), 6)
                        
                        if str(imp_p['tasa0_cuota_p']) == "0.080000":
                            base = to_precision_decimales((base_iva_8 / equivalencia_dr), 6)
                            base_iva_total_8 = base
                            impuesto = to_precision_decimales((impuesto_iva_8 / equivalencia_dr), 6)
                        
                        if str(imp_p['tasa0_cuota_p']) == "0.160000":
                            base = to_precision_decimales((base_iva_16 / equivalencia_dr), 6)
                            base_iva_total_16 = base
                            impuesto = to_precision_decimales((impuesto_iva_16 / equivalencia_dr ), 6)
                            
                        if str(imp_p['tasa0_cuota_p']) == "Exento":
                            base = to_precision_decimales((base_iva_exento / equivalencia_dr), 6)
        ...
        if iva_p_16:
            cp.TotalTrasladosBaseIVA16 = "%.2f" % (base_iva_total_16 * tc)
            cp.TotalTrasladosImpuestoIVA16 = "%.2f" % (impuesto_iva_16 * tc)

        if iva_p_8:
            cp.TotalTrasladosBaseIVA8 = "%.2f" % (base_iva_total_8 * tc)
            cp.TotalTrasladosImpuestoIVA8 = "%.2f" % (impuesto_iva_8 * tc)
        
        if iva_p_0:
            cp.TotalTrasladosBaseIVA0 = "%.2f" % (base_iva_total_0 * tc)
            cp.TotalTrasladosImpuestoIVA0 = "%.2f" % (impuesto_iva_0 * tc)

        if iva_p_exento:
            cp.TotalTrasladosBaseIVAExento = "%.2f" % (base_iva_total_excento * tc)
