...
class QuoteApiBase(utils.QujamApiObject):
    ...
    def _quote_register(self, req, sync=False, timeout=None):
        ...
        return self._sync_return(structs.ToProto(req), sync, timeout, EnumProto.KQuoteMsgID_QuoteRegister,
                                 EnumProto.KQuoteMsgID_RegisterAck)

    def _quote_unregister(self, req, sync=False, timeout=None):
        self._verify_logined()
        return self._sync_return(structs.ToProto(req), sync, timeout, EnumProto.KQuoteMsgID_UnRegister,
                                 EnumProto.KQuoteMsgID_UnRegisterAck)
    ...
    @_func_switch("quote_spi", EnumProto.KQuoteMsgID_RegisterAck)
    def _case_subscribe(self, unitedmsg):
        msg = MsgProto.Quote_RegisterAck()
        if unitedmsg.msg_body.Unpack(msg):
            cbmsg = structs.ToPyobj(msg)
            self._spi.handle_reply(cbmsg, "onSubscribe", EnumProto.KQuoteMsgID_RegisterAck)

    @_func_switch("quote_spi", EnumProto.KQuoteMsgID_UnRegisterAck)
    def _case_unsubscribe(self, unitedmsg):
        msg = MsgProto.Quote_RegisterAck()
        if unitedmsg.msg_body.Unpack(msg):
            cbmsg = structs.ToPyobj(msg)
            self._spi.handle_reply(cbmsg, "onUnsubscribe", EnumProto.KQuoteMsgID_UnRegisterAck)
