...
class DiegoOperator(PythonOperator):
    ...
    def send_file_path_to_diego(self, file_path):
        ...
        print(f"Diego response {response}")

        return response

    def __upload_df_to_s3_temp(self, df: pd.DataFrame, path, pandas_kwargs=None):
        if pandas_kwargs is None:
            pandas_kwargs = dict()
    ...
    def __upload_pandas_df_to_diego(self, df, pandas_kwargs=None):
        if pandas_kwargs is None:
            pandas_kwargs = dict()
        rows = df.shape[0]
        temp_file_path = f"s3://cortex-data-lakehouse-metastore-development/temp-files-diego/{self.task_id}/{self.cube_id}/{str(hash(datetime.utcnow().isoformat()))}"

        if rows <= self.diego_chunksize:
            self.__upload_df_to_s3_temp(df=df, path=temp_file_path, pandas_kwargs=pandas_kwargs)
        else:
            ...
            for i, df_element in enumerate(list_df):
                self.__upload_df_to_s3_temp(df=df_element, path=temp_file_path, pandas_kwargs=pandas_kwargs)
                time.sleep(self.sleep_between_chunks)

        return self.send_file_path_to_diego(temp_file_path)

    def upload_df_to_diego(self, df, pandas_kwargs=None):
        if pandas_kwargs is None:
            pandas_kwargs = dict()
        if isinstance(df, pd.DataFrame):
            return self.__upload_pandas_df_to_diego(df=df, pandas_kwargs=pandas_kwargs)
        else:
    ...
    def __execute_df(self, df):
        ...
        return self.upload_df_to_diego(df, pandas_kwargs=self.pandas_kwargs)
    ...
    def execute(self, context):
        ...
        if isinstance(df, collections.Iterator):
            for df_part in df:
                self.__execute_df(df=df_part)
        else:
            self.__execute_df(df=df)
        if not self.ignore_empty_dataframe:
