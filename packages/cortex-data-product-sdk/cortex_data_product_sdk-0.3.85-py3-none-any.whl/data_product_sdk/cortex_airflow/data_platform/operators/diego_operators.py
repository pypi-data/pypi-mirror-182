...
class DiegoOperator(PythonOperator):
    ...
    def __upload_df_to_s3_temp(self, df: pd.DataFrame, path, part_number, pandas_kwargs=None):
        if pandas_kwargs is None:
            pandas_kwargs = dict()
        df.to_parquet(path=f"path/{part_number}.parquet")
    ...
    def __upload_pandas_df_to_diego(self, df, path, part_number, pandas_kwargs=None):
        ...
        if rows <= self.diego_chunksize:
            self.__upload_df_to_s3_temp(df=df, path=path, pandas_kwargs=pandas_kwargs)
        else:
    ...
    def execute(self, context):
        ...
        if isinstance(df, collections.Iterator):
            for part_number, df_part in enumerate(df):
                self.__execute_df(df=df_part, part_number=part_number, path=temp_file_path)
        else:
            self.__execute_df(df=df, part_number=1, path=temp_file_path)

        self.send_file_path_to_diego(temp_file_path)
