...
class Spider(object):
        ...
        while self.status:
            try:
                ...
                if pres:
                    ...
                    await self.rabbitmq.publish(data=task_info, routing_key=task_queue)
                    await message.ack()
            except Exception as e:
                logger.error(e)
    ...
    async def kafka_task(self):
        task_queue = self.config.kafka().get('task_queue')
        if not self.kafka:
            logger.error('Need database.kafka.Kafka class object')
            return
        while self.status:
            try:
                await self.kafka.consumer(call_back=self.process, topic=task_queue)
            except Exception as e:
                logger.error(e)
