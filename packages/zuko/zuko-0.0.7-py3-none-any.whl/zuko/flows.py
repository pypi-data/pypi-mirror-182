    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.bases.append(
            Unconditional(
                DiagNormal,
                torch.zeros(channels, *space),
                torch.ones(channels, *space),
                ndims=spatial + 1,
                buffer=True,
            )
        )

    def forward(self, y: List[Tensor] = None) -> NormalizingFlow:
        r"""
        Arguments:
            y: A list of contexts :math:`y`. There should be one element :math:`y_i`
        ...
        # Base
        base = self.bases[-1](None)
        dim = -len(base.event_shape)

        batch_shapes = [y_i.shape[:dim] for y_i in y if y_i is not None]
