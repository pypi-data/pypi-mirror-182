...
def process_numeric_data(
    ...
    series = series.round(numeric_precision).astype(str)
    is_transform = True
...
def process_datetime_data(
    ...
    # pipeline.
    return process_numeric_data(
        (series.view(int) / 1e9).astype(int),
        max_len=max_len,
        numeric_precision=0,
...
def tokenize_numeric_col(series, nparts=2, col_zfill=2):
...
def process_data(
    ...
    for c in numeric_cols:
        ...
        series, transform_data = process_numeric_data(
            ...
            transform_data=col_transform_data.get(c),
        )
    ...
    for c in datetime_cols:
        ...
        series, transform_data = process_datetime_data(
            df[c], transform_data=col_transform_data.get(c)
        )
        col_transform_data[c] = transform_data
        series.name = col_name
        processed_series.append(series)
