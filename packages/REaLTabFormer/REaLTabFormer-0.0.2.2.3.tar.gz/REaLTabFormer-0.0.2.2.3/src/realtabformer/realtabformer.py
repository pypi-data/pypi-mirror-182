...
class REaLTabFormer:
    def __init__(
        ...
        **training_args_kwargs,
        ...
        Args:
            ...
            tabular_config: GPT2Config instance to customize the GPT2 model for tabular data generation.
            relational_config: EncoderDecoderConfig instance that defines the encoder and decoder configs for the
             encoder-decoder model used for the relational data generation. See link for example:
            ...
            batch_size: Batch size used for training. Must be adjusted based on the available compute resource.
             TrainingArguments is set to use `gradient_accumulation_steps=4` which will have an effective
             batch_size of 32 for the default value.
            train_size: Fraction of the data that will be passed to the `.fit` method that will be used
             for training. The remaining will be used as validation data.
            output_max_length: Truncation length for the number of output token ids in the relational model.
             This limit applies to the processed data and not the raw number of variables. This is not used in
             the tabular data model.
            early_stopping_patience: Number of evaluation rounds without improvement before stopping the training.
            ...
            mask_rate: The rate of tokens in the transformed observation that will be replaced with the [RMASK] token
             for regularization during training.
            training_args_kwargs: Keyword arguments for the `TrainingArguments` used in training the model.
             Arguments such as `output_dir`, `num_train_epochs`, `per_device_train_batch_size`, `per_device_eval_batch_size`
             if passed will be replaced by `checkpoints_dir`, `epochs`, `batch_size arguments`.
             The comprehensive set of options can be found in
    ...
    def _invalid_model_type(self, model_type):
        raise ValueError(
            f"Model type: {model_type} is not valid. REaLTabFormer only supports `tabular` and `relational` values."
        )

    def _init_tabular(self, tabular_config):
        if tabular_config is not None:
            warnings.warn(
                "The `bos_token_id`, `eos_token_id`, and `vocab_size` attributes will be replaced when the `.fit` method is run."
            )
        else:
    ...
    def _init_relational(self, relational_config):
        if relational_config is not None:
            warnings.warn(
                "The `bos_token_id`, `eos_token_id`, and `vocab_size` attributes for the encoder and decoder will be replaced when the `.fit` method is run."
            )
        else:
            # Default is 12, use 6 for distill-gpt2 as default
    ...
    def fit(
        ...
        n_critic_stop: int = 2,
        gen_rounds: int = 3,
        sensitivity_max_col_nums: int = 50,
        use_ks: bool = False,
        ...
        Args:
            ...
            in_df: Pandas DataFrame containing observations related to `df`, and from which the model
             will generate data. This data goes to the encoder for the relational model.
            join_on: Column name that links the `df` and the `in_df` tables.
            resume_from_checkpoint: If True, resumes training from the latest checkpoint in the checkpoints_dir.
             If path, resumes the training from the given checkpoint.
            ...
            critic_model: A classifier that will be used for the discriminator assessment.
             If no model is specified, a RandomForestClassifier(n_estimators=100, min_samples_leaf=10) will be used.
        ...
        if self.model_type == ModelType.tabular:
            ...
            # else:
            #     trainer = self._train_with_discriminator(
            #         df,
            #         device,
            #         n_critic=n_critic,
    ...
    def _train_with_sensitivity(
        ...
        if isinstance(qt_max, str):
            ...
            warnings.warn(
                f'The qt_max ({qt_max}) set is lower than the duplicate rate ({dup_rate}) in the data. This will not give a reliable early stopping condition. Consider using qt_max="compute" argument.'
            )
        ...
        if dup_rate == 0:
            ...
            warnings.warn(
                f"Duplicate rate ({dup_rate}) in the data is zero. The `qt_interval` will be set to qt_interval_unique={qt_interval_unique}."
            )
        ...
        sensitivity_threshold = SyntheticDataBench.compute_sensitivity_threshold(
            train_data=df,
            num_bootstrap=num_bootstrap,
        ...
        sensitivity_scores = []
        bdm_path = self.checkpoints_dir / "best-disc-model"
        not_bdm_path = self.checkpoints_dir / "not-best-disc-model"
        last_epoch_path = self.checkpoints_dir / "last-epoch-model"
        ...
        for p_epoch in range(last_epoch, self.epochs, n_critic):
                ...
                for g_idx in range(gen_rounds):
                    val_sensitivities.append(
                        SyntheticDataBench.compute_sensitivity_metric(
                            original=df.loc[df.index.difference(hold_df.index)].sample(frac=1 if full_sensitivity else frac),
            ...
            if val_sensitivity < sensitivity_threshold:
                ...
                trainer.state.save_to_json((not_bdm_path / "trainer_state.json").as_posix())
                not_best_val_sensitivity = val_sensitivity - sensitivity_threshold

            print(
                f"Critic round: {p_epoch + n_critic}, sensitivity_threshold: {sensitivity_threshold}, val_sensitivity: {val_sensitivity}, val_sensitivities: {val_sensitivities}"
            )
        ...
        if (bdm_path / "pytorch_model.bin").exists():
            self.model.from_pretrained(bdm_path.as_posix())
        else:
            warnings.warn(
                "No best model was saved. Loading the closest model to the sensitivity_threshold."
            )
            self.model.from_pretrained(not_bdm_path.as_posix())

        return trainer

    def _train_with_discriminator(
        self,
        df: pd.DataFrame,
        device: str = "cuda",
        n_critic: int = 10,
        ...
        # Create a hold out sample for the discriminator model
        hold_df = df.sample(frac=0.2, random_state=self.random_state)
        df = df.loc[df.index.difference(hold_df.index)]

        # Start training
        logging.info("Start training...")
        if critic_model is None:
            critic_model = RandomForestClassifier(
                n_estimators=100,
                min_samples_leaf=10,
        ...
        for p_epoch in range(last_epoch, self.epochs, n_critic):
            ...
            try:
                # Generate samples
                gen_df = self.sample(n_samples=gen_total, device=device)
            except SampleEmptyLimitError:
                # Continue training if the model is still not
                # able to generate stable observations.
                continue

            predictions = SyntheticDataBench.compute_data_copying_predictions(
                original=df.sample(n=len(hold_df), replace=False),
                synthetic=gen_df,
                test=hold_df,
                model=critic_model,
                random_state=self.random_state,
            )
            acc_score = predictions["y_preds"].mean()

            # predictions = SyntheticDataBench.compute_discriminator_predictions(
            #     original=df.sample(n=len(hold_df), replace=False),
            #     synthetic=gen_df,
            #     test=hold_df,
            #     model=critic_model,
            #     random_state=self.random_state,
            # )
            # acc_score = accuracy_score(predictions["y_test"], predictions["y_preds"])

            disc_score = abs(0.5 - acc_score)
            print(
                f"Critic round: {p_epoch + n_critic}, best_disc_score: {best_disc_score}, accuracy: {acc_score}, disc_score: {disc_score}"
            )
            ...
            if len(disc_scores) > n_critic_stop:
                n_no_improve = 0
                prev = 0
                for d_score in disc_scores[-n_critic_stop:]:
                    # We count no improvement if the score is not
                    # better than the best, and that the score is not
                    # better than the previous score.
                    if (d_score > best_disc_score) and (d_score > prev):
                        n_no_improve += 1
    ...
    def sample(
        ...
        **generate_kwargs,
        ...
        Args:
            ...
            gen_batch: Controls the batch size of the data generation process. This parameter should be adjusted based
             on the compute resources.
            device: The device used by the generator. Use torch devices, e.g., `cpu`, `cuda`, `mps` (experimental)
            seed_input: A dictionary of `col_name:values` for the seed data. Only `col_names` that are actually in the
             first sequence of the training input will be used.
            constrain_tokens_gen: Set whether we impose a constraint at each step of the generation limited only to valid
             tokens for the column.
            validator: An instance of `ObservationValidator` for validating the generated samples.
             The validators are applied to observations only, and don't support inter-observation validation.
             See `ObservationValidator` docs on how to set up a validator.
            continuous_invalid_limit: The sampling will raise an exception if `continuous_empty_limit` empty sample batches
             have been produced continuously. This will prevent an infinite loop if the quality of the data generated is
             not good and always produces invalid observations.
            suppress_tokens: (from docs) A list of tokens that will be supressed at generation.
             ...
             This is a useful feature for imputing missing values.
            forced_decoder_ids: (from docs) A list of pairs of integers which indicates a mapping from generation indices
             to token indices that will be forced before sampling. For example, [[1, 123]] means the second generated token
             will always be a token of index 123.
             This is a useful feature for constraining the model to generate only specific stratification variables in
             surveys, e.g., GEO1, URBAN/RURAL variables.
            generate_kwargs: Additional keywords arguments that will be supplied to `.generate` method.
             For a comprehensive list of arguments, see:
    ...
    def predict(
        ...
        **generate_kwargs,
        ...
        Args:
            ...
            target_pos_val: The positive value in the target column to use for binary classification.
                            This is produces a one-to-many prediction relative to `target_pos_val` for
                            targets that are multi-categorical.
            ...
            fillunk: If True, the function will fill any missing values in the data before making predictions.
             Fill unknown tokens with the mode of the batch in the given step.
    ...
    def save(self, path: Union[str, Path], allow_overwrite: Optional[bool] = False):
        ...
        config_file = path / "rtf_config.json"
        model_file = path / "rtf_model.pt"
        ...
        if path.is_dir() and not allow_overwrite:
            if config_file.exists() or model_file.exists():
                ...
                warnings.warn(
                    ...
                    " This will raise an error next time when the model artifacts exist on this directory"
                )
