    ...
    def compute_sensitivity_threshold(
        ...
        for _ in tqdm(range(num_bootstrap), desc="Bootstrap round"):
            if full_sensitivity:
                ...
                _, source = train_test_split(train_data, test_size=3 * frac)
                original = source.sample(frac=frac, replace=False)
                synthetic = source.loc[source.index.difference(original.index)].sample(frac=frac, replace=False)
                test = source.loc[
                    source.index.difference(synthetic.index).diffrence(original.index)]
