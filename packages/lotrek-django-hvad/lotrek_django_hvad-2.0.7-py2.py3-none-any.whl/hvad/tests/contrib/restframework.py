...
from hvad.test_utils.project.app.models import Normal, Related, TranslatedMany, Unique
...
class CombinedUniqueSerializer(TranslationsMixin, TranslatableModelSerializer):
    class Meta:
        model = Unique
        exclude = []
...
class TranslatableModelSerializerTests(HvadTestCase, NormalFixture):
    ...
    def test_serialize_normal(self):
        ...
        self.assertCountEqual(
            data, ['id', 'shared_field', 'translated_field', 'language_code'])
...
class TranslationsMixinTests(HvadTestCase, NormalFixture):
    ...
    def test_translations_mixin_fields(self):
        ...
        self.assertIsInstance(
            serializer.fields['translations'], TranslationListSerializer)
        ...
        self.assertIsInstance(
            serializer.fields['translations'], TranslationListSerializer)
        self.assertIsInstance(
            serializer.fields['translations'].child, CustomTranslationSerializer)
        self.assertCountEqual(
            serializer.fields['translations'].child.fields, ['cheat', 'custom'])

    # ---------------------------------------------------------------------

    def test_serialize(self):
        'Serialize nested translations as a language => fields dict'
        obj = Normal.objects.prefetch_related(
            'translations').get(pk=self.normal_id[1])
        ...
        for language in self.translations:
            ...
            self.assertEqual(
                translation['translated_field'], NORMAL[1].translated_field[language])
    ...
    def test_serialize_custom(self):
        ...
        for language in self.translations:
            ...
            self.assertEqual(translation['custom'],
                             NORMAL[1].translated_field[language])
    ...
    def test_serialize_related(self):
        ...
        for language in ['ja']:
            ...
            self.assertCountEqual(
                translation, ['translated', 'translated_to_translated'])
    ...
    def test_invalid(self):
        ...
        self.assertTrue(
            serializer.errors['translations']['en']['translated_field'])
    ...
    def test_create(self):
        ...
        # insert shared, insert "en", insert "sr"
    ...
    def test_update(self):
        ...
        obj = Normal.objects.untranslated().prefetch_related(
            'translations').get(pk=self.normal_id[1])
        data = {
            'shared_field': 'shared',
            'translations': {
        ...
        # update shared, update "en", insert "sr", delete others
    ...
    def test_update_partial(self):
        ...
        with self.assertNumQueries(1):  # update shared
            obj = serializer.save()
        self.assertEqual(obj.pk, self.normal_id[1])
        qs = Normal.objects.language('all').filter(
            pk=self.normal_id[1], shared_field='shared')
        self.assertCountEqual(
            [item.language_code for item in qs], self.translations)
    ...
    def test_update_translated_many(self):
        ...
        """
        obj = TranslatedMany.objects.language('en').create(
            name='shared', translated_field='English')
        # reload with normal caching conditions so query counting works
        obj = TranslatedMany.objects.untranslated(
        ).prefetch_related('translations').get(pk=obj.pk)
        data = {
            'name': 'shared-updated',
            'translations': {
...
class CombinedTests(HvadTestCase, NormalFixture):
    ...
    def test_serialize(self):
        ...
        obj = Normal.objects.language('ja').prefetch_related(
            'translations').get(pk=self.normal_id[1])

        serializer = CombinedSerializer(instance=obj)
        data = serializer.data
        self.assertCountEqual(
            data, ['id', 'shared_field', 'translated_field', 'language_code', 'translations'])
        self.assertEqual(data['id'], self.normal_id[1])
        self.assertEqual(data['shared_field'], NORMAL[1].shared_field)
        self.assertEqual(data['translated_field'],
                         NORMAL[1].translated_field['ja'])
    ...
    def test_update_unique_translatable(self):
        obj = Unique.objects.language('en').create(
            translated_field='unique en', shared_field='shared all', unique_by_lang='unique by lang en')
        obj.translate('ja')
        obj.translated_field = 'unique ja'
        obj.save()

        data = {
            'shared_field': 'shared all',
            'language_code': 'ignored',
            'translations': {
                'en': {'translated_field': 'unique en', 'unique_by_lang': 'unique by lang'},
                'ja': {'translated_field': 'new ja', 'unique_by_lang': 'unique by lang'},
            },
        }

        serializer = CombinedUniqueSerializer(instance=obj, data=data)
        self.assertTrue(serializer.is_valid())
        obj = serializer.save()
        qs = Unique.objects.language('all').filter(pk=obj.pk)
        self.assertCountEqual([(item.language_code, item.translated_field) for item in qs],
                              [('en', 'unique en'),
                               ('ja', 'new ja')])
        ...
        data = {
            ...
            'language_code': 'ignored',
            'translations': {
                'en': {'translated_field': 'new ja', 'unique_by_lang': 'unique by lang'},
                'ja': {'translated_field': 'new ja', 'unique_by_lang': 'unique by lang'},  # test single translated uniqueness
            },
        }
        ...
        data = {
            ...
            'language_code': 'ignored',
            'translations': {
                'en': {'translated_field': 'unique new en', 'unique_by_lang': 'unique by lang'}, # test uniqueness by lang
            },
        }

        serializer = CombinedUniqueSerializer(data=data)
        self.assertFalse(serializer.is_valid())
