...
class Operator:
    ...
    def draw_auto_exit_with_pic(self, course, picture):
        if course == "OUT":
            self.currentPage.orupActExit(carnum='deff', call_method="manual",
    ...
    def draw_auto_exit_pic(self, picture):
        ...
        # full = (0,0,2592,1944)
        im_r = img.crop((int(2592 / 2 - 600), int(1944 / 2 - 400),
                         int(2592 / 2 + 600), int(1944 / 2 + 200)))
    ...
    def draw_auto_entrance_pic(self, picture):
        ...
        im_r = img.crop((1300, 420, 1850, 1150))
        img_byte_arr = io.BytesIO()
        im_r.save(img_byte_arr, format='PNG')
        img_byte_arr = img_byte_arr.getvalue()
        self.photoimg = ImageTk.PhotoImage(
