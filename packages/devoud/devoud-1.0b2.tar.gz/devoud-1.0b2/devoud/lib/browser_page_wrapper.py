...
class BrowserPageWrapper(QWidget):
    ...
    def load(self, url: str, allow_search=False):
        ...
        elif (self.view is None or not self.view.embedded) and self.return_type_by_ulr(url) is EmbeddedPage:
            self.create_embedded_view(url)
        elif self.view.embedded and self.return_type_by_ulr(url) is EmbeddedPage:
            self.create_embedded_view(url)
