...
class DeviceTimeCoord(xcffib.Struct):
    def __init__(self, unpacker):
        ...
        self.axisvalues = xcffib.List(unpacker, "i", num_axes)
        self.bufsize = unpacker.offset - base
    def pack(self):
        buf = io.BytesIO()
...
class GetDeviceMotionEventsReply(xcffib.Reply):
    def __init__(self, unpacker):
        ...
        self.events = xcffib.List(unpacker, DeviceTimeCoord, self.num_events)
        self.bufsize = unpacker.offset - base
...
class FeedbackState(xcffib.Struct):
    ...
    def pack(self):
        ...
        if class_id & FeedbackClass.Keyboard:
            ...
            buf.write(xcffib.pack_list(self.auto_repeats, "B"))
        if class_id & FeedbackClass.Pointer:
        ...
        if class_id & FeedbackClass.String:
            ...
            buf.write(xcffib.pack_list(self.keysyms, "I"))
        if class_id & FeedbackClass.Integer:
        ...
        if class_id & FeedbackClass.Led:
            self.led_mask = self.data.pop(0)
            self.led_values = self.data.pop(0)
            buf.write(struct.pack("=II", self.led_mask, self.led_values))
        if class_id & FeedbackClass.Bell:
            self.percent = self.data.pop(0)
            self.pitch = self.data.pop(0)
            self.duration = self.data.pop(0)
...
class DeviceState(xcffib.Struct):
    ...
    def pack(self):
        ...
        if control_id & DeviceControl.resolution:
            self.num_valuators = self.data.pop(0)
            ...
            buf.write(xcffib.pack_list(self.resolution_min, "I"))
            buf.write(xcffib.pack_list(self.resolution_max, "I"))
        if control_id & DeviceControl.abs_calib:
...
class DeviceClass(xcffib.Struct):
    ...
    def pack(self):
        ...
        if type & DeviceClassType.Key:
            self.num_keys = self.data.pop(0)
            self.keys = self.data.pop(0)
            buf.write(struct.pack("=H", self.num_keys))
            buf.write(xcffib.pack_list(self.keys, "I"))
        if type & DeviceClassType.Button:
            self.num_buttons = self.data.pop(0)
            ...
            buf.write(xcffib.pack_list(self.state, "I"))
            buf.write(xcffib.pack_list(self.labels, "I"))
        if type & DeviceClassType.Valuator:
            ...
            buf.write(struct.pack("=HIIB3x", self.number, self.label, self.resolution, self.mode))
            buf.write(self.min.pack() if hasattr(self.min, "pack") else FP3232.synthetic(*self.min).pack())
            buf.write(self.max.pack() if hasattr(self.max, "pack") else FP3232.synthetic(*self.max).pack())
            buf.write(self.value.pack() if hasattr(self.value, "pack") else FP3232.synthetic(*self.value).pack())
        if type & DeviceClassType.Scroll:
            self.number = self.data.pop(0)
...
class GesturePinchBeginEvent(xcffib.Event):
    ...
    def pack(self):
        ...
        buf.write(struct.pack("=x2xHIIIIIiiiiiiiiiiH2xI", self.deviceid, self.time, self.detail, self.root, self.event, self.child, self.root_x, self.root_y, self.event_x, self.event_y, self.delta_x, self.delta_y, self.delta_unaccel_x, self.delta_unaccel_y, self.scale, self.delta_angle, self.sourceid, self.flags))
        buf.write(self.mods.pack() if hasattr(self.mods, "pack") else ModifierInfo.synthetic(*self.mods).pack())
        buf.write(self.group.pack() if hasattr(self.group, "pack") else GroupInfo.synthetic(*self.group).pack())
        buf_len = len(buf.getvalue())
        if buf_len < 32:
...
class GestureSwipeBeginEvent(xcffib.Event):
    ...
    def pack(self):
        ...
        buf.write(struct.pack("=x2xHIIIIIiiiiiiiiH2xI", self.deviceid, self.time, self.detail, self.root, self.event, self.child, self.root_x, self.root_y, self.event_x, self.event_y, self.delta_x, self.delta_y, self.delta_unaccel_x, self.delta_unaccel_y, self.sourceid, self.flags))
        buf.write(self.mods.pack() if hasattr(self.mods, "pack") else ModifierInfo.synthetic(*self.mods).pack())
        buf.write(self.group.pack() if hasattr(self.group, "pack") else GroupInfo.synthetic(*self.group).pack())
        buf_len = len(buf.getvalue())
        if buf_len < 32:
...
class xinputExtension(xcffib.Extension):
    ...
    def GetDeviceProperty(self, property, type, offset, len, device_id, delete, is_checked=False):
        buf = io.BytesIO()
    ...
    def XIGetProperty(self, deviceid, delete, property, type, offset, len, is_checked=False):
        buf = io.BytesIO()
