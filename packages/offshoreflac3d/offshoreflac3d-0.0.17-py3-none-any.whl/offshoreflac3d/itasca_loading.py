...
def state_load_factor(case_id,load_s,load_w,load_i):
...
def apply_loading(load):
    for i in range(0,6)
        itasca_loading.loading_sw('loading_ref', load[i], i)


def apply_combined_loadings(combine_load_s,combine_load_w,combine_load_i,pos_load_i,load_patern):
    if combine_load_s == 'YES':
        ...
        if combine_load_w == 'YES':
            ...
            if combine_load_i == 'YES':
                for i in range(0,6)
                    itasca_loading.loading_ice(Load_i[i],pos_load_i,load_patern)
