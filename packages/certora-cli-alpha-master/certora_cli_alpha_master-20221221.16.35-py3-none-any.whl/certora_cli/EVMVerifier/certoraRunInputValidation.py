...
from Shared.certoraUtils import Mode, get_closest_strings, DEFAULT_SOLC, CertoraUserInputError
...
def check_deployment_args(context: CertoraContext) -> None:
    """
    Checks that the user didn't choose both --staging and --cloud
    @param context: A namespace including all CLI arguments provided
    @raise CertoraUserInputError if both --staging and --cloud options are present in context
    """
    if context.staging:
        if context.cloud:
            raise CertoraUserInputError("cannot use both --staging and --cloud")
        context.env = DEFAULT_STAGING_ENV
    else:
        context.env = DEFAULT_CLOUD_ENV
...
def check_args_post_argparse(context: CertoraContext) -> None:
    ...
    check_arg_and_setting_consistency(context)
    check_rule(context)
    certora_root_dir = as_posix(get_certora_root_directory())
    default_jar_path = Path(certora_root_dir) / "emv.jar"
    if context.jar is not None or \
            (default_jar_path.is_file() and context.staging is None and context.cloud is None):
        context.local = True
    else:
        context.local = False
        check_deployment_args(context)
