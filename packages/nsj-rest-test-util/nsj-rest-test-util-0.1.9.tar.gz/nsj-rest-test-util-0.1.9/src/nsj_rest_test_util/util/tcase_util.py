...
class TCaseUtil:
    def __init__(self, current_file, app_name, mope_code, endpoint, port = 5000):
        """
        Args:
        ...
        self.endpoint = f"{self.url_base}:{port}/{(app_name + '/') if app_name else ''}{(mope_code + '/') if mope_code else ''}{endpoint}"
    ...
    def common_request_test(self, json_entrada, json_entrada_nome, http_method, param_mode=ParamMode.QUERY, has_authorization: bool = False):
        ...
        if has_authorization:
            ...
            token_retorno = RequestsUtil.post(
                self.token_url, data=token_body, headers={'content-type':'application/x-www-form-urlencoded'}, has_authorization=True)
            headers['Authorization'] = token_retorno.json()['access_token']

        if HTTPMethod.POST == http_method:
            retorno = RequestsUtil.post(self.endpoint, data=body, headers=headers, has_authorization=has_authorization)
        elif HTTPMethod.GET == http_method:
            retorno = RequestsUtil.get(
                self.endpoint, params=body, param_mode=param_mode, headers=headers, has_authorization=has_authorization)
        elif HTTPMethod.DELETE == http_method:
            retorno = RequestsUtil.delete(
                self.endpoint, params=body, param_mode=param_mode, headers=headers, has_authorization=has_authorization)
        elif HTTPMethod.PUT == http_method:
            retorno = RequestsUtil.put(self.endpoint, data=body, headers=headers, has_authorization=has_authorization)
        else:
            retorno = None
