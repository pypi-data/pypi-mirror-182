...
def test_metricscontainer_1():
    ...
    m.register(lambda df: 1.0, name="a")
    m.register(lambda df: 2.0, name="b")
    ...
    summary = m.compute(
        mm.MOTAccumulator.new_event_dataframe(), metrics=["mul", "add"], name="x"
    )
    assert summary.columns.values.tolist() == ["mul", "add"]
    assert summary.iloc[0]["mul"] == -3.0
    assert summary.iloc[0]["add"] == 3.0
...
def test_motchallenge_files():
    ...
    print(
        mm.io.render_summary(
            summary, namemap=mm.io.motchallenge_metric_names, formatters=mh.formatters
        )
    )
...
def test_paper_metrics():
    ...
    acc.update([1, 2], [1, 3], [[0.6, 0.2], [0.1, 0.6]])
    print(acc.events)
    mh = mm.metrics.create()

    def my_motp(df: mm.metrics.DataFrameMap):
        select = df.full.Type.isin(["MATCH", "SWITCH"])
        subset = df.full[select]
        return subset.D.sum() / subset.shape[0]

    mh.register(my_motp, deps="auto")

    summary = mh.compute(
        acc, metrics=["num_frames", "precision", "recall"], name="my-acc"
    )
