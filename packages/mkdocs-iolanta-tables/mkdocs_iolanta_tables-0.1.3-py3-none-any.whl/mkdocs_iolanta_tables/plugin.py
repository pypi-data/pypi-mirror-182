...
class TablesPlugin(OctadocsMixin):
    ...
    def on_config(self, config, **kwargs):
        ...
        config['extra'].setdefault('contexts', []).append(
            Path(__file__).parent / 'yaml/context.yaml',
        )

    def on_files(self, files: Files, config: Config):
        super().on_files(files=files, config=config)

        self.iolanta.add(
            url=path_to_url(Path(__file__).parent / 'yaml/octadocs-table.yaml'),
            context=config['extra']['default_context'],
        )
