...
class InsertQuery(BaseQuery):
    ...
    def sql(self):
        ...
        -- All the columns in the rows which exist in "staging" and not in
        -- "dimension" will be NULL.
        SELECT staging.<key>, staging.<field> [, ...] FROM <dimension>_staging AS staging
        ...
        if self._sql is None:
            ...
            staging_field_set = ', '.join(f'staging.{field}' for field in self.fields)
            self._sql = (
                'INSERT INTO {table} ( {field_set} ) '
                'SELECT {staging_field_set} FROM {staging} AS staging '
                'LEFT OUTER JOIN {table} AS dimension '
                'ON dimension.{key} = staging.{key} '
                'WHERE dimension.{key} IS NULL'
            ).format(
                table=self.table,
                field_set=field_set,
                staging_field_set=staging_field_set,
                staging=self.staging,
                key=self.key_field
            )
...
class LoadQuery:
    ...
    def _write(self, line):
        ...
        for field in self.fields:
            ...
            if value is None:
                value = ''
            elif isinstance(value, (float, int, bool, Decimal)):
                value = str(value).lower()
            elif isinstance(value, bytes):
                value = '"{}"'.format(value.decode('utf-8').replace('"', '""'))
            else:
                try:
