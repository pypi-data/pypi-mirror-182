...
from ..model import Friend, Member, RainferBaseModel, Stranger, UploadMethod
from ..util import wrap_bracket

if TYPE_CHECKING:
    from ..typing import ReprArgs  # type:ignore
...
class Element(RainferBaseModel):
    """
    指示一个消息中的元素.
    type (str): 元素类型
    """
...
class ForwardNode(RainferBaseModel):
    """表示合并转发中的一个节点"""
...
class MiraiCode(Element):
    """Mirai 码, 并不建议直接使用. Rainfer 也不会提供互转换接口."""
