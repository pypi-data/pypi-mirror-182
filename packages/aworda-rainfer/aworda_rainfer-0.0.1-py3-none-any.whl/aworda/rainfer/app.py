...
from .message.chainx import MessageChainX
...
class RainferMixin:
    ...
    def session_key(self) -> Optional[str]:
        """返回 Rainfer 的 Mirai session key."""
        return self.mirai_session.session_key


class MessageMixin(RainferMixin):
    ...
    async def sendFriendMessage(
        ...
        message: Union[MessageChain, MessageChainX],
        *,
        ...
        if isinstance(message, MessageChainX):
            message = message.toMessageChain()
    ...
    async def sendMessage(
        ...
        *,
        ...
        action: SendMessageActionProtocol["T"] = ...,  # type:ignore
    ) -> "T":
        """
        依据传入的 `target` 自动发送消息.
        ...
        Args:
            ...
            可以在 aworda.rainfer.util.send 查看自带的 action, \
            未传入使用默认 action
...
class RelationshipMixin(RainferMixin):
    """获取各种关系模型的 Mixin 类."""
...
class OperationMixin(RainferMixin):
...
class AnnouncementMixin(RainferMixin):
    """对群公告进行操作的 Mixin 类."""
...
class MultimediaMixin(RainferMixin):
    """用于与多媒体信息交互的 Mixin 类."""
...
class Rainfer(MessageMixin, RelationshipMixin, OperationMixin, AnnouncementMixin, FileMixin, MultimediaMixin):
    """
    RRRRRRAAAIIIINFFEERR (Rainfer).
    ...
    broadcast: Broadcast
    adapter: Adapter
    status: RainferStatus
    running: ClassVar[MutableSet["Rainfer"]] = set()
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            请移步 [ChatLogConfig][aworda.rainfer.model.ChatLogConfig] 查看使用方法. \
        ...
        self.status: RainferStatus = RainferStatus.STOP
    ...
    async def daemon(self, retry_interval: float = 5.0):
        ...
        logger.debug("Rainfer daemon started.")

        while self.status in {RainferStatus.RUNNING, RainferStatus.LAUNCH}:
            try:
                ...
                async for event in yield_with_timeout(
                    ...
                    lambda: (
                        self.adapter.running and self.status in {RainferStatus.RUNNING, RainferStatus.LAUNCH}
                    ),
                ):
                    with enter_context(self, event):
                        sys.audit("RainferPostRemoteEvent", event)
            ...
            if retry_cnt == self.max_retry:
                ...
                break
            if self.status in {RainferStatus.RUNNING, RainferStatus.LAUNCH}:
        ...
        logger.info("Stopping Rainfer...")
        self.status = RainferStatus.CLEANUP
        ...
        logger.info("Posting Rainfer shutdown event...")
        ...
        self.status = RainferStatus.STOP
        logger.success("Stopped Rainfer.")
        return exceptions

    async def launch(self):
        """启动 Rainfer."""
        if self.status is not RainferStatus.STOP:
            return
        self.status = RainferStatus.LAUNCH

        # Logo
        if not self.disable_logo:
            logger.opt(colors=True, raw=True).info(f"<cyan>{RAINFER_ASCII_LOGO}</>")
        ...
        self.status = RainferStatus.RUNNING
    ...
    async def stop(self):
        """请求停止 Rainfer."""
        if self.status is RainferStatus.RUNNING:
            self.status = RainferStatus.SHUTDOWN
            await await_predicate(lambda: self.status in {RainferStatus.CLEANUP, RainferStatus.STOP})
    ...
    async def join(self):
        ...
        if self.status in {RainferStatus.RUNNING, RainferStatus.LAUNCH}:
            await self.stop()
        await await_predicate(lambda: self.status is RainferStatus.STOP)
        if self.daemon_task:
            await self.daemon_task

    async def lifecycle(self):
        """以 async 阻塞方式启动 Rainfer 并等待其停止."""
    ...
    async def __aenter__(self) -> "Rainfer":
        await self.launch()
