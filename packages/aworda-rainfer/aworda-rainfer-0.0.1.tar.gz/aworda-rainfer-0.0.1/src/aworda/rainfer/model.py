...
from aworda.rainfer.util import gen_subclass

if TYPE_CHECKING:
    from .app import Rainfer
    from .message.chain import MessageChain
    from .typing import AbstractSetIntStr, DictStrAny, MappingIntStrAny  # type:ignore
...
class RainferBaseModel(BaseModel):
    """
    Rainfer 一切数据模型的基类.
    """
...
class ChatLogConfig:
    ...
    def initialize(self, app: "Rainfer"):
        """利用 Rainfer 对象注册事件日志处理器"""
...
class MiraiSession(RainferBaseModel):
    """
...
class Stranger(RainferBaseModel):
    """描述 Tencent QQ 中的陌生人."""
...
class GroupConfig(RainferBaseModel):
    """描述群组各项功能的设置."""
...
class MemberInfo(RainferBaseModel):
...
class DownloadInfo(RainferBaseModel):
    """描述一个文件的下载信息."""
...
class Announcement(RainferBaseModel):
    """群公告"""
...
class FileInfo(RainferBaseModel):
    """群组文件详细信息"""
...
class Profile(RainferBaseModel):
    """
    指示某个用户的个人资料
    """
...
class BotMessage(RainferBaseModel):
    """
    指示 Bot 发出的消息.
    """
