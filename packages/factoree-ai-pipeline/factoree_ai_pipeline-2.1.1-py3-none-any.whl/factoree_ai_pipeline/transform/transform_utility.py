...
class TransformUtility(ABC, Generic[DATA_TYPE]):
    ...
    def standardize_value(
        ...
        if sensor_type == SensorType.II and isinstance(value, float) and value < 0.0:
    ...
    def extract_sensor_type(self, tag: str) -> SensorType:
        tag_parts = tag.split('-')
        sensor_type = parse_sensor_type(tag_parts[1]) or parse_sensor_type(tag_parts[2])

        match sensor_type:
            case SensorType.WIC:
                if len(tag_parts) == 3 or (len(tag_parts) == 4 and tag_parts[3] == SET_POINT_SUFFIX):
