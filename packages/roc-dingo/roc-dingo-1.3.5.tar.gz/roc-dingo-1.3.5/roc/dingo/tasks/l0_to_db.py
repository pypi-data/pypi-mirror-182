...
class L0ToDb(Task):
        ...
        for i, current_l0_file in enumerate(self.l0_files):
            ...
            try:
                with h5py.File(current_l0_file, 'r') as l0:
                    ...
                    existing_tm = pd.DataFrame.from_records(self._get_existing_data(TmLog, TmLog.sha,
                                                                                    start_time=query_start,
                                                                                    end_time=query_end))
                    logger.info(f'{existing_tm.shape[0]} TmLog.sha entries found between {query_start} and {query_end}')
                    if existing_tm.shape[0] > 0:
                        self.existing_data = self.existing_data.append(
                            existing_tm, ignore_index=True)
                    logger.debug(f'Getting existing TcLog.sha between {query_start} and {query_end}')
                    existing_tc = pd.DataFrame.from_records(self._get_existing_data(TcLog, TcLog.sha,
                                                                                    start_time=query_start,
                                                                                    end_time=query_end))
                    logger.info(
    ...
    def insert_packet(self, l0, packet_name):
        ...
        if current_df.shape[0] > 0:
            logger.debug(f'Removing {packet_name} data already inserted for {self.current_l0_file}')
    ...
    def packet_to_dataframe(self, l0, packet_name):
        ...
        # Store packet binary data (as hexa)
        ...
        packet_df['utc_time'] = packet_df['utc_time'].apply(
            lambda x: datetime.strptime(x[: -4] + 'Z', TIME_ISO_STRFORMAT))
        ...
        if packet_type == 'TM':
            ...
            packet_info['unique_id'] = l0[
                packet_name]['unique_id'][()]
            packet_info['sequence_name'] = l0[
                packet_name]['sequence_name'][()]
            current_tc_state = l0[
                packet_name]['tc_ack_state'][()]
