...
from iacer.config import BaseConfig, PROJECT, REGIONS, TEMPLATE_CONFIG, TestConfig, IAC_OCTOPUS_NAME, \
...
from iacer.report.generate_reports import ReportBuilder
from iacer.stack import Stacker
...
class Base(metaclass=abc.ABCMeta):
    ...
    async def from_file(cls: Type[T],
        ...
        if project_path:
            ...
            if template:
                ...
                if template_path.is_file() or template_path.is_dir():
        ...
        if not project_name:
            raise IacerException('project name should be specified')
    ...
    async def report(self, output_directory, project_path=None):
        ...
        await reporter.create_logs()
        index = await reporter.generate_report()
        self._upload_to_oss(report_path, index)

    def _upload_to_oss(self, report_path: Path, index: str):
        ...
        if bucket and region:
            ...
            oss_prefix = f'{IAC_OCTOPUS_NAME}/{oss_prefix}'
            oss_plugin = OssPlugin(
                region_id=region, bucket_name=bucket, credential=self.auth.credential)
            for path in report_path.glob(f'{IAC_OCTOPUS_NAME}*.txt'):
                oss_plugin.put_local_file(f'{oss_prefix}/{path.name}', path)
