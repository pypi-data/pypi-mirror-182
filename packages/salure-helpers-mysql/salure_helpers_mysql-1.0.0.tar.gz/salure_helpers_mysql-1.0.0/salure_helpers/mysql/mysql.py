...
class MySQL:
    ...
    def __init__(self, salureconnect_connection: SalureConnect = None, label: Union[str, List] = None, debug=False):
        """
        This class is used for connecting to a mysql database
        :param salureconnect_connection: the salureconnect connection object
        ...
        if os.getenv("MYSQL_HOST") is not None:
            ...
            self.port = 3306 if os.getenv("MYSQL_PORT") is None else os.getenv("MYSQL_PORT")
            self.return_queries = False
        elif salureconnect_connection is not None:
            credentials = salureconnect_connection.get_system_credential(system='mysql', label=label)
            self.host = credentials['host']
            self.user = credentials['username']
            self.password = credentials['password']
            self.database = credentials['schema']
            self.port = 3306 if credentials['port'] is None else credentials['port']
            self.return_queries = False
        else:
            print("Running in query return mode because no credentials were specified")
            self.return_queries = True
        self.debug = debug
