...
def _dx(df, dx=None, eps = 1e-3):
    ...
    if isinstance(df.index,pd.DatetimeIndex):
        return smath.get_dx((df.index[-1] - df.index[0]).total_seconds(), dx, eps, raise_exception=True)
    else:
...
def getPSD(df, dw=0.05, roverlap=0.5, window='hann', detrend='constant', unit="rad"):
    """Compute the power spectral density, with Welch method

    Notes
    -----
    This is simply a wrapper around 'scipy.signal.welch', with input re-parametrisation for easier use.
