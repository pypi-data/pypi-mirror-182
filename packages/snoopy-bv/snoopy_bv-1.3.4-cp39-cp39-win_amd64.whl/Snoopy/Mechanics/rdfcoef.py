...
class RdfCoef(HydroCoefABC):
    """ Contain hydrodynamic database,
    Results of diffraction/radiation calculation
    Mass is not known
    """

    # requirement: all variable is grouped depending on its role and requirement
    data_vars_requirement = ['excitation','added_mass',
                             'wave_damping','cob_point',
                             'hydrostatic_hull']
    data_vars_optional    = ['base_flow_stiffness',
                             'ref_point','ref_wave']
    ...
    attrs_optional        = ['version','software','nb_body',
                             'nb_mode','depth','g','rho','file_type']
    matrans_list = ['hydrostatic_hull',
    ...
    def Build(cls,  nb_body             = 1,
        ...
        nb_mode : int
            number of mode, optional, default = 6

        cob_point : numpy.array or list or xarray.DataArray
            Center of bouyancy, dimension : [nbBody,3]

        ref_point : numpy.array or list or xarray.DataArray
            Reference point, dimension : [nbBody,3]
            Optional, default value: cob_point
        ...
        hydrostatic_hull : numpy.array or list or xarray.DataArray
            hydrostatic stiffness, dimension ["body","mode_i","mode_j"]
        ...
        # Requirements
        data_vars = {}
        assert cob_point is not None             , 'cob_point must be given'
        ...
        if (hydrostatic_hull is not None):
             data_vars['hydrostatic_hull'] =(cls.static_6x6DoF_dims, hydrostatic_hull)
        if (hydrostatic is not None):
             data_vars['hydrostatic'] =(cls.static_6x6DoF_dims, hydrostatic)

        assert ('hydrostatic' in data_vars) or ('hydrostatic_hull' in data_vars),\
            'hydrostatic_hull or hydrostatic must be given'
        ...
        #    wave_length = np.array([w2l( freq, depth = depth ) for freq in frequency])

        #if encounter_frequency is None:
        ...
        #            encounter_frequency[i_head,i_freq] = w2we( freq , head, speed, depth = depth )
        if version is None:
            version = cls.version

        data_vars.update({
            'hydrostatic_hull'     : (cls.static_6x6DoF_dims, hydrostatic_hull),
            ...
            'cob_point'            : (cls.notable_point_dims,cob_point),
