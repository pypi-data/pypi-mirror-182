...
from .mechanicalsolver import MechanicalSolver, HydroStarVMechanics, HydroStarMechanics
from .hydrodatabase import HydroDatabase
def Read_HDF(inputPath):
    xarraydata = xa.open_dataset(inputPath)
    file_type = getattr(xarraydata,"file_type",None)
    if file_type == "hsrdf output":
        cls = RdfCoef
    elif file_type == 'hsmcn input':
        cls = McnInput
