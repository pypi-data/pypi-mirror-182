...
class SeaStatesDF( pd.DataFrame ):
    ...
    def to_ssList( self ) :
        ...
        specCols =  self._getSpectrumCols()
        ssList = []
        for i, row in self.iterrows():
            specList = []
            for imode, specCol in enumerate( specCols ) :
                specClass = getattr( sp, row.loc[ specCol ] )
                args =  { coef : row.loc[ rf"{coef:}_{imode:}"] for coef in specClass.getCoefs_name() }
                spec = specClass( **args , heading = np.deg2rad( row.loc[ f"Heading_{imode:}"] ),
                                           spreading_type = sp.SpreadingType.__members__[row.loc[ f"SpreadingType_{imode:}"]] ,
                                           spreading_value = row.loc[ f"SpreadingValue_{imode:}"],
                                             )
                specList.append( spec )

            ss = sp.SeaState( specList, probability = row.PROB )
            ssList.append( ss )
    ...
    def getCompacted(self, rounding_dict = {}) :
        ...
        for k , v in rounding_dict.items() :
            df.loc[:, k] = np.round ( df.loc[:, k] , v  )

        df._convertNamesToInt()

        col = [i for i in df.columns if i != "PROB"]

        b = np.ascontiguousarray(df.loc[:,col].values).view(np.dtype((np.void, df.loc[:,col].values.dtype.itemsize * df.loc[:,col].values.shape[1])))
        _, idx, inv, count = np.unique(b, return_index=True, return_inverse=True, return_counts=True)

        dfUnique = df.iloc[ idx, : ].copy(deep = True)

        iprob = dfUnique.columns.get_loc("PROB")
        for ipos, iordered in enumerate(idx) :
            dfUnique.iloc[ipos, iprob] = np.sum(dfNew.PROB.iloc[  np.where( inv == ipos )  ])
