    ...
    def import_csv(self, request):
        ...
        try:
            if request.method == "POST":
                ...
                print(f'--- bulk_size: {bulk_size}, single_import_flag: {bulk_size and n < bulk_size}')

                if bulk_size and n <= bulk_size:
                    ...
                    for start_end_i in start_end_ls:
                        thread_i = threading.Thread(target=check_df_format,
                                                    args=(self, df1, md_dc_ls, start_end_i + curr_id, start_end_i, start_end_i + bulk_size, df_rows, my_tqdm, info_dc, q))
                ...
                if (bulk_size and n <= bulk_size) or use_sqlite3:
                    ...
                    for start_end_i in start_end_ls:
                        # print(start_end_i, start_end_i + bulk_size)
                        thread_i = threading.Thread(target=save_md_dc_ls, args=(md_dc_ls, start_end_i, start_end_i + bulk_size, n, save_tqdm, save_q, pk_name))
                        thread_ls.append(thread_i)
                        thread_i.start()
                        thread_i.join()
