...
def login():
    ...
    **You do not need to manually call this method** to use this library.
    ...
    response = requests.post(_build_url('/auth/login'), json=payload, verify=projectal.__verify)
    # Handle errors here
...
def __request(method, endpoint, payload=None, file=None, form=False, is_json=True):
    ...
    if file:
        ...
        kwargs['json'] = payload
    response = fun(_build_url(endpoint), cookies=projectal.cookies, verify=projectal.__verify, **kwargs)
