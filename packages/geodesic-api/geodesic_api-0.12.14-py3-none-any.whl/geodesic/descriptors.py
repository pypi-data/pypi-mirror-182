...
class _BaseDescr:
    ...
    def __set_name__(self, owner: object, name: str):
        ...
        if self.doc is not None:
            ...
            if doc is not None:
                doc = [d.lstrip(" ") for d in doc.split('\n')]
                doc = '\n'.join(doc)
                new_doc += "\n\nDescriptor:\n" + doc
...
class _GeometryDescr(_BaseDescr):
    ...
    __set__ coerces the input into a geojson dict (__geo_interface__) and stores internally to the APIObject dict
...
class _BBoxDescr(_GeometryDescr):
    ...
    Accessors:
        ...
        __set__: coerces the input into a shapely geom before taking the bounds and stores internally to the APIObject dict
    """
...
class _ListDescr(_BaseDescr):
    ...
    Args:
        item_type (tuple or type): A type or tuple of types that are allowed as the inputs to this descriptor.
            ...
            to the first element of the tuple. If you would like to use this to pass in one type (typeA) and return
            another (typeB) then item_type should be `(typeB, typeA)` as well as `coerce=True`.
        min_len (int): a minimum number of elements for this list
        max_len (int): a maximum number of elements for this list
        coerce_items (bool): coerce all elements to the first type in the `item_type` arg when setting the values of the
            list.
