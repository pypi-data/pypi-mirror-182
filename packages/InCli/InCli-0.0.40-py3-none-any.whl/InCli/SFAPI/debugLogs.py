...
def _parseStorage(error=False,printLimits=False):
    ...
    for num,id in enumerate(ids):
        sys.stdout.write("\r%d%%" % int(100*num/len(ids)))
        res = parseLog(logId=id,error=error,printLimits=printLimits)
        

def _parseTail(loguser=None,level=None,whereClause=None,deleteLogs=True,printLimits=False):
    deleteList = []
    def deleteRecords(q):
      while True:
        Id = q.get()
        ...
        while (True):
            ...
            if len(logRecords) > 0:
                ...
                for record in logRecords:
                    ...
                    parseLog(logId=record['Id'], loguser=loguser,level=level,printLimits=printLimits)                
...
def parseLog(logId=None,filepath=None,printLimits=False,lastN=1,loguser=None,level=None,whereClause=None,writeToFile=False,tail=False,deleteLogs=False,store=False,error=False):
    ...
    if tail == True:
        ...
        return _parseStorage(error=error,printLimits=printLimits)
    ...
    return parse(logId,body,printLimits=printLimits,level=level,writeTofile=writeToFile,error=error,limits=printLimits)
...
def parse(logId,logData,printLimits=True,userDebug=True,level=None,writeTofile=False,error=False,limits=False):
    try:
       return  _parse(logId,logData,printLimits,userDebug,level,writeTofile,error,limits)
    except KeyboardInterrupt as e:
        print(f"Parsing for logI {logId} interrupted.")
    except Exception as e:
        raise 

def _parse(logId,logData,printLimits=True,userDebug=True,level=None,writeTofile=False,error=False,limits=False):
   ...
   #     print(num)
        ...
        if len(chunks)>1 and chunks[1] in ['HEAP_ALLOCATE','STATEMENT_EXECUTE','VARIABLE_SCOPE_BEGIN','HEAP_ALLOCATE','SYSTEM_METHOD_ENTRY','SYSTEM_METHOD_EXIT','SOQL_EXECUTE_EXPLAIN']:
            continue
    ...
    printDebugList(context,logId,writeTofile,error=error)
...
def parseLimits(context):
    ...
    if '|LIMIT_USAGE_FOR_NS|' in line:
        ...
        obj['output'] = f"{chunks[1].lower()}  {chunks[1]}"
        context['debugList'].append(obj)
...
def printDebugList(context,logId,toFile=False,error=False):
    if toFile == True:
        ...
        _printDebugList(context,logId,error)

def _printDebugList(context,logId,error=False):
    if context['file_exception'] == False and error == True:
        return 
