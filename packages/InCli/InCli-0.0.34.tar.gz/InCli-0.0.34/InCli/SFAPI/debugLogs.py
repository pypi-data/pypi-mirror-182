...
def _logRecord_toString(logRecord):
    ...
    usernameq = f"select Username from User where Id='{log['LogUserId']}'"
...
def printDebugList(context,logId,toFile=False):
    if toFile == True:
...
def _printDebugList(context,logId):
    print()
    if logId != None:
