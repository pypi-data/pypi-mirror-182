...
def _parseStorage(pc):
    ...
    try:
        _parseIds(ids,pc,printProgress=True)
...
def _parseIds(logIds,pc,raiseKeyBoardInterrupt=False,printProgress=False):
    if 'parsed' not in pc:
        pc['parsed'] = []
    if 'errors' not in pc:
        pc['errors'] = []
...
def _readBody(pc):
    ...
    if file.exists(filename) == True:
        ...
        pc['logRecord'],pc['body'] = _queryLogData(logId) 
        pc['body'] = _logRecord_toString(logRecord=pc['logRecord']) + pc['body']    

def _parseLogId(context):
    _readBody()
...
def parseLog(logId=None,filepath=None,printLimits=False,lastN=1,loguser=None,level=None,whereClause=None,writeToFile=False,tail=False,deleteLogs=False,store=False,error=False):
    ...
    if tail == True:
        return _parseTail(parseContext)

    elif store == True:
        return _parseStorage(parseContext)

    elif filepath != None:
        parseContext['body'] = file.read(filepath)

    elif logId != None:  #check if already downloaded
        _readBody(parseContext)

    else:
        return _parseLogs_LastN(parseContext)
    ...
    parseContext['logToStore'] = True if parseContext['filepath'] == None else False
...
#context,logId,toFile=False,error=False
    logId = pc['logId']
    toFile= pc['writeToFile']
...
def _printDebugList(pc):
    #context,logId,error=False
    logId = pc['logId']
    onlyErrors = pc['onlyErrors']
    context = pc['context']
    if context['file_exception'] == False and onlyErrors == True:
        return 
