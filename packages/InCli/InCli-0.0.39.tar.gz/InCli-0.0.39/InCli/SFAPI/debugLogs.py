...
def parseLog(logId=None,filepath=None,printLimits=True,lastN=1,loguser=None,level=None,whereClause=None,writeToFile=False,tail=False,deleteLogs=False,store=False,error=False):
    body = None
    ...
    if tail == True:
        ...
        return _parseStorage(error=error)
    ...
    return parse(logId,body,printLimits=printLimits,level=level,writeTofile=writeToFile,error=error)
...
def parse(logId,logData,printLimits=True,userDebug=True,level=None,writeTofile=False,error=False):
    try:
        _parse(logId,logData,printLimits,userDebug,level,writeTofile,error)
    except KeyboardInterrupt as e:
        print(f"Parsing for logI {logId} interrupted.")
    except Exception as e:
        raise 

def _parse(logId,logData,printLimits=True,userDebug=True,level=None,writeTofile=False,error=False):
...
def printParsedLog(context,d):
  ...
  #  _sqlcmt2 = d['cmtSOQLQueries'][1] if len(d['cmtSOQLQueries'])>1 else ''
    ...
    print(f"{ms(d['elapsedTime']):10}|{_elapsedPrevD:8}|{d['timeStamp'][0]:12}|{_timeStamp1:12}|{_delta:>10}|{_totalQueriesD:4}|{_totalQueries1:4}|{_cpuPrevD:6}|{_cpuTime0:>6}|{_sql2:>3}|{_sqlcmt2:>3}|{_typeColor}{_type:21}{utils.CEND}|{_apexline:>4}| {val[:150]:50}"+utils.CEND)
