...
from ._transformations import rank_scale_data, scale_data, protect_norm
from ._units import TheFittest, Statictic
...
class SelfCGeneticAlgorithm:
    ...
    def __init__(self, fitness_function, genotype_to_phenotype, iters,
        ...
        self.m_sets = {
            'no': 0,
            'very+': 1/(9*self.str_len),
            'weak': 1/(3*self.str_len),
            'average':  1/(self.str_len),
            'strong': min(1, 3/self.str_len),
            'strong+': min(1, 9/self.str_len)
        }
        self.m_sets = dict(sorted(self.m_sets.items()))

        self.c_sets = {'empty': (self.empty_crossover, 1),
                       'one_point': (self.one_point_crossover, 2),
