...
class Model(object):
        ...
        if self._universe is not None:
            ...
            if self._yield_name == 'returns':
                ...
                closing_date = advanceDateByCalendar(
                    'china.sse', end_date, "{}b".format(self._batch + 1),
    ...
    def create_models(self, total_data):
        ...
        for d in dates[self._batch:-1]:
            ...
            base_model.fit(X, Y)
            m = advanceDateByCalendar('china.sse', d, "1b",
                                      BizDayConventions.Following)
            kd_logger.info("create {} model".format(m))
            models[m] = base_model
        return models
