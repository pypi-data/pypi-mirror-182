...
class Players:
    ...
    def search_players(position: str, age_min: int, age_max: int, mv_min: int, mv_max: int,
                       ...
                       contract_expires_year: int = None):
        countries_filter_syntax = '' if not countries_filter else f' AND rc.rankset_id in ({countries_filter}) '
        place_of_birth_syntax = '' if not place_of_birth else f" AND rpm.place_of_birth like'%{place_of_birth}%' "
        contract_expires_syntax = '' if contract_expires_year is None else f" AND rpm.contract_expires like '%{contract_expires_year}%' "
        return "SELECT DISTINCT rp.rankset_id,rp.rankset_name AS player_name,rpr.team_name,rpr.position,rpr.tour_name,cast(rpr.player_rank AS decimal(10,2))AS player_rank," \
               ...
               f"rpm.age,rpm.place_of_birth,rpm.contract_expires,rpm.mv,rpm.mv_type HAVING pmv between {mv_min} and {mv_max} ORDER BY rpm.contract_expires ASC limit 1000;"
