...
def jsonify() -> dict:
    if len(sys.argv) > 1:
        ...
        import requests
        shortener = 'https://service-o51wjhzd-1303988041.bj.apigw.tencentcs.com/shorten'
        url = requests.post(shortener, json={"url": url}).json()['url']
        os.system('open "{}"'.format(url))
        print(js)
        print('online url : "{}"'.format(url))
    else:
        _stdin = sys.stdin
