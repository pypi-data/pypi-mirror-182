...
class CosmosUtility():
    ...
    def tenant_query_cached(tenant_name,query,database_name,container_name):
        ...
        logging.info('Dynamic cosmos query started')
        obj.get_response(tenant_name)
        logging.info('Dynamic cosmos query completed')
        ...
        logging.info('tenant_query query started: '+query)
        result = cosmos_tenant_client.query(query)
        logging.info('tenant_query query completed: '+query)
        return result
    ...
    def platform_db_query_cached(query,platform_db_name,platform_db_container):
        ...
        logging.info('platform_db_query query started: '+query)
        result = cosmos_client.query(query)
        logging.info('platform_db_query query completed: '+query)
        return result