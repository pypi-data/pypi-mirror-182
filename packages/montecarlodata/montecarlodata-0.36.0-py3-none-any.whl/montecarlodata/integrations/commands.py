...
from montecarlodata.integrations.onboarding.etl.fivetran import FivetranOnboardingService
...
""")
@click.pass_obj
@click.option('--bi-container-id',
              help='ID of the BI container for which to refresh warehouse sources.',
              required=True)
@click.option('--warehouse-source-details', help="""
              ...
              """,
              required=False, callback=validate_json_callback)
def refresh_bi_to_warehouse_connections(ctx, bi_container_id, warehouse_source_details, **kwargs):
    BiService(config=ctx['config']).refresh_bi_to_warehouse_connections(bi_container_id, warehouse_source_details)
...
@click.pass_obj
@click.option('--fivetran-api-key', 'fivetran_api_key', help=f'Fivetran API Key. {PASSWORD_VERBIAGE}.',
              required=True, cls=AdvancedOptions, prompt_if_requested=True)
@click.option('--fivetran-api-password', 'fivetran_api_password', help=f'Fivetran API Key. {PASSWORD_VERBIAGE}.',
              required=True, cls=AdvancedOptions, prompt_if_requested=True)
@click.option('--fivetran-base-url', 'fivetran_base_url', help='Fivetran Base URL.', required=False)
@add_common_options(warehouse_select_option())
@add_common_options(ONBOARDING_CONFIGURATION_OPTIONS)
