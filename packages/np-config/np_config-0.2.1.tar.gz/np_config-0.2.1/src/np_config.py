...
zk_record = LOCAL_DATA_DIR / "config_logs" / f"zk_config-{pathlib.Path().cwd().name}.yaml"
SESSION_ZK_RECORD_FILE = zk_record.with_suffix(f".{session_start_time}{zk_record.suffix}") # . is used later, don't change
...
def cleanup_zk_records():
    ...
    for _, project_records in itertools.groupby(path.glob("*"), key=lambda f: f.stem.split(".")[0]):
        for pair in itertools.pairwise(sorted(project_records, key=lambda f: f.stat().st_ctime, reverse=True)):
            if pair[0].read_bytes() == pair[1].read_bytes():
                logger.debug(f"Removing un-changed zk record: {pair[0].stem.split('.')[0]}")
                pair[0].unlink()
...
class ConfigZK(KazooClient):
    ...
    def __enter__(self):
        ...
        try:
            ...
            if not self.connected:
                logger.warning(f"Could not connect to zookeeper server {self.hosts}", exc_info=True)
                raise
        finally:
            return self
