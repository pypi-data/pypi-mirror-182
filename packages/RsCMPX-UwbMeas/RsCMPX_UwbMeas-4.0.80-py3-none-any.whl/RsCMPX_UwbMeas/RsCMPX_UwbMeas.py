...
class RsCMPX_UwbMeas:
	...
	@classmethod
	def from_existing_session(cls, session: object, options: str = None) -> 'RsCMPX_UwbMeas':
		"""Creates a new RsCMPX_UwbMeas object with the entered 'session' reused. \n
	...
	def __str__(self) -> str:
		if self._core.io:
			return f"RsCMPX_UwbMeas session '{self._core.io.resource_name}'"
		else:
			return f"RsCMPX_UwbMeas with session closed"
	...
	def close(self) -> None:
		"""Closes the active RsCMPX_UwbMeas session."""
		self._core.io.close()
	...
	def _sync_to_custom_properties(self, cloned: 'RsCMPX_UwbMeas') -> None:
	...
	def uwbMeas(self):
		...
		if not hasattr(self, '_uwbMeas'):
			from .Implementations.UwbMeas import UwbMeasCls
	...
	def clone(self) -> 'RsCMPX_UwbMeas':
		"""Creates a deep copy of the RsCMPX_UwbMeas object. Also copies:
		...
		cloned = RsCMPX_UwbMeas.from_existing_session(self.get_session_handle(), self._options)
