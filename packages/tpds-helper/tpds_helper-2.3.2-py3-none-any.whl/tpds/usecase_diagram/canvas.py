...
class CanvasFirmware():
        ...
        if self.cb_project_path():
            ...
            if do_copy or do_overwrite:
                ...
                if do_overwrite is False and any(File.endswith(".h") for File in os.listdir(os.path.dirname(working_dir))):
                    exclude_extn += ['.h', '.c']
                files = [x for x in os.listdir(original_dir) if Path(os.path.join(original_dir, x)).is_file()]
                files = [x for x in files if not x.endswith(tuple(exclude_extn))]
                for fname in files:
                    shutil.copy(os.path.join(original_dir, fname), os.path.join(os.path.dirname(working_dir), fname))
    ...
    def get_c_src_folder(self):
        c_src_folder = None
        if self.cb_project_path():
            original_dir = self.cb_project_path().split('firmware')[0]
            c_src_folder = original_dir if os.path.exists(original_dir) else None
        return c_src_folder
