...
def update_config(run_id: int, config_item_dict: list):
    def makeItem(item):
        key, value = item
    ...
    config_item_dict.append(('cliVer', version.__version__))
    config_item_dict.append(('pythonVer', util.get_python_version()))


    item = map(makeItem, config_item_dict)
...
def upload_log(run_id: int, log_step: int, item_dict: Dict[str, Union[int, str, float, bool]]) -> bool:
    item_list = []
