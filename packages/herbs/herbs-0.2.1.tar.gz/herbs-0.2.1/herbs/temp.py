...
def get_base_start(site_face, x_bias, vox_size, base_thickness):
    zero_vec = np.zeros(len(x_bias))
    x_bias = np.ravel(x_bias)

    if site_face == 0:
        base_loc = x_bias / vox_size
        base_start = np.vstack([zero_vec, base_loc, zero_vec]).T
        base_start[:, 2] = base_start[:, 2] + base_thickness
    elif site_face == 1:
        base_loc = x_bias[::-1] / vox_size
        base_start = np.vstack([zero_vec, base_loc, zero_vec]).T
        base_start[:, 2] = base_start[:, 2] - base_thickness
    elif site_face == 2:
        base_loc = x_bias[::-1] / vox_size
...
def correct_base_sp(base_start, sp, rot_mat, direction, bregma, label_data):
    ...
    for i in range(n_base):
        ct_sp_new[i], error_index_vec[i] = correct_start_pnt(label_data, ct_sp[i], start_vox, direction, bregma)
    ...
    if not np.all(error_index_vec == 0):
        ...
        base_start_new = np.dot(rot_mat.T, rotate_base_start)
        error_index = 0
...
def get_sites_center_base(new_base_start, base_end, y_bias, sites_distance, per_max_sites):
    ...
    for i in range(n_column):
        ...
        if len(center_base) > per_max_sites[i]:
            center_base = center_base[:per_max_sites]
...
def get_base_columns(base_start, sites_center_base):
    ...
    base_columns = []
    for i in range(n_column):
        base_column = np.repeat(np.array([base_start[i]]), len(sites_center_base[i]))
        base_column[:, 0] = sites_center_base + 0.
...
def get_sites(sp, base_columns, rot_mat, bregma, label_data):
    ...
    return sites_loc, sites_label


def get_labels(sites_loc_in_column, bregma, segmentation_data):
    site_vox = sites_loc_in_column + bregma
    sites_label = segmentation_data[site_vox[:, 0], site_vox[:, 1], site_vox[:, 2]]
    sites_label = sites_label.astype(int)
    return sites_label


def get_sites_label(sites_loc, segmentation_data, bregma):
    n_column = len(sites_loc)
    sites_label_list = []
    n_rows = []
    for i in range(len(sites_loc)):
        label_vec = get_labels(sites_loc[i], bregma, segmentation_data)
        sites_label_list.append(label_vec)
        n_rows.append(len(sites_loc[i]))

    n_row = np.max(n_rows)
    sites_label = np.zeros((n_row, n_column), 'i')
    for i in range(n_column):
        sites_label[:n_rows[i], i] = sites_label_list[i].copy()
    ...
    #     region_length[i] = np.sum(sites_label == region_label[i]) * site_height / 2
...
def group_labels(fine_label_mat):
    ...
    for base_level in range(0, n_row):
        ...
        for da_label in unique_label:
            if da_label in previous_row:
                continue
            row_index = 0
            label_check = (fine_label_mat[row_index + base_level] == da_label)
            group_mat[row_index + base_level, label_check] = group_id
    ...
    n_groups = np.max(group_mat)
    weights_vec = np.zeros(n_groups)
    for i in range(n_groups):
        group_loc = np.where(group_mat == i)
        weights = np.sum(group_loc[0]) / len(group_loc[0])
        weights_vec[i] = weights

    order_ind = np.argsort(weights_vec)
    weights_vec = weights_vec[order_ind]
    group_id_label = np.ravel(group_id_label)[order_ind]

    return group_mat, group_id_label, weights_vec


def get_traveling_label_mat(ct_sp, ct_ep, direction, bregma, label_data):
    column_fine_label = []
    n_rows = []
    n_column = len(ct_sp)
    for i in range(n_column):
        column_length = np.sqrt(np.sum((ct_sp[i] - ct_ep[i]) ** 2))
        step_dist = np.arange(0, column_length, 0.5)
        n_pnt = len(step_dist)
    ...
    for i in range(n_column):
        fine_label_mat[:n_rows[i], i] = column_fine_label

    return fine_label_mat


def get_group_bounds(group_mat):
    n_groups = np.max(group_mat)
    ...
    for i in range(n_groups):
        ...
        for j in range(n_column):
            ...
            if stk == 0:
                continue
            gr_start[i, j] = valid_ind[0] * 0.5
            gr_end[i, j] = (valid_ind[0] + stk) * 0.5
            rl += stk
        region_length.append(rl * 0.5 / n_column)

    return gr_start, gr_end, region


def get_n_sites_in_region(gr_start, gr_end, sites_center_base, probe_length_without_tip):
    n_groups, n_column = gr_start.shape

    sc_base = []
    for i in range(n_column):
        sc_base.append(probe_length_without_tip - sites_center_base[i])
    ...
    for i in range(n_groups):
        ...
        for j in range(n_column):
            if np.isnan(gr_start[i, j]):
                continue
            valid_ind = np.logical_and(sites_center_base[j] >= gr_start[i, j], sites_center_base[j] < gr_end[i, j])
            n_sites += np.sum(valid_ind)
        group_n_sites.append(n_sites)

    return group_n_sites
...
def calculate_probe_info(data, label_data, label_info, vxsize_um, probe_settings, bregma, site_face):
    """
    ...
    sites_distance = sites_distance_um / vxsize_um
    ...
    if error_index != 0:
        return data_dict, 15
    new_start_vox = pc_sp + bregma

    pc_ep, probe_length_with_tip_um, probe_length_without_tip_um = correct_end_point(
        pc_sp, pc_end_pnt, direction, vxsize_um, tip_length_um, probe_type)
    new_end_vox = pc_ep + bregma
    new_end_vox = pc_end_vox.astype(int)
    ...
    region_labels_vec, region_length_vec = get_traveling_info(
        new_sp, new_ep, probe_length_with_tip, direction, bregma, label_data)
    ...
    if probe_type_name != 'Tetrode':
        ...
        base_start = get_base_start(site_face, x_bias, vxsize_um, base_thickness)
        ...
        base_start_new, ct_sp, error_index = correct_base_sp(
            base_start, pc_sp, rot_mat, direction, bregma, label_data)
        if error_index != 0:
            return data_dict, 16

        # get traveling information
        region_labels = []
        region_length = []
        for i in range(n_column):
            region_labels_column, region_length_column = get_traveling_info(
                ct_sp[i], ct_ep[i], None, direction, bregma, label_data)
            region_labels.append(region_labels_column)
            region_length.append(region_length_column)
        ...
        sites_loc, sites_labels = get_sites(pc_sp, columns_loc_base, rot_mat, bregma, label_data)

    if probe_type in [0, 1]:

        center_loc_vec = get_center_loc(sp_to_bottom_site_center, sites_distance)

        base_columns = get_np_base_column(base_start_new, center_loc_vec)

        sites_loc = get_np_sites_loc(new_sp, base_columns, rot_mat, probe_type)
        sites_label = get_sites_label(sites_loc, label_data, bregma)
        ...
        sp_to_bottom_site_center = probe_length_without_tip / vxsize_um
        center_loc_vec = get_center_loc(sp_to_bottom_site_center, sites_distance)
        sites_loc = get_linear_silicon_sites_loc(
            new_sp, center_loc_vec, rot_mat, site_face, base_thickness, per_max_n_sites)

        sites_label = get_sites_label(sites_loc, label_data, bregma)

    else:
        sites_loc = np.repeat(np.array([new_ep]), 4, axis=0)
        site_vox = sites_loc + bregma
        site_vox = site_vox.astype(int)
        sites_label = label_data[site_vox[:, 0], site_vox[:, 1], site_vox[:, 2]]
        merged_labels = None
        merged_colors = None
        block_count = None
...
class Probe(object):
    ...
    def set_linear_silicon(self, probe_settings):
        self.probe_length = probe_settings['probe_length']
        self.tip_length = probe_settings['tip_length']
        self.site_width = probe_settings['site_width']
        self.site_height = probe_settings['site_height']
        self.total_sites = probe_settings['total_sites']
        self.site_number_in_banks = None
        self.sites_distance = probe_settings['sites_distance']
        self.probe_type_name = 'Linear-Silicon'
        self.probe_type = 2
        self.probe_thickness = probe_settings['probe_thickness']
    ...
    def get_settings(self):
        data = {'probe_type': self.probe_type,
                ...
                'per_max_sites': self.total_sites,
