...
def file_download(url, secure = True):
    ...
    for i in result:
        ...
        if css:
            ...
            with open("file " + str(css_count) + ".css", "wb") as f:
                f.write(data)
        ...
        if js:
            ...
            with open("file " + str(js_count) + ".js", "wb") as f:
                f.write(data)
...
#scans for ldap injections
def ldap_scanner(url, secure = True, my_file = " ", bot = False):
    #sql errors
    error_message = ["admin"]
    mal_script = "*"
    ...
    for links in my_result:
        ...
        try:
            ...
            for i in error_message:
                if i in result:
                    print(red + "true: " + new_url + " (" + i + ")")
                    my_list.append("true: " + new_url + " (" + i + ")")
        ...
        try:
            ...
            for i in form_input:
                ...
                for ii in error_message:
                    if ii in get:
                        print(red + "true: " + str(links) + " " + str(mal_dict) + " (" + ii + ")")
                        my_list.append("true: " + str(links) + " " + str(mal_dict) + " (" + ii + ")")
...
def link_scanner(url, secure = True, my_file = " ", bot = False):
    if secure == True:
        my_secure = "https://"
...
def login_cracker(url, user_name = "", word_list = "", secure = True):
    ...
    for i in form_data:
        ...
        if "submit" in i:
            ...
            submit_value = re.findall("(name=\")(\S+)(\")", i)
    ...
    with open(word_list, "r") as f:
        for i in f:
            ...
            try:
                result = requests.get(my_url, verify = False, headers = user_agent, timeout = (5, 30)).url
                verify = requests.post(my_url, data = payload, verify = False, headers = user_agent, timeout = (5, 30)).url
...
#scans a log for a specific value
def log_scanner(verify, file = "", folder = ""):
    ...
    print(red + "hits: " + str(len(verify_list)))
...
def nmap():
    ...
    port_list = [1,2,3,5,7,9,11,13,17,18,19,20,21,22,23,25,27,29,31,33,37,38,41,42,43,44,45,46,48,49,50,53,67,68,80,443,853,8080,8443,9050,9051,19132,19133]
            ...
            for iii in range(0, 255):
                for iv in range(0, 255):
                    for port in port_list:
                        my_thread = threading.Thread(target = nmap_thread, args = (port, i, ii, iii, iv,))
                        my_thread.start()
                        time.sleep(0.001)
...
def nmap_thread(port, i, ii, iii, iv):
    
    port_dict = {1:"tcpmux",2:"compressnet (management utility)",3:"compressnet (compression process)",5:"remote job entry",7:"echo protocol",9:"discard protocol",11:"active users (systat service)",13:"daytime protocol",17:"qotd",18:"message send protocol",19:"chargen",20:"ftp data",21:"ftp command",22:"ssh",23:"telnet",25:"SMTP",27:"nsw-fe (nsw user system fe)",29:"msg-icp",31:"msg-auth",33:"dsp",37:"time protocol",38:"rap/rlp",41:"graphics",42:"host name server protocol",43:"whois protocol",44:"mpm-flags",45:"mpm",46:"mpm-snd",48:"auditd",49:"tacacs",50:"re-mail-ck",53:"dns",67:"dhcp",68:"dhcp",80:"http",443:"https",853:"dns over tls",8080:"http proxy",8443:"https proxy",9050:"tor",9051:"tor",19132:"minecraft bedrock server ipv4",19133:"minecraft bedrock server ipv6"}
    ...
    try:
        ...
        print(red + my_ip + ": " + str(port) + " " + port_dict[port])
...
def security_scanner(url, secure = True, my_file = " ", bot = False):
    clear()

    my_subdomain_takeover = subdomain_takeover(url, secure, my_file, bot)
    my_ldap_scanner = ldap_scanner(url, secure, my_file, bot)
    my_sql_injection_scanner = sql_injection_scanner(url, secure, my_file, bot)
    my_xss_scanner = xss_scanner(url, secure, my_file, bot)
    my_xxe_scanner = xxe_scanner(url, secure, my_file, bot)

    clear()

    print(red + "open ports:")
    ...
    print("")
    print(red + "ldap injection:")

    for i in my_ldap_scanner:
        print(red + i)
...
def sql_injection_scanner(url, secure = True, my_file = " ", bot = False):
    if secure == True:
        my_secure = "https://"
...
def subdomain_scanner(url, secure = True, my_file = " ", bot = False):
    domain_list = []

    website = []

    if my_file == " ":
        website = link_scanner(url, secure = secure, bot = bot)
...
def subdomain_takeover(url, secure = True, my_file = " ", bot = False):
    result = subdomain_scanner(url, secure = secure, my_file = my_file, bot = bot)
...
#scans for xss
def xss_scanner(url, secure = True, my_file = " ", bot = False):
    if secure == True:
        my_secure = "https://"
...
#scans for xxe
def xxe_scanner(url, secure = True, my_file = " ", bot = False):
    if secure == True:
        my_secure = "https://"
