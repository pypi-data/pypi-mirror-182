...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~pyroge.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~pyroge.Client.sign_in` or :meth:`~pyroge.Client.sign_up`).

        next_type (:obj:`~pyroge.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~pyroge.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~pyroge.Client.resend_code`.
    """
