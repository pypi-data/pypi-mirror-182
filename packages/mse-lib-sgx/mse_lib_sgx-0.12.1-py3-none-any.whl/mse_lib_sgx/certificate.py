...
class SGXCertificate:
    ...
    def __init__(self, dns_name: str, subject: x509.Name, root_path: Path,
        ...
        self.sk: ec.EllipticCurvePrivateKey = (ec.generate_private_key(
            curve=ec.SECP256R1()) if not self.key_path.exists() else cast(
                ec.EllipticCurvePrivateKey,
                load_pem_private_key(data=self.key_path.read_bytes(),
                                     password=None)))
        ...
        if self.key_path.exists() and self.cert_path.exists():
            ...
            self.quote = Quote.from_bytes(
                get_quote(user_report_data=hashlib.sha256(
                    self.sk.public_key().public_bytes(
                        encoding=Encoding.X962,
                        format=PublicFormat.UncompressedPoint)).digest()))
