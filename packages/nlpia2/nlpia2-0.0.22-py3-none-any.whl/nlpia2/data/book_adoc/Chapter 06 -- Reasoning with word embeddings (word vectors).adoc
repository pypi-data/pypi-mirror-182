...
////
KM: Overall, this is a really nice chapter with some great examples. I have quite a few comments below, though. Most of these are related to formatting and styles. There are some content questions sprinkled throughout as well. Please review my comments and let me know if you have any questions.
HL: OK
////

////
KM: I've taken out the bold below. Bold is not allowed in Manning styles, nor should there be italics in the intro bullets.

There is also bold in other parts of the chapter. I've changed these to italics, to meet Manning style guidelines.
HL: OK
////
...
////
KM: The first sentence of the paragraph below is confusing.
HL: Rewrote all these paragraphs and created a new section
KM: Move text of footnote below to be in the main flow of the chapter. Try parantheses or a note.
HL: Done
////
...
[[word_brain_embedding_figure]]
.Word embeddings in your brain
image::../images/ch06/word-brain-embedding.drawio.png[alt="Figure 6. 1: connections between clusters of neurons representing each word in the phrase 'read the words in'",width=100%,link="../images/ch06/word-brain-embedding.drawio.png"]

You can think of a word embedding as a vector representation of the pattern of neuron firing in your brain when you think about an individual word.
Whenever you think of a word it creates a wave of electrical charges as neurons fire in your brain, like ripples emanating out from a pebble dropped in a pond.
But these electrical signals are selectively flowing out through some neurons and not others.
As you read the words in this sentence you are sparking flashes of activity in your neurons like those in the sketch in figure 6.1.
In fact, researchers have found surprising similarity in the patterns of artificial neural network weights for word embeddings and the activity within your brain as you think about words.footnote:["Robust Evaluation of Language-Brain Encoding Experiments" (https://arxiv.org/abs/1904.02547) by Lisa Beinborn (https://beinborn.eu/)] footnote:[
...
Electrons flowing out from neurons are like children running out of a school doorway and out into the world when the bell rings.
...
It turns out that some motivational speakers have name-squatted the word "NLP" on reddit for their 70's era "Neuro-linguistic Programming" money-making schemes.["Neuro-linguistic programming" explanation on Wikipedia (https://en.wikipedia.org/wiki/Neuro-linguistic_programming) and r/NLP (https://www.reddit.com/r/NLP) vs r/NaturalLanguage (https://www.reddit.com/r/NaturalLanguage)]
...
There's a another "brain on words" to think about.
...
////
KM: Missing footnote info below. If it's a well-known fact in the industry, then no footnote is needed.
////
...
[WARNING]
====
Word embeddings to not represent _your_ intended interpretation of words.
...
////
KM: Move the text of the footnote to the main flow of the chapter. Try parentheses instead.
////
...
They might get better at providing you search results the next time you look for a scientist.footnote:[At least, that is what it did for us in researching this book. We had to use private browser windows to ensure that your search results would be similar to ours.]
...
////
KM: I don't think listing out "A,B,C,D" and saying "choose from" is any more helpful than just saying multiple choice options. I would take those out, so the sentence reads, "It can be difficult to get analogy questions right, even when you have multiple choice options."
////

All those "tip of the tongue" questions are a piece of cake for word embeddings, even though they are not multiple choice.
It can be difficult to get analogy questions right, even when you have **A**, **B**, **C**, and **D** multiple choice options to choose from.
NLP comes to the rescue with word embeddings.

////
KM: Move the text of the footnote below to the main flow of the chapter.
////

Word embeddings can be used to answer even these vague questions and analogy problems.
Word embeddings can help you remember any word or name on the tip of your tongue, as long as the vector for the answer exists in your vocabulary.footnote:[For Google's pretrained Word2Vec model, your word is almost certainly within the 100B word news feed that Google trained it on, unless your word was invented after 2013.]
...
// TODO: find the dataset parsing section and put this section there (also look for Mikolov
=== Word2Vec Innovation
...
Word embeddings were a big leap forward in not only natural language understanding accuracy but also a breakthrough in the hope for general AI.

////
KM: Manning is trying to get away from having so many direct quotes in our books. Please paraphrase what is said below and just provide the citation in a footnote.
HL: What is the goal? To reduce the number of words the reader has to read? To ensure that we don't plagiarize? What is the motivation behind this editorial policy?
HL: I'll try to find a citation that allow the reader to actually appreciate Hofstadter's original list of intelligence attributes.
////

Hofstadter explained general intelligence like this:

.Hofstadter on Intelligence
[quote#hofstadter-intelligence, Douglas R. Hofstadter, "Gödel, Escher, Bach: an Eternal Golden Braid (GEB), p. 26"]
____
No one knows where the borderline between non-intelligent behavior and intelligent behavior lies;...
____

In _GEB_ Hofstadter explains how machines and even software can be intelligent. He suggests that for a machine (or piece of software) to be intelligent it must:

* respond flexibly
* capitalize on good fortune
* make sense of contradictions and ambiguity
* ignore noise and irrelevant information
* focus attention on important information
* find similarities between seemingly different situations
* find differences between seemingly identical situations
* synthesize new thoughts from old ones
* generate new ideas

As Hofstadter lists these essential elements of intelligence he is thinking of words and natural language when he talks about "situations" and "ideas".
You'll soon see how word embeddings enable all of these elements, to various degrees.
For example, word embeddings make it possible "to respond to situations very flexibly" by giving words fuzziness and flexibility that previous language models couldn't accomplish.

In previous iterations of your chatbot you would have to enumerate all the possible ways to say "Hi" if you want your bot to be flexible in its response to this common greeting.
But with word embeddings you can recognize the meaning of the word "hi" as long as you have an embedding for it.
And you can create embeddings for all the words your bot is likely to encounter by just feeding it as much text as you can find.
...
Some children with autism use words and language very precisely and rigidly.
And their inner dialog sometimes creates an exception aptitude at manipulation of formal languages such as musical notation, computer software, and mathematical formula.footnote:[According to the signs and symptoms section of the Wikipedia article on autism (https://proai.org/wiki-autism)]

////
KM: The part of the paragraph below doesn't make sense. Is this an example? If it is, please mark it as such.
////
...
== Word2Vec
////
KM: Move the text of the first 2 footnotes in the paragraph below to the main flow of the chapter. We don't want people to have to move their eye down to the bottom of the page and back up again to get this info. If it's not important, than take it out.
////

In 2012, Tomas Mikolov, an intern at Microsoft, found a way to embed the meaning of words into vector space.footnote:[Word embeddings or word vectors typically have 100 to 500 dimensions, depending on the breadth of information in the corpus used to train them.]
Mikolov trained a neural network footnote:[It is only a single-layer network, so almost any linear machine learning model will also work. Logistic regression, truncated SVD, linear discriminant analysis, or Naive Bayes would all work well.] to predict word occurrences near each target word.
...
////
KM: Nice distinction between these 2 types of learning!
////
...
<2> I'm sure your nessvector dimensions will be much more fun and useful, like "trumpness" and "ghandiness". How about a `nlpia` PR?

You can compute "nessvectors" for any word or _n_-gram in the Word2Vec vocabulary using the tools from  `nlpia` (https://gitlab.com/tangibleai/nlpia2/blob/master/src/nlpia/book/examples/ch06_nessvectors.py). And this approach will work for any "ness" components that you can dream up.

////
KM: Move the text of footnote to the main flow of the text. It might be useful to put this in parantheses.
////
...
You can add and subtract word vectors to _reason_ about the words they represent and answer questions similar to your examples above, like the following.footnote:[For those not up on sports, the Portland Timbers and Seattle Sounders are Major League Soccer teams.]
...
In this chapter, we want to improve on the LSA word vector representations we introduced in the previous chapter.
...
////
KM: Nice examples and figure below!
////
...
[[geometry_of_word2vec_math_figure]]
...
[[compute_the_answer_to_the_soccer_team_question]]
...
[[distance_between_the_singular_and_plural_versions_of_a_word]]
...
[[word_vectors_for_ten_us_cities_projected_onto_a_2d_map_figure]]
...
////
KM: Nice comparison between the Denver omelette and the Denver Nuggets!
////
...
[[training_input_and_output_example_for_the_skip-gram_approach_figure]]
.Training input and output example for the skip-gram approach
image::../images/ch06/we-are-all-one.drawio.png["Flow diagram for creating 2-grams for a window halfwidth of 3 (full width of 6) and skips of 1, 2, 3 and -1, -2, -3",width=100%,alt="Figure 6.3: Skip-gram method for creating training set of paired context and target words",link="../images/ch06/we-are-all-one.drawio.png"]
...
////
KM: In the paragraph below, limit down the text of the footnote to just citation. The rest should be in the main flow of the chapter.
HL: Done
////
...
////
KM: Is there a reason the footnote below is all capitalized?
////

And the creators of SpaCy have come up with an efficient contextualized word embedding algorithm that is as easy to use as the SpaCy package. They called their new package Sense2Vec.footnote:[SENSE2VEC - A FAST AND ACCURATE METHOD FOR WORD SENSE DISAMBIGUATION IN NEURAL WORD EMBEDDINGS (https://arxiv.org/pdf/1511.06388.pdf)]

////
KM: Don't forget to address your to do's below.
////

// FIXME:
// TODO: consolidate the example code snippets in ch06, nlpia2, and nessvec then replace these sections with those examples/snippets

// TODO: improve or delete this as part of the explanation of the pyTorch Word2vec training code

////
KM: This subsection is quite long and is set as a Heading 3 (4 equal signs). Given how long it is, I'm wondering if it should be a Heading 2 (3 equal signs).

Also, can you divide it up into smaller chunks of steps (each big set of steps would be a Heading 3 (4 equal signs).
////
...
class Word2VecDataset(Dataset):
  """
  Takes a HuggingFace dataset as an input, to be used for a
  Word2Vec dataloader.
  """
...
////
KM: The 2 subsections below are at the same level. Did you mean for Computational tricks to be a Heading 2 (3 equal signs) and Frequent bigrams to be a Heading 3 (4 equal signs)?
////

==== Computational tricks of Word2Vec

After the initial publication, the performance of `word2vec` models have been improved through various computational tricks. In this section, we highlight three improvements.
...
//If the words stem:[w_i] and stem:[w_j] result in a high score and the score is above the threshold stem:[\delta], they will be included in the Word2Vec vocabulary as a pair term.
If the words `wpass:n[~i~]` and `wpass:n[~j~]` result in a high score and the score is above the threshold `Δ`, they will be included in the Word2Vec vocabulary as a pair term.
You'll notice that the vocabulary of the model contains terms like "New_York" or "San Francisco".
The token of frequently occurring bigrams are combined through a character (usually "\_").
That way, these terms will be represented as a single one-hot vector instead of two separate ones, such as for "San" and "Francisco".
...
Another accuracy improvement to the original algorithm was to subsample frequent words.
Common words such as the articles "the" and "a" often don't a lot of information and are referred to as stop words.
...
//In the preceding equations, stem:[f(w_i)] represents the frequency of a word across the corpus, and stem:[t] represents a frequency threshold above which you want to apply the subsampling probability.
//The threshold depends on your corpus size, average document length, and the variety of words used in those documents. Values between stem:[10^{-5}] and stem:[10^{-6}] are often found in the literature.
...
//If a word shows up 10 times across your entire corpus, and your corpus has a vocabulary of one million distinct words, and you set the subsampling threshold to stem:[10^{-6}], the probability of keeping the word in any particular _n_-gram is 68%. You would skip it 32% of the time while composing your _n_-grams during tokenization.
...
////
KM: Change the heading below to be more active. Instead of "How to use the `gensim.word2vec` module" change it to "Using the `gensim.word2vec` module."
////
...
////
KM: Move the explanation of the source to the main flow of the text. Only include the citation in the footnote.
HL: Done
////
...
////
KM: Make the heading below more active. Instead of "How to generate your own..." try "Generating your own..."
////
...
////
KM: Make the heading below more active. Instead of "How to use the pretrained fastText models" try "Using the pretrained fastText models."
HL: Done
////
...
//If your LSA matrix of topic vectors is of size stem:[N_{words} \times N_{topics}], the LSA word vectors are the rows of that LSA matrix.
...
////
KM: Move the text of the footnote to the main flow of the text; just the citation should be in the footnote.
HL: Done
////
...
////
KM: Move the text of the footnote to the main flow of the text. Parentheses might work for this one.
HL: TODO: Revise or delete this section
////
...
We just got lucky that "Illini" footnote:[The word "Illini" refers to a group of people, usually football players and fans, rather than a single geographic region like "Illinois" (where most fans of the "Fighting Illini" live).] showed up in your list.
...
////
KM: What are you doing with the text below?
HL: TODO: revise this.
////

////
But that last line has quite a bit of "data wrangling" complexity.
It deals with "South_Carolina" and "South_Dakota", which are surprisingly absent from the Google News word vector embeddings.
Whenever a state's full name isn't available, you just use the abbreviation twice in the sum.
That way city vectors in SC and SD won't be shortchanged on the cultural and semantic features of their parent state names too much.
////
...
[[bubble_plot_of_US_city_word_vectors_code]]
...
// HL: TODO: explain what a sentence/thought embedding is using a single-word summarization of the sentences in this book
// HL: TODO: show how meaning gets "washed out" with longer and longer phrases
// HL: TODO: use pip-installed nessvec and pynndescent index for all of the code examples
...
////
KM: I recommend adding a heading 3 (4 equal signs) here to break the text up a bit. Also, you might want to do that again a few other places in this process since it is so long.
HL: DONE, see section heading above
HL: TODO for additional sections
HL: TODO revise all of John's text here
////
...
>>> subprocess.run(args=['asciidoc3', '-a', '-n', '-a', 'icons', chapt6_adoc]) # <1>
...
>>> plt.show()
----
<1> The argument 'k' denotes the optimal distance between nodes. This number was changed from the default to improve the visual footnote:[See the NetworkX documentation for details: http://mng.bz/gwnl]
...
image::../images/ch06/adjacency_graph_ch_6_with_labels_bold.png[An undirected graph plot of chapter 6 using sentence embeddings and noun phrases as labels,width=100%,link="../images/ch06/adjacency_graph_ch_6_with_labels_bold.png]

////
KM: Add a little bit of text in-between the 2 figures to tell readers what the difference between them is exactly.
////
...
////
KM: Summary should be a heading 1 (2 equal signs). I've changed this for you.
////

== Summary

////
KM: We've change the way that we are asking authors to write Summary bullets. Instead of telling readers WHAT they did/learned. We want to remind readers of definitions, facts, tips, and tricks they should know after reading the chapter. We are actually going to be making this interactive in LiveBook so that readers have complete the sentence themselves.

Instead of, "You've learned how word vectors..." change it to be just a fact: "Word vectors and vector-oriented reasoning can solve subtle problems like analogy questions and nonsynonomy relationships between words."

Please fix these below and add any additional facts you can think of from the chapter. As always, please let me know if you have any questions.
////
...
* You can now train `word2vec` and other word vector embeddings on the words you use in your applications so that your NLP pipeline is "polluted" by the GoogleNews meaning of words inherent in most Word2Vec pretrained models.
* You used `nessvec` to explore, visualize, and even build your own customized word embeddings and "character sheets".
* You've learned how to use word embeddings to discover the surprising hidden meaning of the names of people, places, businesses and even occupations
* A PCA projection of geographic word vectors like US city names cam reveal the cultural closeness of places that are geographically far apart.
* If you respect sentence boundaries with your _n_-grams and are efficient at setting up word pairs for training you can greatly improve the accuracy of your latent semantic analysis word embeddings (see chapter 4).
...
2. Create a graph data structure of another chapter in this book.
3. Create a graph data structure of the full book using sentiment analysis for each chapter.
