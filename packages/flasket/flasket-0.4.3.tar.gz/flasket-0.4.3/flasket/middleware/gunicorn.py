...
class GunicornApplication(BaseApplication):
    ...
    def __init__(self, *args, cfg: t.Dict, rootpath: str = None, **kwargs) -> None:
        """
        ...
        """
        self._flasket = Application(*args, cfg=cfg, rootpath=rootpath, **kwargs)
...
class FlasketCLI(cli.ABCFlasketCli):
    ...
    _forced_cfg: t.Dict = field(default={"server": {"debug": False, "production": True, "gunicorn": True}}, init=False)
    # fmt: on
...
# TODO: Regroup with flasket/middleware/flask.py
def FlasketCliHelper(file: str, pkgname: str, cfgname: str = None) -> FlasketCLI:
    """
    ...
    cfgname: str
        ...
        Name of the file that will be used as configuration file to look for.
    ...
    .. code-block:: python
      ...
      from flasket.middleware.gunicorn import FlasketCliHelper

      if __name__ == "__main__":
          FlasketCliHelper(__file__, "my_pacakge").run()

    """
    return cli.FlasketCliHelper(FlasketCLI, file, pkgname, cfgname)
...
class LocalCmdline(FlasketCmdline):
    def add_arguments(self, parser):
        ...
        # fmt: off
        parser.add_argument(
            "rootpath", default=os.getcwd(), nargs="?",
...
def main():
    cli = FlasketCLI(cmdline_cfg=LocalCmdline)
    cli.ctx = suppress_traceback
    cli.run()
