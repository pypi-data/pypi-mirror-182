...
class ThreadListService:
    ...
    def list(params, page=1, size=20, filter_fields=None, sort=None):
        ...
        @param sort 指定排序字段，仅支持['sort', 'create_time', '-create_time', 'update_time', '-update_time']
        """
        ...
        if sort and sort in ['sort', 'create_time', '-create_time', 'update_time', '-update_time']:
            thread_set.order_by(sort)
        else:
            thread_set.order_by('sort')
        # 指定不需要过滤的类别字段
    ...
    def search_ids(search_prams=None):
        ...
        try:
            thread_set = thread_set \
                ...
                .annotate(thread_classify_value=F("classify_id__value")) \
                .annotate(show_value=F("show_id__value")) \
                .annotate(auth_value=F("auth_id__value")).filter(is_deleted=0)
