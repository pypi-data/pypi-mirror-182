...
def get_class_method(cls):
    ...
    """
    return tuple(
        g for x in dir(cls) if not x.startswith("__") and callable(g := getattr(cls, x))
    )
...
class ComparePerformance:
    ...
    def get_all_instance_methods(self):
        ...
        return tuple(
            x for x in l if callable(getattr(self, x)) and not x.startswith("__")
        )
...
class RunParallel:
    def get_all_instance_methods(self, coroutine):
        ...
        return tuple(
            g
            for x in l
            if callable(g := getattr(self, x))
            and not x.startswith("__")
            and asyncio.iscoroutinefunction(g) == coroutine
        )
    ...
    def dict_results(self, names, results):
        return dict(zip((x.__name__ for x in names), (x.copy() for x in results)))
    ...
    def module_threading(self, *args):
        ...
        r = tuple(threading.Thread(target=v, args=(a[i],)) for i, v in enumerate(args))
        for i in r:
            i.start()
        for i in r:
            i.join()
    ...
    def module_multiprocessing(self, *args):
        ...
        r = tuple(
            multiprocessing.Process(target=v, args=(a[i],)) for i, v in enumerate(args)
        )
        for i in r:
            i.start()
        for i in r:
