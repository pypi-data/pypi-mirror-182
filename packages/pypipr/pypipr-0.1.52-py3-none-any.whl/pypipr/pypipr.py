...
def get_class_method(cls):
    ...
    """
    return tuple(getattr(cls, x) for x in dir(cls) if not x.startswith("__"))


def compare_performance(func_list, n=100000, sort=True):
    """
    ...
    """
    x = tuple(timeit.timeit(i, number=n) for i in func_list)
    x_min = min(x)
    r = tuple((f.__name__, int(x[i] / x_min * 100)) for i, f in enumerate(func_list))
    if sort:
